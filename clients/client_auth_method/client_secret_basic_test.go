...
func TestClientSecretBasicRefreshToken_Epic(t *testing.T) {
	//setup
...
func TestClientSecretBasicIntrospectToken_Epic(t *testing.T) {
	//setup
	oauthConfig := &oauth2.Config{
		ClientID:     "1e3ce324-5c1b-45a9-8ad1-7a0c1f51df43",
		ClientSecret: "PLACEHOLDER_CLIENT_SECRET",
	...
	//test
	response, err := client_auth_method.ClientSecretBasicIntrospectToken(
		context.TODO(),
		logrus.New(),
		endpointDef,
		oauthConfig,
...
func TestClientSecretBasicUserInfoGetPatientId_Epic(t *testing.T) {
	...
	patientID, err := client_auth_method.ClientSecretBasicUserInfoGetPatientId(context.TODO(), logrus.New(), oauthConfig, tokenData, userInfoEndpoint, testHttpClient)
	require.NoError(t, err)
