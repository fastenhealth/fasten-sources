...
func GetSourceFaithMedicalClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFaithMedicalClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Faith Medical Clinic"
	sourceDef.SourceType = pkg.SourceTypeFaithMedicalClinic
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"FAITH MEDICAL CLINIC", "FAITH MEDICAL CLINIC, PLLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1760949200"}}
