...
func GetSourceAdvancedOrthopedicsAndSportsMed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedOrthopedicsAndSportsMed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Advanced Orthopedics and Sports Med"
	sourceDef.SourceType = pkg.SourceTypeAdvancedOrthopedicsAndSportsMed
