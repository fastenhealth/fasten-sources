...
func GetSourceObgynAssociatesOfStAugustineLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeObgynAssociatesOfStAugustineLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "OBGYN ASSOCIATES OF ST AUGUSTINE LLC"
	sourceDef.SourceType = pkg.SourceTypeObgynAssociatesOfStAugustineLlc
