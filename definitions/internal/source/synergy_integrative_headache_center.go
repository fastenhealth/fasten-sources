...
func GetSourceSynergyIntegrativeHeadacheCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSynergyIntegrativeHeadacheCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Synergy Integrative Headache Center"
	sourceDef.SourceType = pkg.SourceTypeSynergyIntegrativeHeadacheCenter
