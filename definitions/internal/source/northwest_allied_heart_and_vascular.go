...
func GetSourceNorthwestAlliedHeartAndVascular(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestAlliedHeartAndVascular]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Allied Heart and Vascular"
	sourceDef.SourceType = pkg.SourceTypeNorthwestAlliedHeartAndVascular
