...
func GetSourceNorthCarolinaEmergencyPartners(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthCarolinaEmergencyPartners]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTH CAROLINA EMERGENCY PARTNERS"
	sourceDef.SourceType = pkg.SourceTypeNorthCarolinaEmergencyPartners
