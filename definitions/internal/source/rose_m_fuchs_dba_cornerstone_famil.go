...
func GetSourceRoseMFuchsDbaCornerstoneFamil(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRoseMFuchsDbaCornerstoneFamil]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Rose M Fuchs DBA Cornerstone Famil"
	sourceDef.SourceType = pkg.SourceTypeRoseMFuchsDbaCornerstoneFamil
