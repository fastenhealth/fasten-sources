...
func GetSourceMidwestSurgeryCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidwestSurgeryCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Midwest Surgery Center"
	sourceDef.SourceType = pkg.SourceTypeMidwestSurgeryCenter
	sourceDef.Category = []string{"261QA1903X", "367500000X"}
	sourceDef.Aliases = []string{"MIDWEST SURGERY CENTER - EAGAN"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1730361478", "1801164355", "1881375319"}}
