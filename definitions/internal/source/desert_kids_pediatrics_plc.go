...
func GetSourceDesertKidsPediatricsPlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDesertKidsPediatricsPlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Desert Kids Pediatrics PLC"
	sourceDef.SourceType = pkg.SourceTypeDesertKidsPediatricsPlc
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1043461171"}}
	sourceDef.PatientAccessUrl = "http://www.desertkidspediatrics.com"
