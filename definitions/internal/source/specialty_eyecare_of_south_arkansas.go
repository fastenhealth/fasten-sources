...
func GetSourceSpecialtyEyecareOfSouthArkansas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSpecialtyEyecareOfSouthArkansas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SPECIALTY EYECARE OF SOUTH ARKANSAS"
	sourceDef.SourceType = pkg.SourceTypeSpecialtyEyecareOfSouthArkansas
