...
func GetSourceNixxi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNixxi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Nixxi"
	sourceDef.SourceType = pkg.SourceTypeNixxi
	sourceDef.Category = []string{"207VM0101X"}
	sourceDef.Aliases = []string{"GARBHA HEALTH LLC", "NIXXI"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1265028948"}}
