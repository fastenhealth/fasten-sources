...
func GetSourceNorthernIllinoisOrthopaedicsAnd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthernIllinoisOrthopaedicsAnd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northern Illinois Orthopaedics And"
	sourceDef.SourceType = pkg.SourceTypeNorthernIllinoisOrthopaedicsAnd
