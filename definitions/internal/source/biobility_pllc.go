...
func GetSourceBiobilityPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBiobilityPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Biobility PLLC"
	sourceDef.SourceType = pkg.SourceTypeBiobilityPllc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"BIOBILITY PLLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1003560350"}}
