...
func GetSourceLakewoodMedCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLakewoodMedCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lakewood Med Center"
	sourceDef.SourceType = pkg.SourceTypeLakewoodMedCenter
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"LAKEWOOD MED CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1841782380"}}
