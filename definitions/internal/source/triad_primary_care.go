...
func GetSourceTriadPrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTriadPrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "TRIAD PRIMARY CARE"
	sourceDef.SourceType = pkg.SourceTypeTriadPrimaryCare
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"TRIAD PRIMARY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1285205708"}}
