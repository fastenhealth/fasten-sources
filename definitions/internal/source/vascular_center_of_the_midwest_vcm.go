...
func GetSourceVascularCenterOfTheMidwestVcm(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVascularCenterOfTheMidwestVcm]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VASCULAR CENTER OF THE MIDWEST VCM"
	sourceDef.SourceType = pkg.SourceTypeVascularCenterOfTheMidwestVcm
