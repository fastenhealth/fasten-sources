...
func GetSourceDiscoveryMedicalNetworkTelehealt(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDiscoveryMedicalNetworkTelehealt]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Discovery Medical Network Telehealt"
	sourceDef.SourceType = pkg.SourceTypeDiscoveryMedicalNetworkTelehealt
