...
func GetSourceSacredHeartMercyHealthCareCent(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSacredHeartMercyHealthCareCent]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sacred Heart Mercy Health Care Cent"
	sourceDef.SourceType = pkg.SourceTypeSacredHeartMercyHealthCareCent
