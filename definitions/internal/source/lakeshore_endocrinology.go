...
func GetSourceLakeshoreEndocrinology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLakeshoreEndocrinology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lakeshore Endocrinology"
	sourceDef.SourceType = pkg.SourceTypeLakeshoreEndocrinology
	sourceDef.Category = []string{"207RE0101X"}
	sourceDef.Aliases = []string{"LAKESHORE ENDOCRINOLOGY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1710384466"}}
