...
func GetSourceAssociatesInGynecologyObstetricsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAssociatesInGynecologyObstetricsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ASSOCIATES IN GYNECOLOGY  OBSTETRICS LLC"
	sourceDef.SourceType = pkg.SourceTypeAssociatesInGynecologyObstetricsLlc
