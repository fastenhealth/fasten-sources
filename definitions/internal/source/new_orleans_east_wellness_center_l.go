...
func GetSourceNewOrleansEastWellnessCenterL(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNewOrleansEastWellnessCenterL]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "New Orleans East Wellness Center L"
	sourceDef.SourceType = pkg.SourceTypeNewOrleansEastWellnessCenterL
