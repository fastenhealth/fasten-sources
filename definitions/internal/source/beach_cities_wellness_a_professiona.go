...
func GetSourceBeachCitiesWellnessAProfessiona(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBeachCitiesWellnessAProfessiona]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BEACH CITIES WELLNESS A PROFESSIONA"
	sourceDef.SourceType = pkg.SourceTypeBeachCitiesWellnessAProfessiona
