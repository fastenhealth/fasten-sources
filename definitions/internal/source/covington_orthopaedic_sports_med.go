...
func GetSourceCovingtonOrthopaedicSportsMed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCovingtonOrthopaedicSportsMed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Covington Orthopaedic  Sports Med"
	sourceDef.SourceType = pkg.SourceTypeCovingtonOrthopaedicSportsMed
