...
func GetSourceHunterdonFirstSurgical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHunterdonFirstSurgical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Hunterdon First Surgical"
	sourceDef.SourceType = pkg.SourceTypeHunterdonFirstSurgical
	sourceDef.Category = []string{"282N00000X"}
	sourceDef.Aliases = []string{"HUNTERDON FIRST SURGICAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1619428836"}}
