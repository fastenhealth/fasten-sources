...
func GetSourceAmsurgWilloughbyAnesthesiaLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmsurgWilloughbyAnesthesiaLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "AmSurg Willoughby Anesthesia LLC"
	sourceDef.SourceType = pkg.SourceTypeAmsurgWilloughbyAnesthesiaLlc
	sourceDef.Category = []string{"367500000X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1548508872"}}
