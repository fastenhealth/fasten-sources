...
func GetSourcePanaceaBrainAndSpineCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePanaceaBrainAndSpineCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Panacea Brain and Spine Center LLC"
	sourceDef.SourceType = pkg.SourceTypePanaceaBrainAndSpineCenterLlc
