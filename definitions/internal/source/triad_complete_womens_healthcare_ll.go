...
func GetSourceTriadCompleteWomensHealthcareLl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTriadCompleteWomensHealthcareLl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Triad Complete Womens Healthcare LL"
	sourceDef.SourceType = pkg.SourceTypeTriadCompleteWomensHealthcareLl
