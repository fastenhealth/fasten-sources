...
func GetSourceAllcareFootAnkleCenterDrImonugo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllcareFootAnkleCenterDrImonugo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ALLCARE FOOT  ANKLE CENTER  Dr Imonugo"
	sourceDef.SourceType = pkg.SourceTypeAllcareFootAnkleCenterDrImonugo
