...
func GetSourceBlueRidgeSpeechHearingCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBlueRidgeSpeechHearingCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Blue Ridge Speech  Hearing Center"
	sourceDef.SourceType = pkg.SourceTypeBlueRidgeSpeechHearingCenter
