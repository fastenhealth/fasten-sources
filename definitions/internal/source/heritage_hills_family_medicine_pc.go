...
func GetSourceHeritageHillsFamilyMedicinePc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHeritageHillsFamilyMedicinePc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Heritage Hills Family Medicine PC"
	sourceDef.SourceType = pkg.SourceTypeHeritageHillsFamilyMedicinePc
