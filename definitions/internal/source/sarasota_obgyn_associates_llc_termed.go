...
func GetSourceSarasotaObgynAssociatesLlcTermed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSarasotaObgynAssociatesLlcTermed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SARASOTA OBGYN ASSOCIATES LLC TERMED"
	sourceDef.SourceType = pkg.SourceTypeSarasotaObgynAssociatesLlcTermed
