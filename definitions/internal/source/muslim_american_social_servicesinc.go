...
func GetSourceMuslimAmericanSocialServicesinc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMuslimAmericanSocialServicesinc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Muslim American Social ServicesInc"
	sourceDef.SourceType = pkg.SourceTypeMuslimAmericanSocialServicesinc
