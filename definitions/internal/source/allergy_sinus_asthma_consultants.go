...
func GetSourceAllergySinusAsthmaConsultants(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllergySinusAsthmaConsultants]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Allergy Sinus  Asthma Consultants"
	sourceDef.SourceType = pkg.SourceTypeAllergySinusAsthmaConsultants
