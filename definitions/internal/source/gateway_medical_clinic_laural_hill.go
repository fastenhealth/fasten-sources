...
func GetSourceGatewayMedicalClinicLauralHill(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGatewayMedicalClinicLauralHill]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gateway Medical Clinic  Laural Hill"
	sourceDef.SourceType = pkg.SourceTypeGatewayMedicalClinicLauralHill
