...
func GetSourceDusonFamilyHealthcare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDusonFamilyHealthcare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DUSON FAMILY HEALTHCARE"
	sourceDef.SourceType = pkg.SourceTypeDusonFamilyHealthcare
	sourceDef.Category = []string{"261QP2300X", "261QR1300X"}
	sourceDef.Aliases = []string{"DUSON FAMILY HEALTHCARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1376986257", "1699930719"}}
	sourceDef.PatientAccessUrl = "http://www.dusonhealth.com"
