...
func GetSourceSandMountainFamilyPracticeCente(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSandMountainFamilyPracticeCente]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sand Mountain Family Practice Cente"
	sourceDef.SourceType = pkg.SourceTypeSandMountainFamilyPracticeCente
