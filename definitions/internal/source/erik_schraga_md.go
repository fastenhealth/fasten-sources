...
func GetSourceErikSchragaMd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeErikSchragaMd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Erik Schraga MD"
	sourceDef.SourceType = pkg.SourceTypeErikSchragaMd
	sourceDef.Category = []string{"207P00000X"}
	sourceDef.Aliases = []string{"BAY MOBILE URGENT CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1457051930"}}
	sourceDef.PatientAccessUrl = "http://www.bayurgent.com"
