...
func GetSourceOldeTowneObstetricsGynecology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOldeTowneObstetricsGynecology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Olde Towne Obstetrics  Gynecology"
	sourceDef.SourceType = pkg.SourceTypeOldeTowneObstetricsGynecology
