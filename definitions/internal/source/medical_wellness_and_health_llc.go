...
func GetSourceMedicalWellnessAndHealthLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMedicalWellnessAndHealthLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MEDICAL WELLNESS AND HEALTH LLC"
	sourceDef.SourceType = pkg.SourceTypeMedicalWellnessAndHealthLlc
	sourceDef.Category = []string{"208D00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1821715434"}}
	sourceDef.PatientAccessUrl = "http://www.nopainoklahoma.com"
