...
func GetSourceMartinsPediatricsAndFamilyCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMartinsPediatricsAndFamilyCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Martins Pediatrics and Family Care"
	sourceDef.SourceType = pkg.SourceTypeMartinsPediatricsAndFamilyCare
