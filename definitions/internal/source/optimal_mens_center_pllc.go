...
func GetSourceOptimalMensCenterPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOptimalMensCenterPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "OPTIMAL MENS CENTER PLLC"
	sourceDef.SourceType = pkg.SourceTypeOptimalMensCenterPllc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1811535974"}}
	sourceDef.PatientAccessUrl = "http://www.optimalmenscenter.com"
