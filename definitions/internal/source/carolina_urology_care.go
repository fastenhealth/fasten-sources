...
func GetSourceCarolinaUrologyCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCarolinaUrologyCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Carolina Urology Care"
	sourceDef.SourceType = pkg.SourceTypeCarolinaUrologyCare
	sourceDef.Category = []string{"208800000X"}
	sourceDef.Aliases = []string{"CAROLINA UROLOGY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1013291442"}}
