...
func GetSourceOhioPulmonaryAndSleepPhysicians(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOhioPulmonaryAndSleepPhysicians]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ohio Pulmonary and Sleep Physicians"
	sourceDef.SourceType = pkg.SourceTypeOhioPulmonaryAndSleepPhysicians
