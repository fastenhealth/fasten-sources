...
func GetSourceSandhyaVenugopalMdLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSandhyaVenugopalMdLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sandhya Venugopal MD LLC"
	sourceDef.SourceType = pkg.SourceTypeSandhyaVenugopalMdLlc
	sourceDef.Category = []string{"207R00000X", "261Q00000X"}
	sourceDef.Aliases = []string{"CARE AND CURE AZ"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1639418247"}}
	sourceDef.PatientAccessUrl = "http://www.careandcureaz.com"
