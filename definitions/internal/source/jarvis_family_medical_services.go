...
func GetSourceJarvisFamilyMedicalServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJarvisFamilyMedicalServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Jarvis Family Medical Services"
	sourceDef.SourceType = pkg.SourceTypeJarvisFamilyMedicalServices
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1013468784"}}
	sourceDef.PatientAccessUrl = "http://www.jarvisfamilymedicalservices.com"
