...
func GetSourceMillenniumCardiovascularAssociate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMillenniumCardiovascularAssociate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Millennium Cardiovascular Associate"
	sourceDef.SourceType = pkg.SourceTypeMillenniumCardiovascularAssociate
