...
func GetSourceTheSource(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheSource]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Source"
	sourceDef.SourceType = pkg.SourceTypeTheSource
	sourceDef.Category = []string{"261QF0050X"}
	sourceDef.Aliases = []string{"THE SOURCE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1922593250"}}
	sourceDef.PatientAccessUrl = "http://www.thesource.org"
