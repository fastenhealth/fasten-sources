...
func GetSourceShenandoahPainAndPalliativeCareClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShenandoahPainAndPalliativeCareClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Shenandoah Pain and Palliative Care Clinic"
	sourceDef.SourceType = pkg.SourceTypeShenandoahPainAndPalliativeCareClinic
