...
func GetSourceWestMetroFireRescue(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWestMetroFireRescue]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "West Metro Fire Rescue"
	sourceDef.SourceType = pkg.SourceTypeWestMetroFireRescue
	sourceDef.Category = []string{"3416L0300X"}
	sourceDef.Aliases = []string{"WEST METRO FIRE RESCUE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1568092385", "1649257007"}}
