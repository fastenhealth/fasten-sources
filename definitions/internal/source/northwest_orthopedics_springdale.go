...
func GetSourceNorthwestOrthopedicsSpringdale(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestOrthopedicsSpringdale]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Orthopedics  Springdale"
	sourceDef.SourceType = pkg.SourceTypeNorthwestOrthopedicsSpringdale
