...
func GetSourceCenterForIntegratedMedicinePa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForIntegratedMedicinePa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center For Integrated Medicine PA"
	sourceDef.SourceType = pkg.SourceTypeCenterForIntegratedMedicinePa
