...
func GetSourceCenterForBoneAndJointSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForBoneAndJointSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center for Bone and Joint Surgery"
	sourceDef.SourceType = pkg.SourceTypeCenterForBoneAndJointSurgery
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"CENTER FOR BONE AND JOINT SURGERY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1962435990"}}
