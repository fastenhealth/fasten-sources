...
func GetSourceLadyOfTheSeaGeneralSurgeryClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLadyOfTheSeaGeneralSurgeryClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lady of the Sea General Surgery Clinic"
	sourceDef.SourceType = pkg.SourceTypeLadyOfTheSeaGeneralSurgeryClinic
	sourceDef.Category = []string{"208600000X"}
	sourceDef.Aliases = []string{"LADY OF THE SEA GENERAL SURGERY CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1265474050"}}
