...
func GetSourceCompleteFootAndAnkleCareOfNorthTexasPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCompleteFootAndAnkleCareOfNorthTexasPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COMPLETE FOOT AND ANKLE CARE OF NORTH TEXAS PA"
	sourceDef.SourceType = pkg.SourceTypeCompleteFootAndAnkleCareOfNorthTexasPa
