...
func GetSourceLpntSpringViewPhysicianPractice(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLpntSpringViewPhysicianPractice]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LPNT Spring View Physician Practice"
	sourceDef.SourceType = pkg.SourceTypeLpntSpringViewPhysicianPractice
