...
func GetSourceComprehensiveHealthCenterMiami(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeComprehensiveHealthCenterMiami]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Comprehensive Health Center  Miami"
	sourceDef.SourceType = pkg.SourceTypeComprehensiveHealthCenterMiami
