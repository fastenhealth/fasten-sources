...
func GetSourceNorthwestAlliedInfectiousDisease(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestAlliedInfectiousDisease]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Allied Infectious Disease"
	sourceDef.SourceType = pkg.SourceTypeNorthwestAlliedInfectiousDisease
