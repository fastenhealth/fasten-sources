...
func GetSourceValleyWomensHealthOfRentonPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeValleyWomensHealthOfRentonPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VALLEY WOMENS HEALTH OF RENTON PLLC"
	sourceDef.SourceType = pkg.SourceTypeValleyWomensHealthOfRentonPllc
