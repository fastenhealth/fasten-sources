...
func GetSourceTatumHighlandsMedicalAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTatumHighlandsMedicalAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "TATUM HIGHLANDS MEDICAL ASSOCIATES"
	sourceDef.SourceType = pkg.SourceTypeTatumHighlandsMedicalAssociates
