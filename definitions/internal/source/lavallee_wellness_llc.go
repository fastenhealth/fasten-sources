...
func GetSourceLavalleeWellnessLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLavalleeWellnessLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lavallee Wellness LLC"
	sourceDef.SourceType = pkg.SourceTypeLavalleeWellnessLlc
	sourceDef.Category = []string{"133V00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1659805257"}}
	sourceDef.PatientAccessUrl = "http://www.lavalleewellness.com/"
