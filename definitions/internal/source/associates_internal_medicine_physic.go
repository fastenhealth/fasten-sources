...
func GetSourceAssociatesInternalMedicinePhysic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAssociatesInternalMedicinePhysic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Associates Internal Medicine Physic"
	sourceDef.SourceType = pkg.SourceTypeAssociatesInternalMedicinePhysic
