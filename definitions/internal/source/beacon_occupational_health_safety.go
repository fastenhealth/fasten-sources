...
func GetSourceBeaconOccupationalHealthSafety(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBeaconOccupationalHealthSafety]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Beacon Occupational Health  Safety"
	sourceDef.SourceType = pkg.SourceTypeBeaconOccupationalHealthSafety
