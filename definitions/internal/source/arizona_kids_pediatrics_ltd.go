...
func GetSourceArizonaKidsPediatricsLtd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArizonaKidsPediatricsLtd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arizona Kids Pediatrics LTD"
	sourceDef.SourceType = pkg.SourceTypeArizonaKidsPediatricsLtd
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1609311109"}}
	sourceDef.PatientAccessUrl = "http://www.arizonakidspediatrics.com"
