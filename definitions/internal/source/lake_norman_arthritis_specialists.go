...
func GetSourceLakeNormanArthritisSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLakeNormanArthritisSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lake Norman Arthritis Specialists"
	sourceDef.SourceType = pkg.SourceTypeLakeNormanArthritisSpecialists
	sourceDef.Category = []string{"207RR0500X"}
	sourceDef.Aliases = []string{"LAKE NORMAN ARTHRITIS SPECIALISTS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1477138527"}}
