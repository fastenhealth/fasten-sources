...
func GetSourceColumbiaFamilyMedicineClinicLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeColumbiaFamilyMedicineClinicLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COLUMBIA FAMILY MEDICINE CLINIC LLC"
	sourceDef.SourceType = pkg.SourceTypeColumbiaFamilyMedicineClinicLlc
