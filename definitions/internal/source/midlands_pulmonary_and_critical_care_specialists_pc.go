...
func GetSourceMidlandsPulmonaryAndCriticalCareSpecialistsPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidlandsPulmonaryAndCriticalCareSpecialistsPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MIDLANDS PULMONARY AND CRITICAL CARE SPECIALISTS PC"
	sourceDef.SourceType = pkg.SourceTypeMidlandsPulmonaryAndCriticalCareSpecialistsPc
