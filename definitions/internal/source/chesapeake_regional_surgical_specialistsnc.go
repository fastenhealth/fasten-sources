...
func GetSourceChesapeakeRegionalSurgicalSpecialistsnc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChesapeakeRegionalSurgicalSpecialistsnc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Chesapeake Regional Surgical SpecialistsNC"
	sourceDef.SourceType = pkg.SourceTypeChesapeakeRegionalSurgicalSpecialistsnc
