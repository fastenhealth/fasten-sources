...
func GetSourcePontchartrainOrthopedicsAndSportsMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePontchartrainOrthopedicsAndSportsMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pontchartrain Orthopedics and Sports Medicine"
	sourceDef.SourceType = pkg.SourceTypePontchartrainOrthopedicsAndSportsMedicine
