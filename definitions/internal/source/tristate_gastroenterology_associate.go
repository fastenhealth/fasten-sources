...
func GetSourceTristateGastroenterologyAssociate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTristateGastroenterologyAssociate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tristate Gastroenterology Associate"
	sourceDef.SourceType = pkg.SourceTypeTristateGastroenterologyAssociate
