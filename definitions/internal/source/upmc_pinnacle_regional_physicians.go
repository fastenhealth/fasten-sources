...
func GetSourceUpmcPinnacleRegionalPhysicians(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUpmcPinnacleRegionalPhysicians]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "UPMC Pinnacle Regional Physicians"
	sourceDef.SourceType = pkg.SourceTypeUpmcPinnacleRegionalPhysicians
