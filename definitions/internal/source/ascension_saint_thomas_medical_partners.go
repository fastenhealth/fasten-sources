...
func GetSourceAscensionSaintThomasMedicalPartners(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAscensionSaintThomasMedicalPartners]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ascension Saint Thomas Medical Partners"
	sourceDef.SourceType = pkg.SourceTypeAscensionSaintThomasMedicalPartners
