...
func GetSourceTennovaLafolletteHealthRehabilitationCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaLafolletteHealthRehabilitationCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova LaFollette Health  Rehabilitation Center"
	sourceDef.SourceType = pkg.SourceTypeTennovaLafolletteHealthRehabilitationCenter
