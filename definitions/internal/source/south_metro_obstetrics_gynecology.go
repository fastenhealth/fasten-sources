...
func GetSourceSouthMetroObstetricsGynecology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthMetroObstetricsGynecology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "South Metro Obstetrics  Gynecology"
	sourceDef.SourceType = pkg.SourceTypeSouthMetroObstetricsGynecology
