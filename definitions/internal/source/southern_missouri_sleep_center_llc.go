...
func GetSourceSouthernMissouriSleepCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernMissouriSleepCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SOUTHERN MISSOURI SLEEP CENTER LLC"
	sourceDef.SourceType = pkg.SourceTypeSouthernMissouriSleepCenterLlc
