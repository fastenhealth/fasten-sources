...
func GetSourceTransitionalLifeCareClinicInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTransitionalLifeCareClinicInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "TRANSITIONAL LIFE CARE CLINIC INC"
	sourceDef.SourceType = pkg.SourceTypeTransitionalLifeCareClinicInc
