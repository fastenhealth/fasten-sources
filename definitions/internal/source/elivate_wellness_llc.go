...
func GetSourceElivateWellnessLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeElivateWellnessLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Elivate Wellness LLC"
	sourceDef.SourceType = pkg.SourceTypeElivateWellnessLlc
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"ELIVATE WELLNESS LLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1457006660"}}
	sourceDef.PatientAccessUrl = "http://www.elivatewellness.com"
