...
func GetSourceCardiovascularAssociatesOfLakeCounty(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCardiovascularAssociatesOfLakeCounty]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Cardiovascular Associates Of Lake County"
	sourceDef.SourceType = pkg.SourceTypeCardiovascularAssociatesOfLakeCounty
