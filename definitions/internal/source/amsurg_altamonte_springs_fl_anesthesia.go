...
func GetSourceAmsurgAltamonteSpringsFlAnesthesia(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmsurgAltamonteSpringsFlAnesthesia]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "AmSurg Altamonte Springs FL Anesthesia"
	sourceDef.SourceType = pkg.SourceTypeAmsurgAltamonteSpringsFlAnesthesia
