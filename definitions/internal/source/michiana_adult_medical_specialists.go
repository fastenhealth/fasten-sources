...
func GetSourceMichianaAdultMedicalSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMichianaAdultMedicalSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Michiana Adult Medical Specialists"
	sourceDef.SourceType = pkg.SourceTypeMichianaAdultMedicalSpecialists
