...
func GetSourceCliffordDGluckMdPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCliffordDGluckMdPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CLIFFORD D GLUCK MD PLLC"
	sourceDef.SourceType = pkg.SourceTypeCliffordDGluckMdPllc
	sourceDef.Category = []string{"207RG0100X", "208800000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1952825671"}}
