...
func GetSourceMedicineAtHome(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMedicineAtHome]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Medicine at Home"
	sourceDef.SourceType = pkg.SourceTypeMedicineAtHome
	sourceDef.Category = []string{"363AM0700X"}
	sourceDef.Aliases = []string{"MEDICINE AT HOME"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1376914481"}}
	sourceDef.PatientAccessUrl = "https://www.medicineathome.net"
