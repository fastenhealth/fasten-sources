...
func GetSourceVeritasFamilyMedicineDentonPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVeritasFamilyMedicineDentonPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VERITAS FAMILY MEDICINE DENTON PLLC"
	sourceDef.SourceType = pkg.SourceTypeVeritasFamilyMedicineDentonPllc
