...
func GetSourceAlaskaCenterForVenousAndLymphaticMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlaskaCenterForVenousAndLymphaticMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Alaska Center for Venous and Lymphatic Medicine"
	sourceDef.SourceType = pkg.SourceTypeAlaskaCenterForVenousAndLymphaticMedicine
