...
func GetSourceSouthwestHandAndMicrosurgeryPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthwestHandAndMicrosurgeryPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southwest Hand and Microsurgery  PA"
	sourceDef.SourceType = pkg.SourceTypeSouthwestHandAndMicrosurgeryPa
