...
func GetSourceGessnerpetersonAlpineUrgentCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGessnerpetersonAlpineUrgentCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GessnerPeterson Alpine Urgent Care"
	sourceDef.SourceType = pkg.SourceTypeGessnerpetersonAlpineUrgentCare
