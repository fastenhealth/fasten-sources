...
func GetSourceActonUrgentCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeActonUrgentCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Acton Urgent Care"
	sourceDef.SourceType = pkg.SourceTypeActonUrgentCare
	sourceDef.Category = []string{"261QU0200X"}
	sourceDef.Aliases = []string{"ACTON URGENT CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1770089997"}}
	sourceDef.PatientAccessUrl = "https://www.actonuc.com/"
