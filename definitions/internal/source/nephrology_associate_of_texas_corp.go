...
func GetSourceNephrologyAssociateOfTexasCorp(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNephrologyAssociateOfTexasCorp]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEPHROLOGY ASSOCIATE OF TEXAS CORP"
	sourceDef.SourceType = pkg.SourceTypeNephrologyAssociateOfTexasCorp
