...
func GetSourceSouthwestNephrologyAssociatesLlp(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthwestNephrologyAssociatesLlp]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SOUTHWEST NEPHROLOGY ASSOCIATES LLP"
	sourceDef.SourceType = pkg.SourceTypeSouthwestNephrologyAssociatesLlp
