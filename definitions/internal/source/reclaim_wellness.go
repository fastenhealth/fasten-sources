...
func GetSourceReclaimWellness(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeReclaimWellness]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Reclaim Wellness"
	sourceDef.SourceType = pkg.SourceTypeReclaimWellness
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{"RECLAIM WELLNESS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1508503152"}}
