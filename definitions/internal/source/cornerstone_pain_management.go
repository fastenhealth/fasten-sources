...
func GetSourceCornerstonePainManagement(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCornerstonePainManagement]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cornerstone Pain Management"
	sourceDef.SourceType = pkg.SourceTypeCornerstonePainManagement
	sourceDef.Category = []string{"207L00000X", "207LP2900X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1780810259"}}
	sourceDef.PatientAccessUrl = "http://www.cornerstonepainmanagement.com"
