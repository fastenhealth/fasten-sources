...
func GetSourceCareplusNissanParkwayFamilyMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCareplusNissanParkwayFamilyMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CarePlus Nissan Parkway Family Medicine"
	sourceDef.SourceType = pkg.SourceTypeCareplusNissanParkwayFamilyMedicine
