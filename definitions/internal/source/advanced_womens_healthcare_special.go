...
func GetSourceAdvancedWomensHealthcareSpecial(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedWomensHealthcareSpecial]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Advanced Womens Healthcare Special"
	sourceDef.SourceType = pkg.SourceTypeAdvancedWomensHealthcareSpecial
