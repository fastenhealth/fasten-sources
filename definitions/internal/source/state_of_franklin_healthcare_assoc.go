...
func GetSourceStateOfFranklinHealthcareAssoc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStateOfFranklinHealthcareAssoc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "State of Franklin Healthcare Assoc"
	sourceDef.SourceType = pkg.SourceTypeStateOfFranklinHealthcareAssoc
