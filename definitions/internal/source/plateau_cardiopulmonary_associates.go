...
func GetSourcePlateauCardiopulmonaryAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePlateauCardiopulmonaryAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Plateau CardioPulmonary Associates"
	sourceDef.SourceType = pkg.SourceTypePlateauCardiopulmonaryAssociates
