...
func GetSourceProvidenceMedicalClinicOfKingsp(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeProvidenceMedicalClinicOfKingsp]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Providence Medical Clinic of Kingsp"
	sourceDef.SourceType = pkg.SourceTypeProvidenceMedicalClinicOfKingsp
