...
func GetSourceGibsonGeneralPhysicianServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGibsonGeneralPhysicianServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gibson General Physician Services"
	sourceDef.SourceType = pkg.SourceTypeGibsonGeneralPhysicianServices
