...
func GetSourceRejuvenateMedspadrZGreenTherapy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRejuvenateMedspadrZGreenTherapy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "REJUVENATE MEDSPADR Z GREEN THERAPY"
	sourceDef.SourceType = pkg.SourceTypeRejuvenateMedspadrZGreenTherapy
