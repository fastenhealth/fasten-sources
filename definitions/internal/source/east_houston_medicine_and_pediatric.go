...
func GetSourceEastHoustonMedicineAndPediatric(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEastHoustonMedicineAndPediatric]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "East Houston Medicine and Pediatric"
	sourceDef.SourceType = pkg.SourceTypeEastHoustonMedicineAndPediatric
