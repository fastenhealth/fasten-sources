...
func GetSourceNorthOaksInternalMedicineAssoc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthOaksInternalMedicineAssoc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Oaks Internal Medicine Assoc"
	sourceDef.SourceType = pkg.SourceTypeNorthOaksInternalMedicineAssoc
