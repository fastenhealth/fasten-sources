...
func GetSourceSouthAtlantaPlmncriticalAssocia(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthAtlantaPlmncriticalAssocia]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "South Atlanta PlmnCritical Associa"
	sourceDef.SourceType = pkg.SourceTypeSouthAtlantaPlmncriticalAssocia
