...
func GetSourceLongIslandHealthAndWellnessFam(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLongIslandHealthAndWellnessFam]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Long Island Health and Wellness Fam"
	sourceDef.SourceType = pkg.SourceTypeLongIslandHealthAndWellnessFam
