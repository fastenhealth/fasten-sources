...
func GetSourceEndoscopyCenterOfSouthBayAnesthesia(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEndoscopyCenterOfSouthBayAnesthesia]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Endoscopy Center Of South Bay Anesthesia"
	sourceDef.SourceType = pkg.SourceTypeEndoscopyCenterOfSouthBayAnesthesia
