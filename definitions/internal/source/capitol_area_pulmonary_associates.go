...
func GetSourceCapitolAreaPulmonaryAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCapitolAreaPulmonaryAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Capitol Area Pulmonary Associates"
	sourceDef.SourceType = pkg.SourceTypeCapitolAreaPulmonaryAssociates
