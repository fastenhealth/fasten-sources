...
func GetSourceBellinghamObstetricsGynAssocPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBellinghamObstetricsGynAssocPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BELLINGHAM OBSTETRICS  GYN ASSOC PLLC"
	sourceDef.SourceType = pkg.SourceTypeBellinghamObstetricsGynAssocPllc
