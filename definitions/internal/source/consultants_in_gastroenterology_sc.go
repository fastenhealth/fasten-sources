...
func GetSourceConsultantsInGastroenterologySc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeConsultantsInGastroenterologySc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Consultants in Gastroenterology SC"
	sourceDef.SourceType = pkg.SourceTypeConsultantsInGastroenterologySc
