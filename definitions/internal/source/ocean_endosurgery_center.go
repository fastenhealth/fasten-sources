...
func GetSourceOceanEndosurgeryCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOceanEndosurgeryCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Ocean Endosurgery Center"
	sourceDef.SourceType = pkg.SourceTypeOceanEndosurgeryCenter
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"OCEAN ENDOSURGERY CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1972541415"}}
