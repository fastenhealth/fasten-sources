...
func GetSourceNorthwestFloridaWomanCareLlcTermed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestFloridaWomanCareLlcTermed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTHWEST FLORIDA WOMAN CARE LLC TERMED"
	sourceDef.SourceType = pkg.SourceTypeNorthwestFloridaWomanCareLlcTermed
