...
func GetSourceBayfrontPhysicianSpecialityServ(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBayfrontPhysicianSpecialityServ]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bayfront Physician Speciality Serv"
	sourceDef.SourceType = pkg.SourceTypeBayfrontPhysicianSpecialityServ
