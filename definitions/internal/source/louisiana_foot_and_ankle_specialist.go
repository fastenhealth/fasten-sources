...
func GetSourceLouisianaFootAndAnkleSpecialist(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLouisianaFootAndAnkleSpecialist]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Louisiana Foot and Ankle Specialist"
	sourceDef.SourceType = pkg.SourceTypeLouisianaFootAndAnkleSpecialist
