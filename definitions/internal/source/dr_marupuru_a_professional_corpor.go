...
func GetSourceDrMarupuruAProfessionalCorpor(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDrMarupuruAProfessionalCorpor]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Dr Marupuru A Professional Corpor"
	sourceDef.SourceType = pkg.SourceTypeDrMarupuruAProfessionalCorpor
