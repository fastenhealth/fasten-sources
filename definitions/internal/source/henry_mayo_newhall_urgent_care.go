...
func GetSourceHenryMayoNewhallUrgentCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHenryMayoNewhallUrgentCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HENRY MAYO NEWHALL URGENT CARE"
	sourceDef.SourceType = pkg.SourceTypeHenryMayoNewhallUrgentCare
