...
func GetSourceWholeSystemsHealthcareSeattle(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWholeSystemsHealthcareSeattle]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Whole Systems Healthcare Seattle"
	sourceDef.SourceType = pkg.SourceTypeWholeSystemsHealthcareSeattle
