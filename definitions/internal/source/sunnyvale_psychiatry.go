...
func GetSourceSunnyvalePsychiatry(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSunnyvalePsychiatry]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sunnyvale Psychiatry"
	sourceDef.SourceType = pkg.SourceTypeSunnyvalePsychiatry
	sourceDef.Category = []string{"2084P0800X"}
	sourceDef.Aliases = []string{"SUNNYVALE PSYCHIATRY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1114595907"}}
	sourceDef.PatientAccessUrl = "http://www.sunnyvalepsychiatry.com"
