...
func GetSourceCompassionPediatricUrgentCareLl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCompassionPediatricUrgentCareLl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Compassion Pediatric Urgent Care LL"
	sourceDef.SourceType = pkg.SourceTypeCompassionPediatricUrgentCareLl
