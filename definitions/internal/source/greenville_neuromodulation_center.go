...
func GetSourceGreenvilleNeuromodulationCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGreenvilleNeuromodulationCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Greenville Neuromodulation Center"
	sourceDef.SourceType = pkg.SourceTypeGreenvilleNeuromodulationCenter
