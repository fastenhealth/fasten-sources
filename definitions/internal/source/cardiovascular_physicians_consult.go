...
func GetSourceCardiovascularPhysiciansConsult(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCardiovascularPhysiciansConsult]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cardiovascular Physicians  Consult"
	sourceDef.SourceType = pkg.SourceTypeCardiovascularPhysiciansConsult
