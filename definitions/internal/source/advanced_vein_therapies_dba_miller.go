...
func GetSourceAdvancedVeinTherapiesDbaMiller(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedVeinTherapiesDbaMiller]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Advanced Vein Therapies DBA Miller"
	sourceDef.SourceType = pkg.SourceTypeAdvancedVeinTherapiesDbaMiller
