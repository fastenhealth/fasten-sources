...
func GetSourceKanawhaCountyEmergencyAmbulance(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKanawhaCountyEmergencyAmbulance]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kanawha County Emergency Ambulance"
	sourceDef.SourceType = pkg.SourceTypeKanawhaCountyEmergencyAmbulance
