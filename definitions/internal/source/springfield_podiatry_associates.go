...
func GetSourceSpringfieldPodiatryAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSpringfieldPodiatryAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Springfield Podiatry Associates"
	sourceDef.SourceType = pkg.SourceTypeSpringfieldPodiatryAssociates
	sourceDef.Category = []string{"213E00000X", "213ES0131X"}
	sourceDef.Aliases = []string{"BENJAMIN C.DICKERT"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1831388552"}}
