...
func GetSourceBufcsacPcDbaBenchmarkUrgentAn(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBufcsacPcDbaBenchmarkUrgentAn]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BUFCSAC PC DBA BENCHMARK URGENT AN"
	sourceDef.SourceType = pkg.SourceTypeBufcsacPcDbaBenchmarkUrgentAn
