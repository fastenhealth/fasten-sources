...
func GetSourceStarFamilyMedicinePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStarFamilyMedicinePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Star Family Medicine PLLC"
	sourceDef.SourceType = pkg.SourceTypeStarFamilyMedicinePllc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"STAR FAMILY MEDICINE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1275027385"}}
