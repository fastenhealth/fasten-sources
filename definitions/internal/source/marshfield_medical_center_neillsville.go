...
func GetSourceMarshfieldMedicalCenterNeillsville(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMarshfieldMedicalCenterNeillsville]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MARSHFIELD MEDICAL CENTER  NEILLSVILLE"
	sourceDef.SourceType = pkg.SourceTypeMarshfieldMedicalCenterNeillsville
