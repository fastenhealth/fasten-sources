...
func GetSourceVitalityMedicalPartnersPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVitalityMedicalPartnersPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Vitality Medical Partners PC"
	sourceDef.SourceType = pkg.SourceTypeVitalityMedicalPartnersPc
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{"CIIT MEDICAL CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1225504327"}}
