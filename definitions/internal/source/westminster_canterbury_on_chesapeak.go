...
func GetSourceWestminsterCanterburyOnChesapeak(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWestminsterCanterburyOnChesapeak]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Westminster Canterbury on Chesapeak"
	sourceDef.SourceType = pkg.SourceTypeWestminsterCanterburyOnChesapeak
