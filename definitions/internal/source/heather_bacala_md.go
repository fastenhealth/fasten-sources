...
func GetSourceHeatherBacalaMd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHeatherBacalaMd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Heather Bacala MD"
	sourceDef.SourceType = pkg.SourceTypeHeatherBacalaMd
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{"MYOBGYN"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1215685136"}}
