...
func GetSourceAdlerFootcareOfGreaterNewYork(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdlerFootcareOfGreaterNewYork]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ADLER FOOTCARE OF GREATER NEW YORK"
	sourceDef.SourceType = pkg.SourceTypeAdlerFootcareOfGreaterNewYork
