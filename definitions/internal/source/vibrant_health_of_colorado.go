...
func GetSourceVibrantHealthOfColorado(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVibrantHealthOfColorado]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Vibrant Health of Colorado"
	sourceDef.SourceType = pkg.SourceTypeVibrantHealthOfColorado
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{"VIBRANT HEALTH OF COLORADO"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1336763143", "1942918768"}}
