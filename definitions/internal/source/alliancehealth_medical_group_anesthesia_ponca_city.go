...
func GetSourceAlliancehealthMedicalGroupAnesthesiaPoncaCity(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupAnesthesiaPoncaCity]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Anesthesia Ponca City"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupAnesthesiaPoncaCity
