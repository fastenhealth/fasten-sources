...
func GetSourceBrPhilipKennedyFamilyHealthCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBrPhilipKennedyFamilyHealthCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BR PHILIP KENNEDY FAMILY HEALTH CENTER"
	sourceDef.SourceType = pkg.SourceTypeBrPhilipKennedyFamilyHealthCenter
