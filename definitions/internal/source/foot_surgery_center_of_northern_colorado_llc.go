...
func GetSourceFootSurgeryCenterOfNorthernColoradoLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFootSurgeryCenterOfNorthernColoradoLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Foot Surgery Center of Northern Colorado LLC"
	sourceDef.SourceType = pkg.SourceTypeFootSurgeryCenterOfNorthernColoradoLlc
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1013116938"}}
