...
func GetSourceAdvancedDermatologyOfMelrosePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedDermatologyOfMelrosePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ADVANCED DERMATOLOGY OF MELROSE PLLC"
	sourceDef.SourceType = pkg.SourceTypeAdvancedDermatologyOfMelrosePllc
