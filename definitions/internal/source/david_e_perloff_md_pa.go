...
func GetSourceDavidEPerloffMdPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDavidEPerloffMdPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "David E Perloff MD PA"
	sourceDef.SourceType = pkg.SourceTypeDavidEPerloffMdPa
	sourceDef.Category = []string{"207RC0000X"}
	sourceDef.Aliases = []string{"PERLOFF CARDIOVASCULAR CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1831310903"}}
