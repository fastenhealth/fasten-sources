...
func GetSourcePacePrimaryCareAndWalkinClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePacePrimaryCareAndWalkinClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pace Primary Care and WalkIn Clinic"
	sourceDef.SourceType = pkg.SourceTypePacePrimaryCareAndWalkinClinic
