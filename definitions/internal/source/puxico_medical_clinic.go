...
func GetSourcePuxicoMedicalClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePuxicoMedicalClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Puxico Medical Clinic"
	sourceDef.SourceType = pkg.SourceTypePuxicoMedicalClinic
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"PUXICO MEDICAL CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1275576654"}}
