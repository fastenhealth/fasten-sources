...
func GetSourceWestfordVeinAestheticSolutions(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWestfordVeinAestheticSolutions]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WESTFORD VEIN  AESTHETIC SOLUTIONS"
	sourceDef.SourceType = pkg.SourceTypeWestfordVeinAestheticSolutions
