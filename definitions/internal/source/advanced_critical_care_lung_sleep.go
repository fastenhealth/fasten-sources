...
func GetSourceAdvancedCriticalCareLungSleep(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedCriticalCareLungSleep]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Advanced Critical Care Lung  Sleep"
	sourceDef.SourceType = pkg.SourceTypeAdvancedCriticalCareLungSleep
