...
func GetSourceNorthValleySurgicalAssociatesI(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthValleySurgicalAssociatesI]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Valley Surgical Associates I"
	sourceDef.SourceType = pkg.SourceTypeNorthValleySurgicalAssociatesI
