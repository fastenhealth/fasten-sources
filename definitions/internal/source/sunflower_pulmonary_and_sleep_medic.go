...
func GetSourceSunflowerPulmonaryAndSleepMedic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSunflowerPulmonaryAndSleepMedic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sunflower Pulmonary and Sleep Medic"
	sourceDef.SourceType = pkg.SourceTypeSunflowerPulmonaryAndSleepMedic
