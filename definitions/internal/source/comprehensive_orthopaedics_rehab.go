...
func GetSourceComprehensiveOrthopaedicsRehab(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeComprehensiveOrthopaedicsRehab]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Comprehensive Orthopaedics  Rehab"
	sourceDef.SourceType = pkg.SourceTypeComprehensiveOrthopaedicsRehab
