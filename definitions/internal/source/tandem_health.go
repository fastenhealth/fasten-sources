...
func GetSourceTandemHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTandemHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tandem Health"
	sourceDef.SourceType = pkg.SourceTypeTandemHealth
	sourceDef.Category = []string{"261QF0400X"}
	sourceDef.Aliases = []string{"SUMTER FAMILY HEALTH CENTER", "TANDEM HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1740641919"}}
