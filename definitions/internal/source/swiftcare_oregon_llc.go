...
func GetSourceSwiftcareOregonLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSwiftcareOregonLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Swiftcare Oregon LLC"
	sourceDef.SourceType = pkg.SourceTypeSwiftcareOregonLlc
	sourceDef.Category = []string{"261QU0200X"}
	sourceDef.Aliases = []string{"SWIFTCARE OREGON LLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1235657156"}}
