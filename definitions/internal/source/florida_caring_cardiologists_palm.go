...
func GetSourceFloridaCaringCardiologistsPalm(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFloridaCaringCardiologistsPalm]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FLORIDA CARING CARDIOLOGISTS PALM"
	sourceDef.SourceType = pkg.SourceTypeFloridaCaringCardiologistsPalm
