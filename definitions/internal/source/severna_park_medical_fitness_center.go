...
func GetSourceSevernaParkMedicalFitnessCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSevernaParkMedicalFitnessCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SEVERNA PARK MEDICAL FITNESS CENTER"
	sourceDef.SourceType = pkg.SourceTypeSevernaParkMedicalFitnessCenter
