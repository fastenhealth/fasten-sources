...
func GetSourceNorthpointSurgicalPrimaryCareAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthpointSurgicalPrimaryCareAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NorthPoint Surgical Primary Care Associates"
	sourceDef.SourceType = pkg.SourceTypeNorthpointSurgicalPrimaryCareAssociates
