...
func GetSourceStJosephInternalMedicineAssocia(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStJosephInternalMedicineAssocia]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "St Joseph Internal Medicine Associa"
	sourceDef.SourceType = pkg.SourceTypeStJosephInternalMedicineAssocia
