...
func GetSourceMasterYourEmotionsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMasterYourEmotionsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Master Your Emotions LLC"
	sourceDef.SourceType = pkg.SourceTypeMasterYourEmotionsLlc
	sourceDef.Category = []string{"101YM0800X"}
	sourceDef.Aliases = []string{"MASTER YOUR EMOTIONS LLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1861127102"}}
