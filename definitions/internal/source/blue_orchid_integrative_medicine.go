...
func GetSourceBlueOrchidIntegrativeMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBlueOrchidIntegrativeMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Blue Orchid Integrative Medicine"
	sourceDef.SourceType = pkg.SourceTypeBlueOrchidIntegrativeMedicine
