...
func GetSourceNirupaVignarajanMdInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNirupaVignarajanMdInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Nirupa Vignarajan MD Inc"
	sourceDef.SourceType = pkg.SourceTypeNirupaVignarajanMdInc
	sourceDef.PatientAccessUrl = "https://www.healthgrades.com/physician/dr-nirupa-vignarajan-yjhs2"
