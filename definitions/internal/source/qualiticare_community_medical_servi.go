...
func GetSourceQualiticareCommunityMedicalServi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeQualiticareCommunityMedicalServi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "QUALITICARE COMMUNITY MEDICAL SERVI"
	sourceDef.SourceType = pkg.SourceTypeQualiticareCommunityMedicalServi
