...
func GetSourceMillerHeightsMedicalAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMillerHeightsMedicalAssociates]; clientIdOk {
	...
	sourceDef.Aliases = []string{"MILLER HEIGHTS MEDICAL ASSOCIATES"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1124542808", "1588777262"}}
