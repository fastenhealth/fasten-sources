...
func GetSourceIdealSelfLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIdealSelfLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "IDEAL SELF LLC"
	sourceDef.SourceType = pkg.SourceTypeIdealSelfLlc
	sourceDef.Category = []string{"363LF0000X", "363LP0808X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1467182774"}}
	sourceDef.PatientAccessUrl = "http://www.idealself.org"
