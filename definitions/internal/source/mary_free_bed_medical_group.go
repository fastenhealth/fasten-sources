...
func GetSourceMaryFreeBedMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMaryFreeBedMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Mary Free Bed Medical Group"
	sourceDef.SourceType = pkg.SourceTypeMaryFreeBedMedicalGroup
	sourceDef.Category = []string{"208100000X"}
	sourceDef.Aliases = []string{"MARY FREE BED MEDICAL GROUP"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1053641589"}}
