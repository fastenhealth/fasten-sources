...
func GetSourceKidneySpecialistsOfNorthHoustonPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKidneySpecialistsOfNorthHoustonPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "KIDNEY SPECIALISTS OF NORTH HOUSTON PLLC"
	sourceDef.SourceType = pkg.SourceTypeKidneySpecialistsOfNorthHoustonPllc
