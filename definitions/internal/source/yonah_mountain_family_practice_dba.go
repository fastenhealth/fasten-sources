...
func GetSourceYonahMountainFamilyPracticeDba(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeYonahMountainFamilyPracticeDba]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Yonah Mountain Family Practice DBA"
	sourceDef.SourceType = pkg.SourceTypeYonahMountainFamilyPracticeDba
