...
func GetSourceAdvancedMedicalAndWeightLoss(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedMedicalAndWeightLoss]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Advanced Medical and Weight Loss"
	sourceDef.SourceType = pkg.SourceTypeAdvancedMedicalAndWeightLoss
