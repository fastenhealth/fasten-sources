...
func GetSourceStiffHealthAndWellnessCarePll(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStiffHealthAndWellnessCarePll]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "STIFF HEALTH AND WELLNESS CARE PLL"
	sourceDef.SourceType = pkg.SourceTypeStiffHealthAndWellnessCarePll
