...
func GetSourceGulfCoastOccupationalSportsAnd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGulfCoastOccupationalSportsAnd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gulf Coast Occupational Sports and"
	sourceDef.SourceType = pkg.SourceTypeGulfCoastOccupationalSportsAnd
