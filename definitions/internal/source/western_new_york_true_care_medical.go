...
func GetSourceWesternNewYorkTrueCareMedical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWesternNewYorkTrueCareMedical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WESTERN NEW YORK TRUE CARE MEDICAL"
	sourceDef.SourceType = pkg.SourceTypeWesternNewYorkTrueCareMedical
