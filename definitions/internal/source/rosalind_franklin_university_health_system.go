...
func GetSourceRosalindFranklinUniversityHealthSystem(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRosalindFranklinUniversityHealthSystem]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Rosalind Franklin University Health System"
	sourceDef.SourceType = pkg.SourceTypeRosalindFranklinUniversityHealthSystem
