...
func GetSourceObgynSpecialistsOfTheTreasureCoastllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeObgynSpecialistsOfTheTreasureCoastllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "OBGYN SPECIALISTS OF THE TREASURE COASTLLC"
	sourceDef.SourceType = pkg.SourceTypeObgynSpecialistsOfTheTreasureCoastllc
