...
func GetSourceKauaiCommunityHealthAllianceDba(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKauaiCommunityHealthAllianceDba]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kauai Community Health Alliance dba"
	sourceDef.SourceType = pkg.SourceTypeKauaiCommunityHealthAllianceDba
