...
func GetSourceAnthonyMedicalAndChiropracticCe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAnthonyMedicalAndChiropracticCe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Anthony Medical and Chiropractic Ce"
	sourceDef.SourceType = pkg.SourceTypeAnthonyMedicalAndChiropracticCe
