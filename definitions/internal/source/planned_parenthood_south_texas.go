...
func GetSourcePlannedParenthoodSouthTexas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePlannedParenthoodSouthTexas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Planned Parenthood South Texas"
	sourceDef.SourceType = pkg.SourceTypePlannedParenthoodSouthTexas
	sourceDef.Category = []string{"291U00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1376833053"}}
	sourceDef.PatientAccessUrl = "http://www.pptrust.org"
