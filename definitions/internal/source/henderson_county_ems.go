...
func GetSourceHendersonCountyEms(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHendersonCountyEms]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Henderson County EMS"
	sourceDef.SourceType = pkg.SourceTypeHendersonCountyEms
	sourceDef.Category = []string{"3416L0300X"}
	sourceDef.Aliases = []string{"HENDERSON COUNTY EMS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1083725592"}}
