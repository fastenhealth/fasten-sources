...
func GetSourceIntegracareClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIntegracareClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "IntegraCare Clinic"
	sourceDef.SourceType = pkg.SourceTypeIntegracareClinic
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"INTEGRACARE CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1265739643"}}
