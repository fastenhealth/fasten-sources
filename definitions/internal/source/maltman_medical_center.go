...
func GetSourceMaltmanMedicalCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMaltmanMedicalCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Maltman Medical Center"
	sourceDef.SourceType = pkg.SourceTypeMaltmanMedicalCenter
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"MALTMAN MEDICAL CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1508320623"}}
