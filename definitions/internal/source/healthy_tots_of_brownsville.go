...
func GetSourceHealthyTotsOfBrownsville(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHealthyTotsOfBrownsville]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Healthy Tots of Brownsville"
	sourceDef.SourceType = pkg.SourceTypeHealthyTotsOfBrownsville
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{"HEALTHY TOTS OF BROWNSVILLE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1588103253"}}
