...
func GetSourceLouisvilleGeriatricAssociatesPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLouisvilleGeriatricAssociatesPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Louisville Geriatric Associates PLLC"
	sourceDef.SourceType = pkg.SourceTypeLouisvilleGeriatricAssociatesPllc
