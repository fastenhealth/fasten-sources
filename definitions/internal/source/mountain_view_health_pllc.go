...
func GetSourceMountainViewHealthPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMountainViewHealthPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Mountain View Health PLLC"
	sourceDef.SourceType = pkg.SourceTypeMountainViewHealthPllc
	sourceDef.Category = []string{"363LP0808X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1568087005"}}
	sourceDef.PatientAccessUrl = "http://www.mtviewhealth.com"
