...
func GetSourceBroadwayFamilyPracticeLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBroadwayFamilyPracticeLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BROADWAY FAMILY PRACTICE LLC"
	sourceDef.SourceType = pkg.SourceTypeBroadwayFamilyPracticeLlc
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1184346090"}}
	sourceDef.PatientAccessUrl = "http://www.broadwayfamilypractice.com"
