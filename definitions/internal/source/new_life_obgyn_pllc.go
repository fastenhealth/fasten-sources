...
func GetSourceNewLifeObgynPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNewLifeObgynPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEW LIFE OBGYN PLLC"
	sourceDef.SourceType = pkg.SourceTypeNewLifeObgynPllc
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{"NEW LIFE CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1558022103"}}
