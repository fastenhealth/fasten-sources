...
func GetSourceAdvancedSurgicalAssociatesOfNor(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedSurgicalAssociatesOfNor]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Advanced Surgical Associates of Nor"
	sourceDef.SourceType = pkg.SourceTypeAdvancedSurgicalAssociatesOfNor
