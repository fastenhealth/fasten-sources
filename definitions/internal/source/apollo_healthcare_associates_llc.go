...
func GetSourceApolloHealthcareAssociatesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeApolloHealthcareAssociatesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Apollo Healthcare Associates LLC"
	sourceDef.SourceType = pkg.SourceTypeApolloHealthcareAssociatesLlc
	sourceDef.Category = []string{"207RE0101X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1114397809"}}
