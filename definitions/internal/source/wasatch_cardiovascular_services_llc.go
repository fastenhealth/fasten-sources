...
func GetSourceWasatchCardiovascularServicesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWasatchCardiovascularServicesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Wasatch Cardiovascular Services LLC"
	sourceDef.SourceType = pkg.SourceTypeWasatchCardiovascularServicesLlc
