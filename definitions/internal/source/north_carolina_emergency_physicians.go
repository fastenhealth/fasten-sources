...
func GetSourceNorthCarolinaEmergencyPhysicians(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthCarolinaEmergencyPhysicians]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTH CAROLINA EMERGENCY PHYSICIANS"
	sourceDef.SourceType = pkg.SourceTypeNorthCarolinaEmergencyPhysicians
