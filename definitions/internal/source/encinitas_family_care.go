...
func GetSourceEncinitasFamilyCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEncinitasFamilyCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Encinitas Family Care"
	sourceDef.SourceType = pkg.SourceTypeEncinitasFamilyCare
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"ENCINITAS FAMILY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1265757041"}}
