...
func GetSourceAlaskaFunctionalMedicineClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlaskaFunctionalMedicineClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Alaska Functional Medicine Clinic"
	sourceDef.SourceType = pkg.SourceTypeAlaskaFunctionalMedicineClinic
