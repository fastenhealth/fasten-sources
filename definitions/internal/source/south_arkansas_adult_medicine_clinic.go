...
func GetSourceSouthArkansasAdultMedicineClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthArkansasAdultMedicineClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "South Arkansas Adult Medicine Clinic"
	sourceDef.SourceType = pkg.SourceTypeSouthArkansasAdultMedicineClinic
