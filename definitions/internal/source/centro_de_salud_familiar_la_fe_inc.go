...
func GetSourceCentroDeSaludFamiliarLaFeInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentroDeSaludFamiliarLaFeInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Centro De Salud Familiar La Fe Inc"
	sourceDef.SourceType = pkg.SourceTypeCentroDeSaludFamiliarLaFeInc
	sourceDef.PatientAccessUrl = "https://lafe-ep.org/"
