...
func GetSourceJeremyLaMotteMd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJeremyLaMotteMd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Jeremy La Motte MD"
	sourceDef.SourceType = pkg.SourceTypeJeremyLaMotteMd
	sourceDef.Category = []string{"208D00000X"}
	sourceDef.Aliases = []string{"STARWEST MEDICAL GROUP"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1659820298"}}
	sourceDef.PatientAccessUrl = "https://seapointemedicalgroup.com/"
