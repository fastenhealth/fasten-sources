...
func GetSourceBeverlymedicalSleepCenterNeurodiagnosticInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBeverlymedicalSleepCenterNeurodiagnosticInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BEVERLYMEDICAL SLEEP CENTER  NEURODIAGNOSTIC INC"
	sourceDef.SourceType = pkg.SourceTypeBeverlymedicalSleepCenterNeurodiagnosticInc
