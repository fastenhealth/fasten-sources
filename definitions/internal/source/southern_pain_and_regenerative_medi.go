...
func GetSourceSouthernPainAndRegenerativeMedi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernPainAndRegenerativeMedi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southern Pain and Regenerative Medi"
	sourceDef.SourceType = pkg.SourceTypeSouthernPainAndRegenerativeMedi
