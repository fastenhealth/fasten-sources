...
func GetSourceShawneeChristianHealthcareCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShawneeChristianHealthcareCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Shawnee Christian Healthcare Center"
	sourceDef.SourceType = pkg.SourceTypeShawneeChristianHealthcareCenter
	sourceDef.PatientAccessUrl = "http://shawneechristianhealthcare.org/"
