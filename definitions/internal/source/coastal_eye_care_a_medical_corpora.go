...
func GetSourceCoastalEyeCareAMedicalCorpora(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCoastalEyeCareAMedicalCorpora]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COASTAL EYE CARE A MEDICAL CORPORA"
	sourceDef.SourceType = pkg.SourceTypeCoastalEyeCareAMedicalCorpora
