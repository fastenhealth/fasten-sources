...
func GetSourceCardiovascularSpecialistOfGerman(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCardiovascularSpecialistOfGerman]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cardiovascular Specialist Of German"
	sourceDef.SourceType = pkg.SourceTypeCardiovascularSpecialistOfGerman
