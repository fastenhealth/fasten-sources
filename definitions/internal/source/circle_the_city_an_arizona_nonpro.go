...
func GetSourceCircleTheCityAnArizonaNonpro(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCircleTheCityAnArizonaNonpro]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Circle The City an Arizona NonPro"
	sourceDef.SourceType = pkg.SourceTypeCircleTheCityAnArizonaNonpro
