...
func GetSourceKingwoodNeurologyAndSleepPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKingwoodNeurologyAndSleepPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kingwood Neurology and Sleep PA"
	sourceDef.SourceType = pkg.SourceTypeKingwoodNeurologyAndSleepPa
	sourceDef.Category = []string{"2084N0400X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1457541237"}}
	sourceDef.PatientAccessUrl = "http://www.kingwoodneurology.com"
