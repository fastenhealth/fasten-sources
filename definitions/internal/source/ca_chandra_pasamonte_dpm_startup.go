...
func GetSourceCaChandraPasamonteDpmStartup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCaChandraPasamonteDpmStartup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CA  Chandra Pasamonte DPM  startup"
	sourceDef.SourceType = pkg.SourceTypeCaChandraPasamonteDpmStartup
