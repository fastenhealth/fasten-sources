...
func GetSourceKhadraOsmanMdOfFtLauderdaleLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKhadraOsmanMdOfFtLauderdaleLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "KHADRA OSMAN MD OF FT LAUDERDALE LLC"
	sourceDef.SourceType = pkg.SourceTypeKhadraOsmanMdOfFtLauderdaleLlc
