...
func GetSourceAdaptiveCounselingAndCoachingSe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdaptiveCounselingAndCoachingSe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Adaptive Counseling and Coaching Se"
	sourceDef.SourceType = pkg.SourceTypeAdaptiveCounselingAndCoachingSe
