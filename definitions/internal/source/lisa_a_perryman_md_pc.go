...
func GetSourceLisaAPerrymanMdPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLisaAPerrymanMdPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lisa A Perryman MD PC"
	sourceDef.SourceType = pkg.SourceTypeLisaAPerrymanMdPc
	sourceDef.Category = []string{"208600000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1932368487"}}
	sourceDef.PatientAccessUrl = "http://www.coloradocolon.com/"
