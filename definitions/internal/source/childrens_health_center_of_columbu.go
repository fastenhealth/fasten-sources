...
func GetSourceChildrensHealthCenterOfColumbu(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChildrensHealthCenterOfColumbu]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CHILDRENS HEALTH CENTER OF COLUMBU"
	sourceDef.SourceType = pkg.SourceTypeChildrensHealthCenterOfColumbu
