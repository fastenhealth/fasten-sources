...
func GetSourceSaintBarnabasOutpatientCenters(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSaintBarnabasOutpatientCenters]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Saint Barnabas Outpatient Centers"
	sourceDef.SourceType = pkg.SourceTypeSaintBarnabasOutpatientCenters
