...
func GetSourceProvidencePrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeProvidencePrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Providence Primary Care"
	sourceDef.SourceType = pkg.SourceTypeProvidencePrimaryCare
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{"PROVIDENCE PRIMARY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1487302857"}}
