...
func GetSourceNursePractitionerProfessionalRes(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNursePractitionerProfessionalRes]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NURSE PRACTITIONER PROFESSIONAL RES"
	sourceDef.SourceType = pkg.SourceTypeNursePractitionerProfessionalRes
