...
func GetSourceNeurologyAndPainConsultantsPll(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeurologyAndPainConsultantsPll]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Neurology and Pain Consultants PLL"
	sourceDef.SourceType = pkg.SourceTypeNeurologyAndPainConsultantsPll
