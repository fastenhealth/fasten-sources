...
func GetSourceWilsonDamClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWilsonDamClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Wilson Dam Clinic"
	sourceDef.SourceType = pkg.SourceTypeWilsonDamClinic
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"WILSON DAM CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1457843708"}}
