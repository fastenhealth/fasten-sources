...
func GetSourceClinictimeInternalMedPrimarycare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeClinictimeInternalMedPrimarycare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ClinicTime Internal Med PrimaryCare"
	sourceDef.SourceType = pkg.SourceTypeClinictimeInternalMedPrimarycare
