...
func GetSourceCoastalSeniorHealthcareInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCoastalSeniorHealthcareInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Coastal Senior Healthcare Inc"
	sourceDef.SourceType = pkg.SourceTypeCoastalSeniorHealthcareInc
	sourceDef.Category = []string{"207QG0300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1497289110"}}
