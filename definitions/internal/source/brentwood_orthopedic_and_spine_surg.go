...
func GetSourceBrentwoodOrthopedicAndSpineSurg(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBrentwoodOrthopedicAndSpineSurg]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BRENTWOOD ORTHOPEDIC AND SPINE SURG"
	sourceDef.SourceType = pkg.SourceTypeBrentwoodOrthopedicAndSpineSurg
