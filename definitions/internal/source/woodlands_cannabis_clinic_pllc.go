...
func GetSourceWoodlandsCannabisClinicPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWoodlandsCannabisClinicPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WOODLANDS CANNABIS CLINIC PLLC"
	sourceDef.SourceType = pkg.SourceTypeWoodlandsCannabisClinicPllc
