...
func GetSourceNitinGuptaMdPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNitinGuptaMdPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NITIN GUPTA MD PC"
	sourceDef.SourceType = pkg.SourceTypeNitinGuptaMdPc
	sourceDef.Category = []string{"207RI0011X"}
	sourceDef.Aliases = []string{"NITIN GUPTA, M.D., P.C."}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1588385512"}}
