...
func GetSourceAdamsBoneAndJointPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdamsBoneAndJointPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ADAMS BONE AND JOINT PLLC"
	sourceDef.SourceType = pkg.SourceTypeAdamsBoneAndJointPllc
	sourceDef.Category = []string{"207X00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1497253934"}}
	sourceDef.PatientAccessUrl = "https://www.adamsboneandjoint.com/"
