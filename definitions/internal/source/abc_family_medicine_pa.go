...
func GetSourceAbcFamilyMedicinePa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAbcFamilyMedicinePa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ABC Family Medicine PA"
	sourceDef.SourceType = pkg.SourceTypeAbcFamilyMedicinePa
	sourceDef.Category = []string{"207QA0505X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1598970725"}}
	sourceDef.PatientAccessUrl = "http://www.abcfamilymedicine.com"
