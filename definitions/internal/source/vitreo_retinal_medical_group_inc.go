...
func GetSourceVitreoRetinalMedicalGroupInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVitreoRetinalMedicalGroupInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Vitreo-Retinal Medical Group Inc"
	sourceDef.SourceType = pkg.SourceTypeVitreoRetinalMedicalGroupInc
	sourceDef.Category = []string{"207W00000X", "207WX0107X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1144733403", "1740437920"}}
