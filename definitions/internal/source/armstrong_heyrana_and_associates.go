...
func GetSourceArmstrongHeyranaAndAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArmstrongHeyranaAndAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Armstrong Heyrana and Associates"
	sourceDef.SourceType = pkg.SourceTypeArmstrongHeyranaAndAssociates
