...
func GetSourceVictorsCrownChristianCounseling(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVictorsCrownChristianCounseling]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Victors Crown Christian Counseling"
	sourceDef.SourceType = pkg.SourceTypeVictorsCrownChristianCounseling
