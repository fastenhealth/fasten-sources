...
func GetSourceNextGenerationOrthopedicSpine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNextGenerationOrthopedicSpine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Next Generation Orthopedic  Spine"
	sourceDef.SourceType = pkg.SourceTypeNextGenerationOrthopedicSpine
