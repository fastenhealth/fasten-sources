...
func GetSourceWisconsin(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWisconsin]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Wisconsin"
	sourceDef.SourceType = pkg.SourceTypeWisconsin
	sourceDef.Category = []string{"208D00000X"}
	sourceDef.Aliases = []string{"WISCONSIN"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1821700188"}}
