...
func GetSourceHealthyTexanPediatricFamilyMe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHealthyTexanPediatricFamilyMe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Healthy Texan Pediatric  Family Me"
	sourceDef.SourceType = pkg.SourceTypeHealthyTexanPediatricFamilyMe
