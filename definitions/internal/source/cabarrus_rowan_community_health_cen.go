...
func GetSourceCabarrusRowanCommunityHealthCen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCabarrusRowanCommunityHealthCen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cabarrus Rowan Community Health Cen"
	sourceDef.SourceType = pkg.SourceTypeCabarrusRowanCommunityHealthCen
