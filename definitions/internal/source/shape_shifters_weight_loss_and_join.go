...
func GetSourceShapeShiftersWeightLossAndJoin(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShapeShiftersWeightLossAndJoin]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Shape Shifters Weight Loss and Join"
	sourceDef.SourceType = pkg.SourceTypeShapeShiftersWeightLossAndJoin
