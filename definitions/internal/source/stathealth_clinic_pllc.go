...
func GetSourceStathealthClinicPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStathealthClinicPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "STAThealth Clinic PLLC"
	sourceDef.SourceType = pkg.SourceTypeStathealthClinicPllc
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"STATHEALTH CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1912298845"}}
