...
func GetSourceUrologySpecialistGroupLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUrologySpecialistGroupLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Urology Specialist Group LLC"
	sourceDef.SourceType = pkg.SourceTypeUrologySpecialistGroupLlc
	sourceDef.Category = []string{"208800000X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1952854853"}}
	sourceDef.PatientAccessUrl = "https://www.besturologyclinic.com/contactus"
