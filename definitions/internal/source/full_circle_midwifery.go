...
func GetSourceFullCircleMidwifery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFullCircleMidwifery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FULL CIRCLE MIDWIFERY"
	sourceDef.SourceType = pkg.SourceTypeFullCircleMidwifery
	sourceDef.Category = []string{"164W00000X", "176B00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1417231135", "1477288132"}}
