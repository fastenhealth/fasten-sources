...
func GetSourceDesotoFootCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDesotoFootCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Desoto Foot Care"
	sourceDef.SourceType = pkg.SourceTypeDesotoFootCare
	sourceDef.Category = []string{"261QP1100X"}
	sourceDef.Aliases = []string{"DESOTO FOOT CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1073557799"}}
	sourceDef.PatientAccessUrl = "http://www.mydesotofootcare.com"
