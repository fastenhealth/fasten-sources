...
func GetSourceCenterForAdultMedicineHyperte(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForAdultMedicineHyperte]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center for Adult Medicine  Hyperte"
	sourceDef.SourceType = pkg.SourceTypeCenterForAdultMedicineHyperte
