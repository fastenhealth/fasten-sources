...
func GetSourceArkansasOrthopedicSurgeryAndWel(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArkansasOrthopedicSurgeryAndWel]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arkansas Orthopedic Surgery and Wel"
	sourceDef.SourceType = pkg.SourceTypeArkansasOrthopedicSurgeryAndWel
