...
func GetSourceHarmonyHealthMd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHarmonyHealthMd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HARMONY HEALTH MD"
	sourceDef.SourceType = pkg.SourceTypeHarmonyHealthMd
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"HARMONY HEALTH MD"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1245505833"}}
	sourceDef.PatientAccessUrl = "http://www.hhmd.com"
