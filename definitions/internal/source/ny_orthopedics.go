...
func GetSourceNyOrthopedics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNyOrthopedics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NY Orthopedics"
	sourceDef.SourceType = pkg.SourceTypeNyOrthopedics
	sourceDef.Category = []string{"207X00000X", "207XX0005X", "332B00000X"}
	sourceDef.Aliases = []string{"NY ORTHOPEDICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1083269047", "1124462627", "1144784851", "1609591023"}}
