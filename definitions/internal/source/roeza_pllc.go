...
func GetSourceRoezaPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRoezaPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Roeza PLLC"
	sourceDef.SourceType = pkg.SourceTypeRoezaPllc
	sourceDef.Category = []string{"363A00000X"}
	sourceDef.Aliases = []string{"SANTE CLINIC AND MED SPA"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1124521851"}}
