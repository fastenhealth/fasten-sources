...
func GetSourceWomensHealthOfStPetersburgLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWomensHealthOfStPetersburgLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WOMENS HEALTH OF ST PETERSBURG LLC"
	sourceDef.SourceType = pkg.SourceTypeWomensHealthOfStPetersburgLlc
