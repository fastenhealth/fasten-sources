...
func GetSourceProactiveRehabPhysicalTherapy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeProactiveRehabPhysicalTherapy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Proactive Rehab Physical Therapy "
	sourceDef.SourceType = pkg.SourceTypeProactiveRehabPhysicalTherapy
