...
func GetSourceClaraJacksonSchermerhornMdPhd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeClaraJacksonSchermerhornMdPhd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CLARA JACKSON SCHERMERHORN MD PHD"
	sourceDef.SourceType = pkg.SourceTypeClaraJacksonSchermerhornMdPhd
