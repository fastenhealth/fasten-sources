...
func GetSourceParkHudsonSurgeryCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeParkHudsonSurgeryCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Park Hudson Surgery Center"
	sourceDef.SourceType = pkg.SourceTypeParkHudsonSurgeryCenter
	sourceDef.BrandLogo = "park-hudson-surgery-center.jpg"
	sourceDef.PatientAccessUrl = "https://texasentandallergy.com/"
