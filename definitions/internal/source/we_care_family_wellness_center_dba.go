...
func GetSourceWeCareFamilyWellnessCenterDba(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWeCareFamilyWellnessCenterDba]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "We Care Family Wellness Center DBA"
	sourceDef.SourceType = pkg.SourceTypeWeCareFamilyWellnessCenterDba
