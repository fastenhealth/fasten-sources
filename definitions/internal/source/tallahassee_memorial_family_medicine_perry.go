...
func GetSourceTallahasseeMemorialFamilyMedicinePerry(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTallahasseeMemorialFamilyMedicinePerry]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tallahassee Memorial Family Medicine Perry"
	sourceDef.SourceType = pkg.SourceTypeTallahasseeMemorialFamilyMedicinePerry
