...
func GetSourcePavilionForWomensHealthAndWellnessLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePavilionForWomensHealthAndWellnessLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PAVILION FOR WOMENS HEALTH AND WELLNESS LLC"
	sourceDef.SourceType = pkg.SourceTypePavilionForWomensHealthAndWellnessLlc
