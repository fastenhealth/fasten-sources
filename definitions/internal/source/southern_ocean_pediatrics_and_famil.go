...
func GetSourceSouthernOceanPediatricsAndFamil(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernOceanPediatricsAndFamil]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southern Ocean Pediatrics and Famil"
	sourceDef.SourceType = pkg.SourceTypeSouthernOceanPediatricsAndFamil
