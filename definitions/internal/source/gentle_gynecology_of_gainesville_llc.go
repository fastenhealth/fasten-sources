...
func GetSourceGentleGynecologyOfGainesvilleLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGentleGynecologyOfGainesvilleLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GENTLE GYNECOLOGY OF GAINESVILLE LLC"
	sourceDef.SourceType = pkg.SourceTypeGentleGynecologyOfGainesvilleLlc
