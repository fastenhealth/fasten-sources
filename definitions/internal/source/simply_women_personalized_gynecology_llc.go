...
func GetSourceSimplyWomenPersonalizedGynecologyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSimplyWomenPersonalizedGynecologyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SIMPLY WOMEN PERSONALIZED GYNECOLOGY LLC"
	sourceDef.SourceType = pkg.SourceTypeSimplyWomenPersonalizedGynecologyLlc
