...
func GetSourceCenterForTheSurgicalArtsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForTheSurgicalArtsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CENTER FOR THE SURGICAL ARTS LLC"
	sourceDef.SourceType = pkg.SourceTypeCenterForTheSurgicalArtsLlc
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"CSA", "THE CENTER FOR THE SURGICAL ARTS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1801855127"}}
