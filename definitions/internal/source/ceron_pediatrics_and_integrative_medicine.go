...
func GetSourceCeronPediatricsAndIntegrativeMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCeronPediatricsAndIntegrativeMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ceron Pediatrics and Integrative Medicine"
	sourceDef.SourceType = pkg.SourceTypeCeronPediatricsAndIntegrativeMedicine
