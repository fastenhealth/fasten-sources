...
func GetSourceNursePractitionerServicesOfMich(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNursePractitionerServicesOfMich]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Nurse Practitioner Services of Mich"
	sourceDef.SourceType = pkg.SourceTypeNursePractitionerServicesOfMich
