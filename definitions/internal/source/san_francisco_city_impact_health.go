...
func GetSourceSanFranciscoCityImpactHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSanFranciscoCityImpactHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "San Francisco City Impact Health "
	sourceDef.SourceType = pkg.SourceTypeSanFranciscoCityImpactHealth
