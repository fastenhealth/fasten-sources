...
func GetSourceCharlesJKollerMdPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCharlesJKollerMdPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CHARLES J KOLLER MD PA"
	sourceDef.SourceType = pkg.SourceTypeCharlesJKollerMdPa
	sourceDef.Category = []string{"208600000X"}
	sourceDef.Aliases = []string{"CHARLES J KOLLER MD PA"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1558915868", "1689982183"}}
