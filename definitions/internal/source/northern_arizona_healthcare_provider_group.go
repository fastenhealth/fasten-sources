...
func GetSourceNorthernArizonaHealthcareProviderGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthernArizonaHealthcareProviderGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTHERN ARIZONA HEALTHCARE PROVIDER GROUP"
	sourceDef.SourceType = pkg.SourceTypeNorthernArizonaHealthcareProviderGroup
