...
func GetSourceCarolinaInterventionalPainInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCarolinaInterventionalPainInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Carolina Interventional Pain Institute"
	sourceDef.SourceType = pkg.SourceTypeCarolinaInterventionalPainInstitute
