...
func GetSourceSupremeAllCareHealthAndWellnes(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSupremeAllCareHealthAndWellnes]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SUPREME ALL CARE HEALTH AND WELLNES"
	sourceDef.SourceType = pkg.SourceTypeSupremeAllCareHealthAndWellnes
