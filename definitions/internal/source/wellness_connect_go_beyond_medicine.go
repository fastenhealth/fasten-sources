...
func GetSourceWellnessConnectGoBeyondMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWellnessConnectGoBeyondMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Wellness Connect Go Beyond Medicine"
	sourceDef.SourceType = pkg.SourceTypeWellnessConnectGoBeyondMedicine
