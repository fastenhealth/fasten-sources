...
func GetSourceLakeAreaMedicalAssociatesAServiceOfClevelandAreaHospital(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLakeAreaMedicalAssociatesAServiceOfClevelandAreaHospital]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lake Area Medical Associates a service of Cleveland Area Hospital"
	sourceDef.SourceType = pkg.SourceTypeLakeAreaMedicalAssociatesAServiceOfClevelandAreaHospital
