...
func GetSourceChsmartinGeneralHospitalClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChsmartinGeneralHospitalClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CHSMartin General Hospital Clinic"
	sourceDef.SourceType = pkg.SourceTypeChsmartinGeneralHospitalClinic
