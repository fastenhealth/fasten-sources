...
func GetSourceClaraAndrewsMdParkhillAffiliate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeClaraAndrewsMdParkhillAffiliate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Clara Andrews MD Parkhill Affiliate"
	sourceDef.SourceType = pkg.SourceTypeClaraAndrewsMdParkhillAffiliate
