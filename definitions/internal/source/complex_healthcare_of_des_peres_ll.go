...
func GetSourceComplexHealthcareOfDesPeresLl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeComplexHealthcareOfDesPeresLl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COMPLEX HEALTHCARE OF DES PERES LL"
	sourceDef.SourceType = pkg.SourceTypeComplexHealthcareOfDesPeresLl
