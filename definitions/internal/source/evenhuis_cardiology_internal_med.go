...
func GetSourceEvenhuisCardiologyInternalMed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEvenhuisCardiologyInternalMed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Evenhuis Cardiology  Internal Med"
	sourceDef.SourceType = pkg.SourceTypeEvenhuisCardiologyInternalMed
