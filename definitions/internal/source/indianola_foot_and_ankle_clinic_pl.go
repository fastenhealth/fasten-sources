...
func GetSourceIndianolaFootAndAnkleClinicPl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIndianolaFootAndAnkleClinicPl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "INDIANOLA FOOT AND ANKLE CLINIC PL"
	sourceDef.SourceType = pkg.SourceTypeIndianolaFootAndAnkleClinicPl
