...
func GetSourceNewBeginningsHealthcareForWomen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNewBeginningsHealthcareForWomen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEW BEGINNINGS HEALTHCARE FOR WOMEN"
	sourceDef.SourceType = pkg.SourceTypeNewBeginningsHealthcareForWomen
