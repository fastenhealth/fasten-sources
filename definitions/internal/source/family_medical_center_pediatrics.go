...
func GetSourceFamilyMedicalCenterPediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFamilyMedicalCenterPediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Family Medical Center Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeFamilyMedicalCenterPediatrics
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{"FAMILY MEDICAL CENTER PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1932363629"}}
