...
func GetSourcePhysiciansRegionalMedicalGroupPharmacy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePhysiciansRegionalMedicalGroupPharmacy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Physicians Regional Medical Group Pharmacy"
	sourceDef.SourceType = pkg.SourceTypePhysiciansRegionalMedicalGroupPharmacy
