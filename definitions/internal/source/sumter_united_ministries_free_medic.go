...
func GetSourceSumterUnitedMinistriesFreeMedic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSumterUnitedMinistriesFreeMedic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sumter United Ministries Free Medic"
	sourceDef.SourceType = pkg.SourceTypeSumterUnitedMinistriesFreeMedic
