...
func GetSourceUtahNavajoHealthSystem(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUtahNavajoHealthSystem]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Utah Navajo Health System"
	sourceDef.SourceType = pkg.SourceTypeUtahNavajoHealthSystem
	sourceDef.Category = []string{"3336C0003X"}
	sourceDef.Aliases = []string{"BLANDING FAMILY PHARMACY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1386870822"}}
