...
func GetSourceWestSuburbanMidwifeAssociatesL(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWestSuburbanMidwifeAssociatesL]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WEST SUBURBAN MIDWIFE ASSOCIATES L"
	sourceDef.SourceType = pkg.SourceTypeWestSuburbanMidwifeAssociatesL
