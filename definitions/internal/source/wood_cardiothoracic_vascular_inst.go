...
func GetSourceWoodCardiothoracicVascularInst(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWoodCardiothoracicVascularInst]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Wood Cardiothoracic  Vascular Inst"
	sourceDef.SourceType = pkg.SourceTypeWoodCardiothoracicVascularInst
