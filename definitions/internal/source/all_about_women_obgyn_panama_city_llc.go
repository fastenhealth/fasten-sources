...
func GetSourceAllAboutWomenObgynPanamaCityLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllAboutWomenObgynPanamaCityLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ALL ABOUT WOMEN OBGYN PANAMA CITY LLC"
	sourceDef.SourceType = pkg.SourceTypeAllAboutWomenObgynPanamaCityLlc
