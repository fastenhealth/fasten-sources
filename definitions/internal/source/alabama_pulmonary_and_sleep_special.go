...
func GetSourceAlabamaPulmonaryAndSleepSpecial(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlabamaPulmonaryAndSleepSpecial]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Alabama Pulmonary and Sleep Special"
	sourceDef.SourceType = pkg.SourceTypeAlabamaPulmonaryAndSleepSpecial
