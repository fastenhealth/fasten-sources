...
func GetSourceInterventionalPainSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInterventionalPainSpecialists]; clientIdOk {
	...
	sourceDef.Category = []string{"363L00000X", "363LF0000X", "208VP0014X", "207LP2900X"}
	sourceDef.Aliases = []string{"INTERVENTIONAL PAIN SPECIALISTS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1174528392"}}
