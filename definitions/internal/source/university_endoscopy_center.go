...
func GetSourceUniversityEndoscopyCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUniversityEndoscopyCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "University Endoscopy Center"
	sourceDef.SourceType = pkg.SourceTypeUniversityEndoscopyCenter
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"UNIVERSITY ENDOSCOPY CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1780656801"}}
