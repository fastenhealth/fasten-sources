...
func GetSourceSanthakumarNeurologyAssociatesPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSanthakumarNeurologyAssociatesPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Santhakumar Neurology Associates PC"
	sourceDef.SourceType = pkg.SourceTypeSanthakumarNeurologyAssociatesPc
