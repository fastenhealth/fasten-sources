...
func GetSourceHoustonKidneySpecialistsCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHoustonKidneySpecialistsCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Houston Kidney Specialists Center"
	sourceDef.SourceType = pkg.SourceTypeHoustonKidneySpecialistsCenter
