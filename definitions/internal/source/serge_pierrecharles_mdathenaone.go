...
func GetSourceSergePierrecharlesMdathenaone(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSergePierrecharlesMdathenaone]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Serge PierreCharles MDathenaOne"
	sourceDef.SourceType = pkg.SourceTypeSergePierrecharlesMdathenaone
