...
func GetSourceNortheastGeorgiaSurgeryCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNortheastGeorgiaSurgeryCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTHEAST GEORGIA SURGERY CENTER LLC"
	sourceDef.SourceType = pkg.SourceTypeNortheastGeorgiaSurgeryCenterLlc
