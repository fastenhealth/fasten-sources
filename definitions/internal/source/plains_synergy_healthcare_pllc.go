...
func GetSourcePlainsSynergyHealthcarePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePlainsSynergyHealthcarePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Plains Synergy Healthcare Pllc"
	sourceDef.SourceType = pkg.SourceTypePlainsSynergyHealthcarePllc
	sourceDef.Category = []string{"207Q00000X", "2084P0800X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1013119171"}}
