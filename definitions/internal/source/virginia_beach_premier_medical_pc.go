...
func GetSourceVirginiaBeachPremierMedicalPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVirginiaBeachPremierMedicalPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Virginia Beach Premier Medical PC"
	sourceDef.SourceType = pkg.SourceTypeVirginiaBeachPremierMedicalPc
