...
func GetSourceDrPritEFamilyCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDrPritEFamilyCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Dr Prit E Family Care"
	sourceDef.SourceType = pkg.SourceTypeDrPritEFamilyCare
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"DR PRIT E FAMILY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1467046870"}}
	sourceDef.PatientAccessUrl = "http://www.drpritefamilycare.com"
