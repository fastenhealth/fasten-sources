...
func GetSourceTheCenterForRuralHealthInnovat(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheCenterForRuralHealthInnovat]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Center for Rural Health Innovat"
	sourceDef.SourceType = pkg.SourceTypeTheCenterForRuralHealthInnovat
