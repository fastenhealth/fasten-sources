...
func GetSourceHooverPodiatry(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHooverPodiatry]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hoover Podiatry"
	sourceDef.SourceType = pkg.SourceTypeHooverPodiatry
	sourceDef.Category = []string{"213ES0103X"}
	sourceDef.Aliases = []string{"HOOVER PODIATRY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1801312442"}}
	sourceDef.PatientAccessUrl = "http://www.hooverpodiatry.com"
