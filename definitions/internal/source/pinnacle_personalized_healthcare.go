...
func GetSourcePinnaclePersonalizedHealthcare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePinnaclePersonalizedHealthcare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pinnacle Personalized Healthcare"
	sourceDef.SourceType = pkg.SourceTypePinnaclePersonalizedHealthcare
