...
func GetSourceCovenantAdvancedPrimaryCareSolu(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCovenantAdvancedPrimaryCareSolu]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Covenant Advanced Primary Care Solu"
	sourceDef.SourceType = pkg.SourceTypeCovenantAdvancedPrimaryCareSolu
