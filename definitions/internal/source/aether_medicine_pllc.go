...
func GetSourceAetherMedicinePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAetherMedicinePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Aether Medicine PLLC"
	sourceDef.SourceType = pkg.SourceTypeAetherMedicinePllc
	sourceDef.Category = []string{"208100000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1659973832"}}
	sourceDef.PatientAccessUrl = "http://www.aethermedicine.com"
