...
func GetSourceTheRightLab(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheRightLab]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Right Lab"
	sourceDef.SourceType = pkg.SourceTypeTheRightLab
	sourceDef.Category = []string{"374700000X"}
	sourceDef.Aliases = []string{"THE RIGHT LAB"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1265128623"}}
