...
func GetSourceNorthOkaloosaPhysicianGroupSurgerySpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthOkaloosaPhysicianGroupSurgerySpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Okaloosa Physician Group  Surgery Specialists"
	sourceDef.SourceType = pkg.SourceTypeNorthOkaloosaPhysicianGroupSurgerySpecialists
