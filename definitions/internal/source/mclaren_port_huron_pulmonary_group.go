...
func GetSourceMclarenPortHuronPulmonaryGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMclarenPortHuronPulmonaryGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "McLaren Port Huron Pulmonary Group"
	sourceDef.SourceType = pkg.SourceTypeMclarenPortHuronPulmonaryGroup
