...
func GetSourceLakeNormanNeurologicalSpinalSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLakeNormanNeurologicalSpinalSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lake Norman Neurological  Spinal Surgery"
	sourceDef.SourceType = pkg.SourceTypeLakeNormanNeurologicalSpinalSurgery
