...
func GetSourceCaremoreMedicalGroupOfNevada(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCaremoreMedicalGroupOfNevada]; clientIdOk {
	...
	sourceDef.Aliases = []string{"CAREMORE MEDICAL GROUP OF NEVADA", "CAREMORE MEDICAL GROUP OF NEVADA (WORKU), PC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1578840724"}}
	sourceDef.BrandLogo = "caremore-medical-group-of-nevada.svg"
