...
func GetSourceTriggCountyOrthopedicFamilyMedi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTriggCountyOrthopedicFamilyMedi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "TRIGG COUNTY ORTHOPEDIC FAMILY MEDI"
	sourceDef.SourceType = pkg.SourceTypeTriggCountyOrthopedicFamilyMedi
