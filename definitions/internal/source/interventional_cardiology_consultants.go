...
func GetSourceInterventionalCardiologyConsultants(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInterventionalCardiologyConsultants]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Interventional Cardiology Consultants"
	sourceDef.SourceType = pkg.SourceTypeInterventionalCardiologyConsultants
	sourceDef.PatientAccessUrl = "https://www.mapquest.com/us/georgia/interventional-cardiology-consultants-372747748"
