...
func GetSourcePalmBeachInternalMedicineLongevityClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePalmBeachInternalMedicineLongevityClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Palm Beach Internal Medicine  Longevity Clinic"
	sourceDef.SourceType = pkg.SourceTypePalmBeachInternalMedicineLongevityClinic
