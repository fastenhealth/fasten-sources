...
func GetSourceJacksonOrthopedicSpineCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJacksonOrthopedicSpineCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Jackson Orthopedic  Spine Center"
	sourceDef.SourceType = pkg.SourceTypeJacksonOrthopedicSpineCenter
