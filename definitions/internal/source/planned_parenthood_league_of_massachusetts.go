...
func GetSourcePlannedParenthoodLeagueOfMassachusetts(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePlannedParenthoodLeagueOfMassachusetts]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Planned Parenthood League of Massachusetts"
	sourceDef.SourceType = pkg.SourceTypePlannedParenthoodLeagueOfMassachusetts
	sourceDef.Category = []string{"261QA0005X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1578693636", "1598829335"}}
