...
func GetSourceVarsityOrthopedicsParkhillAffiliate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVarsityOrthopedicsParkhillAffiliate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Varsity Orthopedics ParkHill Affiliate"
	sourceDef.SourceType = pkg.SourceTypeVarsityOrthopedicsParkhillAffiliate
