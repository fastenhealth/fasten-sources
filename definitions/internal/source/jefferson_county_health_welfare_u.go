...
func GetSourceJeffersonCountyHealthWelfareU(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJeffersonCountyHealthWelfareU]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Jefferson County Health  Welfare U"
	sourceDef.SourceType = pkg.SourceTypeJeffersonCountyHealthWelfareU
