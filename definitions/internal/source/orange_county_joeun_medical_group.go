...
func GetSourceOrangeCountyJoeunMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOrangeCountyJoeunMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ORANGE COUNTY JOEUN MEDICAL GROUP"
	sourceDef.SourceType = pkg.SourceTypeOrangeCountyJoeunMedicalGroup
