...
func GetSourceNephrologyPhysiciansLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNephrologyPhysiciansLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Nephrology Physicians LLC"
	sourceDef.SourceType = pkg.SourceTypeNephrologyPhysiciansLlc
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1003357377"}}
	sourceDef.PatientAccessUrl = "https://www.facebook.com/MichianaRegionalVascularCenter/"
