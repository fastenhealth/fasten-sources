...
func GetSourceBreakthroughRegenerativeOrtho(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBreakthroughRegenerativeOrtho]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Breakthrough Regenerative Ortho"
	sourceDef.SourceType = pkg.SourceTypeBreakthroughRegenerativeOrtho
