...
func GetSourceSchuylkillEndoscopyCenterAnes(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSchuylkillEndoscopyCenterAnes]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Schuylkill Endoscopy Center Anes"
	sourceDef.SourceType = pkg.SourceTypeSchuylkillEndoscopyCenterAnes
