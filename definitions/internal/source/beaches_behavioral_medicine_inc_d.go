...
func GetSourceBeachesBehavioralMedicineIncD(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBeachesBehavioralMedicineIncD]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BEACHES BEHAVIORAL MEDICINE INC D"
	sourceDef.SourceType = pkg.SourceTypeBeachesBehavioralMedicineIncD
