...
func GetSourceCenterForOrthopaedicsSportsMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForOrthopaedicsSportsMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CENTER FOR ORTHOPAEDICS  SPORTS MEDICINE"
	sourceDef.SourceType = pkg.SourceTypeCenterForOrthopaedicsSportsMedicine
