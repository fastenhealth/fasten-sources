...
func GetSourceNeighborhoodMedicalWalkinClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeighborhoodMedicalWalkinClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Neighborhood Medical Walkin Clinic"
	sourceDef.SourceType = pkg.SourceTypeNeighborhoodMedicalWalkinClinic
