...
func GetSourceCentralArkansasCardiovascularInstitutePa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralArkansasCardiovascularInstitutePa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CENTRAL ARKANSAS CARDIOVASCULAR INSTITUTE PA"
	sourceDef.SourceType = pkg.SourceTypeCentralArkansasCardiovascularInstitutePa
