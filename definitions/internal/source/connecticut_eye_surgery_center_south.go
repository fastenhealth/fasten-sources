...
func GetSourceConnecticutEyeSurgeryCenterSouth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeConnecticutEyeSurgeryCenterSouth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Connecticut Eye Surgery Center South"
	sourceDef.SourceType = pkg.SourceTypeConnecticutEyeSurgeryCenterSouth
