...
func GetSourceSurgicalAssistantsOfDallasSuppliesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSurgicalAssistantsOfDallasSuppliesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SURGICAL ASSISTANTS OF DALLAS SUPPLIES LLC"
	sourceDef.SourceType = pkg.SourceTypeSurgicalAssistantsOfDallasSuppliesLlc
