...
func GetSourceCc020FriscoUnifiedWomensHealthPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc020FriscoUnifiedWomensHealthPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC020FRISCO UNIFIED WOMENS HEALTH PLLC"
	sourceDef.SourceType = pkg.SourceTypeCc020FriscoUnifiedWomensHealthPllc
