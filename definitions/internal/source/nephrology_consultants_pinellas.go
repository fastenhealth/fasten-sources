...
func GetSourceNephrologyConsultantsPinellas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNephrologyConsultantsPinellas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Nephrology Consultants Pinellas"
	sourceDef.SourceType = pkg.SourceTypeNephrologyConsultantsPinellas
