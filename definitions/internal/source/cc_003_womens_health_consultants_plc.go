...
func GetSourceCc003WomensHealthConsultantsPlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc003WomensHealthConsultantsPlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC003WOMENS HEALTH CONSULTANTS PLC"
	sourceDef.SourceType = pkg.SourceTypeCc003WomensHealthConsultantsPlc
