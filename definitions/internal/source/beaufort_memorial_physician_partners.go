...
func GetSourceBeaufortMemorialPhysicianPartners(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBeaufortMemorialPhysicianPartners]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Beaufort Memorial Physician Partners"
	sourceDef.SourceType = pkg.SourceTypeBeaufortMemorialPhysicianPartners
