...
func GetSourceHrcBehavioralHealthPsychiatry(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHrcBehavioralHealthPsychiatry]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HRC Behavioral Health  Psychiatry"
	sourceDef.SourceType = pkg.SourceTypeHrcBehavioralHealthPsychiatry
