...
func GetSourceGulfCoastEndoscopyCenterSouth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGulfCoastEndoscopyCenterSouth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Gulf Coast Endoscopy Center South"
	sourceDef.SourceType = pkg.SourceTypeGulfCoastEndoscopyCenterSouth
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"GULF COAST ENDOSCOPY CENTER SOUTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1992777643"}}
