...
func GetSourceRapidCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRapidCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Rapid Care"
	sourceDef.SourceType = pkg.SourceTypeRapidCare
	sourceDef.Category = []string{"261Q00000X", "261QR1300X", "261QU0200X"}
	sourceDef.Aliases = []string{"RAPID CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1265918668", "1609443167", "1710561261"}}
