...
func GetSourceBigHornBasinFamilyCounseling(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBigHornBasinFamilyCounseling]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Big Horn Basin Family Counseling"
	sourceDef.SourceType = pkg.SourceTypeBigHornBasinFamilyCounseling
