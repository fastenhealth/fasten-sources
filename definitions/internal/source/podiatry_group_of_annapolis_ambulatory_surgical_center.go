...
func GetSourcePodiatryGroupOfAnnapolisAmbulatorySurgicalCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePodiatryGroupOfAnnapolisAmbulatorySurgicalCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Podiatry Group of Annapolis Ambulatory Surgical Center"
	sourceDef.SourceType = pkg.SourceTypePodiatryGroupOfAnnapolisAmbulatorySurgicalCenter
