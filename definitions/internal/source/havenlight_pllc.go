...
func GetSourceHavenlightPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHavenlightPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HAVENLIGHT PLLC"
	sourceDef.SourceType = pkg.SourceTypeHavenlightPllc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"HAVENLIGHT HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1568042513"}}
