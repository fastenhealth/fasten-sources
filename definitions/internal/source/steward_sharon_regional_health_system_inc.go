...
func GetSourceStewardSharonRegionalHealthSystemInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStewardSharonRegionalHealthSystemInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "STEWARD SHARON REGIONAL HEALTH SYSTEM INC"
	sourceDef.SourceType = pkg.SourceTypeStewardSharonRegionalHealthSystemInc
