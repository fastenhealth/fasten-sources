...
func GetSourceCedarParkNeurologyAndSleepMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCedarParkNeurologyAndSleepMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cedar Park Neurology and Sleep Medicine"
	sourceDef.SourceType = pkg.SourceTypeCedarParkNeurologyAndSleepMedicine
