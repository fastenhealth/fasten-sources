...
func GetSourceDermatologySkinSurgeryAssociat(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDermatologySkinSurgeryAssociat]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Dermatology  Skin Surgery Associat"
	sourceDef.SourceType = pkg.SourceTypeDermatologySkinSurgeryAssociat
