...
func GetSourceComprehensivePainAndSpineAssoci(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeComprehensivePainAndSpineAssoci]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Comprehensive Pain and Spine Associ"
	sourceDef.SourceType = pkg.SourceTypeComprehensivePainAndSpineAssoci
