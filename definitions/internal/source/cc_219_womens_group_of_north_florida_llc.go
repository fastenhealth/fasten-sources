...
func GetSourceCc219WomensGroupOfNorthFloridaLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc219WomensGroupOfNorthFloridaLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC219WOMENS GROUP OF NORTH FLORIDA LLC"
	sourceDef.SourceType = pkg.SourceTypeCc219WomensGroupOfNorthFloridaLlc
