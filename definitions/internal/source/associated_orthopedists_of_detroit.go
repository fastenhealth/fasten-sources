...
func GetSourceAssociatedOrthopedistsOfDetroit(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAssociatedOrthopedistsOfDetroit]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Associated Orthopedists of Detroit"
	sourceDef.SourceType = pkg.SourceTypeAssociatedOrthopedistsOfDetroit
