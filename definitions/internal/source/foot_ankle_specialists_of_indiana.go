...
func GetSourceFootAnkleSpecialistsOfIndiana(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFootAnkleSpecialistsOfIndiana]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FOOT  ANKLE SPECIALISTS OF INDIANA"
	sourceDef.SourceType = pkg.SourceTypeFootAnkleSpecialistsOfIndiana
