...
func GetSourceChippenhamHospitalPediatricsEmergency(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChippenhamHospitalPediatricsEmergency]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Chippenham Hospital Pediatrics Emergency"
	sourceDef.SourceType = pkg.SourceTypeChippenhamHospitalPediatricsEmergency
