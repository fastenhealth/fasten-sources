...
func GetSourceUpstateLungAndCriticalCareSpecialistsPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUpstateLungAndCriticalCareSpecialistsPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Upstate Lung and Critical Care Specialists PC"
	sourceDef.SourceType = pkg.SourceTypeUpstateLungAndCriticalCareSpecialistsPc
	sourceDef.Category = []string{"207RP1001X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1013963982", "1033141098"}}
