...
func GetSourceCc020HarrisonburgObGynAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc020HarrisonburgObGynAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC020HARRISONBURG OB GYN ASSOCIATES"
	sourceDef.SourceType = pkg.SourceTypeCc020HarrisonburgObGynAssociates
