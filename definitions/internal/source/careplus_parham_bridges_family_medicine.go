...
func GetSourceCareplusParhamBridgesFamilyMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCareplusParhamBridgesFamilyMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CarePlus Parham Bridges Family Medicine"
	sourceDef.SourceType = pkg.SourceTypeCareplusParhamBridgesFamilyMedicine
