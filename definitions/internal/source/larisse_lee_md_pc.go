...
func GetSourceLarisseLeeMdPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLarisseLeeMdPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Larisse Lee MD PC"
	sourceDef.SourceType = pkg.SourceTypeLarisseLeeMdPc
	sourceDef.Category = []string{"2086S0129X"}
	sourceDef.Aliases = []string{"L.A. VEIN CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1376811406"}}
	sourceDef.PatientAccessUrl = "http://www.laveincenter.com/"
