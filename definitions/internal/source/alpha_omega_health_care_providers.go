...
func GetSourceAlphaOmegaHealthCareProviders(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlphaOmegaHealthCareProviders]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Alpha  Omega Health Care Providers"
	sourceDef.SourceType = pkg.SourceTypeAlphaOmegaHealthCareProviders
