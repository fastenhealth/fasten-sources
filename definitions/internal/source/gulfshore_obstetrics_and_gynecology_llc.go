...
func GetSourceGulfshoreObstetricsAndGynecologyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGulfshoreObstetricsAndGynecologyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GULFSHORE OBSTETRICS AND GYNECOLOGY LLC"
	sourceDef.SourceType = pkg.SourceTypeGulfshoreObstetricsAndGynecologyLlc
