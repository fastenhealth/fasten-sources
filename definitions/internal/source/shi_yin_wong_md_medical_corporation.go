...
func GetSourceShiYinWongMdMedicalCorporation(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShiYinWongMdMedicalCorporation]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Shi Yin Wong MD Medical Corporation"
	sourceDef.SourceType = pkg.SourceTypeShiYinWongMdMedicalCorporation
