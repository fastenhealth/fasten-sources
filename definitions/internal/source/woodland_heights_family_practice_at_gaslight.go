...
func GetSourceWoodlandHeightsFamilyPracticeAtGaslight(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWoodlandHeightsFamilyPracticeAtGaslight]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Woodland Heights Family Practice at Gaslight"
	sourceDef.SourceType = pkg.SourceTypeWoodlandHeightsFamilyPracticeAtGaslight
