...
func GetSourceFederalWayNaturopathy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFederalWayNaturopathy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Federal Way Naturopathy"
	sourceDef.SourceType = pkg.SourceTypeFederalWayNaturopathy
	sourceDef.Category = []string{"175F00000X"}
	sourceDef.Aliases = []string{"FEDERAL WAY NATUROPATHY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1467024117"}}
