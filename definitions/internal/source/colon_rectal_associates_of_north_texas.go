...
func GetSourceColonRectalAssociatesOfNorthTexas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeColonRectalAssociatesOfNorthTexas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COLON  RECTAL ASSOCIATES OF NORTH TEXAS"
	sourceDef.SourceType = pkg.SourceTypeColonRectalAssociatesOfNorthTexas
