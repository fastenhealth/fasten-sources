...
func GetSourceAsthmaallergyCenterNorthernNj(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAsthmaallergyCenterNorthernNj]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AsthmaAllergy Center Northern NJ"
	sourceDef.SourceType = pkg.SourceTypeAsthmaallergyCenterNorthernNj
