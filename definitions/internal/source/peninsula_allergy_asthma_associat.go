...
func GetSourcePeninsulaAllergyAsthmaAssociat(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePeninsulaAllergyAsthmaAssociat]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Peninsula Allergy  Asthma Associat"
	sourceDef.SourceType = pkg.SourceTypePeninsulaAllergyAsthmaAssociat
