...
func GetSourceConsanoHealthInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeConsanoHealthInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CONSANO HEALTH INC"
	sourceDef.SourceType = pkg.SourceTypeConsanoHealthInc
	sourceDef.Category = []string{"251B00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1184391831"}}
	sourceDef.PatientAccessUrl = "http://Www.consanohealth.com"
