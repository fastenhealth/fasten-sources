...
func GetSourceNextgenMedicalPractice(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNextgenMedicalPractice]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "NextGen Medical Practice"
	sourceDef.SourceType = pkg.SourceTypeNextgenMedicalPractice
	sourceDef.BrandLogo = "nextgen-medical-practice.svg"
