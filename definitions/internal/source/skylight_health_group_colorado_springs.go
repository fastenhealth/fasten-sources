...
func GetSourceSkylightHealthGroupColoradoSprings(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSkylightHealthGroupColoradoSprings]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Skylight Health Group  Colorado Springs"
	sourceDef.SourceType = pkg.SourceTypeSkylightHealthGroupColoradoSprings
