...
func GetSourcePhoenixMedicalWellnessClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePhoenixMedicalWellnessClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PHOENIX MEDICAL  WELLNESS CLINIC"
	sourceDef.SourceType = pkg.SourceTypePhoenixMedicalWellnessClinic
