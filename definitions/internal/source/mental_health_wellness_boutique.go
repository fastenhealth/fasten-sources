...
func GetSourceMentalHealthWellnessBoutique(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMentalHealthWellnessBoutique]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Mental Health Wellness Boutique"
	sourceDef.SourceType = pkg.SourceTypeMentalHealthWellnessBoutique
	sourceDef.Category = []string{"261QM0801X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1972266757"}}
	sourceDef.PatientAccessUrl = "http://www.mentalwellnessboutique.com"
