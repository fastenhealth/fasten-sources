...
func GetSourceCenterForAdolescentAndChildNeu(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForAdolescentAndChildNeu]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center for Adolescent and Child Neu"
	sourceDef.SourceType = pkg.SourceTypeCenterForAdolescentAndChildNeu
