...
func GetSourceHomevilleMedicalCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHomevilleMedicalCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Homeville Medical Care"
	sourceDef.SourceType = pkg.SourceTypeHomevilleMedicalCare
	sourceDef.Category = []string{"207QA0505X"}
	sourceDef.Aliases = []string{"HOMEVILLE MEDICAL CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1780207910"}}
