...
func GetSourceUpperlineHealthVascularCareGeorgia(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUpperlineHealthVascularCareGeorgia]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Upperline Health Vascular Care Georgia"
	sourceDef.SourceType = pkg.SourceTypeUpperlineHealthVascularCareGeorgia
