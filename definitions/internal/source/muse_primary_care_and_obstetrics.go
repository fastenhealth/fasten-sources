...
func GetSourceMusePrimaryCareAndObstetrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMusePrimaryCareAndObstetrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Muse Primary Care and Obstetrics"
	sourceDef.SourceType = pkg.SourceTypeMusePrimaryCareAndObstetrics
