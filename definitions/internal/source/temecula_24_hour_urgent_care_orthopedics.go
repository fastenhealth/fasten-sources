...
func GetSourceTemecula24HourUrgentCareOrthopedics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTemecula24HourUrgentCareOrthopedics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Temecula 24 Hour Urgent Care  Orthopedics"
	sourceDef.SourceType = pkg.SourceTypeTemecula24HourUrgentCareOrthopedics
