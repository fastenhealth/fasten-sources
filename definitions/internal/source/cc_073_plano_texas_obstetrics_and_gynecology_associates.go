...
func GetSourceCc073PlanoTexasObstetricsAndGynecologyAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc073PlanoTexasObstetricsAndGynecologyAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC073PLANO TEXAS OBSTETRICS AND GYNECOLOGY ASSOCIATES"
	sourceDef.SourceType = pkg.SourceTypeCc073PlanoTexasObstetricsAndGynecologyAssociates
