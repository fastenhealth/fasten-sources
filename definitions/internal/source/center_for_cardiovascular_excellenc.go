...
func GetSourceCenterForCardiovascularExcellenc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForCardiovascularExcellenc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center for Cardiovascular Excellenc"
	sourceDef.SourceType = pkg.SourceTypeCenterForCardiovascularExcellenc
