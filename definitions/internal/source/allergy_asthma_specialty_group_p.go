...
func GetSourceAllergyAsthmaSpecialtyGroupP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllergyAsthmaSpecialtyGroupP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Allergy  Asthma Specialty Group P"
	sourceDef.SourceType = pkg.SourceTypeAllergyAsthmaSpecialtyGroupP
