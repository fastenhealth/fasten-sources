...
func GetSourceWoodlandHeightsPrimaryCareAtDibol(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWoodlandHeightsPrimaryCareAtDibol]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Woodland Heights Primary Care at Dibol"
	sourceDef.SourceType = pkg.SourceTypeWoodlandHeightsPrimaryCareAtDibol
