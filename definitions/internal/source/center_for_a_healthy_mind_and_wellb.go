...
func GetSourceCenterForAHealthyMindAndWellb(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForAHealthyMindAndWellb]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center for a Healthy Mind and Wellb"
	sourceDef.SourceType = pkg.SourceTypeCenterForAHealthyMindAndWellb
