...
func GetSourceCatchingJoyMidwifery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCatchingJoyMidwifery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Catching Joy Midwifery"
	sourceDef.SourceType = pkg.SourceTypeCatchingJoyMidwifery
	sourceDef.Category = []string{"367A00000X"}
	sourceDef.Aliases = []string{"CATCHING JOY MIDWIFERY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1700551272"}}
	sourceDef.PatientAccessUrl = "http://www.catchingjoy.net"
