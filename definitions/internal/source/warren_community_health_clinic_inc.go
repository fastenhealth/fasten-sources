...
func GetSourceWarrenCommunityHealthClinicInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWarrenCommunityHealthClinicInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Warren Community Health Clinic Inc"
	sourceDef.SourceType = pkg.SourceTypeWarrenCommunityHealthClinicInc
