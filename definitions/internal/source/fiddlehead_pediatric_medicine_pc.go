...
func GetSourceFiddleheadPediatricMedicinePc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFiddleheadPediatricMedicinePc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Fiddlehead Pediatric Medicine PC"
	sourceDef.SourceType = pkg.SourceTypeFiddleheadPediatricMedicinePc
