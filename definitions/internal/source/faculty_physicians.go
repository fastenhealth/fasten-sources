...
func GetSourceFacultyPhysicians(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFacultyPhysicians]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Faculty Physicians"
	sourceDef.SourceType = pkg.SourceTypeFacultyPhysicians
	sourceDef.Category = []string{"213ES0103X"}
	sourceDef.Aliases = []string{"FACULTY PHYSICIANS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1134612245"}}
