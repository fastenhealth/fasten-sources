...
func GetSourceNewHealthPainTreatmentCenters(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNewHealthPainTreatmentCenters]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "New Health Pain Treatment Centers"
	sourceDef.SourceType = pkg.SourceTypeNewHealthPainTreatmentCenters
