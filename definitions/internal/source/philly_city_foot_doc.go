...
func GetSourcePhillyCityFootDoc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePhillyCityFootDoc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Philly City Foot Doc"
	sourceDef.SourceType = pkg.SourceTypePhillyCityFootDoc
	sourceDef.Category = []string{"213E00000X"}
	sourceDef.Aliases = []string{"PHILLY CITY FOOT DOC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1912543109"}}
