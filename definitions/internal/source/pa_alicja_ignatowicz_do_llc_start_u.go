...
func GetSourcePaAlicjaIgnatowiczDoLlcStartU(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePaAlicjaIgnatowiczDoLlcStartU]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PA  ALICJA IGNATOWICZ DO LLC  Start U"
	sourceDef.SourceType = pkg.SourceTypePaAlicjaIgnatowiczDoLlcStartU
