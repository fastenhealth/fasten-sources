...
func GetSourceCapeAndIslandsEndoscopyCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCapeAndIslandsEndoscopyCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CAPE AND ISLANDS ENDOSCOPY CENTER LLC"
	sourceDef.SourceType = pkg.SourceTypeCapeAndIslandsEndoscopyCenterLlc
	sourceDef.Category = []string{"261QE0800X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1316094824"}}
