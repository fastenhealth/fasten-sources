...
func GetSourceHoosPediatricAdolescentCarePl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHoosPediatricAdolescentCarePl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HOOS PEDIATRIC  ADOLESCENT CARE PL"
	sourceDef.SourceType = pkg.SourceTypeHoosPediatricAdolescentCarePl
