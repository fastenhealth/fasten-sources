...
func GetSourceAakaishHealthcareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAakaishHealthcareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Aakaish Healthcare Llc"
	sourceDef.SourceType = pkg.SourceTypeAakaishHealthcareLlc
	sourceDef.Category = []string{"207Q00000X", "208D00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1558746289"}}
	sourceDef.PatientAccessUrl = "https://www.cthousedoctor.com/"
