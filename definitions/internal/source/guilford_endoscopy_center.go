...
func GetSourceGuilfordEndoscopyCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGuilfordEndoscopyCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Guilford Endoscopy Center"
	sourceDef.SourceType = pkg.SourceTypeGuilfordEndoscopyCenter
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"GUILFORD ENDOSCOPY CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1811057029"}}
