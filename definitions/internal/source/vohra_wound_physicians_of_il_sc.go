...
func GetSourceVohraWoundPhysiciansOfIlSc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVohraWoundPhysiciansOfIlSc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VOHRA WOUND PHYSICIANS OF IL SC"
	sourceDef.SourceType = pkg.SourceTypeVohraWoundPhysiciansOfIlSc
