...
func GetSourceNeighborhoodResilienceProject(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeighborhoodResilienceProject]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Neighborhood Resilience Project"
	sourceDef.SourceType = pkg.SourceTypeNeighborhoodResilienceProject
