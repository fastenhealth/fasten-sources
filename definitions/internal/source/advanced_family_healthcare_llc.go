...
func GetSourceAdvancedFamilyHealthcareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedFamilyHealthcareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ADVANCED FAMILY HEALTHCARE LLC"
	sourceDef.SourceType = pkg.SourceTypeAdvancedFamilyHealthcareLlc
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1285359000"}}
	sourceDef.PatientAccessUrl = "http://www.advancedfamilyhc.com"
