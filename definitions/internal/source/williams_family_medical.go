...
func GetSourceWilliamsFamilyMedical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWilliamsFamilyMedical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Williams Family Medical"
	sourceDef.SourceType = pkg.SourceTypeWilliamsFamilyMedical
	sourceDef.Category = []string{"207Q00000X", "261QP2300X"}
	sourceDef.Aliases = []string{"WILLIAMS FAMILY MEDICAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1467960369"}}
	sourceDef.PatientAccessUrl = "http://www.williamsfamilymedical.com"
