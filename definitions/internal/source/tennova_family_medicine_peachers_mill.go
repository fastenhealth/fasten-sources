...
func GetSourceTennovaFamilyMedicinePeachersMill(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaFamilyMedicinePeachersMill]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Family Medicine  Peachers Mill"
	sourceDef.SourceType = pkg.SourceTypeTennovaFamilyMedicinePeachersMill
