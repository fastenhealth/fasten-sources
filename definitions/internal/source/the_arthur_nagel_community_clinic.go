...
func GetSourceTheArthurNagelCommunityClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheArthurNagelCommunityClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "THE ARTHUR NAGEL COMMUNITY CLINIC"
	sourceDef.SourceType = pkg.SourceTypeTheArthurNagelCommunityClinic
