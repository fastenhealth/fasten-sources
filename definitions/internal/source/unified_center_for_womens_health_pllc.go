...
func GetSourceUnifiedCenterForWomensHealthPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUnifiedCenterForWomensHealthPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "UNIFIED CENTER FOR WOMENS HEALTH PLLC"
	sourceDef.SourceType = pkg.SourceTypeUnifiedCenterForWomensHealthPllc
