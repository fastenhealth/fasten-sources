...
func GetSourceCarelifeMedical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCarelifeMedical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Carelife Medical"
	sourceDef.SourceType = pkg.SourceTypeCarelifeMedical
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"CARELIFE MEDICAL PLLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1104309194"}}
