...
func GetSourceRockfordRescueMissionhopeClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRockfordRescueMissionhopeClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Rockford Rescue MissionHope Clinic"
	sourceDef.SourceType = pkg.SourceTypeRockfordRescueMissionhopeClinic
