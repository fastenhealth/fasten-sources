...
func GetSourceSouthernIllinoisHealthcareFoundation(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernIllinoisHealthcareFoundation]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southern Illinois Healthcare Foundation"
	sourceDef.SourceType = pkg.SourceTypeSouthernIllinoisHealthcareFoundation
