...
func GetSourceRachelRavenFnpcDbaDotPhysical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRachelRavenFnpcDbaDotPhysical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Rachel Raven FNPC DBA DOT Physical"
	sourceDef.SourceType = pkg.SourceTypeRachelRavenFnpcDbaDotPhysical
