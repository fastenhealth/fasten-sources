...
func GetSourcePollardConciergeHealthWellnessCenters(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePollardConciergeHealthWellnessCenters]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pollard Concierge Health  Wellness Centers"
	sourceDef.SourceType = pkg.SourceTypePollardConciergeHealthWellnessCenters
