...
func GetSourceEastTennesseeMedicalAndSurgical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEastTennesseeMedicalAndSurgical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "EAST TENNESSEE MEDICAL AND SURGICAL"
	sourceDef.SourceType = pkg.SourceTypeEastTennesseeMedicalAndSurgical
