...
func GetSourcePremierInternalMedicineAssociate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePremierInternalMedicineAssociate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Premier Internal Medicine Associate"
	sourceDef.SourceType = pkg.SourceTypePremierInternalMedicineAssociate
