...
func GetSourceRehabilitationneurologicalService(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRehabilitationneurologicalService]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RehabilitationNeurological Service"
	sourceDef.SourceType = pkg.SourceTypeRehabilitationneurologicalService
