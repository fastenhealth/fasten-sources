...
func GetSourceEastCarolinaMedicalAssociatesPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEastCarolinaMedicalAssociatesPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "East Carolina Medical Associates PC"
	sourceDef.SourceType = pkg.SourceTypeEastCarolinaMedicalAssociatesPc
