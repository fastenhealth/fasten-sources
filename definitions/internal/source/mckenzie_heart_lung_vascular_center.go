...
func GetSourceMckenzieHeartLungVascularCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMckenzieHeartLungVascularCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "McKenzie Heart Lung  Vascular Center"
	sourceDef.SourceType = pkg.SourceTypeMckenzieHeartLungVascularCenter
