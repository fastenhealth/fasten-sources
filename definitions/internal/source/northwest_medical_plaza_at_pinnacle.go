...
func GetSourceNorthwestMedicalPlazaAtPinnacle(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestMedicalPlazaAtPinnacle]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Medical Plaza at Pinnacle"
	sourceDef.SourceType = pkg.SourceTypeNorthwestMedicalPlazaAtPinnacle
