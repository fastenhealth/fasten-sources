...
func GetSourceIowaillinoisPainConsultantsPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIowaillinoisPainConsultantsPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "IowaIllinois Pain Consultants PC"
	sourceDef.SourceType = pkg.SourceTypeIowaillinoisPainConsultantsPc
