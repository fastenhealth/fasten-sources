...
func GetSourceInfectiousDiseaseSpecialistsOfC(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInfectiousDiseaseSpecialistsOfC]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Infectious Disease Specialists of C"
	sourceDef.SourceType = pkg.SourceTypeInfectiousDiseaseSpecialistsOfC
