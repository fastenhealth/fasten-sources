...
func GetSourceChrisPittmanMdPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChrisPittmanMdPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Chris Pittman MD PA"
	sourceDef.SourceType = pkg.SourceTypeChrisPittmanMdPa
	sourceDef.Category = []string{"2085R0204X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1295039691"}}
	sourceDef.PatientAccessUrl = "http://www.vein911.com/"
