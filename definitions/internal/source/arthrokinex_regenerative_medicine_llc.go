...
func GetSourceArthrokinexRegenerativeMedicineLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArthrokinexRegenerativeMedicineLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arthrokinex Regenerative Medicine LLC"
	sourceDef.SourceType = pkg.SourceTypeArthrokinexRegenerativeMedicineLlc
