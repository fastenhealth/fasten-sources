...
func GetSourceSalehMedicalRadiologyAndEndovas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSalehMedicalRadiologyAndEndovas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SALEH MEDICAL RADIOLOGY AND ENDOVAS"
	sourceDef.SourceType = pkg.SourceTypeSalehMedicalRadiologyAndEndovas
