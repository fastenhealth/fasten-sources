...
func GetSourceMidwestPainClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidwestPainClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Midwest Pain Clinic"
	sourceDef.SourceType = pkg.SourceTypeMidwestPainClinic
	sourceDef.Category = []string{"261QP3300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1962650218"}}
	sourceDef.PatientAccessUrl = "http://www.midwestpainclinics.com"
