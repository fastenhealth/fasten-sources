...
func GetSourceAffordableHometownHealthcarePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAffordableHometownHealthcarePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AFFORDABLE HOMETOWN HEALTHCARE PLLC"
	sourceDef.SourceType = pkg.SourceTypeAffordableHometownHealthcarePllc
