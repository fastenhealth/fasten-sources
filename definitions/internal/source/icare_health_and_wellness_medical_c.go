...
func GetSourceIcareHealthAndWellnessMedicalC(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIcareHealthAndWellnessMedicalC]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "iCare Health and Wellness Medical C"
	sourceDef.SourceType = pkg.SourceTypeIcareHealthAndWellnessMedicalC
