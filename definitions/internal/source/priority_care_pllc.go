...
func GetSourcePriorityCarePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePriorityCarePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Priority Care PLLC"
	sourceDef.SourceType = pkg.SourceTypePriorityCarePllc
	sourceDef.Category = []string{"207Q00000X", "261QU0200X"}
	sourceDef.Aliases = []string{"PRIORITY FAMILY & URGENT CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1174245609"}}
