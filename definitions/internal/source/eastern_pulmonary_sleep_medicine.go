...
func GetSourceEasternPulmonarySleepMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEasternPulmonarySleepMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Eastern Pulmonary  Sleep Medicine"
	sourceDef.SourceType = pkg.SourceTypeEasternPulmonarySleepMedicine
