...
func GetSourceTexasOaksOrthopaedicAndSportsM(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexasOaksOrthopaedicAndSportsM]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "TEXAS OAKS ORTHOPAEDIC AND SPORTS M"
	sourceDef.SourceType = pkg.SourceTypeTexasOaksOrthopaedicAndSportsM
