...
func GetSourceCoastalRegenerativeOrthopedics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCoastalRegenerativeOrthopedics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Coastal Regenerative Orthopedics "
	sourceDef.SourceType = pkg.SourceTypeCoastalRegenerativeOrthopedics
