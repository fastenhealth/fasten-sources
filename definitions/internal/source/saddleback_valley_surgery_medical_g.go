...
func GetSourceSaddlebackValleySurgeryMedicalG(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSaddlebackValleySurgeryMedicalG]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Saddleback Valley Surgery Medical G"
	sourceDef.SourceType = pkg.SourceTypeSaddlebackValleySurgeryMedicalG
