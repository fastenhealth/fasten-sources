...
func GetSourceWickenburgCommunityHospitalAndClinics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWickenburgCommunityHospitalAndClinics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Wickenburg Community Hospital and Clinics"
	sourceDef.SourceType = pkg.SourceTypeWickenburgCommunityHospitalAndClinics
