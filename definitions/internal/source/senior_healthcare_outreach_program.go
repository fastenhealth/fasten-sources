...
func GetSourceSeniorHealthcareOutreachProgram(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSeniorHealthcareOutreachProgram]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Senior Healthcare Outreach Program"
	sourceDef.SourceType = pkg.SourceTypeSeniorHealthcareOutreachProgram
	sourceDef.Category = []string{"207RG0300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1588707954"}}
