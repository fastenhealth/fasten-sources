...
func GetSourcePacificUrgentCareAndWellnessCe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePacificUrgentCareAndWellnessCe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pacific Urgent Care and Wellness Ce"
	sourceDef.SourceType = pkg.SourceTypePacificUrgentCareAndWellnessCe
