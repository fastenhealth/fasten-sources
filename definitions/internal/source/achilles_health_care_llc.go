...
func GetSourceAchillesHealthCareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAchillesHealthCareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Achilles Health Care LLC"
	sourceDef.SourceType = pkg.SourceTypeAchillesHealthCareLlc
	sourceDef.Category = []string{"302R00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1275906018"}}
	sourceDef.PatientAccessUrl = "http://www.achilleshealthcareservices.com"
