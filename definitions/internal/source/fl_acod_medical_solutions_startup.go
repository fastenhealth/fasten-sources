...
func GetSourceFlAcodMedicalSolutionsStartup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFlAcodMedicalSolutionsStartup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FL  Acod Medical Solutions  Startup"
	sourceDef.SourceType = pkg.SourceTypeFlAcodMedicalSolutionsStartup
