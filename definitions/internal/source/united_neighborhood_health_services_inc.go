...
func GetSourceUnitedNeighborhoodHealthServicesInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUnitedNeighborhoodHealthServicesInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "United Neighborhood Health Services Inc"
	sourceDef.SourceType = pkg.SourceTypeUnitedNeighborhoodHealthServicesInc
	sourceDef.PatientAccessUrl = "https://www.neighborhoodhealthtn.org/our-locations/"
