...
func GetSourceCypressCardiology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCypressCardiology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CYPRESS CARDIOLOGY"
	sourceDef.SourceType = pkg.SourceTypeCypressCardiology
	sourceDef.Category = []string{"207RI0011X"}
	sourceDef.Aliases = []string{"CYPRESS CARDIOLOGY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1528606241"}}
