...
func GetSourceBeyondConciergeMedicalCareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBeyondConciergeMedicalCareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Beyond Concierge Medical Care LLC"
	sourceDef.SourceType = pkg.SourceTypeBeyondConciergeMedicalCareLlc
