...
func GetSourceElPasoNutritionConsultantsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeElPasoNutritionConsultantsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "El Paso Nutrition Consultants LLC"
	sourceDef.SourceType = pkg.SourceTypeElPasoNutritionConsultantsLlc
