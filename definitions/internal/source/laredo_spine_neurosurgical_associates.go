...
func GetSourceLaredoSpineNeurosurgicalAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLaredoSpineNeurosurgicalAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Laredo Spine  Neurosurgical Associates"
	sourceDef.SourceType = pkg.SourceTypeLaredoSpineNeurosurgicalAssociates
