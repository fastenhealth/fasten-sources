...
func GetSourcePrimeHealthcareGadsdenPhysicianMgtPsychiatry(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrimeHealthcareGadsdenPhysicianMgtPsychiatry]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PRIME HEALTHCARE  GADSDEN PHYSICIAN MGT PSYCHIATRY"
	sourceDef.SourceType = pkg.SourceTypePrimeHealthcareGadsdenPhysicianMgtPsychiatry
