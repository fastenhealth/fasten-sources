...
func GetSourceOnpointInternalMedicineRidgeline(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOnpointInternalMedicineRidgeline]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "OnPoint Internal Medicine  Ridgeline"
	sourceDef.SourceType = pkg.SourceTypeOnpointInternalMedicineRidgeline
