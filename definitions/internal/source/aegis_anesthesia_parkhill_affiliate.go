...
func GetSourceAegisAnesthesiaParkhillAffiliate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAegisAnesthesiaParkhillAffiliate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Aegis Anesthesia ParkHill Affiliate"
	sourceDef.SourceType = pkg.SourceTypeAegisAnesthesiaParkhillAffiliate
