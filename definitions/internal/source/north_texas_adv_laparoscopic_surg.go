...
func GetSourceNorthTexasAdvLaparoscopicSurg(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthTexasAdvLaparoscopicSurg]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Texas Adv Laparoscopic Surg"
	sourceDef.SourceType = pkg.SourceTypeNorthTexasAdvLaparoscopicSurg
