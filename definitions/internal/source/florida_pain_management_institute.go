...
func GetSourceFloridaPainManagementInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFloridaPainManagementInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Florida Pain Management Institute"
	sourceDef.SourceType = pkg.SourceTypeFloridaPainManagementInstitute
	sourceDef.Category = []string{"207LP2900X", "208VP0014X"}
	sourceDef.Aliases = []string{"FLORIDA PAIN MANAGEMENT INSTITUTE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1639665219"}}
