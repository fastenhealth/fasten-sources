...
func GetSourceDnaComprehensiveTherapyServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDnaComprehensiveTherapyServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DNA Comprehensive Therapy Services"
	sourceDef.SourceType = pkg.SourceTypeDnaComprehensiveTherapyServices
