...
func GetSourcePrimaryUrgentCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrimaryUrgentCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Primary Urgent Care"
	sourceDef.SourceType = pkg.SourceTypePrimaryUrgentCare
	sourceDef.Category = []string{"261QU0200X"}
	sourceDef.Aliases = []string{"PRIMARY URGENT CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1770935215"}}
