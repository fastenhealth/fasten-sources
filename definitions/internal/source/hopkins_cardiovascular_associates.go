...
func GetSourceHopkinsCardiovascularAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHopkinsCardiovascularAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hopkins Cardiovascular Associates"
	sourceDef.SourceType = pkg.SourceTypeHopkinsCardiovascularAssociates
