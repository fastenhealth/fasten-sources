...
func GetSourceReginaldACoatesMd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeReginaldACoatesMd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Reginald  A. Coates MD"
	sourceDef.SourceType = pkg.SourceTypeReginaldACoatesMd
	sourceDef.PatientAccessUrl = "https://www.healthgrades.com/physician/dr-reginald-coates-3xxrj"
