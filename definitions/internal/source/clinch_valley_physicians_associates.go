...
func GetSourceClinchValleyPhysiciansAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeClinchValleyPhysiciansAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Clinch Valley Physicians Associates"
	sourceDef.SourceType = pkg.SourceTypeClinchValleyPhysiciansAssociates
