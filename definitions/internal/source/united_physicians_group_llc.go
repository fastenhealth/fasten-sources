...
func GetSourceUnitedPhysiciansGroupLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUnitedPhysiciansGroupLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "UNITED PHYSICIANS GROUP LLC"
	sourceDef.SourceType = pkg.SourceTypeUnitedPhysiciansGroupLlc
	sourceDef.Category = []string{"207R00000X", "208M00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1316336845"}}
