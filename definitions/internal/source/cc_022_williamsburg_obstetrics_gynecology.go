...
func GetSourceCc022WilliamsburgObstetricsGynecology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc022WilliamsburgObstetricsGynecology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC022WILLIAMSBURG OBSTETRICS  GYNECOLOGY"
	sourceDef.SourceType = pkg.SourceTypeCc022WilliamsburgObstetricsGynecology
