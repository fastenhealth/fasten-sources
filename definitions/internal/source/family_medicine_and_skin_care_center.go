...
func GetSourceFamilyMedicineAndSkinCareCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFamilyMedicineAndSkinCareCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FAMILY MEDICINE AND SKIN CARE CENTER"
	sourceDef.SourceType = pkg.SourceTypeFamilyMedicineAndSkinCareCenter
