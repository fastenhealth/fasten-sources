...
func GetSourceMontgomeryMedicalAssociatesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMontgomeryMedicalAssociatesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Montgomery Medical Associates LLC"
	sourceDef.SourceType = pkg.SourceTypeMontgomeryMedicalAssociatesLlc
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1023205754"}}
