...
func GetSourceRmc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRmc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RMC"
	sourceDef.SourceType = pkg.SourceTypeRmc
	sourceDef.Category = []string{"208D00000X"}
	sourceDef.Aliases = []string{"RENAISSANCE MEDICAL CARE", "RMC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1649733387"}}
