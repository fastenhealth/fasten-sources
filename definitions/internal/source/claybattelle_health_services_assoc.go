...
func GetSourceClaybattelleHealthServicesAssoc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeClaybattelleHealthServicesAssoc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ClayBattelle Health Services Assoc"
	sourceDef.SourceType = pkg.SourceTypeClaybattelleHealthServicesAssoc
