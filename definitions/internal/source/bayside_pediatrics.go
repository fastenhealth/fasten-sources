...
func GetSourceBaysidePediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBaysidePediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bayside Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeBaysidePediatrics
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{"BAYSIDE PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1386760254", "1881215994"}}
