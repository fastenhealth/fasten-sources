...
func GetSourceIndependenceFootAndAnkleAssoc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIndependenceFootAndAnkleAssoc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Independence Foot and Ankle Assoc"
	sourceDef.SourceType = pkg.SourceTypeIndependenceFootAndAnkleAssoc
