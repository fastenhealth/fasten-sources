...
func GetSourceLakhvinderWadhwaMdLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLakhvinderWadhwaMdLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lakhvinder Wadhwa MD LLC"
	sourceDef.SourceType = pkg.SourceTypeLakhvinderWadhwaMdLlc
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"GERMANTOWN PRIMARY HEALTHCARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1588139653"}}
