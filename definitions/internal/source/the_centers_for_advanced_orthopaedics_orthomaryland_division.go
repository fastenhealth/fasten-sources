...
func GetSourceTheCentersForAdvancedOrthopaedicsOrthomarylandDivision(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheCentersForAdvancedOrthopaedicsOrthomarylandDivision]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Centers for Advanced Orthopaedics OrthoMaryland Division"
	sourceDef.SourceType = pkg.SourceTypeTheCentersForAdvancedOrthopaedicsOrthomarylandDivision
