...
func GetSourceMykPsychiatryLimitedLiability(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMykPsychiatryLimitedLiability]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MYK PSYCHIATRY LIMITED LIABILITY"
	sourceDef.SourceType = pkg.SourceTypeMykPsychiatryLimitedLiability
