...
func GetSourceAdvancedSpineAndPainClinicsOfMinnesota(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedSpineAndPainClinicsOfMinnesota]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Advanced Spine and Pain Clinics of Minnesota"
	sourceDef.SourceType = pkg.SourceTypeAdvancedSpineAndPainClinicsOfMinnesota
