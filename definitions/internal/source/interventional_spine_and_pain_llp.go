...
func GetSourceInterventionalSpineAndPainLlp(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInterventionalSpineAndPainLlp]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Interventional Spine and Pain LLP"
	sourceDef.SourceType = pkg.SourceTypeInterventionalSpineAndPainLlp
