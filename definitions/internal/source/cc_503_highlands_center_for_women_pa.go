...
func GetSourceCc503HighlandsCenterForWomenPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc503HighlandsCenterForWomenPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC503HIGHLANDS CENTER FOR WOMEN PA"
	sourceDef.SourceType = pkg.SourceTypeCc503HighlandsCenterForWomenPa
