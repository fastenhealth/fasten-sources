...
func GetSourceFullLifeHormoneSpecialistsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFullLifeHormoneSpecialistsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Full Life Hormone Specialists LLC"
	sourceDef.SourceType = pkg.SourceTypeFullLifeHormoneSpecialistsLlc
