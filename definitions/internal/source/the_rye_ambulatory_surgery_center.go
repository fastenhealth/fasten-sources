...
func GetSourceTheRyeAmbulatorySurgeryCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheRyeAmbulatorySurgeryCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Rye Ambulatory Surgery Center"
	sourceDef.SourceType = pkg.SourceTypeTheRyeAmbulatorySurgeryCenter
