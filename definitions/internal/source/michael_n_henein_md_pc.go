...
func GetSourceMichaelNHeneinMdPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMichaelNHeneinMdPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Michael N Henein MD PC"
	sourceDef.SourceType = pkg.SourceTypeMichaelNHeneinMdPc
	sourceDef.Category = []string{"208600000X", "208C00000X", "363LF0000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1790927234", "1962671388"}}
