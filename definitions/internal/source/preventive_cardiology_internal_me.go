...
func GetSourcePreventiveCardiologyInternalMe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePreventiveCardiologyInternalMe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Preventive Cardiology  Internal Me"
	sourceDef.SourceType = pkg.SourceTypePreventiveCardiologyInternalMe
