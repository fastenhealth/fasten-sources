...
func GetSourceJuniperPediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJuniperPediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Juniper Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeJuniperPediatrics
	sourceDef.Category = []string{"332900000X"}
	sourceDef.Aliases = []string{"JUNIPER PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1053757666"}}
