...
func GetSourceWomensHealthcarePhysiciansOfNaplesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWomensHealthcarePhysiciansOfNaplesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WOMENS HEALTHCARE PHYSICIANS OF NAPLES LLC"
	sourceDef.SourceType = pkg.SourceTypeWomensHealthcarePhysiciansOfNaplesLlc
