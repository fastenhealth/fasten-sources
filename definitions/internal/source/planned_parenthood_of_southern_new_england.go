...
func GetSourcePlannedParenthoodOfSouthernNewEngland(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePlannedParenthoodOfSouthernNewEngland]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Planned Parenthood of Southern New England"
	sourceDef.SourceType = pkg.SourceTypePlannedParenthoodOfSouthernNewEngland
	sourceDef.Category = []string{"261QA0005X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1730308537"}}
