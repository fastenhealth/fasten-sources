...
func GetSourceCharitonValleyFamilyMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCharitonValleyFamilyMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Chariton Valley Family Medicine"
	sourceDef.SourceType = pkg.SourceTypeCharitonValleyFamilyMedicine
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"CHARITON VALLEY FAMILY MEDICINE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1851713580"}}
