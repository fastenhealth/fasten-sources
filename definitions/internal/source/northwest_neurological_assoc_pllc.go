...
func GetSourceNorthwestNeurologicalAssocPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestNeurologicalAssocPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Neurological Assoc PLLC"
	sourceDef.SourceType = pkg.SourceTypeNorthwestNeurologicalAssocPllc
