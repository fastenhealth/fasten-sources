...
func GetSourceSmithtownMedicalSpecialistsPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSmithtownMedicalSpecialistsPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Smithtown Medical Specialists PC"
	sourceDef.SourceType = pkg.SourceTypeSmithtownMedicalSpecialistsPc
