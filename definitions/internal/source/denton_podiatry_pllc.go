...
func GetSourceDentonPodiatryPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDentonPodiatryPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DENTON PODIATRY PLLC"
	sourceDef.SourceType = pkg.SourceTypeDentonPodiatryPllc
	sourceDef.Category = []string{"213ES0103X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1598431447"}}
	sourceDef.PatientAccessUrl = "http://www.DentonFootandAnkle.com"
