...
func GetSourceLyonsPrimaryUrgentCareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLyonsPrimaryUrgentCareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LYONS PRIMARY URGENT CARE LLC"
	sourceDef.SourceType = pkg.SourceTypeLyonsPrimaryUrgentCareLlc
	sourceDef.Category = []string{"261QU0200X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1407228588"}}
