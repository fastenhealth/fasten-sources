...
func GetSourceScaSouthernCaliforniaAssociatedProviders(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeScaSouthernCaliforniaAssociatedProviders]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SCA  Southern California Associated Providers"
	sourceDef.SourceType = pkg.SourceTypeScaSouthernCaliforniaAssociatedProviders
