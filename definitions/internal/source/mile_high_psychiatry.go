...
func GetSourceMileHighPsychiatry(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMileHighPsychiatry]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Mile High Psychiatry"
	sourceDef.SourceType = pkg.SourceTypeMileHighPsychiatry
	sourceDef.Category = []string{"101YM0800X", "363LP0808X"}
	sourceDef.Aliases = []string{"MILE HIGH PSYCHIATRY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1366099020", "1538706122", "1740883958", "1801490859"}}
