...
func GetSourceKidsAndTeensOrthopaedicSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKidsAndTeensOrthopaedicSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kids and Teens Orthopaedic Surgery"
	sourceDef.SourceType = pkg.SourceTypeKidsAndTeensOrthopaedicSurgery
	sourceDef.Category = []string{"207X00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1891737516"}}
