...
func GetSourceNorthwestAlliedPediatricsAtNorthwestMedicalCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestAlliedPediatricsAtNorthwestMedicalCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Allied Pediatrics at Northwest Medical Center"
	sourceDef.SourceType = pkg.SourceTypeNorthwestAlliedPediatricsAtNorthwestMedicalCenter
