...
func GetSourceGenoaCommunityHospital(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGenoaCommunityHospital]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Genoa Community Hospital"
	sourceDef.SourceType = pkg.SourceTypeGenoaCommunityHospital
	sourceDef.Category = []string{"282NC0060X"}
	sourceDef.Aliases = []string{"GENOA COMMUNITY HOSPITAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1649228511"}}
