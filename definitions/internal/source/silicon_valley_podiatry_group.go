...
func GetSourceSiliconValleyPodiatryGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSiliconValleyPodiatryGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Silicon Valley Podiatry Group"
	sourceDef.SourceType = pkg.SourceTypeSiliconValleyPodiatryGroup
	sourceDef.Category = []string{"213ES0103X"}
	sourceDef.Aliases = []string{"SILICON VALLEY PODIATRY GROUP"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1174675508", "1609284231"}}
