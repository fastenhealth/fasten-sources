...
func GetSourceArthurMedicalAndSportsAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArthurMedicalAndSportsAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arthur Medical and Sports Associates"
	sourceDef.SourceType = pkg.SourceTypeArthurMedicalAndSportsAssociates
