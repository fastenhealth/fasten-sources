...
func GetSourceSteveSikorevichMdAndIrynaPolyakovaDo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSteveSikorevichMdAndIrynaPolyakovaDo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Steve Sikorevich MD and Iryna Polyakova DO"
	sourceDef.SourceType = pkg.SourceTypeSteveSikorevichMdAndIrynaPolyakovaDo
