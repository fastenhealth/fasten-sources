...
func GetSourceTexasSpecialtyPhysiciansBehavioralHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexasSpecialtyPhysiciansBehavioralHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Texas Specialty Physicians Behavioral Health"
	sourceDef.SourceType = pkg.SourceTypeTexasSpecialtyPhysiciansBehavioralHealth
