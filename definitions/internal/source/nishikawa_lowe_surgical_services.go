...
func GetSourceNishikawaLoweSurgicalServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNishikawaLoweSurgicalServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Nishikawa  Lowe Surgical Services"
	sourceDef.SourceType = pkg.SourceTypeNishikawaLoweSurgicalServices
