...
func GetSourceChapelHillDoctorsHealthcareCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChapelHillDoctorsHealthcareCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Chapel Hill Doctors Healthcare Center"
	sourceDef.SourceType = pkg.SourceTypeChapelHillDoctorsHealthcareCenter
