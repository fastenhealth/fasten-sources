...
func GetSourceBristolPulmonologySleepMedici(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBristolPulmonologySleepMedici]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bristol Pulmonology  Sleep Medici"
	sourceDef.SourceType = pkg.SourceTypeBristolPulmonologySleepMedici
