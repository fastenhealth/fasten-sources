...
func GetSourceSamuelURodgersHealthCenterInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSamuelURodgersHealthCenterInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Samuel U Rodgers Health Center INC"
	sourceDef.SourceType = pkg.SourceTypeSamuelURodgersHealthCenterInc
