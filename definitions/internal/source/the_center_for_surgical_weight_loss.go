...
func GetSourceTheCenterForSurgicalWeightLoss(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheCenterForSurgicalWeightLoss]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Center for Surgical Weight Loss"
	sourceDef.SourceType = pkg.SourceTypeTheCenterForSurgicalWeightLoss
	sourceDef.Category = []string{"208600000X"}
	sourceDef.Aliases = []string{"THE CENTER FOR SURGICAL WEIGHT LOSS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1457621799"}}
