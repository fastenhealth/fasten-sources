...
func GetSourceRichmondInterventionalPainMgmt(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRichmondInterventionalPainMgmt]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RICHMOND INTERVENTIONAL PAIN MGMT"
	sourceDef.SourceType = pkg.SourceTypeRichmondInterventionalPainMgmt
