...
func GetSourceSeattleNaturalFamilySportsMed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSeattleNaturalFamilySportsMed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Seattle Natural Family  Sports Med"
	sourceDef.SourceType = pkg.SourceTypeSeattleNaturalFamilySportsMed
