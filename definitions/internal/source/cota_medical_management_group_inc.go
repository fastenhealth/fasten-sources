...
func GetSourceCotaMedicalManagementGroupInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCotaMedicalManagementGroupInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COTA MEDICAL MANAGEMENT GROUP INC"
	sourceDef.SourceType = pkg.SourceTypeCotaMedicalManagementGroupInc
