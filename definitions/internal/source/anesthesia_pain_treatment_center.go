...
func GetSourceAnesthesiaPainTreatmentCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAnesthesiaPainTreatmentCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Anesthesia Pain Treatment Center"
	sourceDef.SourceType = pkg.SourceTypeAnesthesiaPainTreatmentCenter
