...
func GetSourceBostonDermatologyLaserCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBostonDermatologyLaserCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BOSTON DERMATOLOGY  LASER CENTER LLC"
	sourceDef.SourceType = pkg.SourceTypeBostonDermatologyLaserCenterLlc
