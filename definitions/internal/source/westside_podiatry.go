...
func GetSourceWestsidePodiatry(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWestsidePodiatry]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Westside Podiatry"
	sourceDef.SourceType = pkg.SourceTypeWestsidePodiatry
	sourceDef.Category = []string{"213E00000X"}
	sourceDef.Aliases = []string{"WESTSIDE PODIATRY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1497958144"}}
