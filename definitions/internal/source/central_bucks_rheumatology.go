...
func GetSourceCentralBucksRheumatology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralBucksRheumatology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Central Bucks Rheumatology"
	sourceDef.SourceType = pkg.SourceTypeCentralBucksRheumatology
	sourceDef.Category = []string{"207RR0500X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1770178600"}}
	sourceDef.PatientAccessUrl = "http://www.cbrheumatology.com"
