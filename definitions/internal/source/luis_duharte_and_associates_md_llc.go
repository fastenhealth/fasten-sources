...
func GetSourceLuisDuharteAndAssociatesMdLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLuisDuharteAndAssociatesMdLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LUIS DUHARTE AND ASSOCIATES MD LLC"
	sourceDef.SourceType = pkg.SourceTypeLuisDuharteAndAssociatesMdLlc
	sourceDef.Category = []string{"207RI0200X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1194159749"}}
