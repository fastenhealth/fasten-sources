...
func GetSourceCornerstoneFamilyMedicinePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCornerstoneFamilyMedicinePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CORNERSTONE FAMILY MEDICINE PLLC"
	sourceDef.SourceType = pkg.SourceTypeCornerstoneFamilyMedicinePllc
