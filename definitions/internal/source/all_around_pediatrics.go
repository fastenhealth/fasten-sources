...
func GetSourceAllAroundPediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllAroundPediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "All Around Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeAllAroundPediatrics
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{"ALL AROUND PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1598411662"}}
	sourceDef.PatientAccessUrl = "http://allaroundpediatrics.com"
