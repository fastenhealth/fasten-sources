...
func GetSourceTulsaEndoscopyCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTulsaEndoscopyCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Tulsa Endoscopy Center"
	sourceDef.SourceType = pkg.SourceTypeTulsaEndoscopyCenter
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"TULSA ENDOSCOPY CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1417916800"}}
