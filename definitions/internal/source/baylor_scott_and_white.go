// Copyright (C) Fasten Health, Inc. - All Rights Reserved.
//
// THIS FILE IS GENERATED BY https://github.com/fastenhealth/fasten-sources-gen
// PLEASE DO NOT EDIT BY HAND

package source

import (
	definitions "github.com/fastenhealth/fasten-sources/definitions"
	platform "github.com/fastenhealth/fasten-sources/definitions/internal/platform"
	models "github.com/fastenhealth/fasten-sources/definitions/models"
	pkg "github.com/fastenhealth/fasten-sources/pkg"
)

// https://epicproxy.bswhealth.org/FHIR-PRD/BSW/api/FHIR/R4/.well-known/smart-configuration
// https://epicproxy.bswhealth.org/FHIR-PRD/BSW/api/FHIR/R4/metadata
func GetSourceBaylorScottAndWhite(env pkg.FastenEnvType) (models.LighthouseSourceDefinition, error) {
	sourceDef, err := platform.GetSourceEpic(env)
	sourceDef.AuthorizationEndpoint = "https://rxproxy.sw.org/FHIR-PRD/BSW/oauth2/authorize"
	sourceDef.TokenEndpoint = "https://rxproxy.sw.org/FHIR-PRD/BSW/oauth2/token"

	sourceDef.Audience = "https://epicproxy.bswhealth.org/FHIR-PRD/BSW/api/FHIR/R4"

	sourceDef.ApiEndpointBaseUrl = "https://epicproxy.bswhealth.org/FHIR-PRD/BSW/api/FHIR/R4"
	if env == pkg.FastenEnvSandbox {
		sourceDef.ClientId = ""
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(definitions.SourceTypeEpic))

	sourceDef.Display = "Baylor Scott & White"
	sourceDef.SourceType = definitions.SourceTypeBaylorScottAndWhite
	sourceDef.Enabled = true
	sourceDef.SecretKeyPrefix = "epic"

	return sourceDef, err
}
