...
func GetSourceNewDayWomensCareWellnessLlcTermed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNewDayWomensCareWellnessLlcTermed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEW DAY WOMENS CARE  WELLNESS LLC TERMED"
	sourceDef.SourceType = pkg.SourceTypeNewDayWomensCareWellnessLlcTermed
