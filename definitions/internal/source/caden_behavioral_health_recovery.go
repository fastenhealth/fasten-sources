...
func GetSourceCadenBehavioralHealthRecovery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCadenBehavioralHealthRecovery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Caden Behavioral Health Recovery "
	sourceDef.SourceType = pkg.SourceTypeCadenBehavioralHealthRecovery
