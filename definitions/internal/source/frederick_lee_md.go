...
func GetSourceFrederickLeeMd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFrederickLeeMd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Frederick Lee MD"
	sourceDef.SourceType = pkg.SourceTypeFrederickLeeMd
	sourceDef.PatientAccessUrl = "https://www.healthgrades.com/physician/dr-frederick-lee-xj8cy"
