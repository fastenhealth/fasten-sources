...
func GetSourceCoastalOrthopedicSpineInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCoastalOrthopedicSpineInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Coastal Orthopedic  Spine Institute"
	sourceDef.SourceType = pkg.SourceTypeCoastalOrthopedicSpineInstitute
