...
func GetSourceLucinaWomensHealthAndMidwiferyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLucinaWomensHealthAndMidwiferyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LUCINA WOMENS HEALTH AND MIDWIFERY LLC"
	sourceDef.SourceType = pkg.SourceTypeLucinaWomensHealthAndMidwiferyLlc
