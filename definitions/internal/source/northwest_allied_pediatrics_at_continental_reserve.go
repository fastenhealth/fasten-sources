...
func GetSourceNorthwestAlliedPediatricsAtContinentalReserve(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestAlliedPediatricsAtContinentalReserve]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Allied Pediatrics at Continental Reserve"
	sourceDef.SourceType = pkg.SourceTypeNorthwestAlliedPediatricsAtContinentalReserve
