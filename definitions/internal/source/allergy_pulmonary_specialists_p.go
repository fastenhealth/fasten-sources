...
func GetSourceAllergyPulmonarySpecialistsP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllergyPulmonarySpecialistsP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Allergy  Pulmonary Specialists P"
	sourceDef.SourceType = pkg.SourceTypeAllergyPulmonarySpecialistsP
