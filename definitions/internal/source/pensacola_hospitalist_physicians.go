...
func GetSourcePensacolaHospitalistPhysicians(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePensacolaHospitalistPhysicians]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PENSACOLA HOSPITALIST PHYSICIANS"
	sourceDef.SourceType = pkg.SourceTypePensacolaHospitalistPhysicians
