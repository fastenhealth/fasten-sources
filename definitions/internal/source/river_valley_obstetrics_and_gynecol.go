...
func GetSourceRiverValleyObstetricsAndGynecol(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRiverValleyObstetricsAndGynecol]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RIVER VALLEY OBSTETRICS AND GYNECOL"
	sourceDef.SourceType = pkg.SourceTypeRiverValleyObstetricsAndGynecol
