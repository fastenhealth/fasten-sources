...
func GetSourceBoilingSpringLakesFamilyMedicin(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBoilingSpringLakesFamilyMedicin]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Boiling Spring Lakes Family Medicin"
	sourceDef.SourceType = pkg.SourceTypeBoilingSpringLakesFamilyMedicin
