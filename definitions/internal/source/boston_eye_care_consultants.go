...
func GetSourceBostonEyeCareConsultants(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBostonEyeCareConsultants]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Boston Eye Care Consultants"
	sourceDef.SourceType = pkg.SourceTypeBostonEyeCareConsultants
	sourceDef.Category = []string{"152W00000X", "207W00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1689701096"}}
	sourceDef.PatientAccessUrl = "http://www.bostoneyecareconsultants.com"
