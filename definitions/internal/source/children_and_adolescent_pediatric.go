...
func GetSourceChildrenAndAdolescentPediatric(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChildrenAndAdolescentPediatric]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Children and Adolescent Pediatric"
	sourceDef.SourceType = pkg.SourceTypeChildrenAndAdolescentPediatric
