...
func GetSourcePulmonaryAndSleepAssocOfHunterdon(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePulmonaryAndSleepAssocOfHunterdon]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Pulmonary And Sleep Assoc Of Hunterdon"
	sourceDef.SourceType = pkg.SourceTypePulmonaryAndSleepAssocOfHunterdon
