...
func GetSourceCenterForPediatricAndAdolescent(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForPediatricAndAdolescent]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center For Pediatric and Adolescent"
	sourceDef.SourceType = pkg.SourceTypeCenterForPediatricAndAdolescent
