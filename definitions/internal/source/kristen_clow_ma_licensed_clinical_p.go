...
func GetSourceKristenClowMaLicensedClinicalP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKristenClowMaLicensedClinicalP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kristen Clow MA Licensed Clinical P"
	sourceDef.SourceType = pkg.SourceTypeKristenClowMaLicensedClinicalP
