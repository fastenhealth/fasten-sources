...
func GetSourceIndianaTotalTherapy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIndianaTotalTherapy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "INDIANA TOTAL THERAPY"
	sourceDef.SourceType = pkg.SourceTypeIndianaTotalTherapy
	sourceDef.Category = []string{"225100000X", "225X00000X", "332B00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1568694719"}}
