...
func GetSourceGmdNursePractitionerClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGmdNursePractitionerClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GMD Nurse Practitioner Clinic"
	sourceDef.SourceType = pkg.SourceTypeGmdNursePractitionerClinic
	sourceDef.Category = []string{"363L00000X"}
	sourceDef.Aliases = []string{"GMD NURSE PRACTITIONER CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1194490169"}}
