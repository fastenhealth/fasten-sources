...
func GetSourceShahzaibMirzaMdMedicalPartners(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShahzaibMirzaMdMedicalPartners]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SHAHZAIB MIRZA MD MEDICAL PARTNERS"
	sourceDef.SourceType = pkg.SourceTypeShahzaibMirzaMdMedicalPartners
