...
func GetSourceOculoplasticsOphthalmologyAtLi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOculoplasticsOphthalmologyAtLi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Oculoplastics  Ophthalmology at Li"
	sourceDef.SourceType = pkg.SourceTypeOculoplasticsOphthalmologyAtLi
