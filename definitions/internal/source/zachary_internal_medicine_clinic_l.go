...
func GetSourceZacharyInternalMedicineClinicL(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeZacharyInternalMedicineClinicL]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ZACHARY INTERNAL MEDICINE CLINIC L"
	sourceDef.SourceType = pkg.SourceTypeZacharyInternalMedicineClinicL
