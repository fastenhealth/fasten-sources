...
func GetSourceArtisanFootAndAnklePodiatricSpecialistsInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArtisanFootAndAnklePodiatricSpecialistsInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Artisan Foot and Ankle Podiatric Specialists INC"
	sourceDef.SourceType = pkg.SourceTypeArtisanFootAndAnklePodiatricSpecialistsInc
