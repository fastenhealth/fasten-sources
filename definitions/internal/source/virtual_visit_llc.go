...
func GetSourceVirtualVisitLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVirtualVisitLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Virtual Visit LLC"
	sourceDef.SourceType = pkg.SourceTypeVirtualVisitLlc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"MIND MED MENTAL HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1366105199"}}
