...
func GetSourceAllianceHealthMedicalGroupPrimaryCareAtoka(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllianceHealthMedicalGroupPrimaryCareAtoka]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Alliance Health Medical Group Primary Care Atoka"
	sourceDef.SourceType = pkg.SourceTypeAllianceHealthMedicalGroupPrimaryCareAtoka
