...
func GetSourceFlatironFamilyMedicalPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFlatironFamilyMedicalPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FLATIRON FAMILY MEDICAL PC"
	sourceDef.SourceType = pkg.SourceTypeFlatironFamilyMedicalPc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"FLATIRON FAMILY MEDICAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1902324734"}}
