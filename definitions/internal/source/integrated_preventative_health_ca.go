...
func GetSourceIntegratedPreventativeHealthCa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIntegratedPreventativeHealthCa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Integrated  Preventative Health Ca"
	sourceDef.SourceType = pkg.SourceTypeIntegratedPreventativeHealthCa
