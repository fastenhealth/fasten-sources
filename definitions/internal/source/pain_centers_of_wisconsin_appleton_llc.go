...
func GetSourcePainCentersOfWisconsinAppletonLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePainCentersOfWisconsinAppletonLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Pain Centers Of Wisconsin Appleton LLC"
	sourceDef.SourceType = pkg.SourceTypePainCentersOfWisconsinAppletonLlc
	sourceDef.PatientAccessUrl = "https://npino.com/surgery-center/1548502008-pain-centers-of-wisconsin---appleton%2C-llc/"
