...
func GetSourceAlliancehealthMedicalGroupOrthopedics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupOrthopedics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Orthopedics"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupOrthopedics
