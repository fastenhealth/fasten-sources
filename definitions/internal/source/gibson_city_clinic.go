...
func GetSourceGibsonCityClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGibsonCityClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Gibson City Clinic"
	sourceDef.SourceType = pkg.SourceTypeGibsonCityClinic
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"GIBSON CITY CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1841390747"}}
	sourceDef.BrandLogo = "gibson-city-clinic.jpg"
