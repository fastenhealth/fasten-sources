...
func GetSourceNebiPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNebiPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEBI PC"
	sourceDef.SourceType = pkg.SourceTypeNebiPc
	sourceDef.Category = []string{"207RG0100X", "261QM1300X", "261QU0200X"}
	sourceDef.Aliases = []string{"24HOURDOCS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1326651688"}}
