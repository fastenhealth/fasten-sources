...
func GetSourceNorthFlaWomensPhysiciansOfGainesville(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthFlaWomensPhysiciansOfGainesville]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTH FLA WOMENS PHYSICIANS OF GAINESVILLE"
	sourceDef.SourceType = pkg.SourceTypeNorthFlaWomensPhysiciansOfGainesville
