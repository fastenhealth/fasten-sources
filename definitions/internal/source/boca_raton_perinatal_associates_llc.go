...
func GetSourceBocaRatonPerinatalAssociatesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBocaRatonPerinatalAssociatesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BOCA RATON PERINATAL ASSOCIATES LLC"
	sourceDef.SourceType = pkg.SourceTypeBocaRatonPerinatalAssociatesLlc
