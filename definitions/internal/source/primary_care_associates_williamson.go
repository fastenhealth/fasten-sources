...
func GetSourcePrimaryCareAssociatesWilliamson(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrimaryCareAssociatesWilliamson]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Primary Care Associates  Williamson"
	sourceDef.SourceType = pkg.SourceTypePrimaryCareAssociatesWilliamson
