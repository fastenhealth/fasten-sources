...
func GetSourceTrinityOaksFamilyMedicineLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTrinityOaksFamilyMedicineLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Trinity Oaks Family Medicine LLC"
	sourceDef.SourceType = pkg.SourceTypeTrinityOaksFamilyMedicineLlc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1487102620"}}
