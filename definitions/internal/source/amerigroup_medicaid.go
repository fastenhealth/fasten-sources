...
func GetSourceAmerigroupMedicaid(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	sourceDef, err := GetSourceAnthem(env, clientIdLookup)
	sourceDef.AuthorizationEndpoint = "https://patient360.amerigroup.com/P360Member/identityserver/connect/authorize"
	sourceDef.TokenEndpoint = "https://patient360.amerigroup.com/P360Member/identityserver/connect/token"
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmerigroupMedicaid]; clientIdOk {
