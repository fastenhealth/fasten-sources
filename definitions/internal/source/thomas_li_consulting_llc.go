...
func GetSourceThomasLiConsultingLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeThomasLiConsultingLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "THOMAS LI CONSULTING LLC"
	sourceDef.SourceType = pkg.SourceTypeThomasLiConsultingLlc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"TLC CONSULTING"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1013689512"}}
