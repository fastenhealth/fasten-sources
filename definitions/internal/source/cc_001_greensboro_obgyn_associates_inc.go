...
func GetSourceCc001GreensboroObgynAssociatesInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc001GreensboroObgynAssociatesInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC001Greensboro OBGYN Associates Inc"
	sourceDef.SourceType = pkg.SourceTypeCc001GreensboroObgynAssociatesInc
