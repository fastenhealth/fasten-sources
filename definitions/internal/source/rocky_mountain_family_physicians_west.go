...
func GetSourceRockyMountainFamilyPhysiciansWest(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRockyMountainFamilyPhysiciansWest]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Rocky Mountain Family Physicians West"
	sourceDef.SourceType = pkg.SourceTypeRockyMountainFamilyPhysiciansWest
