...
func GetSourceWyomingInterventionalPainManagement(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWyomingInterventionalPainManagement]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WYOMING INTERVENTIONAL PAIN MANAGEMENT"
	sourceDef.SourceType = pkg.SourceTypeWyomingInterventionalPainManagement
