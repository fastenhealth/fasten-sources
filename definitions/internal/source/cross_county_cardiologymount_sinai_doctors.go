...
func GetSourceCrossCountyCardiologymountSinaiDoctors(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCrossCountyCardiologymountSinaiDoctors]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cross County CardiologyMount Sinai Doctors"
	sourceDef.SourceType = pkg.SourceTypeCrossCountyCardiologymountSinaiDoctors
