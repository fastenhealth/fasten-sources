...
func GetSourceAdvancedHeartAndVascularCenterOfNewMexico(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedHeartAndVascularCenterOfNewMexico]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ADVANCED HEART AND VASCULAR CENTER OF NEW MEXICO"
	sourceDef.SourceType = pkg.SourceTypeAdvancedHeartAndVascularCenterOfNewMexico
