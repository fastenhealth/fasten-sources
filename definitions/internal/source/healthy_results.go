...
func GetSourceHealthyResults(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHealthyResults]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Healthy Results"
	sourceDef.SourceType = pkg.SourceTypeHealthyResults
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"HEALTHY RESULTS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1255941407"}}
