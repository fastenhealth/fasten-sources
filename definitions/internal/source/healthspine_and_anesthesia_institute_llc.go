...
func GetSourceHealthspineAndAnesthesiaInstituteLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHealthspineAndAnesthesiaInstituteLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Healthspine and Anesthesia Institute LLC"
	sourceDef.SourceType = pkg.SourceTypeHealthspineAndAnesthesiaInstituteLlc
