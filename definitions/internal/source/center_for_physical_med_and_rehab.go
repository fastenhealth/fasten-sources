...
func GetSourceCenterForPhysicalMedAndRehab(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForPhysicalMedAndRehab]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CENTER FOR PHYSICAL MED AND REHAB"
	sourceDef.SourceType = pkg.SourceTypeCenterForPhysicalMedAndRehab
