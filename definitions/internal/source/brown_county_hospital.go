// Copyright (C) Fasten Health, Inc. - All Rights Reserved.
//
// THIS FILE IS GENERATED BY https://github.com/fastenhealth/fasten-sources-gen
// PLEASE DO NOT EDIT BY HAND

package source

import (
	platform "github.com/fastenhealth/fasten-sources/definitions/internal/platform"
	models "github.com/fastenhealth/fasten-sources/definitions/models"
	pkg "github.com/fastenhealth/fasten-sources/pkg"
)

// https://fhir-myrecord.cerner.com/r4/f24ac13d-3953-486e-b046-3f6f7cc41dd9/metadata
func GetSourceBrownCountyHospital(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	sourceDef, err := platform.GetSourceCerner(env, clientIdLookup)
	sourceDef.AuthorizationEndpoint = "https://authorization.cerner.com/tenants/f24ac13d-3953-486e-b046-3f6f7cc41dd9/protocols/oauth2/profiles/smart-v1/personas/patient/authorize"
	sourceDef.TokenEndpoint = "https://authorization.cerner.com/tenants/f24ac13d-3953-486e-b046-3f6f7cc41dd9/protocols/oauth2/profiles/smart-v1/token"
	sourceDef.IntrospectionEndpoint = "https://authorization.cerner.com/tokeninfo"

	sourceDef.Audience = "https://fhir-myrecord.cerner.com/r4/f24ac13d-3953-486e-b046-3f6f7cc41dd9"

	sourceDef.ApiEndpointBaseUrl = "https://fhir-myrecord.cerner.com/r4/f24ac13d-3953-486e-b046-3f6f7cc41dd9"
	// retrieve client-id, if available
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBrownCountyHospital]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeCerner))

	sourceDef.Display = "Brown County Hospital"
	sourceDef.SourceType = pkg.SourceTypeBrownCountyHospital
	sourceDef.Category = []string{"207P00000X", "207Q00000X", "207Y00000X", "213E00000X", "251E00000X", "261QR1300X", "275N00000X", "282NC0060X", "363L00000X", "367500000X"}
	sourceDef.Aliases = []string{"AINSWORTH FAMILY CLINIC", "BCH PROFESSIONAL SERVICES", "BROWN COUNTY HOSPITAL HOME HEALTH", "BROWN COUNTY HOSPITAL SWING BED"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1073861456", "1265450860", "1417027210", "1760464283", "1891763512"}}
	sourceDef.BrandLogo = "brown-county-hospital.jpg"
	sourceDef.PatientAccessUrl = "https://browncountyhospital.org/"
	sourceDef.SecretKeyPrefix = "cerner"

	return sourceDef, err
}
