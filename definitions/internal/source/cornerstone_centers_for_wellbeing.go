...
func GetSourceCornerstoneCentersForWellbeing(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCornerstoneCentersForWellbeing]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cornerstone Centers for Wellbeing"
	sourceDef.SourceType = pkg.SourceTypeCornerstoneCentersForWellbeing
	sourceDef.Category = []string{"101YM0800X"}
	sourceDef.Aliases = []string{"CORNERSTONE CENTERS FOR WELLBEING"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1174157986"}}
