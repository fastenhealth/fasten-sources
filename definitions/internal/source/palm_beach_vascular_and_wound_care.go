...
func GetSourcePalmBeachVascularAndWoundCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePalmBeachVascularAndWoundCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Palm Beach Vascular and Wound Care"
	sourceDef.SourceType = pkg.SourceTypePalmBeachVascularAndWoundCare
