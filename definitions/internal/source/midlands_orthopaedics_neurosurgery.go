...
func GetSourceMidlandsOrthopaedicsNeurosurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidlandsOrthopaedicsNeurosurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Midlands Orthopaedics  Neurosurgery"
	sourceDef.SourceType = pkg.SourceTypeMidlandsOrthopaedicsNeurosurgery
