...
func GetSourceEntCareForKidsPaParkhillAffiliate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEntCareForKidsPaParkhillAffiliate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ENT Care For Kids PA ParkHill Affiliate"
	sourceDef.SourceType = pkg.SourceTypeEntCareForKidsPaParkhillAffiliate
