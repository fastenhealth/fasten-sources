...
func GetSourceDeltaOrthopedicsAndSportsMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDeltaOrthopedicsAndSportsMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Delta Orthopedics and Sports Medicine"
	sourceDef.SourceType = pkg.SourceTypeDeltaOrthopedicsAndSportsMedicine
