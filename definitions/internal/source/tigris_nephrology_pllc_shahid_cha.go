...
func GetSourceTigrisNephrologyPllcShahidCha(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTigrisNephrologyPllcShahidCha]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tigris Nephrology PLLC Shahid Cha"
	sourceDef.SourceType = pkg.SourceTypeTigrisNephrologyPllcShahidCha
