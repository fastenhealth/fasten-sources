...
func GetSourceEvergreenNeurologyAndSleepCente(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEvergreenNeurologyAndSleepCente]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Evergreen Neurology and Sleep Cente"
	sourceDef.SourceType = pkg.SourceTypeEvergreenNeurologyAndSleepCente
