...
func GetSourceAspiringMindsPsychologicalServic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAspiringMindsPsychologicalServic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Aspiring Minds Psychological servic"
	sourceDef.SourceType = pkg.SourceTypeAspiringMindsPsychologicalServic
