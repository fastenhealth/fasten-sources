...
func GetSourceImpactNw(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeImpactNw]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Impact NW"
	sourceDef.SourceType = pkg.SourceTypeImpactNw
	sourceDef.Category = []string{"251B00000X", "251S00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1053624387", "1417495862"}}
	sourceDef.PatientAccessUrl = "https://impactnw.org/programs/early-childhood/pcts/"
