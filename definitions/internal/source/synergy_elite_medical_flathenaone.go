...
func GetSourceSynergyEliteMedicalFlathenaone(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSynergyEliteMedicalFlathenaone]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Synergy Elite Medical Flathenaone"
	sourceDef.SourceType = pkg.SourceTypeSynergyEliteMedicalFlathenaone
