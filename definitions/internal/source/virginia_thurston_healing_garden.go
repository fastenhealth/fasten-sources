...
func GetSourceVirginiaThurstonHealingGarden(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVirginiaThurstonHealingGarden]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Virginia Thurston Healing Garden"
	sourceDef.SourceType = pkg.SourceTypeVirginiaThurstonHealingGarden
