...
func GetSourceJustLadiesHealthcareOfTheTreasureCoast(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJustLadiesHealthcareOfTheTreasureCoast]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "JUST LADIES HEALTHCARE OF THE TREASURE COAST"
	sourceDef.SourceType = pkg.SourceTypeJustLadiesHealthcareOfTheTreasureCoast
