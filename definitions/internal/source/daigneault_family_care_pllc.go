...
func GetSourceDaigneaultFamilyCarePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDaigneaultFamilyCarePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DAIGNEAULT FAMILY CARE PLLC"
	sourceDef.SourceType = pkg.SourceTypeDaigneaultFamilyCarePllc
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1750050530"}}
	sourceDef.PatientAccessUrl = "http://www.daigneaultfamilycare.com"
