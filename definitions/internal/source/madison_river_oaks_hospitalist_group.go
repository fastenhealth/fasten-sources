...
func GetSourceMadisonRiverOaksHospitalistGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMadisonRiverOaksHospitalistGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Madison River Oaks Hospitalist Group"
	sourceDef.SourceType = pkg.SourceTypeMadisonRiverOaksHospitalistGroup
