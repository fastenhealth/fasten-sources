...
func GetSourceNucleusClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNucleusClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Nucleus Clinic"
	sourceDef.SourceType = pkg.SourceTypeNucleusClinic
	sourceDef.Category = []string{"251K00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1982882742"}}
	sourceDef.PatientAccessUrl = "http://www.nucleusclinic.org"
