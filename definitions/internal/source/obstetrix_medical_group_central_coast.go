...
func GetSourceObstetrixMedicalGroupCentralCoast(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeObstetrixMedicalGroupCentralCoast]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Obstetrix Medical Group-Central Coast"
	sourceDef.SourceType = pkg.SourceTypeObstetrixMedicalGroupCentralCoast
