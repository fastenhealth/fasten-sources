...
func GetSourceInspiradaPediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInspiradaPediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Inspirada Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeInspiradaPediatrics
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{"INSPIRADA PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1144923459"}}
	sourceDef.PatientAccessUrl = "http://www.inspiradapediatrics.com"
