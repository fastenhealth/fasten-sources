...
func GetSourceAshleyAnnRickettsDnpPmhnpLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAshleyAnnRickettsDnpPmhnpLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ASHLEY ANN RICKETTS DNP PMHNP LLC"
	sourceDef.SourceType = pkg.SourceTypeAshleyAnnRickettsDnpPmhnpLlc
	sourceDef.Category = []string{"363LP0808X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1588361562"}}
