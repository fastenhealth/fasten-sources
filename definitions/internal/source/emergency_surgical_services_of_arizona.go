...
func GetSourceEmergencySurgicalServicesOfArizona(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEmergencySurgicalServicesOfArizona]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Emergency Surgical Services Of Arizona"
	sourceDef.SourceType = pkg.SourceTypeEmergencySurgicalServicesOfArizona
