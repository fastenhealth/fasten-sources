...
func GetSourceIlluminareBehavioralHealthServic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIlluminareBehavioralHealthServic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Illuminare Behavioral Health Servic"
	sourceDef.SourceType = pkg.SourceTypeIlluminareBehavioralHealthServic
