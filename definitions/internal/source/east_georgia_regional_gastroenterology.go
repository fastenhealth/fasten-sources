...
func GetSourceEastGeorgiaRegionalGastroenterology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEastGeorgiaRegionalGastroenterology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "East Georgia Regional Gastroenterology"
	sourceDef.SourceType = pkg.SourceTypeEastGeorgiaRegionalGastroenterology
