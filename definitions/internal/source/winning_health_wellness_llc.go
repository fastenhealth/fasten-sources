...
func GetSourceWinningHealthWellnessLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWinningHealthWellnessLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Winning Health Wellness LLC"
	sourceDef.SourceType = pkg.SourceTypeWinningHealthWellnessLlc
	sourceDef.Category = []string{"207QS0010X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1366869190"}}
	sourceDef.PatientAccessUrl = "http://www.winninghealth.md"
