...
func GetSourceNorthwestPrimaryCareJonesClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestPrimaryCareJonesClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Primary Care  Jones Clinic"
	sourceDef.SourceType = pkg.SourceTypeNorthwestPrimaryCareJonesClinic
