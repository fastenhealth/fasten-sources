...
func GetSourceObstetricsGynAssociatesOfCentralFla(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeObstetricsGynAssociatesOfCentralFla]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "OBSTETRICS  GYN ASSOCIATES OF CENTRAL FLA"
	sourceDef.SourceType = pkg.SourceTypeObstetricsGynAssociatesOfCentralFla
