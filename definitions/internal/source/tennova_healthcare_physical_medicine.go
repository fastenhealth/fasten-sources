...
func GetSourceTennovaHealthcarePhysicalMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaHealthcarePhysicalMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Healthcare  Physical Medicine"
	sourceDef.SourceType = pkg.SourceTypeTennovaHealthcarePhysicalMedicine
