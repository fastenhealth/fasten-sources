...
func GetSourceAlliancehealthMedicalGroupPrimaryCareDurant(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupPrimaryCareDurant]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Primary Care Durant"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupPrimaryCareDurant
