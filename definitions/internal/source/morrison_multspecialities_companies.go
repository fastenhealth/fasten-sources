...
func GetSourceMorrisonMultspecialitiesCompanies(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMorrisonMultspecialitiesCompanies]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MORRISON MULTSPECIALITIES COMPANIES"
	sourceDef.SourceType = pkg.SourceTypeMorrisonMultspecialitiesCompanies
