...
func GetSourceKathyJeanSparacinoMdMedicalGro(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKathyJeanSparacinoMdMedicalGro]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kathy Jean Sparacino Md Medical Gro"
	sourceDef.SourceType = pkg.SourceTypeKathyJeanSparacinoMdMedicalGro
