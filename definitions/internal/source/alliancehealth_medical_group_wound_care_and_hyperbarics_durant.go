...
func GetSourceAlliancehealthMedicalGroupWoundCareAndHyperbaricsDurant(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupWoundCareAndHyperbaricsDurant]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Wound Care and Hyperbarics Durant"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupWoundCareAndHyperbaricsDurant
