...
func GetSourceUrologySurgicalCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUrologySurgicalCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Urology Surgical Center LLC"
	sourceDef.SourceType = pkg.SourceTypeUrologySurgicalCenterLlc
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1467490433"}}
	sourceDef.PatientAccessUrl = "https://lincolnurologypc.com/surgical-center-overview/"
