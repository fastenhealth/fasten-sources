...
func GetSourceSaddlebackFamilyAndUrgentCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSaddlebackFamilyAndUrgentCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Saddleback Family and Urgent Care"
	sourceDef.SourceType = pkg.SourceTypeSaddlebackFamilyAndUrgentCare
