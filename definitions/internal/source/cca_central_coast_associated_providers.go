...
func GetSourceCcaCentralCoastAssociatedProviders(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCcaCentralCoastAssociatedProviders]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CCA  Central Coast Associated Providers"
	sourceDef.SourceType = pkg.SourceTypeCcaCentralCoastAssociatedProviders
