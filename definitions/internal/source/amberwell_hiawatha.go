...
func GetSourceAmberwellHiawatha(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmberwellHiawatha]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Amberwell Hiawatha"
	sourceDef.SourceType = pkg.SourceTypeAmberwellHiawatha
	sourceDef.Category = []string{"282NC0060X"}
	sourceDef.Aliases = []string{"AMBERWELL HIAWATHA"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1255421210"}}
