...
func GetSourceNeurologySpecialistsOfGreaterPi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeurologySpecialistsOfGreaterPi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Neurology Specialists of Greater Pi"
	sourceDef.SourceType = pkg.SourceTypeNeurologySpecialistsOfGreaterPi
