...
func GetSourceClevelandFamilyHealthClinicPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeClevelandFamilyHealthClinicPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cleveland Family Health Clinic PLLC"
	sourceDef.SourceType = pkg.SourceTypeClevelandFamilyHealthClinicPllc
