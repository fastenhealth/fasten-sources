...
func GetSourceNorthOkaloosaPhysicianGroupConvenientCareClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthOkaloosaPhysicianGroupConvenientCareClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Okaloosa Physician Group  Convenient Care Clinic"
	sourceDef.SourceType = pkg.SourceTypeNorthOkaloosaPhysicianGroupConvenientCareClinic
