...
func GetSourceParisTherapeuticsInterventions(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeParisTherapeuticsInterventions]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Paris Therapeutics Interventions"
	sourceDef.SourceType = pkg.SourceTypeParisTherapeuticsInterventions
