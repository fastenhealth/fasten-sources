...
func GetSourceAaaFamilyMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAaaFamilyMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AAA Family Medical Group"
	sourceDef.SourceType = pkg.SourceTypeAaaFamilyMedicalGroup
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"AAA FAMILY MEDICAL GROUP"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1396147419"}}
