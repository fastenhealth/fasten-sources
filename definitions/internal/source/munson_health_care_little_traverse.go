...
func GetSourceMunsonHealthCareLittleTraverse(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMunsonHealthCareLittleTraverse]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Munson Health Care Little Traverse"
	sourceDef.SourceType = pkg.SourceTypeMunsonHealthCareLittleTraverse
