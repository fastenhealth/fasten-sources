...
func GetSourceInjuryCareOfColoradoPhysicalTherapy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInjuryCareOfColoradoPhysicalTherapy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Injury Care of Colorado Physical Therapy"
	sourceDef.SourceType = pkg.SourceTypeInjuryCareOfColoradoPhysicalTherapy
