...
func GetSourceAdvancedPracticeHeadacheAndPrim(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedPracticeHeadacheAndPrim]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Advanced Practice Headache and Prim"
	sourceDef.SourceType = pkg.SourceTypeAdvancedPracticeHeadacheAndPrim
