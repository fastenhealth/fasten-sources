...
func GetSourceSouthernCaliforniaWomensHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernCaliforniaWomensHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SOUTHERN CALIFORNIA WOMENS HEALTH"
	sourceDef.SourceType = pkg.SourceTypeSouthernCaliforniaWomensHealth
