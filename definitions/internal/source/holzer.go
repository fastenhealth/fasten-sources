...
func GetSourceHolzer(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHolzer]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Holzer"
	sourceDef.SourceType = pkg.SourceTypeHolzer
	sourceDef.Category = []string{"282NR1301X", "291U00000X"}
	sourceDef.Aliases = []string{"HOLZER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1942256888"}}
