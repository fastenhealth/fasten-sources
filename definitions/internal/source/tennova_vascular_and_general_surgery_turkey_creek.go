...
func GetSourceTennovaVascularAndGeneralSurgeryTurkeyCreek(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaVascularAndGeneralSurgeryTurkeyCreek]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Vascular and General Surgery  Turkey Creek"
	sourceDef.SourceType = pkg.SourceTypeTennovaVascularAndGeneralSurgeryTurkeyCreek
