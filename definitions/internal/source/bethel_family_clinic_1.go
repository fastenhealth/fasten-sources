...
func GetSourceBethelFamilyClinic1(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBethelFamilyClinic1]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bethel Family Clinic"
	sourceDef.SourceType = pkg.SourceTypeBethelFamilyClinic1
	sourceDef.Category = []string{"261Q00000X", "261QF0400X", "261QP2300X"}
	sourceDef.Aliases = []string{"BETHEL FAMILY CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1124078019", "1346958766", "1548866270", "1891279097"}}
	sourceDef.PatientAccessUrl = "https://bethelfamilyclinic.org/"
