...
func GetSourceTennovaThyroidParathyroidSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaThyroidParathyroidSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Thyroid  Parathyroid Surgery"
	sourceDef.SourceType = pkg.SourceTypeTennovaThyroidParathyroidSurgery
