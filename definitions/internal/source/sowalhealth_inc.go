...
func GetSourceSowalhealthInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSowalhealthInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SOWALHEALTH INC"
	sourceDef.SourceType = pkg.SourceTypeSowalhealthInc
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{"SOWAL HEALTH & WELLNESS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1720794522"}}
