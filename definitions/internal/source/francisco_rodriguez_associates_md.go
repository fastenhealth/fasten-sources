...
func GetSourceFranciscoRodriguezAssociatesMd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFranciscoRodriguezAssociatesMd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Francisco Rodriguez  Associates MD"
	sourceDef.SourceType = pkg.SourceTypeFranciscoRodriguezAssociatesMd
