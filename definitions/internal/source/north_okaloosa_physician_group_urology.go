...
func GetSourceNorthOkaloosaPhysicianGroupUrology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthOkaloosaPhysicianGroupUrology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Okaloosa Physician Group  Urology"
	sourceDef.SourceType = pkg.SourceTypeNorthOkaloosaPhysicianGroupUrology
