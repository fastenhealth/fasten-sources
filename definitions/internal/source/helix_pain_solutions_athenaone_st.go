...
func GetSourceHelixPainSolutionsAthenaoneSt(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHelixPainSolutionsAthenaoneSt]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Helix Pain Solutions athenaOne St"
	sourceDef.SourceType = pkg.SourceTypeHelixPainSolutionsAthenaoneSt
