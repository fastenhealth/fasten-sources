...
func GetSourceVohraWoundPhysiciansOfFlLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVohraWoundPhysiciansOfFlLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VOHRA WOUND PHYSICIANS OF FL LLC"
	sourceDef.SourceType = pkg.SourceTypeVohraWoundPhysiciansOfFlLlc
