...
func GetSourceWalkInGynCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWalkInGynCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Walk In GYN Care"
	sourceDef.SourceType = pkg.SourceTypeWalkInGynCare
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{"WALK IN GYN CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1649820614"}}
