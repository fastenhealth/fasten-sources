...
func GetSourceValparaisoMedicalCenterEmergency(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeValparaisoMedicalCenterEmergency]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Valparaiso Medical Center Emergency"
	sourceDef.SourceType = pkg.SourceTypeValparaisoMedicalCenterEmergency
