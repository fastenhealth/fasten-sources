...
func GetSourceCardiologyAssociatesOfMichiganPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCardiologyAssociatesOfMichiganPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Cardiology Associates Of Michigan PC"
	sourceDef.SourceType = pkg.SourceTypeCardiologyAssociatesOfMichiganPc
