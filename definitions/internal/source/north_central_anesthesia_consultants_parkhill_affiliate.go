...
func GetSourceNorthCentralAnesthesiaConsultantsParkhillAffiliate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthCentralAnesthesiaConsultantsParkhillAffiliate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Central Anesthesia Consultants ParkHill Affiliate"
	sourceDef.SourceType = pkg.SourceTypeNorthCentralAnesthesiaConsultantsParkhillAffiliate
