...
func GetSourceGateCityPrimaryCareCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGateCityPrimaryCareCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gate City Primary Care Center"
	sourceDef.SourceType = pkg.SourceTypeGateCityPrimaryCareCenter
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"GATE CITY PRIMARY CARE CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1295265221"}}
