...
func GetSourceMotivatedHealthChoicesPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMotivatedHealthChoicesPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MOTIVATED HEALTH CHOICES PLLC"
	sourceDef.SourceType = pkg.SourceTypeMotivatedHealthChoicesPllc
