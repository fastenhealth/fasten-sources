...
func GetSourceNorthLindenHealthWellnessServ(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthLindenHealthWellnessServ]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTH LINDEN HEALTH  WELLNESS SERV"
	sourceDef.SourceType = pkg.SourceTypeNorthLindenHealthWellnessServ
