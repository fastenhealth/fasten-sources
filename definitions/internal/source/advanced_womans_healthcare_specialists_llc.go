...
func GetSourceAdvancedWomansHealthcareSpecialistsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedWomansHealthcareSpecialistsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ADVANCED WOMANS HEALTHCARE SPECIALISTS LLC"
	sourceDef.SourceType = pkg.SourceTypeAdvancedWomansHealthcareSpecialistsLlc
