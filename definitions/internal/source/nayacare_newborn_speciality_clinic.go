...
func GetSourceNayacareNewbornSpecialityClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNayacareNewbornSpecialityClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NayaCare Newborn Speciality Clinic"
	sourceDef.SourceType = pkg.SourceTypeNayacareNewbornSpecialityClinic
