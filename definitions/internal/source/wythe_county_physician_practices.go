...
func GetSourceWytheCountyPhysicianPractices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWytheCountyPhysicianPractices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WYTHE COUNTY PHYSICIAN PRACTICES"
	sourceDef.SourceType = pkg.SourceTypeWytheCountyPhysicianPractices
