...
func GetSourceProbityHealthcareServicesClini(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeProbityHealthcareServicesClini]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Probity Healthcare Services  Clini"
	sourceDef.SourceType = pkg.SourceTypeProbityHealthcareServicesClini
