...
func GetSourceAspenSpineNeurosurgeryCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAspenSpineNeurosurgeryCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Aspen Spine  Neurosurgery Center"
	sourceDef.SourceType = pkg.SourceTypeAspenSpineNeurosurgeryCenter
