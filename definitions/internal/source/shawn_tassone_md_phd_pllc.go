...
func GetSourceShawnTassoneMdPhdPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShawnTassoneMdPhdPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Shawn Tassone MD PhD PLLC"
	sourceDef.SourceType = pkg.SourceTypeShawnTassoneMdPhdPllc
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1225575244"}}
