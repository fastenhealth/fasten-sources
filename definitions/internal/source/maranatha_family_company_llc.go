...
func GetSourceMaranathaFamilyCompanyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMaranathaFamilyCompanyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Maranatha Family Company LLC"
	sourceDef.SourceType = pkg.SourceTypeMaranathaFamilyCompanyLlc
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{"MARANATHA PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1609584978"}}
