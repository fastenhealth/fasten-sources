...
func GetSourceFmbMedicalLlcDbaEsteroIslandM(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFmbMedicalLlcDbaEsteroIslandM]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FMB Medical LLC DBA Estero Island M"
	sourceDef.SourceType = pkg.SourceTypeFmbMedicalLlcDbaEsteroIslandM
