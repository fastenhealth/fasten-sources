...
func GetSourceFamilyAffairCareLimitedLiabilit(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFamilyAffairCareLimitedLiabilit]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FAMILY AFFAIR CARE LIMITED LIABILIT"
	sourceDef.SourceType = pkg.SourceTypeFamilyAffairCareLimitedLiabilit
