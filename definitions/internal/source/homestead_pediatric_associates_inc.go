...
func GetSourceHomesteadPediatricAssociatesInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHomesteadPediatricAssociatesInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Homestead Pediatric Associates Inc"
	sourceDef.SourceType = pkg.SourceTypeHomesteadPediatricAssociatesInc
