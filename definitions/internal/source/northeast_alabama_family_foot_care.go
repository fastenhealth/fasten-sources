...
func GetSourceNortheastAlabamaFamilyFootCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNortheastAlabamaFamilyFootCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northeast Alabama Family Foot Care"
	sourceDef.SourceType = pkg.SourceTypeNortheastAlabamaFamilyFootCare
