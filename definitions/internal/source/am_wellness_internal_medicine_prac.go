...
func GetSourceAmWellnessInternalMedicinePrac(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmWellnessInternalMedicinePrac]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AM Wellness Internal Medicine Prac"
	sourceDef.SourceType = pkg.SourceTypeAmWellnessInternalMedicinePrac
