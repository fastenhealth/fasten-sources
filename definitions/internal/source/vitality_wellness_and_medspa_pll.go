...
func GetSourceVitalityWellnessAndMedspaPll(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVitalityWellnessAndMedspaPll]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VITALITY WELLNESS AND MEDSPA PLL"
	sourceDef.SourceType = pkg.SourceTypeVitalityWellnessAndMedspaPll
