...
func GetSourceCc901WomensHealthServicesOfBradentonLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc901WomensHealthServicesOfBradentonLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC901WOMENS HEALTH SERVICES OF BRADENTON LLC"
	sourceDef.SourceType = pkg.SourceTypeCc901WomensHealthServicesOfBradentonLlc
