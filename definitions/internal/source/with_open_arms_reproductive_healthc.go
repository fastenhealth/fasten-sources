...
func GetSourceWithOpenArmsReproductiveHealthc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWithOpenArmsReproductiveHealthc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "With Open Arms Reproductive Healthc"
	sourceDef.SourceType = pkg.SourceTypeWithOpenArmsReproductiveHealthc
