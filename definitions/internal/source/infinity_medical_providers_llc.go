...
func GetSourceInfinityMedicalProvidersLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInfinityMedicalProvidersLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "INFINITY MEDICAL PROVIDERS LLC"
	sourceDef.SourceType = pkg.SourceTypeInfinityMedicalProvidersLlc
	sourceDef.Category = []string{"207Q00000X", "261QP2300X"}
	sourceDef.Aliases = []string{"INFINITY MEDICAL PROVIDERS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1154957678"}}
