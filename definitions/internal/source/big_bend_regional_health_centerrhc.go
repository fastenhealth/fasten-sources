...
func GetSourceBigBendRegionalHealthCenterrhc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBigBendRegionalHealthCenterrhc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Big Bend Regional Health CenterRHC"
	sourceDef.SourceType = pkg.SourceTypeBigBendRegionalHealthCenterrhc
