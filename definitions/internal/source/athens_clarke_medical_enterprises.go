...
func GetSourceAthensClarkeMedicalEnterprises(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAthensClarkeMedicalEnterprises]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ATHENS CLARKE MEDICAL ENTERPRISES"
	sourceDef.SourceType = pkg.SourceTypeAthensClarkeMedicalEnterprises
