...
func GetSourceNpHouseCallsOfMassachusettsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNpHouseCallsOfMassachusettsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NP House Calls of Massachusetts LLC"
	sourceDef.SourceType = pkg.SourceTypeNpHouseCallsOfMassachusettsLlc
