...
func GetSourceCarolinaPinesEnt(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCarolinaPinesEnt]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Carolina Pines ENT"
	sourceDef.SourceType = pkg.SourceTypeCarolinaPinesEnt
	sourceDef.Category = []string{"207Y00000X"}
	sourceDef.Aliases = []string{"CAROLINA PINES ENT"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1992316764"}}
