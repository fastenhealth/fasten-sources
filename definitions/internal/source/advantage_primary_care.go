...
func GetSourceAdvantagePrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvantagePrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Advantage Primary Care"
	sourceDef.SourceType = pkg.SourceTypeAdvantagePrimaryCare
	sourceDef.Category = []string{"363A00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1245959287"}}
	sourceDef.PatientAccessUrl = "http://www.advantageRVA.com"
