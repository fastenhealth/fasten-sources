...
func GetSourceSurgicalAssociatesOfBloomington(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSurgicalAssociatesOfBloomington]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Surgical Associates of Bloomington"
	sourceDef.SourceType = pkg.SourceTypeSurgicalAssociatesOfBloomington
