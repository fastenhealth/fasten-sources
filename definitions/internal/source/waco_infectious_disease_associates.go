...
func GetSourceWacoInfectiousDiseaseAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWacoInfectiousDiseaseAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Waco Infectious Disease Associates"
	sourceDef.SourceType = pkg.SourceTypeWacoInfectiousDiseaseAssociates
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1881808566"}}
	sourceDef.PatientAccessUrl = "http://www.wipdapa.com"
