...
func GetSourceEdwardCondonMedicalPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEdwardCondonMedicalPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "EDWARD CONDON MEDICAL PC"
	sourceDef.SourceType = pkg.SourceTypeEdwardCondonMedicalPc
	sourceDef.Category = []string{"207RE0101X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1811174519"}}
