...
func GetSourceOrthopedicSurgeryAndSportsMedic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOrthopedicSurgeryAndSportsMedic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Orthopedic Surgery and Sports Medic"
	sourceDef.SourceType = pkg.SourceTypeOrthopedicSurgeryAndSportsMedic
