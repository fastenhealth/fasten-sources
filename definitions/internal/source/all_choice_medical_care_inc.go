...
func GetSourceAllChoiceMedicalCareInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllChoiceMedicalCareInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ALL CHOICE MEDICAL CARE INC"
	sourceDef.SourceType = pkg.SourceTypeAllChoiceMedicalCareInc
	sourceDef.Category = []string{"261QH0100X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1134712615"}}
