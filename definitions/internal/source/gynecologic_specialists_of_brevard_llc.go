...
func GetSourceGynecologicSpecialistsOfBrevardLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGynecologicSpecialistsOfBrevardLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GYNECOLOGIC SPECIALISTS OF BREVARD LLC"
	sourceDef.SourceType = pkg.SourceTypeGynecologicSpecialistsOfBrevardLlc
