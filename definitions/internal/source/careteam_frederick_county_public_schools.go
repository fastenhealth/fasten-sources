...
func GetSourceCareteamFrederickCountyPublicSchools(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCareteamFrederickCountyPublicSchools]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CareTeam  Frederick County Public Schools"
	sourceDef.SourceType = pkg.SourceTypeCareteamFrederickCountyPublicSchools
