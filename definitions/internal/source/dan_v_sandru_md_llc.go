...
func GetSourceDanVSandruMdLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDanVSandruMdLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DAN V SANDRU MD LLC"
	sourceDef.SourceType = pkg.SourceTypeDanVSandruMdLlc
	sourceDef.Category = []string{"363LA2200X"}
	sourceDef.Aliases = []string{"DOCTOR'S OFFICE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1962921841"}}
