...
func GetSourceSouthwestMinnesotaOrthoAndAsc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthwestMinnesotaOrthoAndAsc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southwest Minnesota Ortho and ASC"
	sourceDef.SourceType = pkg.SourceTypeSouthwestMinnesotaOrthoAndAsc
