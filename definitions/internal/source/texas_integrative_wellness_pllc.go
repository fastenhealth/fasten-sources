...
func GetSourceTexasIntegrativeWellnessPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexasIntegrativeWellnessPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "TEXAS INTEGRATIVE WELLNESS PLLC"
	sourceDef.SourceType = pkg.SourceTypeTexasIntegrativeWellnessPllc
	sourceDef.Category = []string{"208D00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1770107542"}}
	sourceDef.PatientAccessUrl = "http://www.bluvida.com"
