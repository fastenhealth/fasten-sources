...
func GetSourceBrandonOrthopedicAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBrandonOrthopedicAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Brandon Orthopedic Associates"
	sourceDef.SourceType = pkg.SourceTypeBrandonOrthopedicAssociates
	sourceDef.Category = []string{"332B00000X"}
	sourceDef.Aliases = []string{"BRANDON ORTHOPEDIC ASSOCIATES"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1447751706"}}
