...
func GetSourceTennovaLafolletteOutpatientRehabCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaLafolletteOutpatientRehabCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova LaFollette Outpatient Rehab Center"
	sourceDef.SourceType = pkg.SourceTypeTennovaLafolletteOutpatientRehabCenter
