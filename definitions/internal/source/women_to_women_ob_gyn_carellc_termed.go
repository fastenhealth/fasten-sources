...
func GetSourceWomenToWomenObGynCarellcTermed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWomenToWomenObGynCarellcTermed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WOMEN TO WOMEN OB GYN CARELLC TERMED"
	sourceDef.SourceType = pkg.SourceTypeWomenToWomenObGynCarellcTermed
