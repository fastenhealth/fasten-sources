...
func GetSourceAlliancehealthMedicalGroupFamilyPracticeClintonHills(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupFamilyPracticeClintonHills]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Family Practice Clinton Hills"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupFamilyPracticeClintonHills
