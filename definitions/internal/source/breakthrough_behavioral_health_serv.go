...
func GetSourceBreakthroughBehavioralHealthServ(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBreakthroughBehavioralHealthServ]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BREAKTHROUGH BEHAVIORAL HEALTH SERV"
	sourceDef.SourceType = pkg.SourceTypeBreakthroughBehavioralHealthServ
