...
func GetSourceFyzicalTherapyBalanceCenterRo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFyzicalTherapyBalanceCenterRo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Fyzical Therapy  Balance Center Ro"
	sourceDef.SourceType = pkg.SourceTypeFyzicalTherapyBalanceCenterRo
