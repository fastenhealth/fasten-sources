...
func GetSourceSonoranPediatricEndocrinologyAndDiabetesInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSonoranPediatricEndocrinologyAndDiabetesInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sonoran Pediatric Endocrinology and Diabetes Institute"
	sourceDef.SourceType = pkg.SourceTypeSonoranPediatricEndocrinologyAndDiabetesInstitute
