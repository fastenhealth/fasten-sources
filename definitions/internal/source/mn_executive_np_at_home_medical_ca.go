...
func GetSourceMnExecutiveNpAtHomeMedicalCa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMnExecutiveNpAtHomeMedicalCa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MN  Executive NP  At Home Medical Ca"
	sourceDef.SourceType = pkg.SourceTypeMnExecutiveNpAtHomeMedicalCa
