...
func GetSourceGlendaleNeurologicalClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGlendaleNeurologicalClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Glendale Neurological Clinic"
	sourceDef.SourceType = pkg.SourceTypeGlendaleNeurologicalClinic
	sourceDef.Category = []string{"2084N0400X"}
	sourceDef.Aliases = []string{"GLENDALE NEUROLOGICAL CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1720522063"}}
