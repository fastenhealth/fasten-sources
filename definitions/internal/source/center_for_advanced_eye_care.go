...
func GetSourceCenterForAdvancedEyeCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForAdvancedEyeCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Center For Advanced Eye Care"
	sourceDef.SourceType = pkg.SourceTypeCenterForAdvancedEyeCare
	sourceDef.Category = []string{"207W00000X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1285263566"}}
	sourceDef.PatientAccessUrl = "https://www.centerforeyes.com/"
