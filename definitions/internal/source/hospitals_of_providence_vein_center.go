...
func GetSourceHospitalsOfProvidenceVeinCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHospitalsOfProvidenceVeinCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hospitals of Providence Vein Center"
	sourceDef.SourceType = pkg.SourceTypeHospitalsOfProvidenceVeinCenter
