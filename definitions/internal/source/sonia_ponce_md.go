...
func GetSourceSoniaPonceMd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSoniaPonceMd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sonia Ponce MD"
	sourceDef.SourceType = pkg.SourceTypeSoniaPonceMd
	sourceDef.Category = []string{"174400000X"}
	sourceDef.Aliases = []string{"SONIA PONCE MD"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1861096067"}}
