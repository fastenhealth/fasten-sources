...
func GetSourceArizonaCommunityPhysicians(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArizonaCommunityPhysicians]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeEpic))

	sourceDef.Display = "Arizona Community Physicians"
	sourceDef.SourceType = pkg.SourceTypeArizonaCommunityPhysicians
	sourceDef.Category = []string{"261QH0700X", "261QM1300X", "261QM2500X", "261QS1200X"}
