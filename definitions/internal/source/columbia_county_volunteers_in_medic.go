...
func GetSourceColumbiaCountyVolunteersInMedic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeColumbiaCountyVolunteersInMedic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Columbia County Volunteers in Medic"
	sourceDef.SourceType = pkg.SourceTypeColumbiaCountyVolunteersInMedic
