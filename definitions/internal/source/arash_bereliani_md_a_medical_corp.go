...
func GetSourceArashBerelianiMdAMedicalCorp(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArashBerelianiMdAMedicalCorp]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ARASH BERELIANI MD A MEDICAL CORP"
	sourceDef.SourceType = pkg.SourceTypeArashBerelianiMdAMedicalCorp
