...
func GetSourceWholeSystemsHealthcareLakeOswego(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWholeSystemsHealthcareLakeOswego]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Whole Systems Healthcare Lake Oswego"
	sourceDef.SourceType = pkg.SourceTypeWholeSystemsHealthcareLakeOswego
