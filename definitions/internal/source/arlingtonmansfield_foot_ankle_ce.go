...
func GetSourceArlingtonmansfieldFootAnkleCe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArlingtonmansfieldFootAnkleCe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ArlingtonMansfield Foot  Ankle Ce"
	sourceDef.SourceType = pkg.SourceTypeArlingtonmansfieldFootAnkleCe
