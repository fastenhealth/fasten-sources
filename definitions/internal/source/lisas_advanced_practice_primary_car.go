...
func GetSourceLisasAdvancedPracticePrimaryCar(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLisasAdvancedPracticePrimaryCar]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LISAS ADVANCED PRACTICE PRIMARY CAR"
	sourceDef.SourceType = pkg.SourceTypeLisasAdvancedPracticePrimaryCar
