...
func GetSourceDiverisOrthopedicsAndSportsMedi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDiverisOrthopedicsAndSportsMedi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Diveris Orthopedics and Sports Medi"
	sourceDef.SourceType = pkg.SourceTypeDiverisOrthopedicsAndSportsMedi
