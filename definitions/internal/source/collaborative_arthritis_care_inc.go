...
func GetSourceCollaborativeArthritisCareInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCollaborativeArthritisCareInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Collaborative Arthritis Care Inc"
	sourceDef.SourceType = pkg.SourceTypeCollaborativeArthritisCareInc
