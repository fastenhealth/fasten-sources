...
func GetSourceMichiganCenterForHematologyAnd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMichiganCenterForHematologyAnd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MICHIGAN CENTER FOR HEMATOLOGY AND"
	sourceDef.SourceType = pkg.SourceTypeMichiganCenterForHematologyAnd
