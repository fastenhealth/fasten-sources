...
func GetSourceCenterForAdultMedicinePreventativeCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForAdultMedicinePreventativeCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center for Adult Medicine  Preventative Care"
	sourceDef.SourceType = pkg.SourceTypeCenterForAdultMedicinePreventativeCare
