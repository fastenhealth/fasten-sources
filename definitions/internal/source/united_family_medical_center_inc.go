...
func GetSourceUnitedFamilyMedicalCenterInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUnitedFamilyMedicalCenterInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "UNITED FAMILY MEDICAL CENTER INC"
	sourceDef.SourceType = pkg.SourceTypeUnitedFamilyMedicalCenterInc
