...
func GetSourceGessnerPetersonHouseCalls(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGessnerPetersonHouseCalls]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GESSNER PETERSON HOUSE CALLS"
	sourceDef.SourceType = pkg.SourceTypeGessnerPetersonHouseCalls
