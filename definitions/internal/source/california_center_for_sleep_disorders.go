...
func GetSourceCaliforniaCenterForSleepDisorders(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCaliforniaCenterForSleepDisorders]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "California Center for Sleep Disorders"
	sourceDef.SourceType = pkg.SourceTypeCaliforniaCenterForSleepDisorders
