...
func GetSourceNorthwestFamilyMedicineAndObstetrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestFamilyMedicineAndObstetrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Family Medicine and Obstetrics"
	sourceDef.SourceType = pkg.SourceTypeNorthwestFamilyMedicineAndObstetrics
