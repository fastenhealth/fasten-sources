...
func GetSourceOliveTreeIntegrativeHealthLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOliveTreeIntegrativeHealthLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "OLIVE TREE INTEGRATIVE HEALTH LLC"
	sourceDef.SourceType = pkg.SourceTypeOliveTreeIntegrativeHealthLlc
