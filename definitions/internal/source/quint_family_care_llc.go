...
func GetSourceQuintFamilyCareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeQuintFamilyCareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Quint Family Care LLC"
	sourceDef.SourceType = pkg.SourceTypeQuintFamilyCareLlc
	sourceDef.Category = []string{"364SF0001X"}
	sourceDef.Aliases = []string{"QUINT FAMILY CARE LLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1801466461"}}
	sourceDef.PatientAccessUrl = "http://www.quintfamilycare.com"
