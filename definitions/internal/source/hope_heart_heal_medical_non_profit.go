...
func GetSourceHopeHeartHealMedicalNonProfit(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHopeHeartHealMedicalNonProfit]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hope Heart Heal Medical Non Profit"
	sourceDef.SourceType = pkg.SourceTypeHopeHeartHealMedicalNonProfit
