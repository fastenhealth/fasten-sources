...
func GetSourceFrontRangeOrthopaedicsPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFrontRangeOrthopaedicsPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Front Range Orthopaedics PC"
	sourceDef.SourceType = pkg.SourceTypeFrontRangeOrthopaedicsPc
	sourceDef.Category = []string{"207X00000X", "213E00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1841212719"}}
