...
func GetSourceBrooklynUrgentCareOfMarinePark(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBrooklynUrgentCareOfMarinePark]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Brooklyn Urgent Care Of Marine Park"
	sourceDef.SourceType = pkg.SourceTypeBrooklynUrgentCareOfMarinePark
