...
func GetSourceObriaMedicalClinicsOfSouthernCalifornia(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeObriaMedicalClinicsOfSouthernCalifornia]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "OBRIA MEDICAL CLINICS OF SOUTHERN CALIFORNIA"
	sourceDef.SourceType = pkg.SourceTypeObriaMedicalClinicsOfSouthernCalifornia
