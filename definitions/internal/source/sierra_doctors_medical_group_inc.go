...
func GetSourceSierraDoctorsMedicalGroupInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSierraDoctorsMedicalGroupInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sierra Doctors Medical Group Inc"
	sourceDef.SourceType = pkg.SourceTypeSierraDoctorsMedicalGroupInc
	sourceDef.Category = []string{"207P00000X", "207Q00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1669575841"}}
	sourceDef.PatientAccessUrl = "http://www.sierradoctors.com"
