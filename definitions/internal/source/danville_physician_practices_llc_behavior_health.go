...
func GetSourceDanvillePhysicianPracticesLlcBehaviorHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDanvillePhysicianPracticesLlcBehaviorHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DANVILLE PHYSICIAN PRACTICES LLC BEHAVIOR HEALTH"
	sourceDef.SourceType = pkg.SourceTypeDanvillePhysicianPracticesLlcBehaviorHealth
