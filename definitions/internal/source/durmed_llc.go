...
func GetSourceDurmedLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDurmedLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DURMED LLC"
	sourceDef.SourceType = pkg.SourceTypeDurmedLlc
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{"ALEXANDRU DURGHEU"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1568176097"}}
