...
func GetSourceCenterForSleepAndNasalSinusDi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForSleepAndNasalSinusDi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CENTER FOR SLEEP AND NASAL SINUS DI"
	sourceDef.SourceType = pkg.SourceTypeCenterForSleepAndNasalSinusDi
