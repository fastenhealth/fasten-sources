...
func GetSourceNorthShoreHeartAndVascularLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthShoreHeartAndVascularLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Shore Heart and Vascular LLC"
	sourceDef.SourceType = pkg.SourceTypeNorthShoreHeartAndVascularLlc
