...
func GetSourceTherapeuticHealthAndWellnessLl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTherapeuticHealthAndWellnessLl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "THERAPEUTIC HEALTH AND WELLNESS LL"
	sourceDef.SourceType = pkg.SourceTypeTherapeuticHealthAndWellnessLl
