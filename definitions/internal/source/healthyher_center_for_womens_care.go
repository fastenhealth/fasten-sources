...
func GetSourceHealthyherCenterForWomensCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHealthyherCenterForWomensCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HealthyHER Center for Womens Care"
	sourceDef.SourceType = pkg.SourceTypeHealthyherCenterForWomensCare
