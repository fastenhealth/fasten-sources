...
func GetSourceSouthCentralWellnessLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthCentralWellnessLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SOUTH CENTRAL WELLNESS LLC"
	sourceDef.SourceType = pkg.SourceTypeSouthCentralWellnessLlc
	sourceDef.Category = []string{"208800000X"}
	sourceDef.Aliases = []string{"SOUTH LAND UROLOGY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1154805828"}}
