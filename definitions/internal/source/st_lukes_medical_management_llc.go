...
func GetSourceStLukesMedicalManagementLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStLukesMedicalManagementLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ST LUKES MEDICAL MANAGEMENT LLC"
	sourceDef.SourceType = pkg.SourceTypeStLukesMedicalManagementLlc
