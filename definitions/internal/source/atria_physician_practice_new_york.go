...
func GetSourceAtriaPhysicianPracticeNewYork(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAtriaPhysicianPracticeNewYork]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Atria Physician Practice New York"
	sourceDef.SourceType = pkg.SourceTypeAtriaPhysicianPracticeNewYork
