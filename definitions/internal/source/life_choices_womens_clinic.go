...
func GetSourceLifeChoicesWomensClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLifeChoicesWomensClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Life Choices Womens Clinic"
	sourceDef.SourceType = pkg.SourceTypeLifeChoicesWomensClinic
	sourceDef.Category = []string{"251V00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1366574279"}}
	sourceDef.PatientAccessUrl = "http://pcwaz.com"
