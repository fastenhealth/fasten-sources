...
func GetSourceNorthwestPrimaryCareWillowCreek(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestPrimaryCareWillowCreek]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Primary Care  Willow Creek"
	sourceDef.SourceType = pkg.SourceTypeNorthwestPrimaryCareWillowCreek
