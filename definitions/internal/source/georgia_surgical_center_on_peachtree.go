...
func GetSourceGeorgiaSurgicalCenterOnPeachtree(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGeorgiaSurgicalCenterOnPeachtree]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GEORGIA SURGICAL CENTER ON PEACHTREE"
	sourceDef.SourceType = pkg.SourceTypeGeorgiaSurgicalCenterOnPeachtree
