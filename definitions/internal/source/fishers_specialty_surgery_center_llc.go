...
func GetSourceFishersSpecialtySurgeryCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFishersSpecialtySurgeryCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Fishers Specialty Surgery Center LLC"
	sourceDef.SourceType = pkg.SourceTypeFishersSpecialtySurgeryCenterLlc
