...
func GetSourceDmcMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDmcMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DMC Medical Group"
	sourceDef.SourceType = pkg.SourceTypeDmcMedicalGroup
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"DMC MEDICAL GROUP"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1073827101", "1902157498"}}
	sourceDef.PatientAccessUrl = "https://www.dmc.org/home"
