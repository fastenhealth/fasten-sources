...
func GetSourceSullivanWorkforceHealthWellnessClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSullivanWorkforceHealthWellnessClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sullivan Workforce Health  Wellness Clinic"
	sourceDef.SourceType = pkg.SourceTypeSullivanWorkforceHealthWellnessClinic
