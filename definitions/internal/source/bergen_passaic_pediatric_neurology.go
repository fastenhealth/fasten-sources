...
func GetSourceBergenPassaicPediatricNeurology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBergenPassaicPediatricNeurology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bergen Passaic Pediatric Neurology"
	sourceDef.SourceType = pkg.SourceTypeBergenPassaicPediatricNeurology
