...
func GetSourcePanhandleRuralHealthPrimaryCa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePanhandleRuralHealthPrimaryCa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Panhandle Rural Health  Primary Ca"
	sourceDef.SourceType = pkg.SourceTypePanhandleRuralHealthPrimaryCa
