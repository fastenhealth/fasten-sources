...
func GetSourceTennovaPrimaryCareCaryville(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaPrimaryCareCaryville]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Primary Care Caryville"
	sourceDef.SourceType = pkg.SourceTypeTennovaPrimaryCareCaryville
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"TENNOVA PRIMARY CARE CARYVILLE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1023613361"}}
