...
func GetSourceColoradoRiverGastroeneterology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeColoradoRiverGastroeneterology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Colorado River Gastroeneterology"
	sourceDef.SourceType = pkg.SourceTypeColoradoRiverGastroeneterology
