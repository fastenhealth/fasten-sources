...
func GetSourceKidsAndTeensEndocrinologyPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKidsAndTeensEndocrinologyPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kids and Teens Endocrinology PLLC"
	sourceDef.SourceType = pkg.SourceTypeKidsAndTeensEndocrinologyPllc
