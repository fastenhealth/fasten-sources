...
func GetSourceInternalAndPulmonaryMedicineOf(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInternalAndPulmonaryMedicineOf]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "INTERNAL AND PULMONARY MEDICINE OF"
	sourceDef.SourceType = pkg.SourceTypeInternalAndPulmonaryMedicineOf
