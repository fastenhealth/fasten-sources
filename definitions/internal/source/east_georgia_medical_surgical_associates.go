...
func GetSourceEastGeorgiaMedicalSurgicalAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEastGeorgiaMedicalSurgicalAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "East Georgia Medical  Surgical Associates"
	sourceDef.SourceType = pkg.SourceTypeEastGeorgiaMedicalSurgicalAssociates
