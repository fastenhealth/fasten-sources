...
func GetSourceProactiveMdOccupationalHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeProactiveMdOccupationalHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PROACTIVE MD OCCUPATIONAL HEALTH"
	sourceDef.SourceType = pkg.SourceTypeProactiveMdOccupationalHealth
