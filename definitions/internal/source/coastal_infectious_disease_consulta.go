...
func GetSourceCoastalInfectiousDiseaseConsulta(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCoastalInfectiousDiseaseConsulta]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Coastal Infectious Disease Consulta"
	sourceDef.SourceType = pkg.SourceTypeCoastalInfectiousDiseaseConsulta
