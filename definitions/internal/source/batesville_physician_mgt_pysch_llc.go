...
func GetSourceBatesvillePhysicianMgtPyschLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBatesvillePhysicianMgtPyschLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Batesville Physician MGT Pysch LLC"
	sourceDef.SourceType = pkg.SourceTypeBatesvillePhysicianMgtPyschLlc
