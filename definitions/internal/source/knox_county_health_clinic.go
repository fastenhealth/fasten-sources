...
func GetSourceKnoxCountyHealthClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKnoxCountyHealthClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Knox County Health Clinic"
	sourceDef.SourceType = pkg.SourceTypeKnoxCountyHealthClinic
	sourceDef.Category = []string{"261QD0000X"}
	sourceDef.Aliases = []string{"KNOX COUNTY HEALTH CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1407364417"}}
