...
func GetSourcePatriciaWHendersonDoPcDbaJack(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePatriciaWHendersonDoPcDbaJack]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PATRICIA W HENDERSON DO PC DBA Jack"
	sourceDef.SourceType = pkg.SourceTypePatriciaWHendersonDoPcDbaJack
