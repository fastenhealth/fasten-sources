...
func GetSourceCenterCityPodiatryAssociatesP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterCityPodiatryAssociatesP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center City Podiatry Associates P"
	sourceDef.SourceType = pkg.SourceTypeCenterCityPodiatryAssociatesP
