...
func GetSourceSportsPainAndRegenerativeInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSportsPainAndRegenerativeInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sports Pain and Regenerative Institute"
	sourceDef.SourceType = pkg.SourceTypeSportsPainAndRegenerativeInstitute
