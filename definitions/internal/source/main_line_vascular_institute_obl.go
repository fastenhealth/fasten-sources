...
func GetSourceMainLineVascularInstituteObl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMainLineVascularInstituteObl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Main Line Vascular Institute (OBL)"
	sourceDef.SourceType = pkg.SourceTypeMainLineVascularInstituteObl
