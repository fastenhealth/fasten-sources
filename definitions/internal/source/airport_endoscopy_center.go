...
func GetSourceAirportEndoscopyCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAirportEndoscopyCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Airport Endoscopy Center"
	sourceDef.SourceType = pkg.SourceTypeAirportEndoscopyCenter
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"AIRPORT ENDOSCOPY CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1013980317"}}
