...
func GetSourceShenValleyFamilyPracticeDbaShe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShenValleyFamilyPracticeDbaShe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Shen Valley Family Practice DBA She"
	sourceDef.SourceType = pkg.SourceTypeShenValleyFamilyPracticeDbaShe
