...
func GetSourceSeasideHealthCenterAtAtlanticShores(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSeasideHealthCenterAtAtlanticShores]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Seaside Health Center at Atlantic Shores"
	sourceDef.SourceType = pkg.SourceTypeSeasideHealthCenterAtAtlanticShores
