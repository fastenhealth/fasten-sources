...
func GetSourceRigelGeriatricsPllcDbaTexasGer(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRigelGeriatricsPllcDbaTexasGer]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Rigel Geriatrics PLLC Dba Texas Ger"
	sourceDef.SourceType = pkg.SourceTypeRigelGeriatricsPllcDbaTexasGer
