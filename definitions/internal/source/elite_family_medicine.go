...
func GetSourceEliteFamilyMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEliteFamilyMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Elite Family Medicine"
	sourceDef.SourceType = pkg.SourceTypeEliteFamilyMedicine
	sourceDef.Category = []string{"207Q00000X", "261QP2300X"}
	sourceDef.Aliases = []string{"ELITE FAMILY MEDICINE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1053003087", "1316698764", "1962742908"}}
