...
func GetSourceAmazingHealthCareAndServicesIn(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmazingHealthCareAndServicesIn]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AMAZING HEALTH CARE AND SERVICES IN"
	sourceDef.SourceType = pkg.SourceTypeAmazingHealthCareAndServicesIn
