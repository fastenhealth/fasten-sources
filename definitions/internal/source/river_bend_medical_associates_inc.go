...
func GetSourceRiverBendMedicalAssociatesInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRiverBendMedicalAssociatesInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "River Bend Medical Associates Inc"
	sourceDef.SourceType = pkg.SourceTypeRiverBendMedicalAssociatesInc
