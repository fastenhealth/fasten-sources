...
func GetSourceHousecallPhysiciansOfIllinoisS(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHousecallPhysiciansOfIllinoisS]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Housecall Physicians of Illinois S"
	sourceDef.SourceType = pkg.SourceTypeHousecallPhysiciansOfIllinoisS
