...
func GetSourceJacksonvilleCenterForEndoscopy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJacksonvilleCenterForEndoscopy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Jacksonville Center For Endoscopy"
	sourceDef.SourceType = pkg.SourceTypeJacksonvilleCenterForEndoscopy
