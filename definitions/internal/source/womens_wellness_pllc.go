...
func GetSourceWomensWellnessPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWomensWellnessPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Womens Wellness PLLC"
	sourceDef.SourceType = pkg.SourceTypeWomensWellnessPllc
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{"HOLLY A. DICKSON, MD", "WOMENS WELLNESS PLLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1801316179"}}
