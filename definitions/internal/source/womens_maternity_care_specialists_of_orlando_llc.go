...
func GetSourceWomensMaternityCareSpecialistsOfOrlandoLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWomensMaternityCareSpecialistsOfOrlandoLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WOMENS  MATERNITY CARE SPECIALISTS OF ORLANDO LLC"
	sourceDef.SourceType = pkg.SourceTypeWomensMaternityCareSpecialistsOfOrlandoLlc
