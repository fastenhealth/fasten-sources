...
func GetSourceCompleteHealthcareForWomenOfWellington(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCompleteHealthcareForWomenOfWellington]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COMPLETE HEALTHCARE FOR WOMEN OF WELLINGTON"
	sourceDef.SourceType = pkg.SourceTypeCompleteHealthcareForWomenOfWellington
