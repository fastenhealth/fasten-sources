...
func GetSourceNewberryCountyMemorialHospitalPhysicianPractices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNewberryCountyMemorialHospitalPhysicianPractices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Newberry County Memorial Hospital Physician Practices"
	sourceDef.SourceType = pkg.SourceTypeNewberryCountyMemorialHospitalPhysicianPractices
