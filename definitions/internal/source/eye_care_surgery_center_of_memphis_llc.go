...
func GetSourceEyeCareSurgeryCenterOfMemphisLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEyeCareSurgeryCenterOfMemphisLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Eye Care Surgery Center Of Memphis LLC"
	sourceDef.SourceType = pkg.SourceTypeEyeCareSurgeryCenterOfMemphisLlc
