...
func GetSourceHemalPatelInternalMedicinePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHemalPatelInternalMedicinePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HEMAL PATEL INTERNAL MEDICINE PLLC"
	sourceDef.SourceType = pkg.SourceTypeHemalPatelInternalMedicinePllc
