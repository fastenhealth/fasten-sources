...
func GetSourceRestoreWellnessAndPainManagemen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRestoreWellnessAndPainManagemen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Restore Wellness and Pain Managemen"
	sourceDef.SourceType = pkg.SourceTypeRestoreWellnessAndPainManagemen
