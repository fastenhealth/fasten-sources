...
func GetSourceBluebonnetFootAndAnkleStartup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBluebonnetFootAndAnkleStartup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bluebonnet Foot and Ankle Startup"
	sourceDef.SourceType = pkg.SourceTypeBluebonnetFootAndAnkleStartup
