...
func GetSourceFrankFichtelMdFacsNeurological(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFrankFichtelMdFacsNeurological]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Frank Fichtel MD FACS Neurological"
	sourceDef.SourceType = pkg.SourceTypeFrankFichtelMdFacsNeurological
