...
func GetSourceKnoxvilleShifaClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKnoxvilleShifaClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Knoxville Shifa Clinic"
	sourceDef.SourceType = pkg.SourceTypeKnoxvilleShifaClinic
	sourceDef.Category = []string{"207Q00000X", "261QP2300X"}
	sourceDef.Aliases = []string{"KNOXVILLE SHIFA CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1306325816"}}
