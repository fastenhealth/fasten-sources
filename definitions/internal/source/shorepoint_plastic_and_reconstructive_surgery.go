...
func GetSourceShorepointPlasticAndReconstructiveSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShorepointPlasticAndReconstructiveSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ShorePoint Plastic and Reconstructive Surgery"
	sourceDef.SourceType = pkg.SourceTypeShorepointPlasticAndReconstructiveSurgery
