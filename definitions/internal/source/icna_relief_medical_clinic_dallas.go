...
func GetSourceIcnaReliefMedicalClinicDallas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIcnaReliefMedicalClinicDallas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ICNA Relief Medical Clinic Dallas"
	sourceDef.SourceType = pkg.SourceTypeIcnaReliefMedicalClinicDallas
