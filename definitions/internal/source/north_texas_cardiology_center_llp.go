...
func GetSourceNorthTexasCardiologyCenterLlp(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthTexasCardiologyCenterLlp]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Texas Cardiology Center LLP"
	sourceDef.SourceType = pkg.SourceTypeNorthTexasCardiologyCenterLlp
