...
func GetSourceSisuHealthClinicLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSisuHealthClinicLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SISU HEALTH CLINIC LLC"
	sourceDef.SourceType = pkg.SourceTypeSisuHealthClinicLlc
	sourceDef.Category = []string{"363L00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1831817642"}}
	sourceDef.PatientAccessUrl = "http://sisuhealthclinic.com"
