...
func GetSourceRoyaltyNephrologyMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRoyaltyNephrologyMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Royalty Nephrology Medical Group"
	sourceDef.SourceType = pkg.SourceTypeRoyaltyNephrologyMedicalGroup
