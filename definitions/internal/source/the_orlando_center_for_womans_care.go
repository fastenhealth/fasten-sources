...
func GetSourceTheOrlandoCenterForWomansCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheOrlandoCenterForWomansCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "THE ORLANDO CENTER FOR WOMANS CARE"
	sourceDef.SourceType = pkg.SourceTypeTheOrlandoCenterForWomansCare
