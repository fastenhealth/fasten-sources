...
func GetSourceStarFootAnkleSpecialistsPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStarFootAnkleSpecialistsPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "STAR FOOT  ANKLE SPECIALISTS PLLC"
	sourceDef.SourceType = pkg.SourceTypeStarFootAnkleSpecialistsPllc
