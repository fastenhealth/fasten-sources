...
func GetSourceAlliancehealthMedicalGroupSpecialistsPoncaCity(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupSpecialistsPoncaCity]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Specialists  Ponca City"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupSpecialistsPoncaCity
