...
func GetSourcePortlandPainSolutionsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePortlandPainSolutionsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PORTLAND PAIN SOLUTIONS LLC"
	sourceDef.SourceType = pkg.SourceTypePortlandPainSolutionsLlc
	sourceDef.Category = []string{"261QP3300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1104400621"}}
	sourceDef.PatientAccessUrl = "http://www.portlandpainsolutions.com"
