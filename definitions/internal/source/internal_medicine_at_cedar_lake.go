...
func GetSourceInternalMedicineAtCedarLake(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInternalMedicineAtCedarLake]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Internal Medicine at Cedar Lake"
	sourceDef.SourceType = pkg.SourceTypeInternalMedicineAtCedarLake
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"INTERNAL MEDICINE AT CEDAR LAKE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1083971626"}}
