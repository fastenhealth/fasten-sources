...
func GetSourceOologahFamilyMedicineTanaVogel(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOologahFamilyMedicineTanaVogel]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "OOLOGAH FAMILY MEDICINE TANA VOGEL"
	sourceDef.SourceType = pkg.SourceTypeOologahFamilyMedicineTanaVogel
