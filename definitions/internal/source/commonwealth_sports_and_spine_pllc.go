...
func GetSourceCommonwealthSportsAndSpinePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCommonwealthSportsAndSpinePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Commonwealth Sports and Spine PLLC"
	sourceDef.SourceType = pkg.SourceTypeCommonwealthSportsAndSpinePllc
