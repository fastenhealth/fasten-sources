...
func GetSourceEverythingSpeechLlcDbaNdoscopy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEverythingSpeechLlcDbaNdoscopy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "EVERYTHING SPEECH LLC DBA Ndoscopy"
	sourceDef.SourceType = pkg.SourceTypeEverythingSpeechLlcDbaNdoscopy
