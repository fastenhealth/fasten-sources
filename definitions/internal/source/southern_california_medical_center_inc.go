...
func GetSourceSouthernCaliforniaMedicalCenterInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernCaliforniaMedicalCenterInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Southern California Medical Center Inc"
	sourceDef.SourceType = pkg.SourceTypeSouthernCaliforniaMedicalCenterInc
	sourceDef.Hidden = true
	sourceDef.PatientAccessUrl = "https://scmedcenter.org/"
