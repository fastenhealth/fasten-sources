...
func GetSourceFootAndAnkleHealthCentersLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFootAndAnkleHealthCentersLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Foot and Ankle Health Centers LLC"
	sourceDef.SourceType = pkg.SourceTypeFootAndAnkleHealthCentersLlc
