...
func GetSourceLakeOswegoFamilyPhysiciansPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLakeOswegoFamilyPhysiciansPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lake Oswego Family Physicians PC"
	sourceDef.SourceType = pkg.SourceTypeLakeOswegoFamilyPhysiciansPc
