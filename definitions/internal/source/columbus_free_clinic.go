...
func GetSourceColumbusFreeClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeColumbusFreeClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Columbus Free Clinic"
	sourceDef.SourceType = pkg.SourceTypeColumbusFreeClinic
	sourceDef.Category = []string{"261Q00000X", "261QP2300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1831747328"}}
	sourceDef.PatientAccessUrl = "http://www.columbusfreeclinic.com"
