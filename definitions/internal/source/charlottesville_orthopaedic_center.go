...
func GetSourceCharlottesvilleOrthopaedicCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCharlottesvilleOrthopaedicCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Charlottesville Orthopaedic Center"
	sourceDef.SourceType = pkg.SourceTypeCharlottesvilleOrthopaedicCenter
