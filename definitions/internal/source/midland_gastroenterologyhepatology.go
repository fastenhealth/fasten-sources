...
func GetSourceMidlandGastroenterologyhepatology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidlandGastroenterologyhepatology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MIDLAND GASTROENTEROLOGYHEPATOLOGY"
	sourceDef.SourceType = pkg.SourceTypeMidlandGastroenterologyhepatology
