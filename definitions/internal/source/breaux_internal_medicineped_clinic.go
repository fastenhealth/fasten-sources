...
func GetSourceBreauxInternalMedicinepedClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBreauxInternalMedicinepedClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Breaux Internal MedicinePed Clinic"
	sourceDef.SourceType = pkg.SourceTypeBreauxInternalMedicinepedClinic
