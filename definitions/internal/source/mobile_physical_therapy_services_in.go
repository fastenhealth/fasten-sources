...
func GetSourceMobilePhysicalTherapyServicesIn(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMobilePhysicalTherapyServicesIn]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MOBILE PHYSICAL THERAPY SERVICES IN"
	sourceDef.SourceType = pkg.SourceTypeMobilePhysicalTherapyServicesIn
