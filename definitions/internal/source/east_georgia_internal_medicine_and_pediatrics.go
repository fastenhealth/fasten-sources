...
func GetSourceEastGeorgiaInternalMedicineAndPediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEastGeorgiaInternalMedicineAndPediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "East Georgia Internal Medicine and Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeEastGeorgiaInternalMedicineAndPediatrics
