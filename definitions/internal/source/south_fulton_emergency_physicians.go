...
func GetSourceSouthFultonEmergencyPhysicians(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthFultonEmergencyPhysicians]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SOUTH FULTON EMERGENCY PHYSICIANS"
	sourceDef.SourceType = pkg.SourceTypeSouthFultonEmergencyPhysicians
