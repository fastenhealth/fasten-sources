...
func GetSourceOkaloosaOrthopedicAndSportsMedicineClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOkaloosaOrthopedicAndSportsMedicineClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Okaloosa Orthopedic and Sports Medicine Clinic"
	sourceDef.SourceType = pkg.SourceTypeOkaloosaOrthopedicAndSportsMedicineClinic
