...
func GetSourceKentuckyDiabetesEndocrinologyCen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKentuckyDiabetesEndocrinologyCen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "KENTUCKY DIABETES ENDOCRINOLOGY CEN"
	sourceDef.SourceType = pkg.SourceTypeKentuckyDiabetesEndocrinologyCen
