...
func GetSourceNorthwestFamilyMedicineTuscanySquare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestFamilyMedicineTuscanySquare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Family Medicine  Tuscany Square"
	sourceDef.SourceType = pkg.SourceTypeNorthwestFamilyMedicineTuscanySquare
