...
func GetSourceDavisVascularInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDavisVascularInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Davis Vascular Institute"
	sourceDef.SourceType = pkg.SourceTypeDavisVascularInstitute
	sourceDef.Category = []string{"2086S0129X"}
	sourceDef.Aliases = []string{"DAVIS VASCULAR INSTITUTE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1548544984"}}
