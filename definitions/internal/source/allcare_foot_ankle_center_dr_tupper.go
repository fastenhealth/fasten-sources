...
func GetSourceAllcareFootAnkleCenterDrTupper(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllcareFootAnkleCenterDrTupper]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ALLCARE FOOT  ANKLE CENTER  Dr Tupper"
	sourceDef.SourceType = pkg.SourceTypeAllcareFootAnkleCenterDrTupper
