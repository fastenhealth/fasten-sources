...
func GetSourceTrinityWomensCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTrinityWomensCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Trinity Womens Center"
	sourceDef.SourceType = pkg.SourceTypeTrinityWomensCenter
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{"TRINITY WOMENS CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1316128069"}}
