...
func GetSourcePDouglasCochranMdLtd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePDouglasCochranMdLtd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "P Douglas Cochran MD LTD"
	sourceDef.SourceType = pkg.SourceTypePDouglasCochranMdLtd
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"P DOUGLAS COCHRAN MD LTD"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1336320498"}}
