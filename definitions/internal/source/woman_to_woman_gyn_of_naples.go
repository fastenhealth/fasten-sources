...
func GetSourceWomanToWomanGynOfNaples(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWomanToWomanGynOfNaples]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Woman to Woman GYN of Naples"
	sourceDef.SourceType = pkg.SourceTypeWomanToWomanGynOfNaples
	sourceDef.Category = []string{"207VF0040X", "207VG0400X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1376651091"}}
