...
func GetSourceBigSkyOrthopaedicsAndSpineLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBigSkyOrthopaedicsAndSpineLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Big Sky Orthopaedics and Spine LLC"
	sourceDef.SourceType = pkg.SourceTypeBigSkyOrthopaedicsAndSpineLlc
