...
func GetSourceRonEakerMdLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRonEakerMdLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ron Eaker MD LLC"
	sourceDef.SourceType = pkg.SourceTypeRonEakerMdLlc
	sourceDef.Category = []string{"207VB0002X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1992430649"}}
	sourceDef.PatientAccessUrl = "http://mdvip.com/RonEakerMD"
