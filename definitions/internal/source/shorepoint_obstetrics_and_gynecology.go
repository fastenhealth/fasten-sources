...
func GetSourceShorepointObstetricsAndGynecology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShorepointObstetricsAndGynecology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ShorePoint Obstetrics and Gynecology"
	sourceDef.SourceType = pkg.SourceTypeShorepointObstetricsAndGynecology
