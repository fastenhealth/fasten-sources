...
func GetSourceCc072FriscoObstetricsGynecologyPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc072FriscoObstetricsGynecologyPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC072FRISCO OBSTETRICS  GYNECOLOGY PA"
	sourceDef.SourceType = pkg.SourceTypeCc072FriscoObstetricsGynecologyPa
