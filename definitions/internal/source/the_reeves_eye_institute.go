...
func GetSourceTheReevesEyeInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheReevesEyeInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "The Reeves Eye Institute"
	sourceDef.SourceType = pkg.SourceTypeTheReevesEyeInstitute
	sourceDef.Category = []string{"207W00000X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1306115423"}}
	sourceDef.PatientAccessUrl = "https://www.facebook.com/ReevesSurgeryCenter/"
