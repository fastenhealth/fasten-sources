...
func GetSourceBeckerFredericDbaKingOfPrussi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBeckerFredericDbaKingOfPrussi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Becker Frederic dba King of Prussi"
	sourceDef.SourceType = pkg.SourceTypeBeckerFredericDbaKingOfPrussi
