...
func GetSourceCc046WendoverObgynAndInfertilityInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc046WendoverObgynAndInfertilityInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC046WENDOVER OBGYN AND INFERTILITY INC"
	sourceDef.SourceType = pkg.SourceTypeCc046WendoverObgynAndInfertilityInc
