...
func GetSourcePlasticSurgeryAssociatesOfSouth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePlasticSurgeryAssociatesOfSouth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Plastic Surgery Associates of South"
	sourceDef.SourceType = pkg.SourceTypePlasticSurgeryAssociatesOfSouth
