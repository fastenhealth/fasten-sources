...
func GetSourceFaithSolutionsToMentalHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFaithSolutionsToMentalHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Faith Solutions to Mental Health"
	sourceDef.SourceType = pkg.SourceTypeFaithSolutionsToMentalHealth
