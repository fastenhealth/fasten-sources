...
func GetSourceKatherineKempDbaLegacyBehaviora(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKatherineKempDbaLegacyBehaviora]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Katherine Kemp dba Legacy Behaviora"
	sourceDef.SourceType = pkg.SourceTypeKatherineKempDbaLegacyBehaviora
