...
func GetSourceFairOaksOrthopaedicAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFairOaksOrthopaedicAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Fair Oaks Orthopaedic Associates"
	sourceDef.SourceType = pkg.SourceTypeFairOaksOrthopaedicAssociates
