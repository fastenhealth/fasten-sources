...
func GetSourceNutritionHealthAssociatesInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNutritionHealthAssociatesInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Nutrition  Health Associates Inc"
	sourceDef.SourceType = pkg.SourceTypeNutritionHealthAssociatesInc
