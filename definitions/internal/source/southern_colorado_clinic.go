...
func GetSourceSouthernColoradoClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernColoradoClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southern Colorado Clinic"
	sourceDef.SourceType = pkg.SourceTypeSouthernColoradoClinic
	sourceDef.Category = []string{"332B00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1659435535"}}
	sourceDef.PatientAccessUrl = "http://southerncoloradoclinic.com"
