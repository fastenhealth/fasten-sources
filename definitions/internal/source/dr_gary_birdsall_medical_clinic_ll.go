...
func GetSourceDrGaryBirdsallMedicalClinicLl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDrGaryBirdsallMedicalClinicLl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Dr Gary Birdsall Medical Clinic LL"
	sourceDef.SourceType = pkg.SourceTypeDrGaryBirdsallMedicalClinicLl
