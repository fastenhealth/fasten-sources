...
func GetSourceConnecticutNeurosurgeryAndSpine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeConnecticutNeurosurgeryAndSpine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CONNECTICUT NEUROSURGERY AND SPINE"
	sourceDef.SourceType = pkg.SourceTypeConnecticutNeurosurgeryAndSpine
