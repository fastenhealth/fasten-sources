...
func GetSourceCenterForHealthEmpowermentDba(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForHealthEmpowermentDba]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CENTER FOR HEALTH EMPOWERMENT DBA"
	sourceDef.SourceType = pkg.SourceTypeCenterForHealthEmpowermentDba
