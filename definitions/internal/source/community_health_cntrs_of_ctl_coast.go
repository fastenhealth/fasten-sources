...
func GetSourceCommunityHealthCntrsOfCtlCoast(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCommunityHealthCntrsOfCtlCoast]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Community Health Cntrs Of Ctl Coast"
	sourceDef.SourceType = pkg.SourceTypeCommunityHealthCntrsOfCtlCoast
