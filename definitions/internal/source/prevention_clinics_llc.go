...
func GetSourcePreventionClinicsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePreventionClinicsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Prevention Clinics LLC"
	sourceDef.SourceType = pkg.SourceTypePreventionClinicsLlc
	sourceDef.Category = []string{"207R00000X", "261QP2300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1073981361"}}
	sourceDef.PatientAccessUrl = "http://www.preventionclinics.com"
