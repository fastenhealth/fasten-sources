...
func GetSourceMorlenHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMorlenHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MORLEN HEALTH"
	sourceDef.SourceType = pkg.SourceTypeMorlenHealth
	sourceDef.Category = []string{"207RP1001X", "2279P1005X"}
	sourceDef.Aliases = []string{"MORLEN HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1851024954"}}
