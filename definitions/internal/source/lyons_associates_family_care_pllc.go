...
func GetSourceLyonsAssociatesFamilyCarePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLyonsAssociatesFamilyCarePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lyons  Associates Family Care PLLC"
	sourceDef.SourceType = pkg.SourceTypeLyonsAssociatesFamilyCarePllc
