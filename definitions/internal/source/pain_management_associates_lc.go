...
func GetSourcePainManagementAssociatesLc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePainManagementAssociatesLc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PAIN MANAGEMENT ASSOCIATES LC"
	sourceDef.SourceType = pkg.SourceTypePainManagementAssociatesLc
	sourceDef.Category = []string{"207L00000X", "207LP2900X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1720065048"}}
