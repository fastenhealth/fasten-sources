...
func GetSourcePigaPrimaryCareAssociatesPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePigaPrimaryCareAssociatesPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Piga Primary Care Associates PLLC"
	sourceDef.SourceType = pkg.SourceTypePigaPrimaryCareAssociatesPllc
