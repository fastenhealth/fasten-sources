...
func GetSourceAnnaGasparyanMdInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAnnaGasparyanMdInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ANNA GASPARYAN MD INC"
	sourceDef.SourceType = pkg.SourceTypeAnnaGasparyanMdInc
	sourceDef.Category = []string{"2086S0129X"}
	sourceDef.Aliases = []string{"DESERT VASCULAR ASSOCIATES"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1982131785"}}
