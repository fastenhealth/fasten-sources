...
func GetSourceCrossroadsOncologyAndHematologyAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCrossroadsOncologyAndHematologyAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Crossroads Oncology and Hematology Associates"
	sourceDef.SourceType = pkg.SourceTypeCrossroadsOncologyAndHematologyAssociates
