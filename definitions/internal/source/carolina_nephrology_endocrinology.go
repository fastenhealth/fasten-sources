...
func GetSourceCarolinaNephrologyEndocrinology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCarolinaNephrologyEndocrinology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Carolina Nephrology  Endocrinology"
	sourceDef.SourceType = pkg.SourceTypeCarolinaNephrologyEndocrinology
