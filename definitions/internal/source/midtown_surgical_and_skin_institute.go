...
func GetSourceMidtownSurgicalAndSkinInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidtownSurgicalAndSkinInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Midtown Surgical and Skin Institute"
	sourceDef.SourceType = pkg.SourceTypeMidtownSurgicalAndSkinInstitute
