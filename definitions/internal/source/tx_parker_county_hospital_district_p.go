...
func GetSourceTxParkerCountyHospitalDistrictP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTxParkerCountyHospitalDistrictP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "TX  Parker County Hospital District  P"
	sourceDef.SourceType = pkg.SourceTypeTxParkerCountyHospitalDistrictP
