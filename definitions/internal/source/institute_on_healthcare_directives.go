...
func GetSourceInstituteOnHealthcareDirectives(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInstituteOnHealthcareDirectives]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Institute on Healthcare Directives"
	sourceDef.SourceType = pkg.SourceTypeInstituteOnHealthcareDirectives
	sourceDef.Category = []string{"207P00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1366891368"}}
	sourceDef.PatientAccessUrl = "https://institutehcd.com/"
