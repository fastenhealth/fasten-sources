...
func GetSourceHamptonRoadsKidneyInternalMed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHamptonRoadsKidneyInternalMed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hampton Roads Kidney  Internal Med"
	sourceDef.SourceType = pkg.SourceTypeHamptonRoadsKidneyInternalMed
