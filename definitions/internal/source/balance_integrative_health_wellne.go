...
func GetSourceBalanceIntegrativeHealthWellne(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBalanceIntegrativeHealthWellne]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Balance Integrative Health  Wellne"
	sourceDef.SourceType = pkg.SourceTypeBalanceIntegrativeHealthWellne
