...
func GetSourceKidneyAndHypertensionAssociatesOfDallasPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKidneyAndHypertensionAssociatesOfDallasPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "KIDNEY AND HYPERTENSION ASSOCIATES OF DALLAS PLLC"
	sourceDef.SourceType = pkg.SourceTypeKidneyAndHypertensionAssociatesOfDallasPllc
	sourceDef.Category = []string{"207RN0300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1164791398"}}
