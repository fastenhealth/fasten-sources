...
func GetSourceKyleDParishMdPsc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKyleDParishMdPsc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "KYLE D PARISH MD PSC"
	sourceDef.SourceType = pkg.SourceTypeKyleDParishMdPsc
	sourceDef.Category = []string{"207QS0010X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1447449350"}}
	sourceDef.PatientAccessUrl = "http://www.parishmd.com"
