...
func GetSourceChinoHillsFootAndAnkleCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChinoHillsFootAndAnkleCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CHINO HILLS FOOT AND ANKLE CENTER"
	sourceDef.SourceType = pkg.SourceTypeChinoHillsFootAndAnkleCenter
	sourceDef.Category = []string{"213ES0103X"}
	sourceDef.Aliases = []string{"CHINO HILLS FOOT AND ANKLE CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1841811940"}}
