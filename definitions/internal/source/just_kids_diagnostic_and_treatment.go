...
func GetSourceJustKidsDiagnosticAndTreatment(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJustKidsDiagnosticAndTreatment]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "JUST KIDS DIAGNOSTIC AND TREATMENT"
	sourceDef.SourceType = pkg.SourceTypeJustKidsDiagnosticAndTreatment
