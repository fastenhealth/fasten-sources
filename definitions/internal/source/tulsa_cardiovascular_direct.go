...
func GetSourceTulsaCardiovascularDirect(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTulsaCardiovascularDirect]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "TULSA CARDIOVASCULAR DIRECT"
	sourceDef.SourceType = pkg.SourceTypeTulsaCardiovascularDirect
