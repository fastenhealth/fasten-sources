...
func GetSourceCornerstoneFamilyPracticeAndUrg(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCornerstoneFamilyPracticeAndUrg]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cornerstone Family Practice and Urg"
	sourceDef.SourceType = pkg.SourceTypeCornerstoneFamilyPracticeAndUrg
