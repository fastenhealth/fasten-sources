...
func GetSourceBoulderValleyCenterForDermatolo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBoulderValleyCenterForDermatolo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Boulder Valley Center for Dermatolo"
	sourceDef.SourceType = pkg.SourceTypeBoulderValleyCenterForDermatolo
