...
func GetSourceInfiniteNewDayCounselingAndCon(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInfiniteNewDayCounselingAndCon]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "INFINITE NEW DAY COUNSELING AND CON"
	sourceDef.SourceType = pkg.SourceTypeInfiniteNewDayCounselingAndCon
