...
func GetSourceRichmondHillMedicalServicesPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRichmondHillMedicalServicesPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Richmond Hill Medical Services PC"
	sourceDef.SourceType = pkg.SourceTypeRichmondHillMedicalServicesPc
