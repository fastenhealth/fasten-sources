...
func GetSourceNorthOkaloosaMedicalCenterTransitionalCareUnit(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthOkaloosaMedicalCenterTransitionalCareUnit]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Okaloosa Medical Center  Transitional Care Unit"
	sourceDef.SourceType = pkg.SourceTypeNorthOkaloosaMedicalCenterTransitionalCareUnit
