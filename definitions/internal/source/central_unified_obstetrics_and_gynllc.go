...
func GetSourceCentralUnifiedObstetricsAndGynllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralUnifiedObstetricsAndGynllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CENTRAL UNIFIED OBSTETRICS AND GYNLLC"
	sourceDef.SourceType = pkg.SourceTypeCentralUnifiedObstetricsAndGynllc
