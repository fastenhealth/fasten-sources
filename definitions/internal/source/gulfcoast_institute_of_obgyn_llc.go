...
func GetSourceGulfcoastInstituteOfObgynLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGulfcoastInstituteOfObgynLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GULFCOAST INSTITUTE OF OBGYN LLC"
	sourceDef.SourceType = pkg.SourceTypeGulfcoastInstituteOfObgynLlc
