...
func GetSourceChicagoSleepHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChicagoSleepHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CHICAGO SLEEP HEALTH"
	sourceDef.SourceType = pkg.SourceTypeChicagoSleepHealth
	sourceDef.Category = []string{"332B00000X"}
	sourceDef.Aliases = []string{"CHICAGO SLEEP HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1962900191"}}
