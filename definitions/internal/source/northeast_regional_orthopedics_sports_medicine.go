...
func GetSourceNortheastRegionalOrthopedicsSportsMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNortheastRegionalOrthopedicsSportsMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northeast Regional Orthopedics  Sports Medicine"
	sourceDef.SourceType = pkg.SourceTypeNortheastRegionalOrthopedicsSportsMedicine
