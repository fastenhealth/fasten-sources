...
func GetSourceAgarwalNephrologyHypertension(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAgarwalNephrologyHypertension]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Agarwal Nephrology  Hypertension"
	sourceDef.SourceType = pkg.SourceTypeAgarwalNephrologyHypertension
