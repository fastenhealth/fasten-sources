...
func GetSourceEndoscopyCenterAtMeridianAnesthesia(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEndoscopyCenterAtMeridianAnesthesia]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Endoscopy Center At Meridian Anesthesia"
	sourceDef.SourceType = pkg.SourceTypeEndoscopyCenterAtMeridianAnesthesia
