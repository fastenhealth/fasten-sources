...
func GetSourcePermianBasinWomensResourceCent(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePermianBasinWomensResourceCent]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PERMIAN BASIN WOMENS RESOURCE CENT"
	sourceDef.SourceType = pkg.SourceTypePermianBasinWomensResourceCent
