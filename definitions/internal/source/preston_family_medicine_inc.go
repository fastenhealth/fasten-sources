...
func GetSourcePrestonFamilyMedicineInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrestonFamilyMedicineInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Preston Family Medicine Inc"
	sourceDef.SourceType = pkg.SourceTypePrestonFamilyMedicineInc
	sourceDef.Category = []string{"207Q00000X", "261QR1300X"}
	sourceDef.Aliases = []string{"PRESTON FAMILY MEDICINE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1053802553"}}
