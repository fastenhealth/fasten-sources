...
func GetSourceKetchikanIndianCommunityTribalHealthClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKetchikanIndianCommunityTribalHealthClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ketchikan Indian Community  Tribal Health Clinic"
	sourceDef.SourceType = pkg.SourceTypeKetchikanIndianCommunityTribalHealthClinic
