...
func GetSourceMedcityFamilyClinicLimitedLiabi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMedcityFamilyClinicLimitedLiabi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MEDCITY FAMILY CLINIC LIMITED LIABI"
	sourceDef.SourceType = pkg.SourceTypeMedcityFamilyClinicLimitedLiabi
