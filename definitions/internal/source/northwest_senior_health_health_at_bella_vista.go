...
func GetSourceNorthwestSeniorHealthHealthAtBellaVista(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestSeniorHealthHealthAtBellaVista]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Senior Health Health at Bella Vista"
	sourceDef.SourceType = pkg.SourceTypeNorthwestSeniorHealthHealthAtBellaVista
