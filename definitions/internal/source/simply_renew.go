...
func GetSourceSimplyRenew(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSimplyRenew]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Simply Renew"
	sourceDef.SourceType = pkg.SourceTypeSimplyRenew
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{"SIMPLY RENEW"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1639740327"}}
