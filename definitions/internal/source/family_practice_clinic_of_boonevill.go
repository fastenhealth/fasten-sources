...
func GetSourceFamilyPracticeClinicOfBoonevill(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFamilyPracticeClinicOfBoonevill]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Family Practice Clinic of Boonevill"
	sourceDef.SourceType = pkg.SourceTypeFamilyPracticeClinicOfBoonevill
