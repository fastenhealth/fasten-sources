...
func GetSourceAlliancehealthMedicalGroupNeurosurgeryPoncaCity(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupNeurosurgeryPoncaCity]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Neurosurgery Ponca City"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupNeurosurgeryPoncaCity
