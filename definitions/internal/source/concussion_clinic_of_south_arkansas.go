...
func GetSourceConcussionClinicOfSouthArkansas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeConcussionClinicOfSouthArkansas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Concussion Clinic of South Arkansas"
	sourceDef.SourceType = pkg.SourceTypeConcussionClinicOfSouthArkansas
