...
func GetSourceFreestoneHealthClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFreestoneHealthClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Freestone Health Clinic"
	sourceDef.SourceType = pkg.SourceTypeFreestoneHealthClinic
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"FREESTONE HEALTH CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1093263501"}}
