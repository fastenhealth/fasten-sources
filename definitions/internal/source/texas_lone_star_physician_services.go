...
func GetSourceTexasLoneStarPhysicianServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexasLoneStarPhysicianServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "TEXAS LONE STAR PHYSICIAN SERVICES"
	sourceDef.SourceType = pkg.SourceTypeTexasLoneStarPhysicianServices
