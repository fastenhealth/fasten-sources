...
func GetSourceMgaGastroDiagAndTherapCtrMetairie(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMgaGastroDiagAndTherapCtrMetairie]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "MGA Gastro Diag And Therap Ctr Metairie"
	sourceDef.SourceType = pkg.SourceTypeMgaGastroDiagAndTherapCtrMetairie
