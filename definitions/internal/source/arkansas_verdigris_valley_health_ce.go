...
func GetSourceArkansasVerdigrisValleyHealthCe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArkansasVerdigrisValleyHealthCe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arkansas Verdigris Valley Health Ce"
	sourceDef.SourceType = pkg.SourceTypeArkansasVerdigrisValleyHealthCe
