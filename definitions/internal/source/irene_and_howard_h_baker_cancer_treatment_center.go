...
func GetSourceIreneAndHowardHBakerCancerTreatmentCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIreneAndHowardHBakerCancerTreatmentCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Irene and Howard H Baker Cancer Treatment Center"
	sourceDef.SourceType = pkg.SourceTypeIreneAndHowardHBakerCancerTreatmentCenter
