...
func GetSourceWilmingtonSurgicalAssociatesPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWilmingtonSurgicalAssociatesPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Wilmington Surgical Associates PA"
	sourceDef.SourceType = pkg.SourceTypeWilmingtonSurgicalAssociatesPa
	sourceDef.Category = []string{"174400000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1003821547"}}
	sourceDef.PatientAccessUrl = "http://www.wilmingtonsurgical.com"
