...
func GetSourceTennovaCardiothoracicClarksville(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaCardiothoracicClarksville]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Cardiothoracic  Clarksville"
	sourceDef.SourceType = pkg.SourceTypeTennovaCardiothoracicClarksville
