...
func GetSourceOriginsFamilyMedicalWeightLos(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOriginsFamilyMedicalWeightLos]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Origins Family Medical  Weight Los"
	sourceDef.SourceType = pkg.SourceTypeOriginsFamilyMedicalWeightLos
