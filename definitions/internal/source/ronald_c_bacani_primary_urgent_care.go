...
func GetSourceRonaldCBacaniPrimaryUrgentCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRonaldCBacaniPrimaryUrgentCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ronald C Bacani Primary Urgent Care"
	sourceDef.SourceType = pkg.SourceTypeRonaldCBacaniPrimaryUrgentCare
