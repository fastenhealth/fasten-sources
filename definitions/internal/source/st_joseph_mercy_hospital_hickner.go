...
func GetSourceStJosephMercyHospitalHickner(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStJosephMercyHospitalHickner]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ST JOSEPH MERCY HOSPITAL  HICKNER"
	sourceDef.SourceType = pkg.SourceTypeStJosephMercyHospitalHickner
