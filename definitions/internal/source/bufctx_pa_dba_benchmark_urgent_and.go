...
func GetSourceBufctxPaDbaBenchmarkUrgentAnd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBufctxPaDbaBenchmarkUrgentAnd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BUFCTX PA DBA BENCHMARK URGENT AND"
	sourceDef.SourceType = pkg.SourceTypeBufctxPaDbaBenchmarkUrgentAnd
