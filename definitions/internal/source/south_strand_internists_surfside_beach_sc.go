...
func GetSourceSouthStrandInternistsSurfsideBeachSc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthStrandInternistsSurfsideBeachSc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "South Strand Internists  Surfside Beach SC"
	sourceDef.SourceType = pkg.SourceTypeSouthStrandInternistsSurfsideBeachSc
