...
func GetSourceAriseMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAriseMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arise Medical Group"
	sourceDef.SourceType = pkg.SourceTypeAriseMedicalGroup
	sourceDef.Category = []string{"208600000X"}
	sourceDef.Aliases = []string{"THE PRACTICE AT WESTLAKE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1003549767"}}
