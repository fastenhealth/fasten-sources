...
func GetSourceBrokenArrowOrthopedicsInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBrokenArrowOrthopedicsInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Broken Arrow Orthopedics Inc"
	sourceDef.SourceType = pkg.SourceTypeBrokenArrowOrthopedicsInc
	sourceDef.Category = []string{"207XS0106X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1245256486"}}
