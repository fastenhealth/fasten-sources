...
func GetSourceBowmanFamilyMedicalClinicLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBowmanFamilyMedicalClinicLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Bowman Family Medical Clinic LLC"
	sourceDef.SourceType = pkg.SourceTypeBowmanFamilyMedicalClinicLlc
