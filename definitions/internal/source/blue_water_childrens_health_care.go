...
func GetSourceBlueWaterChildrensHealthCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBlueWaterChildrensHealthCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BLUE WATER CHILDRENS HEALTH CARE"
	sourceDef.SourceType = pkg.SourceTypeBlueWaterChildrensHealthCare
