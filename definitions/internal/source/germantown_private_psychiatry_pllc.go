...
func GetSourceGermantownPrivatePsychiatryPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGermantownPrivatePsychiatryPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Germantown Private Psychiatry PLLC"
	sourceDef.SourceType = pkg.SourceTypeGermantownPrivatePsychiatryPllc
