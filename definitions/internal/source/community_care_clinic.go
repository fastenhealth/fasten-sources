...
func GetSourceCommunityCareClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCommunityCareClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Community Care Clinic"
	sourceDef.SourceType = pkg.SourceTypeCommunityCareClinic
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"COMMUNITY CARE CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1376261099"}}
