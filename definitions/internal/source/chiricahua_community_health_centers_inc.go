...
func GetSourceChiricahuaCommunityHealthCentersInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChiricahuaCommunityHealthCentersInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Chiricahua Community Health Centers Inc"
	sourceDef.SourceType = pkg.SourceTypeChiricahuaCommunityHealthCentersInc
