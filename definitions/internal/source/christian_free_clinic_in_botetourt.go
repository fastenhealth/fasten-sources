...
func GetSourceChristianFreeClinicInBotetourt(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChristianFreeClinicInBotetourt]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Christian Free Clinic In Botetourt"
	sourceDef.SourceType = pkg.SourceTypeChristianFreeClinicInBotetourt
