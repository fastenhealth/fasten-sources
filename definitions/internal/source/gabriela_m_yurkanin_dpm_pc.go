...
func GetSourceGabrielaMYurkaninDpmPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGabrielaMYurkaninDpmPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gabriela M Yurkanin DPM PC"
	sourceDef.SourceType = pkg.SourceTypeGabrielaMYurkaninDpmPc
	sourceDef.Category = []string{"213E00000X", "213ES0103X"}
	sourceDef.Aliases = []string{"YURKANIN FOOT & ANKLE RECONSTRUCTIVE CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1528533163"}}
