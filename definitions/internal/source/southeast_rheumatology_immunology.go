...
func GetSourceSoutheastRheumatologyImmunology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSoutheastRheumatologyImmunology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southeast Rheumatology  Immunology"
	sourceDef.SourceType = pkg.SourceTypeSoutheastRheumatologyImmunology
