...
func GetSourceWashingtonOrthopaedicCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWashingtonOrthopaedicCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Washington Orthopaedic Center"
	sourceDef.SourceType = pkg.SourceTypeWashingtonOrthopaedicCenter
	sourceDef.Category = []string{"207X00000X"}
	sourceDef.Aliases = []string{"WASHINGTON ORTHOPAEDIC CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1437701463", "1952388134"}}
