...
func GetSourceChildrensCardiologyAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChildrensCardiologyAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Childrens Cardiology Associates"
	sourceDef.SourceType = pkg.SourceTypeChildrensCardiologyAssociates
	sourceDef.PatientAccessUrl = "https://www.pediatrix.com/find-care/practices/cardiologyhouston"
