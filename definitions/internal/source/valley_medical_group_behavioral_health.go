...
func GetSourceValleyMedicalGroupBehavioralHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeValleyMedicalGroupBehavioralHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Valley Medical Group Behavioral Health"
	sourceDef.SourceType = pkg.SourceTypeValleyMedicalGroupBehavioralHealth
