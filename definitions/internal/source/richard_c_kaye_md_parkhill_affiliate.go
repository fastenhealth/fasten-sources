...
func GetSourceRichardCKayeMdParkhillAffiliate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRichardCKayeMdParkhillAffiliate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Richard C Kaye MD ParkHill Affiliate"
	sourceDef.SourceType = pkg.SourceTypeRichardCKayeMdParkhillAffiliate
