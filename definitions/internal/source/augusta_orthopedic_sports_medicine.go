...
func GetSourceAugustaOrthopedicSportsMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAugustaOrthopedicSportsMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Augusta Orthopedic Sports Medicine"
	sourceDef.SourceType = pkg.SourceTypeAugustaOrthopedicSportsMedicine
