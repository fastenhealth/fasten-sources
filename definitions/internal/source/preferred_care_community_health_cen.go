...
func GetSourcePreferredCareCommunityHealthCen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePreferredCareCommunityHealthCen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Preferred Care Community Health Cen"
	sourceDef.SourceType = pkg.SourceTypePreferredCareCommunityHealthCen
