...
func GetSourceNorthOkaloosaMedicalCenterSurgeryCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthOkaloosaMedicalCenterSurgeryCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Okaloosa Medical Center Surgery Center"
	sourceDef.SourceType = pkg.SourceTypeNorthOkaloosaMedicalCenterSurgeryCenter
