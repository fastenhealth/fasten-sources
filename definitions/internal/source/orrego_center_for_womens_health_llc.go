...
func GetSourceOrregoCenterForWomensHealthLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOrregoCenterForWomensHealthLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ORREGO CENTER FOR WOMENS HEALTH LLC"
	sourceDef.SourceType = pkg.SourceTypeOrregoCenterForWomensHealthLlc
