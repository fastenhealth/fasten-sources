...
func GetSourceFortyFortAnesthesiaAssociatesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFortyFortAnesthesiaAssociatesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Forty Fort Anesthesia Associates LLC"
	sourceDef.SourceType = pkg.SourceTypeFortyFortAnesthesiaAssociatesLlc
	sourceDef.Category = []string{"367500000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1346600038"}}
