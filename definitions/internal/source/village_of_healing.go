...
func GetSourceVillageOfHealing(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVillageOfHealing]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Village of Healing"
	sourceDef.SourceType = pkg.SourceTypeVillageOfHealing
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{"THE VILLAGE OF HEALING CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1154991099"}}
	sourceDef.PatientAccessUrl = "http://www.villageofhealingcle.com"
