...
func GetSourceCc031WomensHealthAllianceMidwiferyPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc031WomensHealthAllianceMidwiferyPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC031WOMENS HEALTH ALLIANCE MIDWIFERY PA"
	sourceDef.SourceType = pkg.SourceTypeCc031WomensHealthAllianceMidwiferyPa
