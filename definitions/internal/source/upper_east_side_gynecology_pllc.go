...
func GetSourceUpperEastSideGynecologyPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUpperEastSideGynecologyPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Upper East Side Gynecology PLLC"
	sourceDef.SourceType = pkg.SourceTypeUpperEastSideGynecologyPllc
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1265771307"}}
	sourceDef.PatientAccessUrl = "http://www.uesgynecology.com"
