...
func GetSourceGoldbergPsychiatry(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGoldbergPsychiatry]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Goldberg Psychiatry"
	sourceDef.SourceType = pkg.SourceTypeGoldbergPsychiatry
	sourceDef.Category = []string{"2084P0800X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1942956255"}}
	sourceDef.PatientAccessUrl = "http://www.goldbergpsychiatry.com"
