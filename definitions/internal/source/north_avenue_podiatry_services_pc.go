...
func GetSourceNorthAvenuePodiatryServicesPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthAvenuePodiatryServicesPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTH AVENUE PODIATRY SERVICES PC"
	sourceDef.SourceType = pkg.SourceTypeNorthAvenuePodiatryServicesPc
	sourceDef.Category = []string{"213E00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1558531152"}}
	sourceDef.PatientAccessUrl = "http://www.northavenuepodiatry.com"
