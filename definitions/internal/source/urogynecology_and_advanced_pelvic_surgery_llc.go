...
func GetSourceUrogynecologyAndAdvancedPelvicSurgeryLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUrogynecologyAndAdvancedPelvicSurgeryLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "UROGYNECOLOGY AND ADVANCED PELVIC SURGERY LLC"
	sourceDef.SourceType = pkg.SourceTypeUrogynecologyAndAdvancedPelvicSurgeryLlc
