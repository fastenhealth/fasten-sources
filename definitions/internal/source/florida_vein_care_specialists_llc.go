...
func GetSourceFloridaVeinCareSpecialistsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFloridaVeinCareSpecialistsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Florida Vein Care Specialists LLC"
	sourceDef.SourceType = pkg.SourceTypeFloridaVeinCareSpecialistsLlc
	sourceDef.Category = []string{"2086S0129X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1689917445"}}
	sourceDef.PatientAccessUrl = "http://www.myfloridaveinspecialists.com/"
