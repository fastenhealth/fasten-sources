...
func GetSourceShepherdEyeSurgicenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShepherdEyeSurgicenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Shepherd Eye Surgicenter"
	sourceDef.SourceType = pkg.SourceTypeShepherdEyeSurgicenter
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"SHEPHERD EYE SURGICENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1982677191"}}
	sourceDef.BrandLogo = "shepherd-eye-surgicenter.jpg"
