...
func GetSource360PhysiciansAndHealthPartners(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceType360PhysiciansAndHealthPartners]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "360 Physicians and Health Partners"
	sourceDef.SourceType = pkg.SourceType360PhysiciansAndHealthPartners
