...
func GetSourceFloridaWellnessGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFloridaWellnessGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Florida Wellness Group"
	sourceDef.SourceType = pkg.SourceTypeFloridaWellnessGroup
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"FLORIDA WELLNESS GROUP"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1407406820"}}
