...
func GetSourceFreyaHealthLlcDbaEsteroIsland(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFreyaHealthLlcDbaEsteroIsland]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Freya Health LLC DBA Estero Island"
	sourceDef.SourceType = pkg.SourceTypeFreyaHealthLlcDbaEsteroIsland
