...
func GetSourceVillageMedicalCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVillageMedicalCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VILLAGE MEDICAL CENTER LLC"
	sourceDef.SourceType = pkg.SourceTypeVillageMedicalCenterLlc
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1629704069"}}
	sourceDef.PatientAccessUrl = "http://vmchealthcare.com"
