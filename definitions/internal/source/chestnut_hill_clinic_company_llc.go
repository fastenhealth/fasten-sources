...
func GetSourceChestnutHillClinicCompanyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChestnutHillClinicCompanyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CHESTNUT HILL CLINIC COMPANY LLC"
	sourceDef.SourceType = pkg.SourceTypeChestnutHillClinicCompanyLlc
