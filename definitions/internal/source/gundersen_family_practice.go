...
func GetSourceGundersenFamilyPractice(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGundersenFamilyPractice]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gundersen Family Practice"
	sourceDef.SourceType = pkg.SourceTypeGundersenFamilyPractice
	sourceDef.Category = []string{"363L00000X"}
	sourceDef.Aliases = []string{"GUNDERSEN FAMILY PRACTICE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1457002503"}}
