...
func GetSourceBoxbarVascular(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBoxbarVascular]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BoxBar Vascular"
	sourceDef.SourceType = pkg.SourceTypeBoxbarVascular
	sourceDef.Category = []string{"208600000X", "2086S0129X"}
	sourceDef.Aliases = []string{"BOXBAR VASCULAR"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1841712072"}}
