...
func GetSourceCc032GeorgetownObgynUnifiedPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc032GeorgetownObgynUnifiedPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC032GEORGETOWN OBGYN UNIFIED PLLC"
	sourceDef.SourceType = pkg.SourceTypeCc032GeorgetownObgynUnifiedPllc
