...
func GetSourceGosselinkPeterliveOakMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGosselinkPeterliveOakMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gosselink PeterLive Oak Medicine"
	sourceDef.SourceType = pkg.SourceTypeGosselinkPeterliveOakMedicine
