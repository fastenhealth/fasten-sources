...
func GetSourceShepperdMedicalClinicSoleProp(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShepperdMedicalClinicSoleProp]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Shepperd Medical Clinic Sole Prop"
	sourceDef.SourceType = pkg.SourceTypeShepperdMedicalClinicSoleProp
