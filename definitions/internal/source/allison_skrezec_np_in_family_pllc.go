...
func GetSourceAllisonSkrezecNpInFamilyPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllisonSkrezecNpInFamilyPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ALLISON SKREZEC NP IN FAMILY PLLC"
	sourceDef.SourceType = pkg.SourceTypeAllisonSkrezecNpInFamilyPllc
