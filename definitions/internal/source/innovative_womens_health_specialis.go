...
func GetSourceInnovativeWomensHealthSpecialis(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInnovativeWomensHealthSpecialis]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Innovative Womens Health Specialis"
	sourceDef.SourceType = pkg.SourceTypeInnovativeWomensHealthSpecialis
