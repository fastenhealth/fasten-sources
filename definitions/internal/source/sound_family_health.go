...
func GetSourceSoundFamilyHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSoundFamilyHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sound Family Health"
	sourceDef.SourceType = pkg.SourceTypeSoundFamilyHealth
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"SOUND FAMILY HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1912148933"}}
