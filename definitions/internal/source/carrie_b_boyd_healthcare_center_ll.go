...
func GetSourceCarrieBBoydHealthcareCenterLl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCarrieBBoydHealthcareCenterLl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Carrie B Boyd Healthcare Center LL"
	sourceDef.SourceType = pkg.SourceTypeCarrieBBoydHealthcareCenterLl
