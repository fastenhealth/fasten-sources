...
func GetSourceBumpsNBruisesPediatricUrgentC(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBumpsNBruisesPediatricUrgentC]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bumps N Bruises Pediatric Urgent C"
	sourceDef.SourceType = pkg.SourceTypeBumpsNBruisesPediatricUrgentC
