...
func GetSourceFoothillAllergyAsthmaAndImmuno(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFoothillAllergyAsthmaAndImmuno]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Foothill Allergy  Asthma and Immuno"
	sourceDef.SourceType = pkg.SourceTypeFoothillAllergyAsthmaAndImmuno
