...
func GetSourceEndoscopyCenterOfTheCentralCoast(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEndoscopyCenterOfTheCentralCoast]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Endoscopy Center of the Central Coast"
	sourceDef.SourceType = pkg.SourceTypeEndoscopyCenterOfTheCentralCoast
