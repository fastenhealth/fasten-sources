...
func GetSourceCc080MoontowerMidwiferyAndWellness(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc080MoontowerMidwiferyAndWellness]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC080MOONTOWER MIDWIFERY AND WELLNESS"
	sourceDef.SourceType = pkg.SourceTypeCc080MoontowerMidwiferyAndWellness
