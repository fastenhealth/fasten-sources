...
func GetSourceNeighbormdPartnersOfFloridaLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeighbormdPartnersOfFloridaLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEIGHBORMD PARTNERS OF FLORIDA LLC"
	sourceDef.SourceType = pkg.SourceTypeNeighbormdPartnersOfFloridaLlc
