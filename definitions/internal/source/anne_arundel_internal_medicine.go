...
func GetSourceAnneArundelInternalMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAnneArundelInternalMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Anne Arundel Internal Medicine"
	sourceDef.SourceType = pkg.SourceTypeAnneArundelInternalMedicine
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"ANNE ARUNDEL INTERNAL MEDICINE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1487272167"}}
