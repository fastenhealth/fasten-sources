...
func GetSourceProlianceSportsTherapyOfBellevue(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeProlianceSportsTherapyOfBellevue]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Proliance Sports Therapy Of Bellevue"
	sourceDef.SourceType = pkg.SourceTypeProlianceSportsTherapyOfBellevue
