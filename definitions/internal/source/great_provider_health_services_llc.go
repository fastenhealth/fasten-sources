...
func GetSourceGreatProviderHealthServicesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGreatProviderHealthServicesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GREAT PROVIDER HEALTH SERVICES LLC"
	sourceDef.SourceType = pkg.SourceTypeGreatProviderHealthServicesLlc
