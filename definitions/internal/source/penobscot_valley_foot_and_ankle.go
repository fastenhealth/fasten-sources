...
func GetSourcePenobscotValleyFootAndAnkle(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePenobscotValleyFootAndAnkle]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Penobscot Valley Foot and Ankle"
	sourceDef.SourceType = pkg.SourceTypePenobscotValleyFootAndAnkle
	sourceDef.Category = []string{"213ES0103X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1053886184"}}
