...
func GetSourceColoradoCenterForOrthopaedicExcellence(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeColoradoCenterForOrthopaedicExcellence]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Colorado Center for Orthopaedic Excellence"
	sourceDef.SourceType = pkg.SourceTypeColoradoCenterForOrthopaedicExcellence
