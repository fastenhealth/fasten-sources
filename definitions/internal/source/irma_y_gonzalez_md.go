...
func GetSourceIrmaYGonzalezMd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIrmaYGonzalezMd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Irma Y Gonzalez MD"
	sourceDef.SourceType = pkg.SourceTypeIrmaYGonzalezMd
	sourceDef.PatientAccessUrl = "http://irmagonzalezmd.com/"
