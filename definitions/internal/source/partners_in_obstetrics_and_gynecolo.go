...
func GetSourcePartnersInObstetricsAndGynecolo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePartnersInObstetricsAndGynecolo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Partners in Obstetrics and Gynecolo"
	sourceDef.SourceType = pkg.SourceTypePartnersInObstetricsAndGynecolo
