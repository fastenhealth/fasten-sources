...
func GetSourceHunterdonSurgicalAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHunterdonSurgicalAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Hunterdon Surgical Associates"
	sourceDef.SourceType = pkg.SourceTypeHunterdonSurgicalAssociates
	sourceDef.BrandLogo = "hunterdon-behavioral-health.svg"
	sourceDef.PatientAccessUrl = "https://www.hunterdonsurgical.com/"
