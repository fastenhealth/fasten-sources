...
func GetSourceRobertCByrdClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRobertCByrdClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ROBERT C BYRD CLINIC"
	sourceDef.SourceType = pkg.SourceTypeRobertCByrdClinic
	sourceDef.Category = []string{"207Q00000X", "208000000X"}
	sourceDef.Aliases = []string{"ROBERT C BYRD CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1871539296"}}
