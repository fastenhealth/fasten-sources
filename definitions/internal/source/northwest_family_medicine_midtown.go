...
func GetSourceNorthwestFamilyMedicineMidtown(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestFamilyMedicineMidtown]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Family Medicine  Midtown"
	sourceDef.SourceType = pkg.SourceTypeNorthwestFamilyMedicineMidtown
