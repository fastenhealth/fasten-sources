...
func GetSourceAxisSpineCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAxisSpineCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Axis Spine Center"
	sourceDef.SourceType = pkg.SourceTypeAxisSpineCenter
	sourceDef.Category = []string{"261QM1300X"}
	sourceDef.Aliases = []string{"AXIS SPINE CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1740917111"}}
