...
func GetSourceCentraMedicalGroupSouthsideLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentraMedicalGroupSouthsideLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CENTRA MEDICAL GROUP SOUTHSIDE LLC"
	sourceDef.SourceType = pkg.SourceTypeCentraMedicalGroupSouthsideLlc
