...
func GetSourceHutchinsonRegionalPhysicianNetwork(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHutchinsonRegionalPhysicianNetwork]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hutchinson Regional Physician Network"
	sourceDef.SourceType = pkg.SourceTypeHutchinsonRegionalPhysicianNetwork
