...
func GetSourceUhmpEntDrsHassanAndFadiAbbass(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUhmpEntDrsHassanAndFadiAbbass]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "UHMP ENT Drs Hassan and Fadi Abbass"
	sourceDef.SourceType = pkg.SourceTypeUhmpEntDrsHassanAndFadiAbbass
