...
func GetSourceAlpsRoadFamilyFootAndAnkle(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlpsRoadFamilyFootAndAnkle]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Alps Road Family Foot and Ankle"
	sourceDef.SourceType = pkg.SourceTypeAlpsRoadFamilyFootAndAnkle
	sourceDef.Category = []string{"261QP1100X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1245480003"}}
	sourceDef.PatientAccessUrl = "http://www.alpsroadfootandankle.com"
