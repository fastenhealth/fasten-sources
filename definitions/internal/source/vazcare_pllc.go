...
func GetSourceVazcarePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVazcarePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VAZCARE PLLC"
	sourceDef.SourceType = pkg.SourceTypeVazcarePllc
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"VAZCARE PLLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1710680699"}}
