...
func GetSourceShorepointInternalMedicineAndPediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShorepointInternalMedicineAndPediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ShorePoint Internal Medicine and Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeShorepointInternalMedicineAndPediatrics
