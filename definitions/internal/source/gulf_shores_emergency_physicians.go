...
func GetSourceGulfShoresEmergencyPhysicians(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGulfShoresEmergencyPhysicians]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GULF SHORES EMERGENCY PHYSICIANS"
	sourceDef.SourceType = pkg.SourceTypeGulfShoresEmergencyPhysicians
