...
func GetSourceVSuttonHealthcareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVSuttonHealthcareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "V Sutton Healthcare LLC"
	sourceDef.SourceType = pkg.SourceTypeVSuttonHealthcareLlc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"SUTTON HOLISTIC HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1467127274"}}
