...
func GetSourceCentralTexasSpineInstitutePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralTexasSpineInstitutePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Central Texas Spine Institute PLLC"
	sourceDef.SourceType = pkg.SourceTypeCentralTexasSpineInstitutePllc
