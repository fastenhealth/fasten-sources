...
func GetSourceSteeleFamilyRuralHealthClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSteeleFamilyRuralHealthClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Steele Family Rural Health Clinic"
	sourceDef.SourceType = pkg.SourceTypeSteeleFamilyRuralHealthClinic
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"STEELE FAMILY RURAL HEALTH CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1831689058"}}
