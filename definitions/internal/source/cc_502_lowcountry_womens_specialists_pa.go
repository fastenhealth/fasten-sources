...
func GetSourceCc502LowcountryWomensSpecialistsPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc502LowcountryWomensSpecialistsPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC502LOWCOUNTRY WOMENS SPECIALISTS PA"
	sourceDef.SourceType = pkg.SourceTypeCc502LowcountryWomensSpecialistsPa
