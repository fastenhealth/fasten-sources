...
func GetSourceCardiovascularSpecialistsOfSouth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCardiovascularSpecialistsOfSouth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CARDIOVASCULAR SPECIALISTS OF SOUTH"
	sourceDef.SourceType = pkg.SourceTypeCardiovascularSpecialistsOfSouth
