...
func GetSourceTennovaInfectiousDiseaseCleveland(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaInfectiousDiseaseCleveland]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Infectious Disease  Cleveland"
	sourceDef.SourceType = pkg.SourceTypeTennovaInfectiousDiseaseCleveland
