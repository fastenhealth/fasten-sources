...
func GetSourceSchirmerPsychiatricmentalHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSchirmerPsychiatricmentalHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Schirmer PsychiatricMental Health"
	sourceDef.SourceType = pkg.SourceTypeSchirmerPsychiatricmentalHealth
