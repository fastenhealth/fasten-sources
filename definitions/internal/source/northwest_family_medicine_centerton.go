...
func GetSourceNorthwestFamilyMedicineCenterton(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestFamilyMedicineCenterton]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Family Medicine  Centerton"
	sourceDef.SourceType = pkg.SourceTypeNorthwestFamilyMedicineCenterton
