...
func GetSourceMcculleyAllergySinusAsthmaCe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMcculleyAllergySinusAsthmaCe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "McCulley Allergy Sinus  Asthma Ce"
	sourceDef.SourceType = pkg.SourceTypeMcculleyAllergySinusAsthmaCe
