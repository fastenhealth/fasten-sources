...
func GetSourceEliteObstetricsAndGynecologyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEliteObstetricsAndGynecologyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ELITE OBSTETRICS AND GYNECOLOGY LLC"
	sourceDef.SourceType = pkg.SourceTypeEliteObstetricsAndGynecologyLlc
