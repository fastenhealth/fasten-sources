...
func GetSourceCardiologyAssociatesOfNorthernN(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCardiologyAssociatesOfNorthernN]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cardiology Associates of Northern N"
	sourceDef.SourceType = pkg.SourceTypeCardiologyAssociatesOfNorthernN
