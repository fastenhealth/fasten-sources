...
func GetSourceCaInstCardiovascularLsrVnSurge(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCaInstCardiovascularLsrVnSurge]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CA Inst Cardiovascular Lsr Vn Surge"
	sourceDef.SourceType = pkg.SourceTypeCaInstCardiovascularLsrVnSurge
