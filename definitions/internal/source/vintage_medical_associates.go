...
func GetSourceVintageMedicalAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVintageMedicalAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Vintage Medical Associates"
	sourceDef.SourceType = pkg.SourceTypeVintageMedicalAssociates
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"VINTAGE MEDICAL ASSOCIATES"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1134420821"}}
