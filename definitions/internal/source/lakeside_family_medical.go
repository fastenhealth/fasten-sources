...
func GetSourceLakesideFamilyMedical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLakesideFamilyMedical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lakeside Family Medical"
	sourceDef.SourceType = pkg.SourceTypeLakesideFamilyMedical
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"LAKESIDE FAMILY MEDICAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1497363303"}}
