...
func GetSourceTheBreastfeedingSuccessCompanyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheBreastfeedingSuccessCompanyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Breastfeeding Success Company LLC"
	sourceDef.SourceType = pkg.SourceTypeTheBreastfeedingSuccessCompanyLlc
