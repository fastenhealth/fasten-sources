...
func GetSourceTheWomensCenterForTotalHealthLlcTermed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheWomensCenterForTotalHealthLlcTermed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "THE WOMENS CENTER FOR TOTAL HEALTH LLC TERMED"
	sourceDef.SourceType = pkg.SourceTypeTheWomensCenterForTotalHealthLlcTermed
