...
func GetSourcePotomacOnCallLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePotomacOnCallLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Potomac on Call LLC"
	sourceDef.SourceType = pkg.SourceTypePotomacOnCallLlc
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"POTOMAC ELDERCARE PHYSICIANS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1922647775"}}
