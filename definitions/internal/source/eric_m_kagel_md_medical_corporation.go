...
func GetSourceEricMKagelMdMedicalCorporation(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEricMKagelMdMedicalCorporation]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Eric M Kagel MD Medical Corporation"
	sourceDef.SourceType = pkg.SourceTypeEricMKagelMdMedicalCorporation
