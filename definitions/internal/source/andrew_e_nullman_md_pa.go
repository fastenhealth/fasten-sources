...
func GetSourceAndrewENullmanMdPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAndrewENullmanMdPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Andrew E Nullman MD PA"
	sourceDef.SourceType = pkg.SourceTypeAndrewENullmanMdPa
	sourceDef.Category = []string{"207RG0100X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1457459489"}}
	sourceDef.PatientAccessUrl = "http://www.drnullman.com"
