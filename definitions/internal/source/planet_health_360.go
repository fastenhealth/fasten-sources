...
func GetSourcePlanetHealth360(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePlanetHealth360]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Planet Health 360"
	sourceDef.SourceType = pkg.SourceTypePlanetHealth360
	sourceDef.Category = []string{"261QU0200X"}
	sourceDef.Aliases = []string{"PLANET HEALTH 360"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1114431145"}}
