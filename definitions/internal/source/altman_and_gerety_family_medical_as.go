...
func GetSourceAltmanAndGeretyFamilyMedicalAs(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAltmanAndGeretyFamilyMedicalAs]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Altman and Gerety Family Medical As"
	sourceDef.SourceType = pkg.SourceTypeAltmanAndGeretyFamilyMedicalAs
