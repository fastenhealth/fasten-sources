...
func GetSourceAmsurgOaklandAnesthesiaLp(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmsurgOaklandAnesthesiaLp]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "AmSurg Oakland Anesthesia LP"
	sourceDef.SourceType = pkg.SourceTypeAmsurgOaklandAnesthesiaLp
	sourceDef.Category = []string{"207L00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1679847727"}}
