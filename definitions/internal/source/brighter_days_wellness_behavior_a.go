...
func GetSourceBrighterDaysWellnessBehaviorA(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBrighterDaysWellnessBehaviorA]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BRIGHTER DAYS WELLNESS  BEHAVIOR A"
	sourceDef.SourceType = pkg.SourceTypeBrighterDaysWellnessBehaviorA
