...
func GetSourceEliseLeonardMdLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEliseLeonardMdLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Elise Leonard MD LLC"
	sourceDef.SourceType = pkg.SourceTypeEliseLeonardMdLlc
	sourceDef.PatientAccessUrl = "https://eliseleonardmd.com/"
