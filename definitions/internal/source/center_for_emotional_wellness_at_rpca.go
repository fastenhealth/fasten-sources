...
func GetSourceCenterForEmotionalWellnessAtRpca(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForEmotionalWellnessAtRpca]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center for Emotional Wellness at RPCA"
	sourceDef.SourceType = pkg.SourceTypeCenterForEmotionalWellnessAtRpca
