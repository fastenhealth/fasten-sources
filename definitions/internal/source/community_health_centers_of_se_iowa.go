...
func GetSourceCommunityHealthCentersOfSeIowa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCommunityHealthCentersOfSeIowa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Community Health Centers Of SE Iowa"
	sourceDef.SourceType = pkg.SourceTypeCommunityHealthCentersOfSeIowa
