...
func GetSourceCc015AssociatesInWomensHealthcare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc015AssociatesInWomensHealthcare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC015ASSOCIATES IN WOMENS HEALTHCARE"
	sourceDef.SourceType = pkg.SourceTypeCc015AssociatesInWomensHealthcare
