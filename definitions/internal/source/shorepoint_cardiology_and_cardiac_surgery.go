...
func GetSourceShorepointCardiologyAndCardiacSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShorepointCardiologyAndCardiacSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ShorePoint Cardiology and Cardiac Surgery"
	sourceDef.SourceType = pkg.SourceTypeShorepointCardiologyAndCardiacSurgery
