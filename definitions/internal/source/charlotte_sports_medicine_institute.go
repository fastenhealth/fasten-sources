...
func GetSourceCharlotteSportsMedicineInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCharlotteSportsMedicineInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Charlotte Sports Medicine Institute"
	sourceDef.SourceType = pkg.SourceTypeCharlotteSportsMedicineInstitute
