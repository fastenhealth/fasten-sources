...
func GetSourceSollisHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSollisHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sollis Health"
	sourceDef.SourceType = pkg.SourceTypeSollisHealth
	sourceDef.Category = []string{"261QU0200X"}
	sourceDef.Aliases = []string{"SOLLIS HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1013531607", "1922685569"}}
