...
func GetSourcePeachtreePsychiatricProfessionals(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePeachtreePsychiatricProfessionals]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Peachtree Psychiatric Professionals"
	sourceDef.SourceType = pkg.SourceTypePeachtreePsychiatricProfessionals
