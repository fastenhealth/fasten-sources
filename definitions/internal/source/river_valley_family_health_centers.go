...
func GetSourceRiverValleyFamilyHealthCenters(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRiverValleyFamilyHealthCenters]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "River Valley Family Health Centers"
	sourceDef.SourceType = pkg.SourceTypeRiverValleyFamilyHealthCenters
	sourceDef.Category = []string{"3336C0003X"}
	sourceDef.Aliases = []string{"RIVER VALLEY PHARMACY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1780232215"}}
