...
func GetSourceSportsMedicineOrthopedicCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSportsMedicineOrthopedicCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sports Medicine  Orthopedic Center"
	sourceDef.SourceType = pkg.SourceTypeSportsMedicineOrthopedicCenter
