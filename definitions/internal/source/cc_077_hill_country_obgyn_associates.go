...
func GetSourceCc077HillCountryObgynAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc077HillCountryObgynAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC077HILL COUNTRY OBGYN ASSOCIATES"
	sourceDef.SourceType = pkg.SourceTypeCc077HillCountryObgynAssociates
