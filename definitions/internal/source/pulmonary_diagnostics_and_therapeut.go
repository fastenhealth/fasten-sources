...
func GetSourcePulmonaryDiagnosticsAndTherapeut(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePulmonaryDiagnosticsAndTherapeut]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pulmonary Diagnostics and Therapeut"
	sourceDef.SourceType = pkg.SourceTypePulmonaryDiagnosticsAndTherapeut
