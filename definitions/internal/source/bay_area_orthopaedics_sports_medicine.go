...
func GetSourceBayAreaOrthopaedicsSportsMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBayAreaOrthopaedicsSportsMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bay Area Orthopaedics  Sports Medicine"
	sourceDef.SourceType = pkg.SourceTypeBayAreaOrthopaedicsSportsMedicine
