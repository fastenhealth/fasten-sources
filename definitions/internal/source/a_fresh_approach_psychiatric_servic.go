...
func GetSourceAFreshApproachPsychiatricServic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAFreshApproachPsychiatricServic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "A Fresh Approach Psychiatric Servic"
	sourceDef.SourceType = pkg.SourceTypeAFreshApproachPsychiatricServic
