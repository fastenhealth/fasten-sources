...
func GetSourcePreventiveMedicineInstitutePa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePreventiveMedicineInstitutePa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Preventive Medicine Institute PA"
	sourceDef.SourceType = pkg.SourceTypePreventiveMedicineInstitutePa
