...
func GetSourceEastTexasPulmonaryAndSleepCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEastTexasPulmonaryAndSleepCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "East Texas Pulmonary and Sleep Center"
	sourceDef.SourceType = pkg.SourceTypeEastTexasPulmonaryAndSleepCenter
