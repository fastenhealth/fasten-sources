...
func GetSourceRadicalRecoveryTreatmentCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRadicalRecoveryTreatmentCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Radical Recovery Treatment Center"
	sourceDef.SourceType = pkg.SourceTypeRadicalRecoveryTreatmentCenter
