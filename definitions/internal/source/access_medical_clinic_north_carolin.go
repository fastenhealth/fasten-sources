...
func GetSourceAccessMedicalClinicNorthCarolin(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAccessMedicalClinicNorthCarolin]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Access Medical Clinic North Carolin"
	sourceDef.SourceType = pkg.SourceTypeAccessMedicalClinicNorthCarolin
