...
func GetSourceWholeSystemsHealthcarePortland(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWholeSystemsHealthcarePortland]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Whole Systems Healthcare Portland"
	sourceDef.SourceType = pkg.SourceTypeWholeSystemsHealthcarePortland
