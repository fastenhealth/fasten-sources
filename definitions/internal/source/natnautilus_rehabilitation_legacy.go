...
func GetSourceNatnautilusRehabilitationLegacy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNatnautilusRehabilitationLegacy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NATNautilus Rehabilitation Legacy"
	sourceDef.SourceType = pkg.SourceTypeNatnautilusRehabilitationLegacy
