...
func GetSourceEasternRegionalPainSpecialistsP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEasternRegionalPainSpecialistsP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Eastern Regional Pain Specialists P"
	sourceDef.SourceType = pkg.SourceTypeEasternRegionalPainSpecialistsP
