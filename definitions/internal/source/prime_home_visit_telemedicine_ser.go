...
func GetSourcePrimeHomeVisitTelemedicineSer(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrimeHomeVisitTelemedicineSer]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Prime Home Visit  Telemedicine Ser"
	sourceDef.SourceType = pkg.SourceTypePrimeHomeVisitTelemedicineSer
