...
func GetSourceGlobalHealthAndRecoveryServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGlobalHealthAndRecoveryServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Global Health and Recovery Services"
	sourceDef.SourceType = pkg.SourceTypeGlobalHealthAndRecoveryServices
