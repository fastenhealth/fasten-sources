...
func GetSourcePrimaryCareClinicOfNorthDallas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrimaryCareClinicOfNorthDallas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Primary Care Clinic of North Dallas"
	sourceDef.SourceType = pkg.SourceTypePrimaryCareClinicOfNorthDallas
