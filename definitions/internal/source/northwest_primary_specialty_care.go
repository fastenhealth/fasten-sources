...
func GetSourceNorthwestPrimarySpecialtyCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestPrimarySpecialtyCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Primary  Specialty Care"
	sourceDef.SourceType = pkg.SourceTypeNorthwestPrimarySpecialtyCare
