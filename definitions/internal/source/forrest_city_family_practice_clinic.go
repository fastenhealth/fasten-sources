...
func GetSourceForrestCityFamilyPracticeClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeForrestCityFamilyPracticeClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Forrest City Family Practice Clinic"
	sourceDef.SourceType = pkg.SourceTypeForrestCityFamilyPracticeClinic
