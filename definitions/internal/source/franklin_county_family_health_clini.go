...
func GetSourceFranklinCountyFamilyHealthClini(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFranklinCountyFamilyHealthClini]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Franklin County Family Health Clini"
	sourceDef.SourceType = pkg.SourceTypeFranklinCountyFamilyHealthClini
