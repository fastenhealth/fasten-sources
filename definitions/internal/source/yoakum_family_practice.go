...
func GetSourceYoakumFamilyPractice(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeYoakumFamilyPractice]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Yoakum Family Practice"
	sourceDef.SourceType = pkg.SourceTypeYoakumFamilyPractice
	sourceDef.Category = []string{"208D00000X"}
	sourceDef.Aliases = []string{"YOAKUM FAMILY PRACTICE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1619399128"}}
