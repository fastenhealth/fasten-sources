...
func GetSourceHarmonyPsychiatricServicesPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHarmonyPsychiatricServicesPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Harmony Psychiatric Services PLLC"
	sourceDef.SourceType = pkg.SourceTypeHarmonyPsychiatricServicesPllc
