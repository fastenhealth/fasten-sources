...
func GetSourceWomensHealthCollective(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWomensHealthCollective]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Womens Health Collective"
	sourceDef.SourceType = pkg.SourceTypeWomensHealthCollective
	sourceDef.Category = []string{"207VG0400X"}
	sourceDef.Aliases = []string{"WOMEN'S HEALTH COLLECTIVE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1629615646"}}
