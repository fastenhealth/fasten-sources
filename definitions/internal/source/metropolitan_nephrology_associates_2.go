...
func GetSourceMetropolitanNephrologyAssociates2(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMetropolitanNephrologyAssociates2]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Metropolitan Nephrology Associates 2"
	sourceDef.SourceType = pkg.SourceTypeMetropolitanNephrologyAssociates2
