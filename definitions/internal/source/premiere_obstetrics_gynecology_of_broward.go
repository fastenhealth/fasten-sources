...
func GetSourcePremiereObstetricsGynecologyOfBroward(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePremiereObstetricsGynecologyOfBroward]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PREMIERE OBSTETRICS  GYNECOLOGY OF BROWARD"
	sourceDef.SourceType = pkg.SourceTypePremiereObstetricsGynecologyOfBroward
