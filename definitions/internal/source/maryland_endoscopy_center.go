...
func GetSourceMarylandEndoscopyCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMarylandEndoscopyCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Maryland Endoscopy Center"
	sourceDef.SourceType = pkg.SourceTypeMarylandEndoscopyCenter
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"MARYLAND ENDOSCOPY CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1710952130"}}
