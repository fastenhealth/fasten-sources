...
func GetSourceGynecologicalSpecialtyCareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGynecologicalSpecialtyCareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GYNECOLOGICAL SPECIALTY CARE LLC"
	sourceDef.SourceType = pkg.SourceTypeGynecologicalSpecialtyCareLlc
