...
func GetSourceNorthwestCenterForInfectiousDiseases(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestCenterForInfectiousDiseases]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Center for Infectious Diseases"
	sourceDef.SourceType = pkg.SourceTypeNorthwestCenterForInfectiousDiseases
