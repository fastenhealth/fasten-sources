...
func GetSourceEndocrinologyAssociatesPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEndocrinologyAssociatesPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ENDOCRINOLOGY ASSOCIATES PA"
	sourceDef.SourceType = pkg.SourceTypeEndocrinologyAssociatesPa
	sourceDef.Category = []string{"207RE0101X"}
	sourceDef.Aliases = []string{"DIANE R KRIEGER, MD"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1083165658"}}
