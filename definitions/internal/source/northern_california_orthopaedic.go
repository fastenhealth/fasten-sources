...
func GetSourceNorthernCaliforniaOrthopaedic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthernCaliforniaOrthopaedic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTHERN CALIFORNIA ORTHOPAEDIC"
	sourceDef.SourceType = pkg.SourceTypeNorthernCaliforniaOrthopaedic
