...
func GetSourcePrimaryCareAssociatesOfLaredo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrimaryCareAssociatesOfLaredo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Primary Care Associates of Laredo"
	sourceDef.SourceType = pkg.SourceTypePrimaryCareAssociatesOfLaredo
