...
func GetSourceMontegaFamilyMedicinePa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMontegaFamilyMedicinePa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MONTEGA FAMILY MEDICINE PA"
	sourceDef.SourceType = pkg.SourceTypeMontegaFamilyMedicinePa
	sourceDef.Category = []string{"207QA0505X"}
	sourceDef.Aliases = []string{"MONTEGA MEDICAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1306410634"}}
