...
func GetSourceKevinJKesslerMdPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKevinJKesslerMdPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kevin J Kessler MD PA"
	sourceDef.SourceType = pkg.SourceTypeKevinJKesslerMdPa
	sourceDef.Category = []string{"207X00000X"}
	sourceDef.Aliases = []string{"KESSLER LIPMAN ORTHOPEDICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1528034402"}}
