...
func GetSourceAlliancehealthMedicalGroupSpecialistsClinton(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupSpecialistsClinton]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Specialists  Clinton"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupSpecialistsClinton
