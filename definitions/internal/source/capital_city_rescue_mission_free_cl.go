...
func GetSourceCapitalCityRescueMissionFreeCl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCapitalCityRescueMissionFreeCl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Capital City Rescue Mission Free Cl"
	sourceDef.SourceType = pkg.SourceTypeCapitalCityRescueMissionFreeCl
