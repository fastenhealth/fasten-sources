...
func GetSourceObstetrixMedicalGroupOfOklahomaPC(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeObstetrixMedicalGroupOfOklahomaPC]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Obstetrix Medical Group Of Oklahoma P.C"
	sourceDef.SourceType = pkg.SourceTypeObstetrixMedicalGroupOfOklahomaPC
