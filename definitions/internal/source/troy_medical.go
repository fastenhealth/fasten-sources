...
func GetSourceTroyMedical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTroyMedical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Troy Medical"
	sourceDef.SourceType = pkg.SourceTypeTroyMedical
	sourceDef.Category = []string{"261QM1300X"}
	sourceDef.Aliases = []string{"TROY MEDICAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1760077747"}}
	sourceDef.PatientAccessUrl = "http://www.troymed.com"
