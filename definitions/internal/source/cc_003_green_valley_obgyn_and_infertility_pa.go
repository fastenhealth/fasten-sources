...
func GetSourceCc003GreenValleyObgynAndInfertilityPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc003GreenValleyObgynAndInfertilityPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC003Green Valley OBGYN and Infertility PA"
	sourceDef.SourceType = pkg.SourceTypeCc003GreenValleyObgynAndInfertilityPa
