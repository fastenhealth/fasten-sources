...
func GetSourceKyTririversHealthcarePllcSalem(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKyTririversHealthcarePllcSalem]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "KY  TriRivers Healthcare  PLLC  Salem"
	sourceDef.SourceType = pkg.SourceTypeKyTririversHealthcarePllcSalem
