...
func GetSourceAustinBreastImaging(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAustinBreastImaging]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Austin Breast Imaging"
	sourceDef.SourceType = pkg.SourceTypeAustinBreastImaging
	sourceDef.Category = []string{"2085R0202X", "2085R0204X"}
	sourceDef.Aliases = []string{"AUSTIN BREAST IMAGING"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1851796171"}}
