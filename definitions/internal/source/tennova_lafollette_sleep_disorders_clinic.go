...
func GetSourceTennovaLafolletteSleepDisordersClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaLafolletteSleepDisordersClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova LaFollette Sleep Disorders Clinic"
	sourceDef.SourceType = pkg.SourceTypeTennovaLafolletteSleepDisordersClinic
