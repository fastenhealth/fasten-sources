...
func GetSourcePaChsSunburyClinicCompanyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePaChsSunburyClinicCompanyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PA  CHS  Sunbury Clinic Company LLC"
	sourceDef.SourceType = pkg.SourceTypePaChsSunburyClinicCompanyLlc
