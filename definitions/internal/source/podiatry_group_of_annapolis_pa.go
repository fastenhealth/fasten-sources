...
func GetSourcePodiatryGroupOfAnnapolisPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePodiatryGroupOfAnnapolisPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Podiatry Group of Annapolis PA"
	sourceDef.SourceType = pkg.SourceTypePodiatryGroupOfAnnapolisPa
	sourceDef.Category = []string{"213E00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1427167923"}}
