...
func GetSourceRainbowMedicalAssociationMdP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRainbowMedicalAssociationMdP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RAINBOW MEDICAL ASSOCIATION MD P"
	sourceDef.SourceType = pkg.SourceTypeRainbowMedicalAssociationMdP
