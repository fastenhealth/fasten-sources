...
func GetSourceCarneiroInstituteForHandSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCarneiroInstituteForHandSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Carneiro Institute for Hand Surgery"
	sourceDef.SourceType = pkg.SourceTypeCarneiroInstituteForHandSurgery
