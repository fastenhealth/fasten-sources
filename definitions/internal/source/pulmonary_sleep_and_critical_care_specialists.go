...
func GetSourcePulmonarySleepAndCriticalCareSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePulmonarySleepAndCriticalCareSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pulmonary Sleep and Critical Care Specialists"
	sourceDef.SourceType = pkg.SourceTypePulmonarySleepAndCriticalCareSpecialists
