...
func GetSourceEliteUpperExtremityAndPlasticSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEliteUpperExtremityAndPlasticSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Elite Upper Extremity and Plastic Surgery"
	sourceDef.SourceType = pkg.SourceTypeEliteUpperExtremityAndPlasticSurgery
