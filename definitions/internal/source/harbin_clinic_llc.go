...
func GetSourceHarbinClinicLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHarbinClinicLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Harbin Clinic LLC"
	sourceDef.SourceType = pkg.SourceTypeHarbinClinicLlc
	sourceDef.Category = []string{"333600000X", "3336C0003X"}
	sourceDef.Aliases = []string{"HARBIN CLINIC CANCER CENTER PHARMACY", "HARBIN CLINIC PHARMACY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1184923492", "1952318834"}}
