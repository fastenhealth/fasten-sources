...
func GetSourceSaintClairAllergyAndAsthmaCent(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSaintClairAllergyAndAsthmaCent]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Saint Clair Allergy And Asthma Cent"
	sourceDef.SourceType = pkg.SourceTypeSaintClairAllergyAndAsthmaCent
