...
func GetSourceTheGoodClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheGoodClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Good Clinic"
	sourceDef.SourceType = pkg.SourceTypeTheGoodClinic
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"THE GOOD CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1003423252", "1568120319"}}
