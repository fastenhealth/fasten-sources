...
func GetSourceMdvipDrWendyWalkerDrThomasLoomis(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMdvipDrWendyWalkerDrThomasLoomis]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MDVIP  Dr Wendy Walker  Dr Thomas Loomis"
	sourceDef.SourceType = pkg.SourceTypeMdvipDrWendyWalkerDrThomasLoomis
