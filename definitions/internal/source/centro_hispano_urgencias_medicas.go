...
func GetSourceCentroHispanoUrgenciasMedicas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentroHispanoUrgenciasMedicas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Centro Hispano Urgencias Medicas"
	sourceDef.SourceType = pkg.SourceTypeCentroHispanoUrgenciasMedicas
	sourceDef.Category = []string{"261QU0200X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1366711590", "1497139042"}}
