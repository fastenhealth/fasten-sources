...
func GetSourceSunCountryPediatricsMainOfficeSpringHill(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSunCountryPediatricsMainOfficeSpringHill]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sun Country Pediatrics MAIN OFFICE  Spring Hill"
	sourceDef.SourceType = pkg.SourceTypeSunCountryPediatricsMainOfficeSpringHill
