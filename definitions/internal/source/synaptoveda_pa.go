...
func GetSourceSynaptovedaPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSynaptovedaPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SYNAPTOVEDA PA"
	sourceDef.SourceType = pkg.SourceTypeSynaptovedaPa
	sourceDef.Category = []string{"2084N0400X"}
	sourceDef.Aliases = []string{"NEUROMUSCULAR AND EMG SPECIALISTS OF TEXAS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1710408265"}}
