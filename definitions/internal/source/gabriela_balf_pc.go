...
func GetSourceGabrielaBalfPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGabrielaBalfPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gabriela Balf PC"
	sourceDef.SourceType = pkg.SourceTypeGabrielaBalfPc
	sourceDef.Category = []string{"2084P0800X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1215625645"}}
	sourceDef.PatientAccessUrl = "http://www.dr-gabriela.com"
