...
func GetSourceHelpingHandBehavioralServicesL(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHelpingHandBehavioralServicesL]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Helping Hand Behavioral Services L"
	sourceDef.SourceType = pkg.SourceTypeHelpingHandBehavioralServicesL
