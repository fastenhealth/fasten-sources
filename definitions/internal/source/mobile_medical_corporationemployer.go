...
func GetSourceMobileMedicalCorporationemployer(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMobileMedicalCorporationemployer]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Mobile Medical CorporationEmployer"
	sourceDef.SourceType = pkg.SourceTypeMobileMedicalCorporationemployer
