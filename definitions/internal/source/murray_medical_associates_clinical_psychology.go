...
func GetSourceMurrayMedicalAssociatesClinicalPsychology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMurrayMedicalAssociatesClinicalPsychology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Murray Medical Associates Clinical Psychology"
	sourceDef.SourceType = pkg.SourceTypeMurrayMedicalAssociatesClinicalPsychology
