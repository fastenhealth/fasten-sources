...
func GetSourcePersonalizedWomensHealthcareParkhillAffiliate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePersonalizedWomensHealthcareParkhillAffiliate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Personalized Womens Healthcare ParkHill Affiliate"
	sourceDef.SourceType = pkg.SourceTypePersonalizedWomensHealthcareParkhillAffiliate
