...
func GetSourceBlairFamilyMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBlairFamilyMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Blair Family Medicine"
	sourceDef.SourceType = pkg.SourceTypeBlairFamilyMedicine
	sourceDef.Category = []string{"363L00000X"}
	sourceDef.Aliases = []string{"BLAIR FAMILY MEDICINE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1134620107"}}
	sourceDef.PatientAccessUrl = "http://www.blairfamilymed.com"
