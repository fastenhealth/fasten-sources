...
func GetSourceInpatientClinicalsTestHospital(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInpatientClinicalsTestHospital]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Inpatient Clinicals Test Hospital"
	sourceDef.SourceType = pkg.SourceTypeInpatientClinicalsTestHospital
