...
func GetSourceRockyMountainUrgentCareFamilyMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRockyMountainUrgentCareFamilyMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Rocky Mountain Urgent Care  Family Medicine"
	sourceDef.SourceType = pkg.SourceTypeRockyMountainUrgentCareFamilyMedicine
