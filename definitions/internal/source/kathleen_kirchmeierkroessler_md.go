...
func GetSourceKathleenKirchmeierkroesslerMd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKathleenKirchmeierkroesslerMd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kathleen KirchmeierKroessler MD"
	sourceDef.SourceType = pkg.SourceTypeKathleenKirchmeierkroesslerMd
