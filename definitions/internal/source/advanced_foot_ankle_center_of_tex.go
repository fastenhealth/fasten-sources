...
func GetSourceAdvancedFootAnkleCenterOfTex(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedFootAnkleCenterOfTex]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Advanced Foot  Ankle Center of Tex"
	sourceDef.SourceType = pkg.SourceTypeAdvancedFootAnkleCenterOfTex
