...
func GetSourceTotalFootAndAnkleCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTotalFootAndAnkleCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Total Foot and Ankle Center"
	sourceDef.SourceType = pkg.SourceTypeTotalFootAndAnkleCenter
	sourceDef.Category = []string{"261QP1100X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1154919090"}}
	sourceDef.PatientAccessUrl = "http://honesdalepodiatrist.com"
