...
func GetSourceOrthopedicAndSpineInstituteOfSocal(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOrthopedicAndSpineInstituteOfSocal]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Orthopedic & Spine Institute Of SoCal"
	sourceDef.SourceType = pkg.SourceTypeOrthopedicAndSpineInstituteOfSocal
