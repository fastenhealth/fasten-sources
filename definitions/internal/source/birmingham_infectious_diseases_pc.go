...
func GetSourceBirminghamInfectiousDiseasesPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBirminghamInfectiousDiseasesPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Birmingham Infectious Diseases PC"
	sourceDef.SourceType = pkg.SourceTypeBirminghamInfectiousDiseasesPc
