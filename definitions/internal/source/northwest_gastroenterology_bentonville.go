...
func GetSourceNorthwestGastroenterologyBentonville(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestGastroenterologyBentonville]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Gastroenterology  Bentonville"
	sourceDef.SourceType = pkg.SourceTypeNorthwestGastroenterologyBentonville
