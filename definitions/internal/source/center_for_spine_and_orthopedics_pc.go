...
func GetSourceCenterForSpineAndOrthopedicsPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForSpineAndOrthopedicsPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center For Spine And Orthopedics PC"
	sourceDef.SourceType = pkg.SourceTypeCenterForSpineAndOrthopedicsPc
