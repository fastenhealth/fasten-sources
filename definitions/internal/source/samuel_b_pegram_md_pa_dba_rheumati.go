...
func GetSourceSamuelBPegramMdPaDbaRheumati(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSamuelBPegramMdPaDbaRheumati]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Samuel B Pegram MD PA DBA Rheumati"
	sourceDef.SourceType = pkg.SourceTypeSamuelBPegramMdPaDbaRheumati
