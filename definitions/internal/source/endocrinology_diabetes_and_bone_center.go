...
func GetSourceEndocrinologyDiabetesAndBoneCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEndocrinologyDiabetesAndBoneCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ENDOCRINOLOGY DIABETES AND BONE CENTER"
	sourceDef.SourceType = pkg.SourceTypeEndocrinologyDiabetesAndBoneCenter
