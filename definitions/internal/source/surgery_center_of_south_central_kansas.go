...
func GetSourceSurgeryCenterOfSouthCentralKansas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSurgeryCenterOfSouthCentralKansas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Surgery Center Of South Central Kansas"
	sourceDef.SourceType = pkg.SourceTypeSurgeryCenterOfSouthCentralKansas
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"SURGERY CENTER OF SOUTH CENTRAL KANSAS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1043283294"}}
