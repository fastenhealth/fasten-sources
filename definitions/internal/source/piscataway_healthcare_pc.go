...
func GetSourcePiscatawayHealthcarePc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePiscatawayHealthcarePc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PISCATAWAY HEALTHCARE PC"
	sourceDef.SourceType = pkg.SourceTypePiscatawayHealthcarePc
	sourceDef.Category = []string{"261QC1800X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1508001827"}}
