...
func GetSourceHighlandsRanchFamilyMedicineP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHighlandsRanchFamilyMedicineP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Highlands Ranch Family Medicine P"
	sourceDef.SourceType = pkg.SourceTypeHighlandsRanchFamilyMedicineP
