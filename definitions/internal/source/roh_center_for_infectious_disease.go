...
func GetSourceRohCenterForInfectiousDisease(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRohCenterForInfectiousDisease]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ROH Center for Infectious Disease"
	sourceDef.SourceType = pkg.SourceTypeRohCenterForInfectiousDisease
