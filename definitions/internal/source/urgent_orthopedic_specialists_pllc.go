...
func GetSourceUrgentOrthopedicSpecialistsPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUrgentOrthopedicSpecialistsPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Urgent Orthopedic Specialists PLLC"
	sourceDef.SourceType = pkg.SourceTypeUrgentOrthopedicSpecialistsPllc
