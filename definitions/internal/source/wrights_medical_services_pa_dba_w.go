...
func GetSourceWrightsMedicalServicesPaDbaW(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWrightsMedicalServicesPaDbaW]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Wrights Medical Services PA dba W"
	sourceDef.SourceType = pkg.SourceTypeWrightsMedicalServicesPaDbaW
