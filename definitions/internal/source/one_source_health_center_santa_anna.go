...
func GetSourceOneSourceHealthCenterSantaAnna(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOneSourceHealthCenterSantaAnna]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "One Source Health Center  Santa Anna"
	sourceDef.SourceType = pkg.SourceTypeOneSourceHealthCenterSantaAnna
