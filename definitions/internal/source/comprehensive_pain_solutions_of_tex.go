...
func GetSourceComprehensivePainSolutionsOfTex(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeComprehensivePainSolutionsOfTex]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Comprehensive Pain Solutions of Tex"
	sourceDef.SourceType = pkg.SourceTypeComprehensivePainSolutionsOfTex
