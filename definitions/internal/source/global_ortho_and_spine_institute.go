...
func GetSourceGlobalOrthoAndSpineInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGlobalOrthoAndSpineInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Global Ortho and Spine Institute"
	sourceDef.SourceType = pkg.SourceTypeGlobalOrthoAndSpineInstitute
