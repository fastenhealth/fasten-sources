...
func GetSourceSportsMedicineAndOrthopaedicCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSportsMedicineAndOrthopaedicCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sports Medicine and Orthopaedic Center"
	sourceDef.SourceType = pkg.SourceTypeSportsMedicineAndOrthopaedicCenter
	sourceDef.Category = []string{"225100000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1386303832"}}
	sourceDef.PatientAccessUrl = "https://smoc-pt.com/"
