...
func GetSourceNorthScottsdaleBehavioralHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthScottsdaleBehavioralHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Scottsdale Behavioral Health"
	sourceDef.SourceType = pkg.SourceTypeNorthScottsdaleBehavioralHealth
