...
func GetSourceGraceCenterForFamilyCommunity(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGraceCenterForFamilyCommunity]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Grace Center for Family  Community"
	sourceDef.SourceType = pkg.SourceTypeGraceCenterForFamilyCommunity
