...
func GetSourceHighlandUrgentCareFamilyMedic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHighlandUrgentCareFamilyMedic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Highland Urgent Care  Family Medic"
	sourceDef.SourceType = pkg.SourceTypeHighlandUrgentCareFamilyMedic
