...
func GetSourceKentuckianaSurgicalSpecialistsP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKentuckianaSurgicalSpecialistsP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kentuckiana Surgical Specialists P"
	sourceDef.SourceType = pkg.SourceTypeKentuckianaSurgicalSpecialistsP
