...
func GetSourceManaswisOrthopedicAndJointRepl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeManaswisOrthopedicAndJointRepl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Manaswis Orthopedic and Joint Repl"
	sourceDef.SourceType = pkg.SourceTypeManaswisOrthopedicAndJointRepl
