...
func GetSourceNextcareNebraska(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNextcareNebraska]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "NextCare Nebraska"
	sourceDef.SourceType = pkg.SourceTypeNextcareNebraska
	sourceDef.PatientAccessUrl = "http://m.scottsdalechamber.chambermaster.com/Mobile/Directory/memberdetails/nextcare-urgent-care-oak"
