...
func GetSourceTennovaFamilyMedicineTinyTown(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaFamilyMedicineTinyTown]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Family Medicine  Tiny Town"
	sourceDef.SourceType = pkg.SourceTypeTennovaFamilyMedicineTinyTown
