...
func GetSourceStudentHealthImpactProject(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStudentHealthImpactProject]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "STUDENT HEALTH IMPACT PROJECT"
	sourceDef.SourceType = pkg.SourceTypeStudentHealthImpactProject
	sourceDef.Category = []string{"1223G0001X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1972089993"}}
