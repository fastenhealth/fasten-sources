...
func GetSourceInclusiveCareGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInclusiveCareGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Inclusive Care Group"
	sourceDef.SourceType = pkg.SourceTypeInclusiveCareGroup
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"INCLUSIVE CARE GROUP"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1801540190"}}
	sourceDef.PatientAccessUrl = "http://www.inclusivecaregroup.com"
