...
func GetSourceMidtownReproductiveMedicinePc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidtownReproductiveMedicinePc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Midtown Reproductive Medicine PC"
	sourceDef.SourceType = pkg.SourceTypeMidtownReproductiveMedicinePc
