...
func GetSourceBayAreaWomensCareOfClearwaterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBayAreaWomensCareOfClearwaterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BAY AREA WOMENS CARE OF CLEARWATER LLC"
	sourceDef.SourceType = pkg.SourceTypeBayAreaWomensCareOfClearwaterLlc
