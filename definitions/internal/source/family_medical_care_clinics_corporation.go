...
func GetSourceFamilyMedicalCareClinicsCorporation(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFamilyMedicalCareClinicsCorporation]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Family Medical Care Clinics Corporation"
	sourceDef.SourceType = pkg.SourceTypeFamilyMedicalCareClinicsCorporation
