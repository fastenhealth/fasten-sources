...
func GetSourceYakimaValleyFamilyClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeYakimaValleyFamilyClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Yakima Valley Family Clinic"
	sourceDef.SourceType = pkg.SourceTypeYakimaValleyFamilyClinic
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{"YAKIMA VALLEY FAMILY CLINIC, LLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1821621863"}}
