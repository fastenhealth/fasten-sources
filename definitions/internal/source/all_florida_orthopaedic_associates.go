...
func GetSourceAllFloridaOrthopaedicAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllFloridaOrthopaedicAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "All Florida Orthopaedic Associates"
	sourceDef.SourceType = pkg.SourceTypeAllFloridaOrthopaedicAssociates
