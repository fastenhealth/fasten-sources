...
func GetSourceMorenoValleyCommunityMedicalGro(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMorenoValleyCommunityMedicalGro]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Moreno Valley Community Medical Gro"
	sourceDef.SourceType = pkg.SourceTypeMorenoValleyCommunityMedicalGro
