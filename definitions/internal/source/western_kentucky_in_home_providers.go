...
func GetSourceWesternKentuckyInHomeProviders(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWesternKentuckyInHomeProviders]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Western Kentucky In Home Providers"
	sourceDef.SourceType = pkg.SourceTypeWesternKentuckyInHomeProviders
