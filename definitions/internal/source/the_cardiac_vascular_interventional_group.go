...
func GetSourceTheCardiacVascularInterventionalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheCardiacVascularInterventionalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "THE CARDIAC  VASCULAR INTERVENTIONAL GROUP"
	sourceDef.SourceType = pkg.SourceTypeTheCardiacVascularInterventionalGroup
