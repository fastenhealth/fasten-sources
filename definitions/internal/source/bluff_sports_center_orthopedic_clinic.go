...
func GetSourceBluffSportsCenterOrthopedicClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBluffSportsCenterOrthopedicClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bluff Sports Center  Orthopedic Clinic"
	sourceDef.SourceType = pkg.SourceTypeBluffSportsCenterOrthopedicClinic
