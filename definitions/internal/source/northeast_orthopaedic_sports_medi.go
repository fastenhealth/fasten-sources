...
func GetSourceNortheastOrthopaedicSportsMedi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNortheastOrthopaedicSportsMedi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northeast Orthopaedic  Sports Medi"
	sourceDef.SourceType = pkg.SourceTypeNortheastOrthopaedicSportsMedi
