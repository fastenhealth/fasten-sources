...
func GetSourceSouthFloridaGynecologyWomensHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthFloridaGynecologyWomensHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SOUTH FLORIDA GYNECOLOGY  WOMENS HEALTH"
	sourceDef.SourceType = pkg.SourceTypeSouthFloridaGynecologyWomensHealth
