...
func GetSourceStephensCountyHospital(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStephensCountyHospital]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Stephens County Hospital"
	sourceDef.SourceType = pkg.SourceTypeStephensCountyHospital
	sourceDef.Category = []string{"310400000X"}
	sourceDef.Aliases = []string{"STEPHENS COUNTY HOSPITAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1427224153", "1982870390"}}
