...
func GetSourceClarkFamilyPractice(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeClarkFamilyPractice]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Clark Family Practice"
	sourceDef.SourceType = pkg.SourceTypeClarkFamilyPractice
	sourceDef.Category = []string{"363L00000X"}
	sourceDef.Aliases = []string{"CLARK FAMILY PRACTICE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1528799186"}}
	sourceDef.PatientAccessUrl = "https://clarkfamilypracticellc.com/"
