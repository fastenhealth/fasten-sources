...
func GetSourceArthrexMedicalCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArthrexMedicalCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arthrex Medical Center"
	sourceDef.SourceType = pkg.SourceTypeArthrexMedicalCenter
	sourceDef.Category = []string{"261QC1800X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1750668059"}}
	sourceDef.PatientAccessUrl = "http://www.arthrexmedicalcenter.com"
