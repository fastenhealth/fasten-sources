...
func GetSourceDelawareValleyNephrologyAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDelawareValleyNephrologyAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Delaware Valley Nephrology Associates"
	sourceDef.SourceType = pkg.SourceTypeDelawareValleyNephrologyAssociates
