...
func GetSourceSportsOrthopedicAdvancedRehabili(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSportsOrthopedicAdvancedRehabili]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sports Orthopedic Advanced Rehabili"
	sourceDef.SourceType = pkg.SourceTypeSportsOrthopedicAdvancedRehabili
