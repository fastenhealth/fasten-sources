...
func GetSourcePerisseiaPrimaryCareAndWellness(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePerisseiaPrimaryCareAndWellness]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Perisseia Primary Care and Wellness"
	sourceDef.SourceType = pkg.SourceTypePerisseiaPrimaryCareAndWellness
