...
func GetSourceElevatePsychiatry(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeElevatePsychiatry]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Elevate Psychiatry"
	sourceDef.SourceType = pkg.SourceTypeElevatePsychiatry
	sourceDef.Category = []string{"2084P0800X"}
	sourceDef.Aliases = []string{"ELEVATE PSYCHIATRY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1902346513"}}
	sourceDef.PatientAccessUrl = "http://www.elevatepsychiatry.com"
