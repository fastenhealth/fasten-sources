...
func GetSourceSouthernTnLawrenceburgPhysicianServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernTnLawrenceburgPhysicianServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southern TN Lawrenceburg Physician Services"
	sourceDef.SourceType = pkg.SourceTypeSouthernTnLawrenceburgPhysicianServices
