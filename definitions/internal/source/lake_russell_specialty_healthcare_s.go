...
func GetSourceLakeRussellSpecialtyHealthcareS(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLakeRussellSpecialtyHealthcareS]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lake Russell Specialty Healthcare S"
	sourceDef.SourceType = pkg.SourceTypeLakeRussellSpecialtyHealthcareS
