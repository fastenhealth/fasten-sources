...
func GetSourceRheumatologyServicesOfFlorida(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRheumatologyServicesOfFlorida]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Rheumatology Services of Florida"
	sourceDef.SourceType = pkg.SourceTypeRheumatologyServicesOfFlorida
	sourceDef.Category = []string{"207RR0500X"}
	sourceDef.Aliases = []string{"RHEUMATOLOGY SERVICES OF FLORIDA"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1952633158"}}
