...
func GetSourceWatersMedicalClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWatersMedicalClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Waters Medical Clinic"
	sourceDef.SourceType = pkg.SourceTypeWatersMedicalClinic
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"WATERS MEDICAL CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1205580990", "1861146201"}}
