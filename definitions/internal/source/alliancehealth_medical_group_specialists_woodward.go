...
func GetSourceAlliancehealthMedicalGroupSpecialistsWoodward(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupSpecialistsWoodward]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Specialists  Woodward"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupSpecialistsWoodward
