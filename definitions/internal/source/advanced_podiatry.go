...
func GetSourceAdvancedPodiatry(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedPodiatry]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Advanced Podiatry"
	sourceDef.SourceType = pkg.SourceTypeAdvancedPodiatry
	sourceDef.Category = []string{"213ES0103X"}
	sourceDef.Aliases = []string{"ADVANCED PODIATRY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1477983906"}}
