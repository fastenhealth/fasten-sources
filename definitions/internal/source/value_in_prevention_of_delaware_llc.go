...
func GetSourceValueInPreventionOfDelawareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeValueInPreventionOfDelawareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Value in Prevention of Delaware LLC"
	sourceDef.SourceType = pkg.SourceTypeValueInPreventionOfDelawareLlc
