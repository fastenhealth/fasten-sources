...
func GetSourceHealingConnectionBehavioralHealt(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHealingConnectionBehavioralHealt]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Healing Connection Behavioral Healt"
	sourceDef.SourceType = pkg.SourceTypeHealingConnectionBehavioralHealt
