...
func GetSourceHealthAllianceForTheUninsured(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHealthAllianceForTheUninsured]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Health Alliance for the Uninsured"
	sourceDef.SourceType = pkg.SourceTypeHealthAllianceForTheUninsured
