...
func GetSourcePracticallyPerfectPhysicalTherapyConsultingLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePracticallyPerfectPhysicalTherapyConsultingLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PRACTICALLY PERFECT PHYSICAL THERAPY CONSULTING LLC"
	sourceDef.SourceType = pkg.SourceTypePracticallyPerfectPhysicalTherapyConsultingLlc
