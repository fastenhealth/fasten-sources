...
func GetSourceAllergyAsthmaImmunologyInstitu(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllergyAsthmaImmunologyInstitu]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Allergy Asthma  Immunology Institu"
	sourceDef.SourceType = pkg.SourceTypeAllergyAsthmaImmunologyInstitu
