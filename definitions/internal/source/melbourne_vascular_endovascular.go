...
func GetSourceMelbourneVascularEndovascular(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMelbourneVascularEndovascular]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MELBOURNE VASCULAR  ENDOVASCULAR"
	sourceDef.SourceType = pkg.SourceTypeMelbourneVascularEndovascular
