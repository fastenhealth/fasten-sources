...
func GetSourceTennovaOrthopedicsTurkeyCreek(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaOrthopedicsTurkeyCreek]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Orthopedics  Turkey Creek"
	sourceDef.SourceType = pkg.SourceTypeTennovaOrthopedicsTurkeyCreek
