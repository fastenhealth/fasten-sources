...
func GetSourceLanderCountyCommunityHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLanderCountyCommunityHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lander County Community Health"
	sourceDef.SourceType = pkg.SourceTypeLanderCountyCommunityHealth
	sourceDef.Category = []string{"261QC1500X", "261QF0050X"}
	sourceDef.Aliases = []string{"LANDER COUNTY COMMUNITY HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1447743703"}}
