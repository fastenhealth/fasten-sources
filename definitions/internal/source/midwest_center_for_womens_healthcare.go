...
func GetSourceMidwestCenterForWomensHealthcare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidwestCenterForWomensHealthcare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MIDWEST CENTER FOR WOMENS HEALTHCARE"
	sourceDef.SourceType = pkg.SourceTypeMidwestCenterForWomensHealthcare
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1821079518"}}
	sourceDef.PatientAccessUrl = "http://mcwhc.com"
