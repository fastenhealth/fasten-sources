...
func GetSourceCapeAndIslandsDigestiveDiseaseAssociatesPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCapeAndIslandsDigestiveDiseaseAssociatesPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CAPE AND ISLANDS DIGESTIVE DISEASE ASSOCIATES PC"
	sourceDef.SourceType = pkg.SourceTypeCapeAndIslandsDigestiveDiseaseAssociatesPc
