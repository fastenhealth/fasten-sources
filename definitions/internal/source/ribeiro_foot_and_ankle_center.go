...
func GetSourceRibeiroFootAndAnkleCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRibeiroFootAndAnkleCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ribeiro Foot and Ankle Center"
	sourceDef.SourceType = pkg.SourceTypeRibeiroFootAndAnkleCenter
	sourceDef.Category = []string{"213E00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1770990764"}}
	sourceDef.PatientAccessUrl = "http://www.ribeirofootandankle.com"
