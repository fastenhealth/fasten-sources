...
func GetSourceSolidFoundationPsychiatry(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSolidFoundationPsychiatry]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Solid Foundation Psychiatry"
	sourceDef.SourceType = pkg.SourceTypeSolidFoundationPsychiatry
	sourceDef.Category = []string{"363LP0808X"}
	sourceDef.Aliases = []string{"SOLID FOUNDATION PSYCHIATRY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1801535414"}}
