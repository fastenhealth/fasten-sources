...
func GetSourcePulaskiFreeClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePulaskiFreeClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pulaski Free Clinic"
	sourceDef.SourceType = pkg.SourceTypePulaskiFreeClinic
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"PULASKI FREE CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1225619117"}}
