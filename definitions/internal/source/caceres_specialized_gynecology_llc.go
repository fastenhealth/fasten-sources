...
func GetSourceCaceresSpecializedGynecologyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCaceresSpecializedGynecologyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Caceres Specialized Gynecology LLC"
	sourceDef.SourceType = pkg.SourceTypeCaceresSpecializedGynecologyLlc
