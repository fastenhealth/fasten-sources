...
func GetSourcePpgwni(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePpgwni]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PPGWNI"
	sourceDef.SourceType = pkg.SourceTypePpgwni
	sourceDef.Category = []string{"207VG0400X", "363LW0102X"}
	sourceDef.Aliases = []string{"PPGWNI"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1356596845", "1578946539"}}
