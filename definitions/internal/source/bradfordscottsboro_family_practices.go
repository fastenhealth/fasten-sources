...
func GetSourceBradfordscottsboroFamilyPractices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBradfordscottsboroFamilyPractices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BradfordScottsboro Family Practices"
	sourceDef.SourceType = pkg.SourceTypeBradfordscottsboroFamilyPractices
