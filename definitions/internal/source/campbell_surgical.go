...
func GetSourceCampbellSurgical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCampbellSurgical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Campbell Surgical"
	sourceDef.SourceType = pkg.SourceTypeCampbellSurgical
	sourceDef.Category = []string{"208600000X"}
	sourceDef.Aliases = []string{"CAMPBELL SURGICAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1790110146"}}
