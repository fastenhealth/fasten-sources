...
func GetSourceTheCenterForInternalAndIntegra(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheCenterForInternalAndIntegra]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Center for Internal and Integra"
	sourceDef.SourceType = pkg.SourceTypeTheCenterForInternalAndIntegra
