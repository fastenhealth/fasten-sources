...
func GetSourceChesapeakeRegionalMedicalSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChesapeakeRegionalMedicalSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Chesapeake Regional Medical Specialists"
	sourceDef.SourceType = pkg.SourceTypeChesapeakeRegionalMedicalSpecialists
