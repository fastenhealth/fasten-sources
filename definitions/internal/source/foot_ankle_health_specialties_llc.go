...
func GetSourceFootAnkleHealthSpecialtiesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFootAnkleHealthSpecialtiesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Foot  Ankle Health Specialties LLC"
	sourceDef.SourceType = pkg.SourceTypeFootAnkleHealthSpecialtiesLlc
