...
func GetSourceNeurologyPartnersProfessionalCor(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeurologyPartnersProfessionalCor]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEUROLOGY PARTNERS PROFESSIONAL COR"
	sourceDef.SourceType = pkg.SourceTypeNeurologyPartnersProfessionalCor
