...
func GetSourceNeDrNpPatelMdPcUrologyAssocia(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeDrNpPatelMdPcUrologyAssocia]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NE  Dr NP Patel MD PC  Urology Associa"
	sourceDef.SourceType = pkg.SourceTypeNeDrNpPatelMdPcUrologyAssocia
