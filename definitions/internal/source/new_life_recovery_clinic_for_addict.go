...
func GetSourceNewLifeRecoveryClinicForAddict(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNewLifeRecoveryClinicForAddict]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "New Life Recovery Clinic For Addict"
	sourceDef.SourceType = pkg.SourceTypeNewLifeRecoveryClinicForAddict
