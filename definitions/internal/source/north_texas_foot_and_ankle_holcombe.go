...
func GetSourceNorthTexasFootAndAnkleHolcombe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthTexasFootAndAnkleHolcombe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Texas Foot and Ankle Holcombe"
	sourceDef.SourceType = pkg.SourceTypeNorthTexasFootAndAnkleHolcombe
