...
func GetSourcePrecisionHealthcareSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrecisionHealthcareSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Precision Healthcare Specialists"
	sourceDef.SourceType = pkg.SourceTypePrecisionHealthcareSpecialists
	sourceDef.Category = []string{"207N00000X"}
	sourceDef.Aliases = []string{"PRECISION DERMATOLOGY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1417640046"}}
