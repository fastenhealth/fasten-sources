...
func GetSourceTedKovacevMd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTedKovacevMd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ted Kovacev MD"
	sourceDef.SourceType = pkg.SourceTypeTedKovacevMd
	sourceDef.Category = []string{"208600000X"}
	sourceDef.Aliases = []string{"TED KOVACEV MD"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1588215933"}}
	sourceDef.PatientAccessUrl = "http://www.LJSurgery.com"
