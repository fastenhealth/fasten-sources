...
func GetSourceHeartFailureSurvivalCenterOfAm(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHeartFailureSurvivalCenterOfAm]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Heart Failure Survival Center Of Am"
	sourceDef.SourceType = pkg.SourceTypeHeartFailureSurvivalCenterOfAm
