...
func GetSourceInfectiousDiseasesAssociatesOfB(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInfectiousDiseasesAssociatesOfB]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Infectious Diseases Associates Of B"
	sourceDef.SourceType = pkg.SourceTypeInfectiousDiseasesAssociatesOfB
