...
func GetSourceExpressClinicLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeExpressClinicLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "EXPRESS CLINIC LLC"
	sourceDef.SourceType = pkg.SourceTypeExpressClinicLlc
	sourceDef.Category = []string{"207R00000X", "261QP2300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1285220830"}}
	sourceDef.PatientAccessUrl = "http://www.expressclinicmd.com"
