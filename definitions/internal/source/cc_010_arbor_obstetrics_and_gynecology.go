...
func GetSourceCc010ArborObstetricsAndGynecology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc010ArborObstetricsAndGynecology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC010ARBOR OBSTETRICS AND GYNECOLOGY"
	sourceDef.SourceType = pkg.SourceTypeCc010ArborObstetricsAndGynecology
