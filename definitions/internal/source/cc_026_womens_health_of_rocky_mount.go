...
func GetSourceCc026WomensHealthOfRockyMount(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc026WomensHealthOfRockyMount]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC026WOMENS HEALTH OF ROCKY MOUNT"
	sourceDef.SourceType = pkg.SourceTypeCc026WomensHealthOfRockyMount
