...
func GetSourceSteddyHealthcareNpInAdultHealt(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSteddyHealthcareNpInAdultHealt]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Steddy Healthcare NP in Adult Healt"
	sourceDef.SourceType = pkg.SourceTypeSteddyHealthcareNpInAdultHealt
