...
func GetSourceLifestyleMedicineCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLifestyleMedicineCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lifestyle Medicine Center LLC"
	sourceDef.SourceType = pkg.SourceTypeLifestyleMedicineCenterLlc
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1578980736"}}
	sourceDef.PatientAccessUrl = "http://www.lifestylemedcenter.com"
