...
func GetSourceCharlestonSurgicalHospital(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCharlestonSurgicalHospital]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Charleston Surgical Hospital"
	sourceDef.SourceType = pkg.SourceTypeCharlestonSurgicalHospital
	sourceDef.Category = []string{"282N00000X"}
	sourceDef.Aliases = []string{"CHARLESTON SURGICAL HOSPITAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1669429965"}}
