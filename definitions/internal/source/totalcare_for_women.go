...
func GetSourceTotalcareForWomen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTotalcareForWomen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Totalcare for Women"
	sourceDef.SourceType = pkg.SourceTypeTotalcareForWomen
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{"TOTALCARE FOR WOMEN"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1568593952"}}
