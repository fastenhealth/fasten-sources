...
func GetSourcePediatricsMed3(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePediatricsMed3]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Pediatrics (Med 3)"
	sourceDef.SourceType = pkg.SourceTypePediatricsMed3
	sourceDef.PatientAccessUrl = "https://www.pediatricscostamesa.com/"
