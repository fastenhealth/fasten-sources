...
func GetSourceIllinoisSpineAndPainCenterSc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIllinoisSpineAndPainCenterSc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ILLINOIS SPINE AND PAIN CENTER SC"
	sourceDef.SourceType = pkg.SourceTypeIllinoisSpineAndPainCenterSc
	sourceDef.Category = []string{"207LP2900X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1801185616"}}
