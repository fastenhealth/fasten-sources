...
func GetSourceDrRandyFolkerEntAndAllergyPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDrRandyFolkerEntAndAllergyPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Dr Randy Folker ENT and Allergy PC"
	sourceDef.SourceType = pkg.SourceTypeDrRandyFolkerEntAndAllergyPc
