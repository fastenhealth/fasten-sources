...
func GetSourceMorelifeSolutionsMentalHealthL(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMorelifeSolutionsMentalHealthL]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MORELIFE SOLUTIONS MENTAL HEALTH L"
	sourceDef.SourceType = pkg.SourceTypeMorelifeSolutionsMentalHealthL
