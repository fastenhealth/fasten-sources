...
func GetSourceCarolinaOrthopaedicSurgeryAssociatesPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCarolinaOrthopaedicSurgeryAssociatesPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CAROLINA ORTHOPAEDIC SURGERY ASSOCIATES PA"
	sourceDef.SourceType = pkg.SourceTypeCarolinaOrthopaedicSurgeryAssociatesPa
