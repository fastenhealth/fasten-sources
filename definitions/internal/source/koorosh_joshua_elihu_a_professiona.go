...
func GetSourceKooroshJoshuaElihuAProfessiona(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKooroshJoshuaElihuAProfessiona]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "KOOROSH JOSHUA ELIHU A PROFESSIONA"
	sourceDef.SourceType = pkg.SourceTypeKooroshJoshuaElihuAProfessiona
