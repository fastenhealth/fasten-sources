...
func GetSourceTheHeartVascularCenterAtRiverRegion(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheHeartVascularCenterAtRiverRegion]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Heart  Vascular Center at River Region"
	sourceDef.SourceType = pkg.SourceTypeTheHeartVascularCenterAtRiverRegion
