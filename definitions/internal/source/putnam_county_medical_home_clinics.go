...
func GetSourcePutnamCountyMedicalHomeClinics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePutnamCountyMedicalHomeClinics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Putnam County Medical Home Clinics"
	sourceDef.SourceType = pkg.SourceTypePutnamCountyMedicalHomeClinics
