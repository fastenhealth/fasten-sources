...
func GetSourceAnesthesiaPainManagementDoctors(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAnesthesiaPainManagementDoctors]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Anesthesia Pain Management Doctors"
	sourceDef.SourceType = pkg.SourceTypeAnesthesiaPainManagementDoctors
