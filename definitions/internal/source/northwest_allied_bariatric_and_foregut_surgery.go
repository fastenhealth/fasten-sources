...
func GetSourceNorthwestAlliedBariatricAndForegutSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestAlliedBariatricAndForegutSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Allied Bariatric and Foregut Surgery"
	sourceDef.SourceType = pkg.SourceTypeNorthwestAlliedBariatricAndForegutSurgery
