...
func GetSourcePlateauClinicGreenbrierRiverEnt(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePlateauClinicGreenbrierRiverEnt]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Plateau Clinic Greenbrier River ENT"
	sourceDef.SourceType = pkg.SourceTypePlateauClinicGreenbrierRiverEnt
