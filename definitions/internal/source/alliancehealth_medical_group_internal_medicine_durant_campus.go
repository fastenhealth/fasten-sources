...
func GetSourceAlliancehealthMedicalGroupInternalMedicineDurantCampus(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupInternalMedicineDurantCampus]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Internal Medicine Durant Campus"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupInternalMedicineDurantCampus
