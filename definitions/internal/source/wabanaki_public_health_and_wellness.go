...
func GetSourceWabanakiPublicHealthAndWellness(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWabanakiPublicHealthAndWellness]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WABANAKI PUBLIC HEALTH AND WELLNESS"
	sourceDef.SourceType = pkg.SourceTypeWabanakiPublicHealthAndWellness
