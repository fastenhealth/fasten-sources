...
func GetSourceMetroOrthopedicsSportsTherapy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMetroOrthopedicsSportsTherapy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Metro Orthopedics  Sports Therapy"
	sourceDef.SourceType = pkg.SourceTypeMetroOrthopedicsSportsTherapy
