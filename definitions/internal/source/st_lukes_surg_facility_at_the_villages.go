...
func GetSourceStLukesSurgFacilityAtTheVillages(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStLukesSurgFacilityAtTheVillages]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "St Lukes Surg Facility at the Villages"
	sourceDef.SourceType = pkg.SourceTypeStLukesSurgFacilityAtTheVillages
