...
func GetSourceQuintinLawMdDbaExecutiveMedic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeQuintinLawMdDbaExecutiveMedic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Quintin Law MD DBA Executive Medic"
	sourceDef.SourceType = pkg.SourceTypeQuintinLawMdDbaExecutiveMedic
