...
func GetSourceDallasPodiatryWorks(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDallasPodiatryWorks]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Dallas Podiatry Works"
	sourceDef.SourceType = pkg.SourceTypeDallasPodiatryWorks
	sourceDef.Category = []string{"213E00000X"}
	sourceDef.Aliases = []string{"DALLAS PODIATRY WORKS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1578260972", "1831854983"}}
