...
func GetSourceTreasuredPediatricCareHilaryNic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTreasuredPediatricCareHilaryNic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Treasured Pediatric Care Hilary Nic"
	sourceDef.SourceType = pkg.SourceTypeTreasuredPediatricCareHilaryNic
