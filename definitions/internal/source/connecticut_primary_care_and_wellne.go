...
func GetSourceConnecticutPrimaryCareAndWellne(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeConnecticutPrimaryCareAndWellne]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Connecticut Primary Care and Wellne"
	sourceDef.SourceType = pkg.SourceTypeConnecticutPrimaryCareAndWellne
