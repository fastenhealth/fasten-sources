...
func GetSourceAmsurgCincinnatiAnesthesiaLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmsurgCincinnatiAnesthesiaLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "AMSURG Cincinnati Anesthesia LLC"
	sourceDef.SourceType = pkg.SourceTypeAmsurgCincinnatiAnesthesiaLlc
	sourceDef.Category = []string{"367500000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1053713743"}}
