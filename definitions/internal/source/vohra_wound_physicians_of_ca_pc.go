...
func GetSourceVohraWoundPhysiciansOfCaPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVohraWoundPhysiciansOfCaPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VOHRA WOUND PHYSICIANS OF CA PC"
	sourceDef.SourceType = pkg.SourceTypeVohraWoundPhysiciansOfCaPc
