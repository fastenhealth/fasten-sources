...
func GetSourceCenterForMinimallyInvasiveAndReconstructiveGynecology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForMinimallyInvasiveAndReconstructiveGynecology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center for Minimally Invasive and Reconstructive Gynecology"
	sourceDef.SourceType = pkg.SourceTypeCenterForMinimallyInvasiveAndReconstructiveGynecology
