...
func GetSourceCc017TheJonesCenterForWomensHealthLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc017TheJonesCenterForWomensHealthLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC017THE JONES CENTER FOR WOMENS HEALTH LLC"
	sourceDef.SourceType = pkg.SourceTypeCc017TheJonesCenterForWomensHealthLlc
