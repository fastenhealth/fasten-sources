...
func GetSourceRisePediatricsPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRisePediatricsPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RISE PEDIATRICS PLLC"
	sourceDef.SourceType = pkg.SourceTypeRisePediatricsPllc
	sourceDef.Category = []string{"363LP0200X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1235803727"}}
	sourceDef.PatientAccessUrl = "http://risepeds.com"
