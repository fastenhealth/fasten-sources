...
func GetSourceStatesvillePsychiatry(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStatesvillePsychiatry]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Statesville Psychiatry"
	sourceDef.SourceType = pkg.SourceTypeStatesvillePsychiatry
	sourceDef.Category = []string{"2084P0800X"}
	sourceDef.Aliases = []string{"STATESVILLE PSYCHIATRY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1538443973"}}
