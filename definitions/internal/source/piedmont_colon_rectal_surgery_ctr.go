...
func GetSourcePiedmontColonRectalSurgeryCtr(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePiedmontColonRectalSurgeryCtr]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Piedmont Colon  Rectal Surgery Ctr"
	sourceDef.SourceType = pkg.SourceTypePiedmontColonRectalSurgeryCtr
