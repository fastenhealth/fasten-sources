...
func GetSourceVanosPainManagementSolutionsPs(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVanosPainManagementSolutionsPs]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VANOS PAIN MANAGEMENT SOLUTIONS PS"
	sourceDef.SourceType = pkg.SourceTypeVanosPainManagementSolutionsPs
