...
func GetSourceMorenoVallyCommunityMedicalGrp(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMorenoVallyCommunityMedicalGrp]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Moreno Vally Community Medical Grp"
	sourceDef.SourceType = pkg.SourceTypeMorenoVallyCommunityMedicalGrp
