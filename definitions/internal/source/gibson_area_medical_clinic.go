...
func GetSourceGibsonAreaMedicalClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGibsonAreaMedicalClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Gibson Area Medical Clinic"
	sourceDef.SourceType = pkg.SourceTypeGibsonAreaMedicalClinic
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"GIBSON AREA MEDICAL CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1154702181"}}
