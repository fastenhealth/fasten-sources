...
func GetSourceNorthwestSeniorHealthAtSchmieding(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestSeniorHealthAtSchmieding]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Senior Health at Schmieding"
	sourceDef.SourceType = pkg.SourceTypeNorthwestSeniorHealthAtSchmieding
