...
func GetSourceFauquierMedicalAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFauquierMedicalAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "FAUQUIER MEDICAL ASSOCIATES"
	sourceDef.SourceType = pkg.SourceTypeFauquierMedicalAssociates
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1376601567"}}
	sourceDef.PatientAccessUrl = "https://www.fauquierhealth.org/"
