...
func GetSourcePediatricClinicOfNLittleRock(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePediatricClinicOfNLittleRock]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PEDIATRIC CLINIC OF N LITTLE ROCK"
	sourceDef.SourceType = pkg.SourceTypePediatricClinicOfNLittleRock
