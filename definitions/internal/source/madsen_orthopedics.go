...
func GetSourceMadsenOrthopedics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMadsenOrthopedics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Madsen Orthopedics"
	sourceDef.SourceType = pkg.SourceTypeMadsenOrthopedics
	sourceDef.Category = []string{"207X00000X"}
	sourceDef.Aliases = []string{"MADSEN ORTHOPEDICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1023712601"}}
