...
func GetSourceUltraHealthProvidersOfFlorida(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUltraHealthProvidersOfFlorida]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ultra Health Providers of Florida"
	sourceDef.SourceType = pkg.SourceTypeUltraHealthProvidersOfFlorida
	sourceDef.Category = []string{"207RG0300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1528549490"}}
	sourceDef.PatientAccessUrl = "https://www.ultrahealthproviders.com/"
