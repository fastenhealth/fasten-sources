...
func GetSourceMedicalCenterBarbour(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMedicalCenterBarbour]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Medical Center Barbour"
	sourceDef.SourceType = pkg.SourceTypeMedicalCenterBarbour
	sourceDef.Category = []string{"282N00000X"}
	sourceDef.Aliases = []string{"MEDICAL CENTER BARBOUR"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1730692641"}}
	sourceDef.PatientAccessUrl = "http://medctrbarbour.org"
