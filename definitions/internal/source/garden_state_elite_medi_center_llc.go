...
func GetSourceGardenStateEliteMediCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGardenStateEliteMediCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GARDEN STATE ELITE MEDI CENTER LLC"
	sourceDef.SourceType = pkg.SourceTypeGardenStateEliteMediCenterLlc
