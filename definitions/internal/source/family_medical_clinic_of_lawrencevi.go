...
func GetSourceFamilyMedicalClinicOfLawrencevi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFamilyMedicalClinicOfLawrencevi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Family Medical Clinic of Lawrencevi"
	sourceDef.SourceType = pkg.SourceTypeFamilyMedicalClinicOfLawrencevi
