...
func GetSourceSouthernWvMedicalAndSurgicalSp(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernWvMedicalAndSurgicalSp]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SOUTHERN WV MEDICAL AND SURGICAL SP"
	sourceDef.SourceType = pkg.SourceTypeSouthernWvMedicalAndSurgicalSp
