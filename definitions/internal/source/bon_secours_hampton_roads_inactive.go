...
func GetSourceBonSecoursHamptonRoadsInactive(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBonSecoursHamptonRoadsInactive]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bon Secours Hampton Roads Inactive"
	sourceDef.SourceType = pkg.SourceTypeBonSecoursHamptonRoadsInactive
