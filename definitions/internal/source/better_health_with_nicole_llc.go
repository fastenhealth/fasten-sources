...
func GetSourceBetterHealthWithNicoleLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBetterHealthWithNicoleLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Better Health with Nicole LLC"
	sourceDef.SourceType = pkg.SourceTypeBetterHealthWithNicoleLlc
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1568184174"}}
	sourceDef.PatientAccessUrl = "http://www.betterhealthnicole.com"
