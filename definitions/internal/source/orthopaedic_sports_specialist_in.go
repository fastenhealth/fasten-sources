...
func GetSourceOrthopaedicSportsSpecialistIn(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOrthopaedicSportsSpecialistIn]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Orthopaedic  Sports Specialist In"
	sourceDef.SourceType = pkg.SourceTypeOrthopaedicSportsSpecialistIn
