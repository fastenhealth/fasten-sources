...
func GetSourceNorthwestPrimaryCareHarBerMeadows(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestPrimaryCareHarBerMeadows]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Primary Care  Har Ber Meadows"
	sourceDef.SourceType = pkg.SourceTypeNorthwestPrimaryCareHarBerMeadows
