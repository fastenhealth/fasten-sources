...
func GetSourceNaturalRhythmsIntegrativeMedicin(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNaturalRhythmsIntegrativeMedicin]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Natural Rhythms Integrative Medicin"
	sourceDef.SourceType = pkg.SourceTypeNaturalRhythmsIntegrativeMedicin
