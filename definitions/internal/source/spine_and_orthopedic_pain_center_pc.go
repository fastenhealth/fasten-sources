...
func GetSourceSpineAndOrthopedicPainCenterPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSpineAndOrthopedicPainCenterPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Spine and Orthopedic Pain Center PC"
	sourceDef.SourceType = pkg.SourceTypeSpineAndOrthopedicPainCenterPc
