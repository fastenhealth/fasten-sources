...
func GetSourceWomensCenterforintegrativeHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWomensCenterforintegrativeHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Womens CenterforIntegrative Health"
	sourceDef.SourceType = pkg.SourceTypeWomensCenterforintegrativeHealth
