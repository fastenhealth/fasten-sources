...
func GetSourcePhlebotomyInkAlliedHealthcareSe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePhlebotomyInkAlliedHealthcareSe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Phlebotomy Ink Allied Healthcare Se"
	sourceDef.SourceType = pkg.SourceTypePhlebotomyInkAlliedHealthcareSe
