...
func GetSourceNorthOkaloosaMedicalCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthOkaloosaMedicalCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Okaloosa Medical Center"
	sourceDef.SourceType = pkg.SourceTypeNorthOkaloosaMedicalCenter
	sourceDef.Category = []string{"275N00000X", "282N00000X"}
	sourceDef.Aliases = []string{"NORTH OKALOOSA MEDICAL CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1104897859", "1457864357"}}
