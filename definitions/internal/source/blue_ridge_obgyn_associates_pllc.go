...
func GetSourceBlueRidgeObgynAssociatesPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBlueRidgeObgynAssociatesPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Blue Ridge OBGYN Associates PLLC"
	sourceDef.SourceType = pkg.SourceTypeBlueRidgeObgynAssociatesPllc
