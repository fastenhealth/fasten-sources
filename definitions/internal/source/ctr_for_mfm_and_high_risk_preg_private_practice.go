...
func GetSourceCtrForMfmAndHighRiskPregPrivatePractice(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCtrForMfmAndHighRiskPregPrivatePractice]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CTR FOR MFM AND HIGH RISK PREG PRIVATE PRACTICE"
	sourceDef.SourceType = pkg.SourceTypeCtrForMfmAndHighRiskPregPrivatePractice
