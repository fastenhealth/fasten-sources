...
func GetSourceCoalCreekFamilyMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCoalCreekFamilyMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Coal Creek Family Medicine"
	sourceDef.SourceType = pkg.SourceTypeCoalCreekFamilyMedicine
	sourceDef.Category = []string{"174400000X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1942407622"}}
	sourceDef.BrandLogo = "coal-creek-family-medicine.jpg"
	sourceDef.PatientAccessUrl = "https://coalcreekfamilymedicine.com/"
