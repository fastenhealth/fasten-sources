...
func GetSourceJellicoWellnessConvenientCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJellicoWellnessConvenientCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Jellico Wellness  Convenient Care"
	sourceDef.SourceType = pkg.SourceTypeJellicoWellnessConvenientCare
