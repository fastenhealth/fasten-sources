...
func GetSourceInterventionalSpineSportsInsti(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInterventionalSpineSportsInsti]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Interventional Spine  Sports Insti"
	sourceDef.SourceType = pkg.SourceTypeInterventionalSpineSportsInsti
