...
func GetSourceAndrewJHutchinsonDoPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAndrewJHutchinsonDoPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Andrew J Hutchinson DO PA"
	sourceDef.SourceType = pkg.SourceTypeAndrewJHutchinsonDoPa
	sourceDef.Category = []string{"2086S0129X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1912054974"}}
