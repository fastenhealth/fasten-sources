...
func GetSourceAffiliatedEndoscopyCentersLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAffiliatedEndoscopyCentersLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Affiliated Endoscopy Centers LLC"
	sourceDef.SourceType = pkg.SourceTypeAffiliatedEndoscopyCentersLlc
