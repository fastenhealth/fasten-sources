...
func GetSourceAlpenglowGynecology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlpenglowGynecology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Alpenglow Gynecology"
	sourceDef.SourceType = pkg.SourceTypeAlpenglowGynecology
	sourceDef.Category = []string{"207VG0400X"}
	sourceDef.Aliases = []string{"ALPENGLOW GYNECOLOGY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1215440920"}}
