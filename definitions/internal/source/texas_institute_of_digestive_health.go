...
func GetSourceTexasInstituteOfDigestiveHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexasInstituteOfDigestiveHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Texas Institute of Digestive Health"
	sourceDef.SourceType = pkg.SourceTypeTexasInstituteOfDigestiveHealth
