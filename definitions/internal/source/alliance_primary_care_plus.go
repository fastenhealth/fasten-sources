...
func GetSourceAlliancePrimaryCarePlus(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancePrimaryCarePlus]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Alliance Primary Care Plus"
	sourceDef.SourceType = pkg.SourceTypeAlliancePrimaryCarePlus
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1265055081"}}
	sourceDef.PatientAccessUrl = "http://www.allianceprimarycareplus.com"
