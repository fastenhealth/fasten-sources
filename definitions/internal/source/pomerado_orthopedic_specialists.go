...
func GetSourcePomeradoOrthopedicSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePomeradoOrthopedicSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pomerado Orthopedic Specialists"
	sourceDef.SourceType = pkg.SourceTypePomeradoOrthopedicSpecialists
