...
func GetSourceOsceolaCountyCouncilOnAging(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOsceolaCountyCouncilOnAging]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Osceola County Council On Aging"
	sourceDef.SourceType = pkg.SourceTypeOsceolaCountyCouncilOnAging
	sourceDef.Category = []string{"251B00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1790800142"}}
