...
func GetSourceMckennaRuggeroliAndHelmiPainS(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMckennaRuggeroliAndHelmiPainS]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "McKenna Ruggeroli and Helmi Pain S"
	sourceDef.SourceType = pkg.SourceTypeMckennaRuggeroliAndHelmiPainS
