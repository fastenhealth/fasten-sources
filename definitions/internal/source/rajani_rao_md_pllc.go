...
func GetSourceRajaniRaoMdPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRajaniRaoMdPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RAJANI RAO MD PLLC"
	sourceDef.SourceType = pkg.SourceTypeRajaniRaoMdPllc
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{"VITTAL CARE4U"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1194293795"}}
