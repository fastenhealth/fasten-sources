...
func GetSourceSkinWellnessDermatologyAssociate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSkinWellnessDermatologyAssociate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Skin Wellness Dermatology Associate"
	sourceDef.SourceType = pkg.SourceTypeSkinWellnessDermatologyAssociate
