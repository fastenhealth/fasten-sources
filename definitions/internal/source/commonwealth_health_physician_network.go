...
func GetSourceCommonwealthHealthPhysicianNetwork(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCommonwealthHealthPhysicianNetwork]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Commonwealth Health Physician Network"
	sourceDef.SourceType = pkg.SourceTypeCommonwealthHealthPhysicianNetwork
