...
func GetSourcePiedmontRheumatologyConsultants(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePiedmontRheumatologyConsultants]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Piedmont Rheumatology Consultants"
	sourceDef.SourceType = pkg.SourceTypePiedmontRheumatologyConsultants
