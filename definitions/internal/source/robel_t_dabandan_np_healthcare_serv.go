...
func GetSourceRobelTDabandanNpHealthcareServ(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRobelTDabandanNpHealthcareServ]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Robel T Dabandan NP Healthcare Serv"
	sourceDef.SourceType = pkg.SourceTypeRobelTDabandanNpHealthcareServ
