...
func GetSourceMagnoliaBarkWoundCareSolutions(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMagnoliaBarkWoundCareSolutions]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MAGNOLIA BARK WOUND CARE SOLUTIONS"
	sourceDef.SourceType = pkg.SourceTypeMagnoliaBarkWoundCareSolutions
