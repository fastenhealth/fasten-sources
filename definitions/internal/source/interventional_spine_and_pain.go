...
func GetSourceInterventionalSpineAndPain(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInterventionalSpineAndPain]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "INTERVENTIONAL SPINE AND PAIN"
	sourceDef.SourceType = pkg.SourceTypeInterventionalSpineAndPain
