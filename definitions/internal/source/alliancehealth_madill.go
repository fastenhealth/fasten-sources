...
func GetSourceAlliancehealthMadill(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMadill]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Madill"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMadill
	sourceDef.Category = []string{"275N00000X", "282NC0060X"}
	sourceDef.Aliases = []string{"ALLIANCEHEALTH MADILL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1053414037", "1467476556"}}
