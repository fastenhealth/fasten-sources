...
func GetSourcePerryCountyMemorialHospital(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePerryCountyMemorialHospital]; clientIdOk {
	...
	sourceDef.Category = []string{"282NC0060X", "3416L0300X"}
	sourceDef.Aliases = []string{"PERRY COUNTY MEMORIAL HOSPITAL", "PERRY COUNTY MEMORIAL HOSPITAL AMBULANCE SERVICE LLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1245360213", "1407158678", "1982699328"}}
