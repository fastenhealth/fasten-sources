...
func GetSourceBreckinridgeHealthPodiatryClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBreckinridgeHealthPodiatryClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BRECKINRIDGE HEALTH PODIATRY CLINIC"
	sourceDef.SourceType = pkg.SourceTypeBreckinridgeHealthPodiatryClinic
