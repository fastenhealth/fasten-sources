...
func GetSourceHighRiskPregnancyCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHighRiskPregnancyCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HIGH RISK PREGNANCY CENTER"
	sourceDef.SourceType = pkg.SourceTypeHighRiskPregnancyCenter
	sourceDef.Category = []string{"207VM0101X"}
	sourceDef.Aliases = []string{"HIGH RISK PREGNANCY CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1467496240", "1992351316"}}
