...
func GetSourceWashingtonSurgicalSpecialistLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWashingtonSurgicalSpecialistLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Washington Surgical Specialist LLC"
	sourceDef.SourceType = pkg.SourceTypeWashingtonSurgicalSpecialistLlc
