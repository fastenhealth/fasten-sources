...
func GetSourcePatelAnesthesiaAndPainSpecialis(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePatelAnesthesiaAndPainSpecialis]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PATEL ANESTHESIA AND PAIN SPECIALIS"
	sourceDef.SourceType = pkg.SourceTypePatelAnesthesiaAndPainSpecialis
