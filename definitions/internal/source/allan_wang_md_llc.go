...
func GetSourceAllanWangMdLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllanWangMdLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Allan Wang MD LLC"
	sourceDef.SourceType = pkg.SourceTypeAllanWangMdLlc
	sourceDef.Category = []string{"207K00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1700055217"}}
	sourceDef.PatientAccessUrl = "http://www.HIAllergy.com"
