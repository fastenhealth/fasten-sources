...
func GetSourcePediatricGastroenterologyOfCharl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePediatricGastroenterologyOfCharl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PEDIATRIC GASTROENTEROLOGY OF CHARL"
	sourceDef.SourceType = pkg.SourceTypePediatricGastroenterologyOfCharl
