...
func GetSourceVaCentraHealthAddictionMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVaCentraHealthAddictionMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VA  Centra Health  Addiction Medicine"
	sourceDef.SourceType = pkg.SourceTypeVaCentraHealthAddictionMedicine
