...
func GetSourceObstetricsGynecologyAssociatesOfNMia(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeObstetricsGynecologyAssociatesOfNMia]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "OBSTETRICS  GYNECOLOGY ASSOCIATES OF N MIA"
	sourceDef.SourceType = pkg.SourceTypeObstetricsGynecologyAssociatesOfNMia
