...
func GetSourceNebraskaPainManagementPhysicians(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNebraskaPainManagementPhysicians]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEBRASKA PAIN MANAGEMENT PHYSICIANS"
	sourceDef.SourceType = pkg.SourceTypeNebraskaPainManagementPhysicians
