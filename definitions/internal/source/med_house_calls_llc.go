...
func GetSourceMedHouseCallsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMedHouseCallsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MED House Calls LLC"
	sourceDef.SourceType = pkg.SourceTypeMedHouseCallsLlc
	sourceDef.Category = []string{"363LA2200X", "363LF0000X"}
	sourceDef.Aliases = []string{"MOBILE WOUND CARE SERVICES OF LOUISIANA"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1619218757"}}
