...
func GetSourceMusicCityPhysiciansGroupPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMusicCityPhysiciansGroupPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Music City Physicians Group PLLC"
	sourceDef.SourceType = pkg.SourceTypeMusicCityPhysiciansGroupPllc
	sourceDef.Category = []string{"261QH0100X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1740915719"}}
