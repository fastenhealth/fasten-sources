...
func GetSourceCultivateMentalHealthPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCultivateMentalHealthPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cultivate Mental Health PC"
	sourceDef.SourceType = pkg.SourceTypeCultivateMentalHealthPc
	sourceDef.Category = []string{"363LP0808X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1023691490"}}
	sourceDef.PatientAccessUrl = "http://www.cultivatemhpc.com"
