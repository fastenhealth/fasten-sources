...
func GetSourceColumbiaGorgeNeurologySpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeColumbiaGorgeNeurologySpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Columbia Gorge Neurology Specialists"
	sourceDef.SourceType = pkg.SourceTypeColumbiaGorgeNeurologySpecialists
