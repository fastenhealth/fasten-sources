...
func GetSourceBrevardHealthGroupLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBrevardHealthGroupLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BREVARD HEALTH GROUP LLC"
	sourceDef.SourceType = pkg.SourceTypeBrevardHealthGroupLlc
	sourceDef.Category = []string{"208VP0014X"}
	sourceDef.Aliases = []string{"BREVARD SPINE AND REHAB INSTITUTE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1346723244"}}
