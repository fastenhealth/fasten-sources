...
func GetSourceNunezMedicalServicesPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNunezMedicalServicesPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NUNEZ MEDICAL SERVICES PC"
	sourceDef.SourceType = pkg.SourceTypeNunezMedicalServicesPc
