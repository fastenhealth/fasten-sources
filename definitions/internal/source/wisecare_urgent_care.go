...
func GetSourceWisecareUrgentCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWisecareUrgentCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WiseCare Urgent Care"
	sourceDef.SourceType = pkg.SourceTypeWisecareUrgentCare
	sourceDef.Category = []string{"261QU0200X"}
	sourceDef.Aliases = []string{"WISECARE URGENT CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1255868519"}}
