...
func GetSourceDecaturLittleEaglePediatricsP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDecaturLittleEaglePediatricsP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DECATUR LITTLE EAGLE PEDIATRICS P"
	sourceDef.SourceType = pkg.SourceTypeDecaturLittleEaglePediatricsP
