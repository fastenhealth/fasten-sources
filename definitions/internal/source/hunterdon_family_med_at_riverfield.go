...
func GetSourceHunterdonFamilyMedAtRiverfield(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHunterdonFamilyMedAtRiverfield]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Hunterdon Family Med At Riverfield"
	sourceDef.SourceType = pkg.SourceTypeHunterdonFamilyMedAtRiverfield
