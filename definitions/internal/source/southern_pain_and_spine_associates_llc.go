...
func GetSourceSouthernPainAndSpineAssociatesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernPainAndSpineAssociatesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SOUTHERN PAIN AND SPINE ASSOCIATES LLC"
	sourceDef.SourceType = pkg.SourceTypeSouthernPainAndSpineAssociatesLlc
