...
func GetSourceGeorgeannaJHuangMdProfessionalCorporation(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGeorgeannaJHuangMdProfessionalCorporation]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GEORGEANNA J HUANG MD PROFESSIONAL CORPORATION"
	sourceDef.SourceType = pkg.SourceTypeGeorgeannaJHuangMdProfessionalCorporation
	sourceDef.Category = []string{"208200000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1740385137"}}
