...
func GetSourceNevadaKidneyDiseaseHypertensionCentersLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNevadaKidneyDiseaseHypertensionCentersLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Nevada Kidney Disease  Hypertension Centers LLC"
	sourceDef.SourceType = pkg.SourceTypeNevadaKidneyDiseaseHypertensionCentersLlc
