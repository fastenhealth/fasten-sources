...
func GetSourceKentuckianaCardiovascularSpeciali(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKentuckianaCardiovascularSpeciali]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kentuckiana Cardiovascular Speciali"
	sourceDef.SourceType = pkg.SourceTypeKentuckianaCardiovascularSpeciali
