...
func GetSourceNorthwestPrimaryCareBentonville(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestPrimaryCareBentonville]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Primary Care  Bentonville"
	sourceDef.SourceType = pkg.SourceTypeNorthwestPrimaryCareBentonville
