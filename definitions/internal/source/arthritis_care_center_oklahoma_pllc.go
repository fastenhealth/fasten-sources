...
func GetSourceArthritisCareCenterOklahomaPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArthritisCareCenterOklahomaPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Arthritis Care Center Oklahoma PLLC"
	sourceDef.SourceType = pkg.SourceTypeArthritisCareCenterOklahomaPllc
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1669626263"}}
	sourceDef.PatientAccessUrl = "https://okarthritis.com/"
