...
func GetSourceReliantFootAnkleSpecialistPll(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeReliantFootAnkleSpecialistPll]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RELIANT FOOT  ANKLE SPECIALIST PLL"
	sourceDef.SourceType = pkg.SourceTypeReliantFootAnkleSpecialistPll
