...
func GetSourceAngelsMedicalLlcDbaCamdenSpine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAngelsMedicalLlcDbaCamdenSpine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Angels Medical LLC dba Camden Spine"
	sourceDef.SourceType = pkg.SourceTypeAngelsMedicalLlcDbaCamdenSpine
