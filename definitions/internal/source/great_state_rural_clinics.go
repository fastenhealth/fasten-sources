...
func GetSourceGreatStateRuralClinics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGreatStateRuralClinics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Great State Rural Clinics"
	sourceDef.SourceType = pkg.SourceTypeGreatStateRuralClinics
	sourceDef.Category = []string{"207N00000X"}
	sourceDef.Aliases = []string{"GREAT STATE ALABAMA", "GREAT STATE RURAL CLINICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1124757588"}}
