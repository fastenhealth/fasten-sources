...
func GetSourceUpperlineHealthFloridaVeinCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUpperlineHealthFloridaVeinCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Upperline Health Florida Vein Center"
	sourceDef.SourceType = pkg.SourceTypeUpperlineHealthFloridaVeinCenter
