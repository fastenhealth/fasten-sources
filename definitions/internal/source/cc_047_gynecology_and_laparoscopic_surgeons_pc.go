...
func GetSourceCc047GynecologyAndLaparoscopicSurgeonsPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc047GynecologyAndLaparoscopicSurgeonsPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC047GYNECOLOGY AND LAPAROSCOPIC SURGEONS PC"
	sourceDef.SourceType = pkg.SourceTypeCc047GynecologyAndLaparoscopicSurgeonsPc
