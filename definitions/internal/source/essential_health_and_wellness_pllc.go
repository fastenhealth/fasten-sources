...
func GetSourceEssentialHealthAndWellnessPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEssentialHealthAndWellnessPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ESSENTIAL HEALTH AND WELLNESS PLLC"
	sourceDef.SourceType = pkg.SourceTypeEssentialHealthAndWellnessPllc
