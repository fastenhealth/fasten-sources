...
func GetSourceElevatePhysicalMedicineAndRehab(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeElevatePhysicalMedicineAndRehab]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Elevate Physical Medicine and Rehab"
	sourceDef.SourceType = pkg.SourceTypeElevatePhysicalMedicineAndRehab
