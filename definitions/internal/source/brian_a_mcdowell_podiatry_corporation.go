...
func GetSourceBrianAMcdowellPodiatryCorporation(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBrianAMcdowellPodiatryCorporation]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Brian A McDowell Podiatry Corporation"
	sourceDef.SourceType = pkg.SourceTypeBrianAMcdowellPodiatryCorporation
