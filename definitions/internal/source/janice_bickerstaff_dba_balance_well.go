...
func GetSourceJaniceBickerstaffDbaBalanceWell(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJaniceBickerstaffDbaBalanceWell]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Janice Bickerstaff DBA Balance Well"
	sourceDef.SourceType = pkg.SourceTypeJaniceBickerstaffDbaBalanceWell
