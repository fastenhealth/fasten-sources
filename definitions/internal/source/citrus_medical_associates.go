...
func GetSourceCitrusMedicalAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCitrusMedicalAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Citrus Medical Associates"
	sourceDef.SourceType = pkg.SourceTypeCitrusMedicalAssociates
	sourceDef.Category = []string{"207R00000X", "261QP2300X"}
	sourceDef.Aliases = []string{"CITRUS MEDICAL ASSOCIATES"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1417423344", "1659049732"}}
