...
func GetSourceMyndspaceMentalHealthEducation(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMyndspaceMentalHealthEducation]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MyNDSpace Mental Health Education "
	sourceDef.SourceType = pkg.SourceTypeMyndspaceMentalHealthEducation
