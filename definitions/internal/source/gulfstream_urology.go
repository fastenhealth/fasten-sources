...
func GetSourceGulfstreamUrology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGulfstreamUrology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gulfstream Urology"
	sourceDef.SourceType = pkg.SourceTypeGulfstreamUrology
	sourceDef.Category = []string{"208800000X"}
	sourceDef.Aliases = []string{"GULFSTREAM UROLOGY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1518105394"}}
