...
func GetSourceBayfrontOrthopaedicAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBayfrontOrthopaedicAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bayfront Orthopaedic Associates"
	sourceDef.SourceType = pkg.SourceTypeBayfrontOrthopaedicAssociates
