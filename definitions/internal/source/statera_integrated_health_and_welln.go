...
func GetSourceStateraIntegratedHealthAndWelln(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStateraIntegratedHealthAndWelln]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Statera Integrated Health and Welln"
	sourceDef.SourceType = pkg.SourceTypeStateraIntegratedHealthAndWelln
