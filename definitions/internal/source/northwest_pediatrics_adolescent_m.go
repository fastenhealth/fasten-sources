...
func GetSourceNorthwestPediatricsAdolescentM(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestPediatricsAdolescentM]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Pediatrics  Adolescent M"
	sourceDef.SourceType = pkg.SourceTypeNorthwestPediatricsAdolescentM
