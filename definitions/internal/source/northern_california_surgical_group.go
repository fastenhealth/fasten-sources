...
func GetSourceNorthernCaliforniaSurgicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthernCaliforniaSurgicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTHERN CALIFORNIA SURGICAL GROUP"
	sourceDef.SourceType = pkg.SourceTypeNorthernCaliforniaSurgicalGroup
