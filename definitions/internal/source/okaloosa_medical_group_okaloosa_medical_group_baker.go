...
func GetSourceOkaloosaMedicalGroupOkaloosaMedicalGroupBaker(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOkaloosaMedicalGroupOkaloosaMedicalGroupBaker]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Okaloosa Medical Group Okaloosa Medical Group  Baker"
	sourceDef.SourceType = pkg.SourceTypeOkaloosaMedicalGroupOkaloosaMedicalGroupBaker
