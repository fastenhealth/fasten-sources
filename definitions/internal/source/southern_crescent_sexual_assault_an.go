...
func GetSourceSouthernCrescentSexualAssaultAn(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernCrescentSexualAssaultAn]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southern Crescent Sexual Assault an"
	sourceDef.SourceType = pkg.SourceTypeSouthernCrescentSexualAssaultAn
