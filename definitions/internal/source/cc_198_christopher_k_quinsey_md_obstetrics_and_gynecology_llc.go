...
func GetSourceCc198ChristopherKQuinseyMdObstetricsAndGynecologyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc198ChristopherKQuinseyMdObstetricsAndGynecologyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC198CHRISTOPHER K QUINSEY MD OBSTETRICS AND GYNECOLOGY LLC"
	sourceDef.SourceType = pkg.SourceTypeCc198ChristopherKQuinseyMdObstetricsAndGynecologyLlc
