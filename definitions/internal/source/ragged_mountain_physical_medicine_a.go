...
func GetSourceRaggedMountainPhysicalMedicineA(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRaggedMountainPhysicalMedicineA]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ragged Mountain Physical Medicine a"
	sourceDef.SourceType = pkg.SourceTypeRaggedMountainPhysicalMedicineA
