...
func GetSourceEauClaireCooperativeHealthCente(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEauClaireCooperativeHealthCente]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Eau Claire Cooperative Health Cente"
	sourceDef.SourceType = pkg.SourceTypeEauClaireCooperativeHealthCente
