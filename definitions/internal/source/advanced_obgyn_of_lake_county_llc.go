...
func GetSourceAdvancedObgynOfLakeCountyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedObgynOfLakeCountyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ADVANCED OBGYN OF LAKE COUNTY LLC"
	sourceDef.SourceType = pkg.SourceTypeAdvancedObgynOfLakeCountyLlc
