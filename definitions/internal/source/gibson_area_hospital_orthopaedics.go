...
func GetSourceGibsonAreaHospitalOrthopaedics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGibsonAreaHospitalOrthopaedics]; clientIdOk {
	...
	sourceDef.Aliases = []string{"GIBSON AREA HOSPITAL ORTHOPAEDICS", "GIBSON AREA HOSPITAL ORTHOPAEDICS CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1578799797"}}
