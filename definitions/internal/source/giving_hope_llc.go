...
func GetSourceGivingHopeLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGivingHopeLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Giving Hope LLC"
	sourceDef.SourceType = pkg.SourceTypeGivingHopeLlc
	sourceDef.Category = []string{"207LP2900X"}
	sourceDef.Aliases = []string{"ALPENGLOW PAIN & WELLNESS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1508321639"}}
	sourceDef.PatientAccessUrl = "http://www.AlpenglowPain.com"
