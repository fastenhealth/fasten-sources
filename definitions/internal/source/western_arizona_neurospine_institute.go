...
func GetSourceWesternArizonaNeurospineInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWesternArizonaNeurospineInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Western Arizona NeuroSpine Institute"
	sourceDef.SourceType = pkg.SourceTypeWesternArizonaNeurospineInstitute
