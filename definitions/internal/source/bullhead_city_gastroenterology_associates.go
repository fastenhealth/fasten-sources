...
func GetSourceBullheadCityGastroenterologyAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBullheadCityGastroenterologyAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bullhead City Gastroenterology Associates"
	sourceDef.SourceType = pkg.SourceTypeBullheadCityGastroenterologyAssociates
