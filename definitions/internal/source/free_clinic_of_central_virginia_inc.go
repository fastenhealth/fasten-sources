...
func GetSourceFreeClinicOfCentralVirginiaInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFreeClinicOfCentralVirginiaInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FREE CLINIC OF CENTRAL VIRGINIA INC"
	sourceDef.SourceType = pkg.SourceTypeFreeClinicOfCentralVirginiaInc
