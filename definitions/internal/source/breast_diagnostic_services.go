...
func GetSourceBreastDiagnosticServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBreastDiagnosticServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BREAST DIAGNOSTIC SERVICES"
	sourceDef.SourceType = pkg.SourceTypeBreastDiagnosticServices
