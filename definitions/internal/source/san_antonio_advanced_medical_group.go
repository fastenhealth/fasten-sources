...
func GetSourceSanAntonioAdvancedMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSanAntonioAdvancedMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "San Antonio Advanced Medical Group"
	sourceDef.SourceType = pkg.SourceTypeSanAntonioAdvancedMedicalGroup
