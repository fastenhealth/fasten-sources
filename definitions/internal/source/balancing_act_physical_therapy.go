...
func GetSourceBalancingActPhysicalTherapy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBalancingActPhysicalTherapy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Balancing Act Physical Therapy"
	sourceDef.SourceType = pkg.SourceTypeBalancingActPhysicalTherapy
	sourceDef.Category = []string{"261QP2000X"}
	sourceDef.Aliases = []string{"BALANCING ACT PHYSICAL THERAPY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1982946950"}}
