...
func GetSourceTheEndoscopyCenterOfSantaFe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheEndoscopyCenterOfSantaFe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "The Endoscopy Center Of Santa Fe"
	sourceDef.SourceType = pkg.SourceTypeTheEndoscopyCenterOfSantaFe
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"THE ENDOSCOPY CENTER OF SANTA FE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1548249675"}}
