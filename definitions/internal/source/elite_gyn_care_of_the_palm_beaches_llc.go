...
func GetSourceEliteGynCareOfThePalmBeachesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEliteGynCareOfThePalmBeachesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ELITE GYN CARE OF THE PALM BEACHES LLC"
	sourceDef.SourceType = pkg.SourceTypeEliteGynCareOfThePalmBeachesLlc
