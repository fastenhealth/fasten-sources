...
func GetSourceManateeGynecologyAssociatesLlcTermed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeManateeGynecologyAssociatesLlcTermed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MANATEE GYNECOLOGY ASSOCIATES LLC TERMED"
	sourceDef.SourceType = pkg.SourceTypeManateeGynecologyAssociatesLlcTermed
