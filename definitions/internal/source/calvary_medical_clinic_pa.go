...
func GetSourceCalvaryMedicalClinicPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCalvaryMedicalClinicPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Calvary Medical Clinic PA"
	sourceDef.SourceType = pkg.SourceTypeCalvaryMedicalClinicPa
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1235303264"}}
	sourceDef.PatientAccessUrl = "http://calvarymedicalclinic.com"
