...
func GetSourceMediproOrthopedicSpineCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMediproOrthopedicSpineCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MediPro Orthopedic  Spine Center"
	sourceDef.SourceType = pkg.SourceTypeMediproOrthopedicSpineCenter
