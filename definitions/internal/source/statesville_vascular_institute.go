...
func GetSourceStatesvilleVascularInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStatesvilleVascularInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Statesville Vascular Institute"
	sourceDef.SourceType = pkg.SourceTypeStatesvilleVascularInstitute
	sourceDef.Category = []string{"208600000X", "2086S0129X"}
	sourceDef.Aliases = []string{"STATESVILLE VASCULAR INSTITUTE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1184060717"}}
