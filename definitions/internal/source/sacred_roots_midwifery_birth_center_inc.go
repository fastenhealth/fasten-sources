...
func GetSourceSacredRootsMidwiferyBirthCenterInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSacredRootsMidwiferyBirthCenterInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SACRED ROOTS MIDWIFERY BIRTH CENTER INC"
	sourceDef.SourceType = pkg.SourceTypeSacredRootsMidwiferyBirthCenterInc
