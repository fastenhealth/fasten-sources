...
func GetSourceArizonaColorectalExpertsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArizonaColorectalExpertsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arizona Colorectal Experts LLC"
	sourceDef.SourceType = pkg.SourceTypeArizonaColorectalExpertsLlc
	sourceDef.Category = []string{"208C00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1720744543"}}
	sourceDef.PatientAccessUrl = "http://www.aceclinic.org"
