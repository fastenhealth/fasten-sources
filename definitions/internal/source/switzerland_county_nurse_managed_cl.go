...
func GetSourceSwitzerlandCountyNurseManagedCl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSwitzerlandCountyNurseManagedCl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Switzerland County Nurse Managed Cl"
	sourceDef.SourceType = pkg.SourceTypeSwitzerlandCountyNurseManagedCl
