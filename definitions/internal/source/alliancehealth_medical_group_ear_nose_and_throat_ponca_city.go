...
func GetSourceAlliancehealthMedicalGroupEarNoseAndThroatPoncaCity(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupEarNoseAndThroatPoncaCity]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Ear Nose and Throat Ponca City"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupEarNoseAndThroatPoncaCity
