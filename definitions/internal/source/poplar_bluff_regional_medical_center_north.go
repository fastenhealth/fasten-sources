...
func GetSourcePoplarBluffRegionalMedicalCenterNorth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePoplarBluffRegionalMedicalCenterNorth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Poplar Bluff Regional Medical Center  North"
	sourceDef.SourceType = pkg.SourceTypePoplarBluffRegionalMedicalCenterNorth
