...
func GetSourceCc006HawthorneObgynAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc006HawthorneObgynAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC006Hawthorne OBGYN Associates"
	sourceDef.SourceType = pkg.SourceTypeCc006HawthorneObgynAssociates
