...
func GetSourceCentralValleyHealthCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralValleyHealthCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Central Valley Health Care"
	sourceDef.SourceType = pkg.SourceTypeCentralValleyHealthCare
	sourceDef.PatientAccessUrl = "http://cvhnclinics.org/"
