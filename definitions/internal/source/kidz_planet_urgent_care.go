...
func GetSourceKidzPlanetUrgentCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKidzPlanetUrgentCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kidz Planet Urgent Care"
	sourceDef.SourceType = pkg.SourceTypeKidzPlanetUrgentCare
	sourceDef.Category = []string{"261QU0200X"}
	sourceDef.Aliases = []string{"KIDZ PLANET URGENT CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1740899111"}}
