...
func GetSourceChildrensNationalCardiologyRichmond(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChildrensNationalCardiologyRichmond]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CHILDRENS NATIONAL CARDIOLOGY RICHMOND"
	sourceDef.SourceType = pkg.SourceTypeChildrensNationalCardiologyRichmond
