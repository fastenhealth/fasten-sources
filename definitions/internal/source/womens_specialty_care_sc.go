...
func GetSourceWomensSpecialtyCareSc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWomensSpecialtyCareSc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Womens Specialty Care SC"
	sourceDef.SourceType = pkg.SourceTypeWomensSpecialtyCareSc
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{"DR KAREN J MASS MD"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1477611028"}}
