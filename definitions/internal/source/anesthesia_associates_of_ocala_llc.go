...
func GetSourceAnesthesiaAssociatesOfOcalaLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAnesthesiaAssociatesOfOcalaLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Anesthesia Associates Of Ocala LLC"
	sourceDef.SourceType = pkg.SourceTypeAnesthesiaAssociatesOfOcalaLlc
