...
func GetSourceOceanwayPediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOceanwayPediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Oceanway Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeOceanwayPediatrics
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{"OCEANWAY PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1851008726"}}
