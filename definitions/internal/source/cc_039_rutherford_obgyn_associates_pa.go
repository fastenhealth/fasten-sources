...
func GetSourceCc039RutherfordObgynAssociatesPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc039RutherfordObgynAssociatesPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC039RUTHERFORD OBGYN ASSOCIATES PA"
	sourceDef.SourceType = pkg.SourceTypeCc039RutherfordObgynAssociatesPa
