...
func GetSourceClinicalSupportProviders(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeClinicalSupportProviders]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Clinical Support Providers"
	sourceDef.SourceType = pkg.SourceTypeClinicalSupportProviders
	sourceDef.Category = []string{"261QM1300X"}
	sourceDef.Aliases = []string{"CLINICAL SUPPORT PROVIDERS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1922583616"}}
