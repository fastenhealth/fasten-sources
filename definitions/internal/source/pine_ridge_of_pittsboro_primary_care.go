...
func GetSourcePineRidgeOfPittsboroPrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePineRidgeOfPittsboroPrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pine Ridge of Pittsboro Primary Care"
	sourceDef.SourceType = pkg.SourceTypePineRidgeOfPittsboroPrimaryCare
