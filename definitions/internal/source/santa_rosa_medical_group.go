...
func GetSourceSantaRosaMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSantaRosaMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Santa Rosa Medical Group"
	sourceDef.SourceType = pkg.SourceTypeSantaRosaMedicalGroup
	sourceDef.Category = []string{"207Q00000X", "207R00000X"}
	sourceDef.Aliases = []string{"SANTA ROSA MEDICAL GROUP"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1801235445", "1891093100"}}
