...
func GetSourceWinningHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWinningHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Winning Health"
	sourceDef.SourceType = pkg.SourceTypeWinningHealth
	sourceDef.Category = []string{"332B00000X", "332BC3200X"}
	sourceDef.Aliases = []string{"WINNING HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1740644558"}}
