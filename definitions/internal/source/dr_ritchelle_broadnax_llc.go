...
func GetSourceDrRitchelleBroadnaxLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDrRitchelleBroadnaxLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Dr Ritchelle Broadnax LLC"
	sourceDef.SourceType = pkg.SourceTypeDrRitchelleBroadnaxLlc
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{"BROADNAX HEALTHCARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1750852950"}}
