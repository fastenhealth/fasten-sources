...
func GetSourceNephrologyAssociatesOfSouthMiam(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNephrologyAssociatesOfSouthMiam]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Nephrology Associates of South Miam"
	sourceDef.SourceType = pkg.SourceTypeNephrologyAssociatesOfSouthMiam
