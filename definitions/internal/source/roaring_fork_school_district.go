...
func GetSourceRoaringForkSchoolDistrict(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRoaringForkSchoolDistrict]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Roaring Fork School District"
	sourceDef.SourceType = pkg.SourceTypeRoaringForkSchoolDistrict
	sourceDef.Category = []string{"251300000X"}
	sourceDef.Aliases = []string{"ROARING FORK SCHOOL DISTRICT"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1194853150"}}
