...
func GetSourcePiedmontOrthopedicsOrthoatlanta(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePiedmontOrthopedicsOrthoatlanta]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Piedmont Orthopedics  OrthoAtlanta"
	sourceDef.SourceType = pkg.SourceTypePiedmontOrthopedicsOrthoatlanta
