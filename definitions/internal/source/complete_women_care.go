...
func GetSourceCompleteWomenCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCompleteWomenCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Complete Women Care"
	sourceDef.SourceType = pkg.SourceTypeCompleteWomenCare
	sourceDef.Category = []string{"332900000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1811442601"}}
	sourceDef.PatientAccessUrl = "http://www.completewomencare.com"
