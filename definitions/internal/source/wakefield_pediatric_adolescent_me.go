...
func GetSourceWakefieldPediatricAdolescentMe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWakefieldPediatricAdolescentMe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Wakefield Pediatric  Adolescent Me"
	sourceDef.SourceType = pkg.SourceTypeWakefieldPediatricAdolescentMe
