...
func GetSourceShawPainClinicPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShawPainClinicPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SHAW PAIN CLINIC PLLC"
	sourceDef.SourceType = pkg.SourceTypeShawPainClinicPllc
	sourceDef.Category = []string{"208VP0014X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1104479146"}}
	sourceDef.PatientAccessUrl = "http://www.shawpainclinic.com"
