...
func GetSourceWillisClinicPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWillisClinicPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Willis Clinic PLLC"
	sourceDef.SourceType = pkg.SourceTypeWillisClinicPllc
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"WILLIS CLINIC PLLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1063619567"}}
