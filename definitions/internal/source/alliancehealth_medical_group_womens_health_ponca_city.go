...
func GetSourceAlliancehealthMedicalGroupWomensHealthPoncaCity(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupWomensHealthPoncaCity]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Womens Health Ponca City"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupWomensHealthPoncaCity
