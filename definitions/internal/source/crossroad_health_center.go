...
func GetSourceCrossroadHealthCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCrossroadHealthCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Crossroad Health Center"
	sourceDef.SourceType = pkg.SourceTypeCrossroadHealthCenter
	sourceDef.Category = []string{"261QF0400X"}
	sourceDef.Aliases = []string{"CROSSROAD HEALTH CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1639574148"}}
