...
func GetSourceImperialHealthcareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeImperialHealthcareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Imperial Healthcare LLC"
	sourceDef.SourceType = pkg.SourceTypeImperialHealthcareLlc
	sourceDef.Category = []string{"363L00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1457938078"}}
	sourceDef.PatientAccessUrl = "http://www.imperialhealth-fnp.com"
