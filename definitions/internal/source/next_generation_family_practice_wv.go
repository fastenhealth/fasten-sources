...
func GetSourceNextGenerationFamilyPracticeWv(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNextGenerationFamilyPracticeWv]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Next Generation Family Practice WV"
	sourceDef.SourceType = pkg.SourceTypeNextGenerationFamilyPracticeWv
