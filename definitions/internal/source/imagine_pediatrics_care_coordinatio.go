...
func GetSourceImaginePediatricsCareCoordinatio(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeImaginePediatricsCareCoordinatio]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Imagine Pediatrics Care Coordinatio"
	sourceDef.SourceType = pkg.SourceTypeImaginePediatricsCareCoordinatio
