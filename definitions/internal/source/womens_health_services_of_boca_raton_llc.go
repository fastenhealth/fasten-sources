...
func GetSourceWomensHealthServicesOfBocaRatonLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWomensHealthServicesOfBocaRatonLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WOMENS HEALTH SERVICES OF BOCA RATON LLC"
	sourceDef.SourceType = pkg.SourceTypeWomensHealthServicesOfBocaRatonLlc
