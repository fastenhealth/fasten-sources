...
func GetSourceRaybonObstetricsAndGynecologyAssociatesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRaybonObstetricsAndGynecologyAssociatesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RAYBON OBSTETRICS AND GYNECOLOGY ASSOCIATES LLC"
	sourceDef.SourceType = pkg.SourceTypeRaybonObstetricsAndGynecologyAssociatesLlc
