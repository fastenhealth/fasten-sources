...
func GetSourceCentralTexasCardiovascularSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralTexasCardiovascularSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Central Texas Cardiovascular Surgery"
	sourceDef.SourceType = pkg.SourceTypeCentralTexasCardiovascularSurgery
