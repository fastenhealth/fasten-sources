...
func GetSourcePennsylvaniaPhysicianServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePennsylvaniaPhysicianServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pennsylvania Physician Services"
	sourceDef.SourceType = pkg.SourceTypePennsylvaniaPhysicianServices
