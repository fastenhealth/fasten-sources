...
func GetSourceRegenerativeMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRegenerativeMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Regenerative Medical Group"
	sourceDef.SourceType = pkg.SourceTypeRegenerativeMedicalGroup
	sourceDef.Category = []string{"207Q00000X", "207QG0300X"}
	sourceDef.Aliases = []string{"REGENERATIVE MEDICAL GROUP", "RMG GARDEN GROVE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1578928867", "1821557281", "1881104222"}}
