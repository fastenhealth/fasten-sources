...
func GetSourceInfectiousDiseaseAssociatesOfNa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInfectiousDiseaseAssociatesOfNa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Infectious Disease Associates of Na"
	sourceDef.SourceType = pkg.SourceTypeInfectiousDiseaseAssociatesOfNa
