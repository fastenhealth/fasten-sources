...
func GetSourceNcComprehensiveHeadacheClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNcComprehensiveHeadacheClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NC Comprehensive Headache Clinic"
	sourceDef.SourceType = pkg.SourceTypeNcComprehensiveHeadacheClinic
