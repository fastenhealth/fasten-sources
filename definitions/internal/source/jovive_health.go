...
func GetSourceJoviveHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJoviveHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Jovive Health"
	sourceDef.SourceType = pkg.SourceTypeJoviveHealth
	sourceDef.Category = []string{"207P00000X", "207Q00000X", "261QU0200X"}
	sourceDef.Aliases = []string{"JOVIVE HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1255042248", "1336818574"}}
