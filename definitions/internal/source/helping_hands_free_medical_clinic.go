...
func GetSourceHelpingHandsFreeMedicalClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHelpingHandsFreeMedicalClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Helping Hands Free Medical Clinic"
	sourceDef.SourceType = pkg.SourceTypeHelpingHandsFreeMedicalClinic
