...
func GetSourcePrestigeOutpatientProvidersLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrestigeOutpatientProvidersLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Prestige Outpatient Providers LLC"
	sourceDef.SourceType = pkg.SourceTypePrestigeOutpatientProvidersLlc
