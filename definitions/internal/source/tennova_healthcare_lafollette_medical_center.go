...
func GetSourceTennovaHealthcareLafolletteMedicalCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaHealthcareLafolletteMedicalCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Healthcare  LaFollette Medical Center"
	sourceDef.SourceType = pkg.SourceTypeTennovaHealthcareLafolletteMedicalCenter
