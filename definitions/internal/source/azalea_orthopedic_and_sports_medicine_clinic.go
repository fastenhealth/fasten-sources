...
func GetSourceAzaleaOrthopedicAndSportsMedicineClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAzaleaOrthopedicAndSportsMedicineClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Azalea Orthopedic and Sports Medicine Clinic"
	sourceDef.SourceType = pkg.SourceTypeAzaleaOrthopedicAndSportsMedicineClinic
