...
func GetSourceWesternArizonaOrthopaedicAndSportsMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWesternArizonaOrthopaedicAndSportsMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Western Arizona Orthopaedic and Sports Medicine"
	sourceDef.SourceType = pkg.SourceTypeWesternArizonaOrthopaedicAndSportsMedicine
