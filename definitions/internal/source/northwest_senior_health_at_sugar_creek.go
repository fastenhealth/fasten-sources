...
func GetSourceNorthwestSeniorHealthAtSugarCreek(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestSeniorHealthAtSugarCreek]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Senior Health at Sugar Creek"
	sourceDef.SourceType = pkg.SourceTypeNorthwestSeniorHealthAtSugarCreek
