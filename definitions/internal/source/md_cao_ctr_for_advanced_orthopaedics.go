...
func GetSourceMdCaoCtrForAdvancedOrthopaedics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMdCaoCtrForAdvancedOrthopaedics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MD  CAO  Ctr for Advanced Orthopaedics"
	sourceDef.SourceType = pkg.SourceTypeMdCaoCtrForAdvancedOrthopaedics
