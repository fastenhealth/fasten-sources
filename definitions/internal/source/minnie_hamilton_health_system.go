...
func GetSourceMinnieHamiltonHealthSystem(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMinnieHamiltonHealthSystem]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Minnie Hamilton Health System"
	sourceDef.SourceType = pkg.SourceTypeMinnieHamiltonHealthSystem
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"MINNIE HAMILTON HEALTH SYSTEM"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1548993603"}}
