...
func GetSourceQhcPfEvanstonHospitalCorporatio(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeQhcPfEvanstonHospitalCorporatio]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "QHC PF Evanston Hospital Corporatio"
	sourceDef.SourceType = pkg.SourceTypeQhcPfEvanstonHospitalCorporatio
