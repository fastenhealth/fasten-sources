...
func GetSourceOrmondMedicalArtsFamilyPractice(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOrmondMedicalArtsFamilyPractice]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ormond Medical Arts Family Practice"
	sourceDef.SourceType = pkg.SourceTypeOrmondMedicalArtsFamilyPractice
