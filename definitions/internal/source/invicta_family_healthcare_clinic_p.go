...
func GetSourceInvictaFamilyHealthcareClinicP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInvictaFamilyHealthcareClinicP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Invicta Family Healthcare Clinic P"
	sourceDef.SourceType = pkg.SourceTypeInvictaFamilyHealthcareClinicP
