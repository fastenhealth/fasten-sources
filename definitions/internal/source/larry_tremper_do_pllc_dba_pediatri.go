...
func GetSourceLarryTremperDoPllcDbaPediatri(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLarryTremperDoPllcDbaPediatri]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Larry Tremper DO PLLC DBA Pediatri"
	sourceDef.SourceType = pkg.SourceTypeLarryTremperDoPllcDbaPediatri
