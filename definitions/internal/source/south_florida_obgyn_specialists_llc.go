...
func GetSourceSouthFloridaObgynSpecialistsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthFloridaObgynSpecialistsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SOUTH FLORIDA OBGYN SPECIALISTS LLC"
	sourceDef.SourceType = pkg.SourceTypeSouthFloridaObgynSpecialistsLlc
