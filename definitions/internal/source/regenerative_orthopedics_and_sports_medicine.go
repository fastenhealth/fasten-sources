...
func GetSourceRegenerativeOrthopedicsAndSportsMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRegenerativeOrthopedicsAndSportsMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Regenerative Orthopedics and Sports Medicine"
	sourceDef.SourceType = pkg.SourceTypeRegenerativeOrthopedicsAndSportsMedicine
