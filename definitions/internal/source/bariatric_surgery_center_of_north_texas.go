...
func GetSourceBariatricSurgeryCenterOfNorthTexas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBariatricSurgeryCenterOfNorthTexas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BARIATRIC SURGERY CENTER OF NORTH TEXAS"
	sourceDef.SourceType = pkg.SourceTypeBariatricSurgeryCenterOfNorthTexas
