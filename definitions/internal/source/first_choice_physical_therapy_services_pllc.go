...
func GetSourceFirstChoicePhysicalTherapyServicesPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFirstChoicePhysicalTherapyServicesPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FIRST CHOICE PHYSICAL THERAPY SERVICES PLLC"
	sourceDef.SourceType = pkg.SourceTypeFirstChoicePhysicalTherapyServicesPllc
