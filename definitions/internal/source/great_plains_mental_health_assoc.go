...
func GetSourceGreatPlainsMentalHealthAssoc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGreatPlainsMentalHealthAssoc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Great Plains Mental Health Assoc"
	sourceDef.SourceType = pkg.SourceTypeGreatPlainsMentalHealthAssoc
