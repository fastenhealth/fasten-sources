...
func GetSourceNortheasternNevadaPhysicianPractices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNortheasternNevadaPhysicianPractices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northeastern Nevada Physician Practices"
	sourceDef.SourceType = pkg.SourceTypeNortheasternNevadaPhysicianPractices
