...
func GetSourceGlendoraDigestiveDiseaseInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGlendoraDigestiveDiseaseInstitute]; clientIdOk {
	...
	sourceDef.Aliases = []string{"GLENDORA DIGESTIVE DISEASE INSTITUTE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1821285073"}}
