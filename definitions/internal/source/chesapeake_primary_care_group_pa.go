...
func GetSourceChesapeakePrimaryCareGroupPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChesapeakePrimaryCareGroupPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Chesapeake Primary Care Group PA"
	sourceDef.SourceType = pkg.SourceTypeChesapeakePrimaryCareGroupPa
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1609989730"}}
	sourceDef.PatientAccessUrl = "https://chesapeakemedicinenow.com/"
