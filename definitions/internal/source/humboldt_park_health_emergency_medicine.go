...
func GetSourceHumboldtParkHealthEmergencyMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHumboldtParkHealthEmergencyMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HUMBOLDT PARK HEALTH EMERGENCY MEDICINE"
	sourceDef.SourceType = pkg.SourceTypeHumboldtParkHealthEmergencyMedicine
