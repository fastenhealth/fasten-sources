...
func GetSourcePediatricAssociatesOfJacksonville(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePediatricAssociatesOfJacksonville]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pediatric Associates of Jacksonville"
	sourceDef.SourceType = pkg.SourceTypePediatricAssociatesOfJacksonville
