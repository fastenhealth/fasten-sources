...
func GetSourceConcordInternalMedicineAssociate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeConcordInternalMedicineAssociate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CONCORD INTERNAL MEDICINE ASSOCIATE"
	sourceDef.SourceType = pkg.SourceTypeConcordInternalMedicineAssociate
