...
func GetSourceCc903WomensHealthServicesOfWellingtonLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc903WomensHealthServicesOfWellingtonLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC903Womens Health Services of Wellington LLC"
	sourceDef.SourceType = pkg.SourceTypeCc903WomensHealthServicesOfWellingtonLlc
