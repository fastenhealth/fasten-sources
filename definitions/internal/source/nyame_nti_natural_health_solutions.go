...
func GetSourceNyameNtiNaturalHealthSolutions(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNyameNtiNaturalHealthSolutions]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NYAME NTI NATURAL HEALTH SOLUTIONS"
	sourceDef.SourceType = pkg.SourceTypeNyameNtiNaturalHealthSolutions
