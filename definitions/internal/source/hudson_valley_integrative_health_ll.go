...
func GetSourceHudsonValleyIntegrativeHealthLl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHudsonValleyIntegrativeHealthLl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HUDSON VALLEY INTEGRATIVE HEALTH LL"
	sourceDef.SourceType = pkg.SourceTypeHudsonValleyIntegrativeHealthLl
