...
func GetSourceRetinaAssociatesOfKentucky(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRetinaAssociatesOfKentucky]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Retina Associates Of Kentucky"
	sourceDef.SourceType = pkg.SourceTypeRetinaAssociatesOfKentucky
	sourceDef.BrandLogo = "retina-associates-of-kentucky.jpg"
	sourceDef.PatientAccessUrl = "https://www.retinaky.com/"
