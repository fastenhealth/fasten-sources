...
func GetSourceBridgewayRehabiltationServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBridgewayRehabiltationServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bridgeway Rehabiltation Services"
	sourceDef.SourceType = pkg.SourceTypeBridgewayRehabiltationServices
