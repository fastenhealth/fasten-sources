...
func GetSourceOpenHeartsHealingMindsCorporati(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOpenHeartsHealingMindsCorporati]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Open Hearts Healing Minds Corporati"
	sourceDef.SourceType = pkg.SourceTypeOpenHeartsHealingMindsCorporati
