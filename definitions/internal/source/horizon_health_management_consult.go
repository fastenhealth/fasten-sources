...
func GetSourceHorizonHealthManagementConsult(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHorizonHealthManagementConsult]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HORIZON HEALTH MANAGEMENT  CONSULT"
	sourceDef.SourceType = pkg.SourceTypeHorizonHealthManagementConsult
