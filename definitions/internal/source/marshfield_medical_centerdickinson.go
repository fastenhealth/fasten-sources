...
func GetSourceMarshfieldMedicalCenterdickinson(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMarshfieldMedicalCenterdickinson]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Marshfield Medical CenterDickinson"
	sourceDef.SourceType = pkg.SourceTypeMarshfieldMedicalCenterdickinson
