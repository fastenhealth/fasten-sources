...
func GetSourceDrAobClinicLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDrAobClinicLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DR AOB CLINIC LLC"
	sourceDef.SourceType = pkg.SourceTypeDrAobClinicLlc
	sourceDef.Category = []string{"261QM1300X"}
	sourceDef.Aliases = []string{"MELIORA CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1023695830"}}
	sourceDef.PatientAccessUrl = "http://www.melioraclinic.com"
