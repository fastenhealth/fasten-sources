...
func GetSourcePrivateHealthDallas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrivateHealthDallas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Private Health Dallas"
	sourceDef.SourceType = pkg.SourceTypePrivateHealthDallas
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"PRIVATE HEALTH DALLAS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1609337054"}}
	sourceDef.PatientAccessUrl = "http://www.privatehealthdallas.com"
