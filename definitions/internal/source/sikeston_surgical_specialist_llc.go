...
func GetSourceSikestonSurgicalSpecialistLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSikestonSurgicalSpecialistLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sikeston Surgical Specialist LLC"
	sourceDef.SourceType = pkg.SourceTypeSikestonSurgicalSpecialistLlc
