...
func GetSourceMancinaCardiovascularMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMancinaCardiovascularMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Mancina Cardiovascular Medicine"
	sourceDef.SourceType = pkg.SourceTypeMancinaCardiovascularMedicine
	sourceDef.Category = []string{"207RC0000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1083010458"}}
	sourceDef.PatientAccessUrl = "http://msmancinamdcardiology.com"
