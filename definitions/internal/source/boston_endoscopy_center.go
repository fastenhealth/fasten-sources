...
func GetSourceBostonEndoscopyCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBostonEndoscopyCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Boston Endoscopy Center"
	sourceDef.SourceType = pkg.SourceTypeBostonEndoscopyCenter
	sourceDef.Category = []string{"261QE0800X"}
	sourceDef.Aliases = []string{"BOSTON ENDOSCOPY CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1477802387"}}
	sourceDef.PatientAccessUrl = "https://bostonendoscopycenter.com"
