...
func GetSourceNeuroanalyticsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeuroanalyticsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NeuroAnalytics LLC"
	sourceDef.SourceType = pkg.SourceTypeNeuroanalyticsLlc
	sourceDef.Category = []string{"261QM0801X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1104334796"}}
	sourceDef.PatientAccessUrl = "https://www.neuroanalyticsllc.com"
