...
func GetSourceWomenCaringPlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWomenCaringPlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Women Caring PLC"
	sourceDef.SourceType = pkg.SourceTypeWomenCaringPlc
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{"WOMEN CARING PLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1306965264"}}
