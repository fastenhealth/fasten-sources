...
func GetSourceInterimHealthcareSupportiveServi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInterimHealthcareSupportiveServi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Interim Healthcare Supportive Servi"
	sourceDef.SourceType = pkg.SourceTypeInterimHealthcareSupportiveServi
