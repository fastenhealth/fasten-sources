...
func GetSourceCommunityMedicalClinicOfAikenC(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCommunityMedicalClinicOfAikenC]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Community Medical Clinic of Aiken C"
	sourceDef.SourceType = pkg.SourceTypeCommunityMedicalClinicOfAikenC
