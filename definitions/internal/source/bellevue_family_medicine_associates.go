...
func GetSourceBellevueFamilyMedicineAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBellevueFamilyMedicineAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bellevue Family Medicine Associates"
	sourceDef.SourceType = pkg.SourceTypeBellevueFamilyMedicineAssociates
