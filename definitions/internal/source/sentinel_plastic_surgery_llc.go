...
func GetSourceSentinelPlasticSurgeryLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSentinelPlasticSurgeryLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sentinel Plastic Surgery LLC"
	sourceDef.SourceType = pkg.SourceTypeSentinelPlasticSurgeryLlc
	sourceDef.Category = []string{"208200000X"}
	sourceDef.Aliases = []string{"WILLIAM H. FRAZIER, MD"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1265941413"}}
