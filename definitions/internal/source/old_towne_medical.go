...
func GetSourceOldTowneMedical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOldTowneMedical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Old Towne Medical"
	sourceDef.SourceType = pkg.SourceTypeOldTowneMedical
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{"OLD TOWNE MEDICAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1013312172"}}
