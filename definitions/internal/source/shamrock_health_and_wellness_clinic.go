...
func GetSourceShamrockHealthAndWellnessClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShamrockHealthAndWellnessClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SHAMROCK HEALTH AND WELLNESS CLINIC"
	sourceDef.SourceType = pkg.SourceTypeShamrockHealthAndWellnessClinic
