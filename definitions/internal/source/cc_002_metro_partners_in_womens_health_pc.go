...
func GetSourceCc002MetroPartnersInWomensHealthPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc002MetroPartnersInWomensHealthPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC002METRO PARTNERS IN WOMENS HEALTH PC"
	sourceDef.SourceType = pkg.SourceTypeCc002MetroPartnersInWomensHealthPc
