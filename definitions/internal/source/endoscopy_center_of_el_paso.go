...
func GetSourceEndoscopyCenterOfElPaso(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEndoscopyCenterOfElPaso]; clientIdOk {
	...
	sourceDef.Aliases = []string{"ENDOSCOPY CENTER OF EL PASO"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1033181862"}}
