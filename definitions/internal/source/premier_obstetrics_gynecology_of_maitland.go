...
func GetSourcePremierObstetricsGynecologyOfMaitland(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePremierObstetricsGynecologyOfMaitland]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PREMIER OBSTETRICS  GYNECOLOGY OF MAITLAND"
	sourceDef.SourceType = pkg.SourceTypePremierObstetricsGynecologyOfMaitland
