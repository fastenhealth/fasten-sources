...
func GetSourceArizonaInfectiousDiseasePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArizonaInfectiousDiseasePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arizona Infectious Disease PLLC"
	sourceDef.SourceType = pkg.SourceTypeArizonaInfectiousDiseasePllc
	sourceDef.Category = []string{"207RI0200X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1265904338"}}
	sourceDef.PatientAccessUrl = "http://www.azinfection.com"
