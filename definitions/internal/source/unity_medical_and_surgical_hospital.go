...
func GetSourceUnityMedicalAndSurgicalHospital(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUnityMedicalAndSurgicalHospital]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Unity Medical and Surgical Hospital"
	sourceDef.SourceType = pkg.SourceTypeUnityMedicalAndSurgicalHospital
