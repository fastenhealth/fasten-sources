...
func GetSourceAffordacare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAffordacare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Affordacare"
	sourceDef.SourceType = pkg.SourceTypeAffordacare
	sourceDef.Category = []string{"207Q00000X", "261QU0200X"}
	sourceDef.Aliases = []string{"AFFORDACARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1619235629"}}
	sourceDef.PatientAccessUrl = "http://affordacare.com"
