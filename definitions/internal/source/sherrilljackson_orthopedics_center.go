...
func GetSourceSherrilljacksonOrthopedicsCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSherrilljacksonOrthopedicsCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SherrillJackson Orthopedics Center"
	sourceDef.SourceType = pkg.SourceTypeSherrilljacksonOrthopedicsCenter
