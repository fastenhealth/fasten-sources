...
func GetSourceBayAreaPhysicalTherapyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBayAreaPhysicalTherapyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bay Area Physical Therapy LLC"
	sourceDef.SourceType = pkg.SourceTypeBayAreaPhysicalTherapyLlc
	sourceDef.Category = []string{"261QP2000X"}
	sourceDef.Aliases = []string{"BAY AREA PHYSICAL THERAPY", "FITNESS QUEST BAY AREA"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1871582742"}}
