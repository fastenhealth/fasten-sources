...
func GetSourceSequoiaUrology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSequoiaUrology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sequoia Urology"
	sourceDef.SourceType = pkg.SourceTypeSequoiaUrology
	sourceDef.Category = []string{"208800000X"}
	sourceDef.Aliases = []string{"SEQUOIA UROLOGY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1770763930"}}
	sourceDef.PatientAccessUrl = "http://www.sequoiaurology.care"
