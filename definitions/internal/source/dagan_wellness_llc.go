...
func GetSourceDaganWellnessLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDaganWellnessLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Dagan Wellness LLC"
	sourceDef.SourceType = pkg.SourceTypeDaganWellnessLlc
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"DAGAN WELLNESS CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1982065983"}}
	sourceDef.PatientAccessUrl = "http://www.daganwellnessclinic.com"
