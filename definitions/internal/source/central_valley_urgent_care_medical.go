...
func GetSourceCentralValleyUrgentCareMedical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralValleyUrgentCareMedical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CENTRAL VALLEY URGENT CARE MEDICAL"
	sourceDef.SourceType = pkg.SourceTypeCentralValleyUrgentCareMedical
