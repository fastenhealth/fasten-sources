...
func GetSourceChildrensAndFamilyMedicalClini(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChildrensAndFamilyMedicalClini]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Childrens and Family Medical Clini"
	sourceDef.SourceType = pkg.SourceTypeChildrensAndFamilyMedicalClini
