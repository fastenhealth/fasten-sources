...
func GetSourceJmjMedical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJmjMedical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "JMJ Medical"
	sourceDef.SourceType = pkg.SourceTypeJmjMedical
	sourceDef.Category = []string{"208000000X", "208D00000X"}
	sourceDef.Aliases = []string{"JMJ MEDICAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1093235129"}}
