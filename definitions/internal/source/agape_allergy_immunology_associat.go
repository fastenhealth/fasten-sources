...
func GetSourceAgapeAllergyImmunologyAssociat(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAgapeAllergyImmunologyAssociat]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AGAPE ALLERGY  IMMUNOLOGY ASSOCIAT"
	sourceDef.SourceType = pkg.SourceTypeAgapeAllergyImmunologyAssociat
