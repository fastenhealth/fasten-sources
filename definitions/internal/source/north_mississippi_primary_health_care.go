...
func GetSourceNorthMississippiPrimaryHealthCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthMississippiPrimaryHealthCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTH MISSISSIPPI PRIMARY HEALTH CARE"
	sourceDef.SourceType = pkg.SourceTypeNorthMississippiPrimaryHealthCare
