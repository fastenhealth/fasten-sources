...
func GetSourceTheWomensCenterOfOrlandoLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheWomensCenterOfOrlandoLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "THE WOMENS CENTER OF ORLANDO LLC"
	sourceDef.SourceType = pkg.SourceTypeTheWomensCenterOfOrlandoLlc
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1295305878", "1386214963", "1902476591"}}
