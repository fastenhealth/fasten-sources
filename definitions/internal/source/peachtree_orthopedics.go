...
func GetSourcePeachtreeOrthopedics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePeachtreeOrthopedics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Peachtree Orthopedics"
	sourceDef.SourceType = pkg.SourceTypePeachtreeOrthopedics
	sourceDef.Category = []string{"332B00000X"}
	sourceDef.Aliases = []string{"PEACHTREE ORTHOPEDICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1700460326"}}
