...
func GetSourceCollinMedicalWellnessGroupPll(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCollinMedicalWellnessGroupPll]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COLLIN MEDICAL  WELLNESS GROUP PLL"
	sourceDef.SourceType = pkg.SourceTypeCollinMedicalWellnessGroupPll
