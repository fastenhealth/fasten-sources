...
func GetSourceChesapeakeRegionalNeurosciencesneuroEndovascularSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChesapeakeRegionalNeurosciencesneuroEndovascularSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Chesapeake Regional NeurosciencesNeuro Endovascular Surgery"
	sourceDef.SourceType = pkg.SourceTypeChesapeakeRegionalNeurosciencesneuroEndovascularSurgery
