...
func GetSourceDesotoAdultMedicineAssociatesPl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDesotoAdultMedicineAssociatesPl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Desoto Adult Medicine Associates Pl"
	sourceDef.SourceType = pkg.SourceTypeDesotoAdultMedicineAssociatesPl
