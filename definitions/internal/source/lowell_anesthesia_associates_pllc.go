...
func GetSourceLowellAnesthesiaAssociatesPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLowellAnesthesiaAssociatesPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Lowell Anesthesia Associates PLLC"
	sourceDef.SourceType = pkg.SourceTypeLowellAnesthesiaAssociatesPllc
	sourceDef.Category = []string{"367500000X"}
	sourceDef.Aliases = []string{"LOWELL ANESTHESIA ASSOCIATES"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1063938934"}}
