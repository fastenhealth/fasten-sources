...
func GetSourceAriseBehavioralHealthWellness(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAriseBehavioralHealthWellness]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arise Behavioral Health  Wellness"
	sourceDef.SourceType = pkg.SourceTypeAriseBehavioralHealthWellness
