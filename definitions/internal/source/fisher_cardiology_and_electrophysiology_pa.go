...
func GetSourceFisherCardiologyAndElectrophysiologyPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFisherCardiologyAndElectrophysiologyPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Fisher Cardiology and Electrophysiology PA"
	sourceDef.SourceType = pkg.SourceTypeFisherCardiologyAndElectrophysiologyPa
