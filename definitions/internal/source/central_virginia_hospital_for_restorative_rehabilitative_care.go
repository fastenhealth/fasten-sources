...
func GetSourceCentralVirginiaHospitalForRestorativeRehabilitativeCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralVirginiaHospitalForRestorativeRehabilitativeCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CENTRAL VIRGINIA HOSPITAL FOR RESTORATIVE  REHABILITATIVE CARE"
	sourceDef.SourceType = pkg.SourceTypeCentralVirginiaHospitalForRestorativeRehabilitativeCare
