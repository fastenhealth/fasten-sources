...
func GetSourceWayneCountyHealthDepartment(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWayneCountyHealthDepartment]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Wayne County Health Department"
	sourceDef.SourceType = pkg.SourceTypeWayneCountyHealthDepartment
	sourceDef.Category = []string{"251K00000X"}
	sourceDef.Aliases = []string{"WAYNE COUNTY HEALTH DEPARTMENT"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1093246332"}}
