...
func GetSourcePhysicianPracticesOfRiverRegion(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePhysicianPracticesOfRiverRegion]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Physician Practices of River Region"
	sourceDef.SourceType = pkg.SourceTypePhysicianPracticesOfRiverRegion
