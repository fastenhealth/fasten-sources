...
func GetSourceHeartMountainVolunteerMedicalCl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHeartMountainVolunteerMedicalCl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Heart Mountain Volunteer Medical Cl"
	sourceDef.SourceType = pkg.SourceTypeHeartMountainVolunteerMedicalCl
