...
func GetSourceNorthwestGeorgiaSurgicalAssociat(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestGeorgiaSurgicalAssociat]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Georgia Surgical Associat"
	sourceDef.SourceType = pkg.SourceTypeNorthwestGeorgiaSurgicalAssociat
