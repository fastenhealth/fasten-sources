...
func GetSourceHintonsIntegrativeMedicineHim(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHintonsIntegrativeMedicineHim]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hintons Integrative Medicine HIM"
	sourceDef.SourceType = pkg.SourceTypeHintonsIntegrativeMedicineHim
