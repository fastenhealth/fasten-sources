...
func GetSourceWeeCarePediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWeeCarePediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Wee Care Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeWeeCarePediatrics
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{"WEE CARE PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1194956292", "1356496020", "1518181841"}}
