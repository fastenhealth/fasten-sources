...
func GetSourceNorthHoustonDiabetesInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthHoustonDiabetesInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTH HOUSTON DIABETES INSTITUTE"
	sourceDef.SourceType = pkg.SourceTypeNorthHoustonDiabetesInstitute
