...
func GetSourceAssociatedSpecialistsInMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAssociatedSpecialistsInMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ASSOCIATED SPECIALISTS IN MEDICINE"
	sourceDef.SourceType = pkg.SourceTypeAssociatedSpecialistsInMedicine
