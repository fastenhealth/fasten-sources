...
func GetSourceSantosPrimaryCareWalkInUrgentCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSantosPrimaryCareWalkInUrgentCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Santos Primary Care  Walk In Urgent Care"
	sourceDef.SourceType = pkg.SourceTypeSantosPrimaryCareWalkInUrgentCare
