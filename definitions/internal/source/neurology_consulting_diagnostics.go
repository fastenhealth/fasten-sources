...
func GetSourceNeurologyConsultingDiagnostics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeurologyConsultingDiagnostics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEUROLOGY CONSULTING  DIAGNOSTICS"
	sourceDef.SourceType = pkg.SourceTypeNeurologyConsultingDiagnostics
