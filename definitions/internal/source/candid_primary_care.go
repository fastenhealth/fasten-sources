...
func GetSourceCandidPrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCandidPrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Candid Primary Care"
	sourceDef.SourceType = pkg.SourceTypeCandidPrimaryCare
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"CANDID PRIMARY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1528795218"}}
	sourceDef.PatientAccessUrl = "http://candidprimarycare.com"
