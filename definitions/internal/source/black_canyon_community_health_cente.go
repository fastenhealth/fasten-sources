...
func GetSourceBlackCanyonCommunityHealthCente(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBlackCanyonCommunityHealthCente]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Black Canyon Community Health Cente"
	sourceDef.SourceType = pkg.SourceTypeBlackCanyonCommunityHealthCente
