...
func GetSourceNorthwestMedicalPlazaAtSugarCreek(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestMedicalPlazaAtSugarCreek]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Medical Plaza at Sugar Creek"
	sourceDef.SourceType = pkg.SourceTypeNorthwestMedicalPlazaAtSugarCreek
