...
func GetSourceVirtualHealthNpInFamilyHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVirtualHealthNpInFamilyHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VIRTUAL HEALTH NP IN FAMILY HEALTH"
	sourceDef.SourceType = pkg.SourceTypeVirtualHealthNpInFamilyHealth
