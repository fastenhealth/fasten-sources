...
func GetSourceMonroeRheumatologyAndEndocrinolo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMonroeRheumatologyAndEndocrinolo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MONROE RHEUMATOLOGY AND ENDOCRINOLO"
	sourceDef.SourceType = pkg.SourceTypeMonroeRheumatologyAndEndocrinolo
