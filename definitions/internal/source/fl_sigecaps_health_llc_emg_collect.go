...
func GetSourceFlSigecapsHealthLlcEmgCollect(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFlSigecapsHealthLlcEmgCollect]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FL  Sigecaps Health LLC EMG  Collect"
	sourceDef.SourceType = pkg.SourceTypeFlSigecapsHealthLlcEmgCollect
