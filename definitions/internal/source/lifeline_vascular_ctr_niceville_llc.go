...
func GetSourceLifelineVascularCtrNicevilleLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLifelineVascularCtrNicevilleLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Lifeline Vascular Ctr Niceville LLC"
	sourceDef.SourceType = pkg.SourceTypeLifelineVascularCtrNicevilleLlc
