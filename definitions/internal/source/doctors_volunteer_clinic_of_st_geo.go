...
func GetSourceDoctorsVolunteerClinicOfStGeo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDoctorsVolunteerClinicOfStGeo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Doctors Volunteer Clinic of St Geo"
	sourceDef.SourceType = pkg.SourceTypeDoctorsVolunteerClinicOfStGeo
