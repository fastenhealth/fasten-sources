...
func GetSourceNortheastTexasOncologicAndReco(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNortheastTexasOncologicAndReco]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTHEAST TEXAS ONCOLOGIC AND RECO"
	sourceDef.SourceType = pkg.SourceTypeNortheastTexasOncologicAndReco
