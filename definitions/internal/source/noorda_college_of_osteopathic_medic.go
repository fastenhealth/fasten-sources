...
func GetSourceNoordaCollegeOfOsteopathicMedic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNoordaCollegeOfOsteopathicMedic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Noorda College Of Osteopathic Medic"
	sourceDef.SourceType = pkg.SourceTypeNoordaCollegeOfOsteopathicMedic
