...
func GetSourceTennovaLafolletteGeriatricPsychiatricUnit(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaLafolletteGeriatricPsychiatricUnit]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova LaFollette Geriatric Psychiatric Unit"
	sourceDef.SourceType = pkg.SourceTypeTennovaLafolletteGeriatricPsychiatricUnit
