...
func GetSourceLibertyPainAssociatesSathyaPB(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLibertyPainAssociatesSathyaPB]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Liberty Pain Associates Sathya P B"
	sourceDef.SourceType = pkg.SourceTypeLibertyPainAssociatesSathyaPB
