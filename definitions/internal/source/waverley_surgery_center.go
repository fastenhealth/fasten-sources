...
func GetSourceWaverleySurgeryCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWaverleySurgeryCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Waverley Surgery Center"
	sourceDef.SourceType = pkg.SourceTypeWaverleySurgeryCenter
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"WAVERLEY SURGERY CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1619944824"}}
