...
func GetSourceRaleighGeneralPhysicianServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRaleighGeneralPhysicianServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Raleigh General Physician Services"
	sourceDef.SourceType = pkg.SourceTypeRaleighGeneralPhysicianServices
