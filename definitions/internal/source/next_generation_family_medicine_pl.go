...
func GetSourceNextGenerationFamilyMedicinePl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNextGenerationFamilyMedicinePl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Next Generation Family Medicine PL"
	sourceDef.SourceType = pkg.SourceTypeNextGenerationFamilyMedicinePl
