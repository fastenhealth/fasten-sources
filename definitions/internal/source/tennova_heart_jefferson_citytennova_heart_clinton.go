...
func GetSourceTennovaHeartJeffersonCitytennovaHeartClinton(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaHeartJeffersonCitytennovaHeartClinton]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Heart  Jefferson CityTennova Heart  Clinton"
	sourceDef.SourceType = pkg.SourceTypeTennovaHeartJeffersonCitytennovaHeartClinton
