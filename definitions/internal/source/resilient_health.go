...
func GetSourceResilientHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeResilientHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Resilient Health"
	sourceDef.SourceType = pkg.SourceTypeResilientHealth
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"RESILIENT HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1730325846"}}
	sourceDef.PatientAccessUrl = "http://resilienthealthaustin.com/"
