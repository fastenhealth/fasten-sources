...
func GetSourceCentralOhioNephrologyAndHyperte(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralOhioNephrologyAndHyperte]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Central Ohio Nephrology and Hyperte"
	sourceDef.SourceType = pkg.SourceTypeCentralOhioNephrologyAndHyperte
