...
func GetSourceNeurologySolutionsConsultantsP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeurologySolutionsConsultantsP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Neurology Solutions Consultants P"
	sourceDef.SourceType = pkg.SourceTypeNeurologySolutionsConsultantsP
