...
func GetSourceOmegaMedicalAndWellnessCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOmegaMedicalAndWellnessCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Omega Medical and Wellness Center"
	sourceDef.SourceType = pkg.SourceTypeOmegaMedicalAndWellnessCenter
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"OMEGA MEDICAL AND WELLNESS CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1922573609"}}
