...
func GetSourceDavisInternalMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDavisInternalMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Davis Internal Medicine"
	sourceDef.SourceType = pkg.SourceTypeDavisInternalMedicine
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"DAVIS INTERNAL MEDICINE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1457657868"}}
