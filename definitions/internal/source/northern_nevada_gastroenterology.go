...
func GetSourceNorthernNevadaGastroenterology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthernNevadaGastroenterology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northern Nevada Gastroenterology"
	sourceDef.SourceType = pkg.SourceTypeNorthernNevadaGastroenterology
