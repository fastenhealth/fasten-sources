...
func GetSourceAssociatedNephrologyOfNortheast(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAssociatedNephrologyOfNortheast]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Associated Nephrology of Northeast"
	sourceDef.SourceType = pkg.SourceTypeAssociatedNephrologyOfNortheast
