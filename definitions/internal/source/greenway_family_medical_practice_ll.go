...
func GetSourceGreenwayFamilyMedicalPracticeLl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGreenwayFamilyMedicalPracticeLl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Greenway Family Medical Practice LL"
	sourceDef.SourceType = pkg.SourceTypeGreenwayFamilyMedicalPracticeLl
