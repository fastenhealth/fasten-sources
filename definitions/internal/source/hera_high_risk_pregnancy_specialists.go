...
func GetSourceHeraHighRiskPregnancySpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHeraHighRiskPregnancySpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HERA HIGH RISK PREGNANCY SPECIALISTS"
	sourceDef.SourceType = pkg.SourceTypeHeraHighRiskPregnancySpecialists
