...
func GetSourceHeartCareCenterOfTheValley(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHeartCareCenterOfTheValley]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Heart Care Center of the Valley"
	sourceDef.SourceType = pkg.SourceTypeHeartCareCenterOfTheValley
	sourceDef.Category = []string{"207RC0000X"}
	sourceDef.Aliases = []string{"HEART CARE CENTER OF THE VALLEY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1194248252"}}
