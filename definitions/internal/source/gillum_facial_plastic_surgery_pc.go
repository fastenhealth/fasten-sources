...
func GetSourceGillumFacialPlasticSurgeryPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGillumFacialPlasticSurgeryPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GILLUM FACIAL PLASTIC SURGERY PC"
	sourceDef.SourceType = pkg.SourceTypeGillumFacialPlasticSurgeryPc
