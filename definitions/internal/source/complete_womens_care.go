...
func GetSourceCompleteWomensCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCompleteWomensCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Complete Womens Care"
	sourceDef.SourceType = pkg.SourceTypeCompleteWomensCare
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{"COMPLETE WOMENS CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1851339246"}}
