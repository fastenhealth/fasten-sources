...
func GetSourceEverestCenterForMensHealthLl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEverestCenterForMensHealthLl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "EveresT Center for Mens Health LL"
	sourceDef.SourceType = pkg.SourceTypeEverestCenterForMensHealthLl
