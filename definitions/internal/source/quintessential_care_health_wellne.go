...
func GetSourceQuintessentialCareHealthWellne(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeQuintessentialCareHealthWellne]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Quintessential Care Health  Wellne"
	sourceDef.SourceType = pkg.SourceTypeQuintessentialCareHealthWellne
