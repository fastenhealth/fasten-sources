...
func GetSourceTexasOrthopedicsSportsAndRehabilitationAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexasOrthopedicsSportsAndRehabilitationAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Texas Orthopedics Sports and Rehabilitation Associates"
	sourceDef.SourceType = pkg.SourceTypeTexasOrthopedicsSportsAndRehabilitationAssociates
