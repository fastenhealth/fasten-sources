...
func GetSourceCallenLordeCommunityHealthCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCallenLordeCommunityHealthCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Callen-Lorde Community Health Center"
	sourceDef.SourceType = pkg.SourceTypeCallenLordeCommunityHealthCenter
	sourceDef.Category = []string{"261QF0400X", "291U00000X"}
	sourceDef.Aliases = []string{"CALLEN-LORDE COMMUNITY HEALTH CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1790289072", "1962040246"}}
