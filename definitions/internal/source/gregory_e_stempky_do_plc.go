...
func GetSourceGregoryEStempkyDoPlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGregoryEStempkyDoPlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GREGORY E STEMPKY DO PLC"
	sourceDef.SourceType = pkg.SourceTypeGregoryEStempkyDoPlc
	sourceDef.Category = []string{"261QM2500X"}
	sourceDef.Aliases = []string{"FOREWARD HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1073114922"}}
