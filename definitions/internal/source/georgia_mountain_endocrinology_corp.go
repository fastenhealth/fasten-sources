...
func GetSourceGeorgiaMountainEndocrinologyCorp(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGeorgiaMountainEndocrinologyCorp]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Georgia Mountain Endocrinology Corp"
	sourceDef.SourceType = pkg.SourceTypeGeorgiaMountainEndocrinologyCorp
