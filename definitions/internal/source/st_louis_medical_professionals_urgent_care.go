...
func GetSourceStLouisMedicalProfessionalsUrgentCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStLouisMedicalProfessionalsUrgentCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "St Louis Medical Professionals Urgent Care"
	sourceDef.SourceType = pkg.SourceTypeStLouisMedicalProfessionalsUrgentCare
