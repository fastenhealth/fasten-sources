...
func GetSourceBonneTerreHealthcareClinicLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBonneTerreHealthcareClinicLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BONNE TERRE HEALTHCARE CLINIC LLC"
	sourceDef.SourceType = pkg.SourceTypeBonneTerreHealthcareClinicLlc
