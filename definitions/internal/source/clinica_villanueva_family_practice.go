...
func GetSourceClinicaVillanuevaFamilyPractice(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeClinicaVillanuevaFamilyPractice]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Clinica Villanueva Family Practice"
	sourceDef.SourceType = pkg.SourceTypeClinicaVillanuevaFamilyPractice
