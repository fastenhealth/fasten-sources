...
func GetSourceCc043RaleighGynecologyWellnessPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc043RaleighGynecologyWellnessPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC043RALEIGH GYNECOLOGY  WELLNESS PA"
	sourceDef.SourceType = pkg.SourceTypeCc043RaleighGynecologyWellnessPa
