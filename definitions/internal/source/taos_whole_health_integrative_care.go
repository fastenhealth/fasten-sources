...
func GetSourceTaosWholeHealthIntegrativeCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTaosWholeHealthIntegrativeCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Taos Whole Health Integrative Care"
	sourceDef.SourceType = pkg.SourceTypeTaosWholeHealthIntegrativeCare
