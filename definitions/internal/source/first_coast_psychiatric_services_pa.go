...
func GetSourceFirstCoastPsychiatricServicesPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFirstCoastPsychiatricServicesPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "First Coast Psychiatric Services Pa"
	sourceDef.SourceType = pkg.SourceTypeFirstCoastPsychiatricServicesPa
