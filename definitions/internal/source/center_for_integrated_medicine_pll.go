...
func GetSourceCenterForIntegratedMedicinePll(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForIntegratedMedicinePll]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center for Integrated Medicine PLL"
	sourceDef.SourceType = pkg.SourceTypeCenterForIntegratedMedicinePll
