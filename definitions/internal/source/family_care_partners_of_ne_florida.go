...
func GetSourceFamilyCarePartnersOfNeFlorida(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFamilyCarePartnersOfNeFlorida]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Family Care Partners of NE Florida"
	sourceDef.SourceType = pkg.SourceTypeFamilyCarePartnersOfNeFlorida
