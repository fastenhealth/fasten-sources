...
func GetSourcePrairieFieldsFamilyMedicinePc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrairieFieldsFamilyMedicinePc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Prairie Fields Family Medicine PC"
	sourceDef.SourceType = pkg.SourceTypePrairieFieldsFamilyMedicinePc
