...
func GetSourceShorepointCardiothoracicSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShorepointCardiothoracicSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ShorePoint Cardiothoracic Surgery"
	sourceDef.SourceType = pkg.SourceTypeShorepointCardiothoracicSurgery
