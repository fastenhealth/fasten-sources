...
func GetSourceNorthMetroSurgeryCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthMetroSurgeryCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "North Metro Surgery Center"
	sourceDef.SourceType = pkg.SourceTypeNorthMetroSurgeryCenter
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"NORTH METRO SURGERY CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1760616031"}}
