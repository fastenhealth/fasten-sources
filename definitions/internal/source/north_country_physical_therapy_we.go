...
func GetSourceNorthCountryPhysicalTherapyWe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthCountryPhysicalTherapyWe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTH COUNTRY PHYSICAL THERAPY  WE"
	sourceDef.SourceType = pkg.SourceTypeNorthCountryPhysicalTherapyWe
