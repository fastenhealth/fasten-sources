...
func GetSourceDermatologyCareCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDermatologyCareCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Dermatology Care Center"
	sourceDef.SourceType = pkg.SourceTypeDermatologyCareCenter
	sourceDef.Category = []string{"207N00000X"}
	sourceDef.Aliases = []string{"DERMATOLOGY CARE CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1346479250"}}
