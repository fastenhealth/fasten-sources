...
func GetSourceCompleteHealthAtGreenOaksPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCompleteHealthAtGreenOaksPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COMPLETE HEALTH AT GREEN OAKS PLLC"
	sourceDef.SourceType = pkg.SourceTypeCompleteHealthAtGreenOaksPllc
	sourceDef.Category = []string{"261QH0100X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1245833243"}}
	sourceDef.PatientAccessUrl = "http://www.completehealthgreenoaks.com"
