...
func GetSourceShorepointPrimaryCareAndWalkin(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShorepointPrimaryCareAndWalkin]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ShorePoint Primary Care and WalkIn"
	sourceDef.SourceType = pkg.SourceTypeShorepointPrimaryCareAndWalkin
