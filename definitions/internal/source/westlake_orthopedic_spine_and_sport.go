...
func GetSourceWestlakeOrthopedicSpineAndSport(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWestlakeOrthopedicSpineAndSport]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Westlake Orthopedic Spine and Sport"
	sourceDef.SourceType = pkg.SourceTypeWestlakeOrthopedicSpineAndSport
