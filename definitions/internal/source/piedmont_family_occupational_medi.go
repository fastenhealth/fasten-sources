...
func GetSourcePiedmontFamilyOccupationalMedi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePiedmontFamilyOccupationalMedi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Piedmont Family  Occupational Medi"
	sourceDef.SourceType = pkg.SourceTypePiedmontFamilyOccupationalMedi
