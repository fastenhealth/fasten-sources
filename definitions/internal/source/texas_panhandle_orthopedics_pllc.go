...
func GetSourceTexasPanhandleOrthopedicsPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexasPanhandleOrthopedicsPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "TEXAS PANHANDLE ORTHOPEDICS PLLC"
	sourceDef.SourceType = pkg.SourceTypeTexasPanhandleOrthopedicsPllc
