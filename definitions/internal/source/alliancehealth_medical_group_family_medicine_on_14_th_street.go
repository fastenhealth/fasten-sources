...
func GetSourceAlliancehealthMedicalGroupFamilyMedicineOn14ThStreet(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupFamilyMedicineOn14ThStreet]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Family Medicine on 14th Street"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupFamilyMedicineOn14ThStreet
