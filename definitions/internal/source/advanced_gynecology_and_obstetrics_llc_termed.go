...
func GetSourceAdvancedGynecologyAndObstetricsLlcTermed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedGynecologyAndObstetricsLlcTermed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ADVANCED GYNECOLOGY AND OBSTETRICS LLC TERMED"
	sourceDef.SourceType = pkg.SourceTypeAdvancedGynecologyAndObstetricsLlcTermed
