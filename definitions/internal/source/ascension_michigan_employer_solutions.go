...
func GetSourceAscensionMichiganEmployerSolutions(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAscensionMichiganEmployerSolutions]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ascension Michigan Employer Solutions"
	sourceDef.SourceType = pkg.SourceTypeAscensionMichiganEmployerSolutions
