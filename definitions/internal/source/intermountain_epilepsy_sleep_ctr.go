...
func GetSourceIntermountainEpilepsySleepCtr(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIntermountainEpilepsySleepCtr]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Intermountain Epilepsy  Sleep Ctr"
	sourceDef.SourceType = pkg.SourceTypeIntermountainEpilepsySleepCtr
