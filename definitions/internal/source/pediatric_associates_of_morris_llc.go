...
func GetSourcePediatricAssociatesOfMorrisLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePediatricAssociatesOfMorrisLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pediatric Associates of Morris LLC"
	sourceDef.SourceType = pkg.SourceTypePediatricAssociatesOfMorrisLlc
