...
func GetSourceFirstInFlightNeurology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFirstInFlightNeurology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "First in Flight Neurology"
	sourceDef.SourceType = pkg.SourceTypeFirstInFlightNeurology
	sourceDef.Category = []string{"2084N0400X"}
	sourceDef.Aliases = []string{"FIRST IN FLIGHT NEUROLOGY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1518361914"}}
