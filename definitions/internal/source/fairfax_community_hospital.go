...
func GetSourceFairfaxCommunityHospital(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFairfaxCommunityHospital]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Fairfax Community Hospital"
	sourceDef.SourceType = pkg.SourceTypeFairfaxCommunityHospital
	sourceDef.Category = []string{"275N00000X", "282NC0060X"}
	sourceDef.Aliases = []string{"FAIRFAX COMMUNITY HOSPITAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1134451396"}}
