...
func GetSourceIslamicCenterOfMorrisCountyInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIslamicCenterOfMorrisCountyInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ISLAMIC CENTER OF MORRIS COUNTY INC"
	sourceDef.SourceType = pkg.SourceTypeIslamicCenterOfMorrisCountyInc
