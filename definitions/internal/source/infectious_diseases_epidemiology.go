...
func GetSourceInfectiousDiseasesEpidemiology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInfectiousDiseasesEpidemiology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Infectious Diseases  Epidemiology"
	sourceDef.SourceType = pkg.SourceTypeInfectiousDiseasesEpidemiology
