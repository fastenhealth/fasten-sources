...
func GetSourceAlliancehealthMedicalGroupGeneralSurgeryOn14ThStreet(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupGeneralSurgeryOn14ThStreet]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group General Surgery on 14th Street"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupGeneralSurgeryOn14ThStreet
