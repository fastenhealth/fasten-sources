...
func GetSourceBroadviewMedicalCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBroadviewMedicalCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Broadview Medical Center"
	sourceDef.SourceType = pkg.SourceTypeBroadviewMedicalCenter
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{"BROADVIEW MEDICAL CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1093322984"}}
