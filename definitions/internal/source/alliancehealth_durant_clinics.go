...
func GetSourceAlliancehealthDurantClinics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthDurantClinics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Durant Clinics"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthDurantClinics
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"ALLIANCEHEALTH DURANT CLINICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1063910925"}}
