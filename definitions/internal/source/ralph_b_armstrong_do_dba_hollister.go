...
func GetSourceRalphBArmstrongDoDbaHollister(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRalphBArmstrongDoDbaHollister]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ralph B Armstrong DO dba Hollister"
	sourceDef.SourceType = pkg.SourceTypeRalphBArmstrongDoDbaHollister
