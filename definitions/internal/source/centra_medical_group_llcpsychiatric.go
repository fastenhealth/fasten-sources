...
func GetSourceCentraMedicalGroupLlcpsychiatric(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentraMedicalGroupLlcpsychiatric]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CENTRA MEDICAL GROUP LLCPSYCHIATRIC"
	sourceDef.SourceType = pkg.SourceTypeCentraMedicalGroupLlcpsychiatric
