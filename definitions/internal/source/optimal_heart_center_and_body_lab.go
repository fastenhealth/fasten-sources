...
func GetSourceOptimalHeartCenterAndBodyLab(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOptimalHeartCenterAndBodyLab]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Optimal Heart Center and Body Lab"
	sourceDef.SourceType = pkg.SourceTypeOptimalHeartCenterAndBodyLab
