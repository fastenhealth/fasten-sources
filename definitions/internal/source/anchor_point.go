...
func GetSourceAnchorPoint(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAnchorPoint]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Anchor Point"
	sourceDef.SourceType = pkg.SourceTypeAnchorPoint
	sourceDef.Category = []string{"251S00000X", "261Q00000X"}
	sourceDef.Aliases = []string{"ANCHOR POINT"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1578093613"}}
