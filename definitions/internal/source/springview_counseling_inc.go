...
func GetSourceSpringviewCounselingInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSpringviewCounselingInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Springview Counseling Inc"
	sourceDef.SourceType = pkg.SourceTypeSpringviewCounselingInc
	sourceDef.Category = []string{"101Y00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1922679810"}}
	sourceDef.PatientAccessUrl = "http://SpringviewCounseling.com"
