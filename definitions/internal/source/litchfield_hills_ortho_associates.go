...
func GetSourceLitchfieldHillsOrthoAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLitchfieldHillsOrthoAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Litchfield Hills Ortho Associates"
	sourceDef.SourceType = pkg.SourceTypeLitchfieldHillsOrthoAssociates
