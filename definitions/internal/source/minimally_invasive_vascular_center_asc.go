...
func GetSourceMinimallyInvasiveVascularCenterAsc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMinimallyInvasiveVascularCenterAsc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Minimally Invasive Vascular Center ASC"
	sourceDef.SourceType = pkg.SourceTypeMinimallyInvasiveVascularCenterAsc
