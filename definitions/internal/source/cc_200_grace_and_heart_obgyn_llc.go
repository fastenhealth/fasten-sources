...
func GetSourceCc200GraceAndHeartObgynLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc200GraceAndHeartObgynLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC200GRACE AND HEART OBGYN LLC"
	sourceDef.SourceType = pkg.SourceTypeCc200GraceAndHeartObgynLlc
