...
func GetSourceNorthCarolinaAnesthesiaPartners(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthCarolinaAnesthesiaPartners]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTH CAROLINA ANESTHESIA PARTNERS"
	sourceDef.SourceType = pkg.SourceTypeNorthCarolinaAnesthesiaPartners
