...
func GetSourceLakeNormanOrthopedicsAndHandSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLakeNormanOrthopedicsAndHandSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lake Norman Orthopedics and Hand Surgery"
	sourceDef.SourceType = pkg.SourceTypeLakeNormanOrthopedicsAndHandSurgery
