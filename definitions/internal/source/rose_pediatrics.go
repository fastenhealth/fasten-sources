...
func GetSourceRosePediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRosePediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Rose Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeRosePediatrics
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{"ROSE PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1366029274", "1669078416", "1699360313", "1992971402"}}
