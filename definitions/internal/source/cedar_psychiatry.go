...
func GetSourceCedarPsychiatry(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCedarPsychiatry]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cedar Psychiatry"
	sourceDef.SourceType = pkg.SourceTypeCedarPsychiatry
	sourceDef.Category = []string{"2084P0800X"}
	sourceDef.Aliases = []string{"CEDAR PSYCHIATRY", "FOUNDATIONS FOR CHANGE", "NUMINUS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1265092282"}}
