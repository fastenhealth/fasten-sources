...
func GetSourceCc222ObgynSpecialistsOfBrevardIiLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc222ObgynSpecialistsOfBrevardIiLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC222OBGYN SPECIALISTS OF BREVARD II LLC"
	sourceDef.SourceType = pkg.SourceTypeCc222ObgynSpecialistsOfBrevardIiLlc
