...
func GetSourceGwinnettsProgressiveHealthCareForWomen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGwinnettsProgressiveHealthCareForWomen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GWINNETTS PROGRESSIVE HEALTH CARE FOR WOMEN"
	sourceDef.SourceType = pkg.SourceTypeGwinnettsProgressiveHealthCareForWomen
