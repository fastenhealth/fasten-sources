...
func GetSourceFortSmithMedicalCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFortSmithMedicalCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Fort Smith Medical Center LLC"
	sourceDef.SourceType = pkg.SourceTypeFortSmithMedicalCenterLlc
	sourceDef.Category = []string{"363L00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1770871881"}}
	sourceDef.PatientAccessUrl = "http://www.fortsmithmedicalcenter.com"
