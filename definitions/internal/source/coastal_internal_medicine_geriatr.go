...
func GetSourceCoastalInternalMedicineGeriatr(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCoastalInternalMedicineGeriatr]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Coastal Internal Medicine  Geriatr"
	sourceDef.SourceType = pkg.SourceTypeCoastalInternalMedicineGeriatr
