...
func GetSourceCaArmstrongUrgentCareStartup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCaArmstrongUrgentCareStartup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CA  Armstrong Urgent Care  Startup"
	sourceDef.SourceType = pkg.SourceTypeCaArmstrongUrgentCareStartup
