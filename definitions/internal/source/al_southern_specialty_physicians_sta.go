...
func GetSourceAlSouthernSpecialtyPhysiciansSta(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlSouthernSpecialtyPhysiciansSta]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AL  Southern Specialty Physicians  Sta"
	sourceDef.SourceType = pkg.SourceTypeAlSouthernSpecialtyPhysiciansSta
