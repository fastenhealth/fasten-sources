...
func GetSourceCc010TidewaterPhysiciansForWomen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc010TidewaterPhysiciansForWomen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC010TIDEWATER PHYSICIANS FOR WOMEN"
	sourceDef.SourceType = pkg.SourceTypeCc010TidewaterPhysiciansForWomen
