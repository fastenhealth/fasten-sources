...
func GetSourceLouisburgFamilyPracticeAndPain(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLouisburgFamilyPracticeAndPain]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LOUISBURG FAMILY PRACTICE AND PAIN"
	sourceDef.SourceType = pkg.SourceTypeLouisburgFamilyPracticeAndPain
