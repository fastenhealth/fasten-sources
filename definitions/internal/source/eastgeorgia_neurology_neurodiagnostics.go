...
func GetSourceEastgeorgiaNeurologyNeurodiagnostics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEastgeorgiaNeurologyNeurodiagnostics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "EastGeorgia Neurology  Neurodiagnostics"
	sourceDef.SourceType = pkg.SourceTypeEastgeorgiaNeurologyNeurodiagnostics
