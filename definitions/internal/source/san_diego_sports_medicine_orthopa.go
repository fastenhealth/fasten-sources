...
func GetSourceSanDiegoSportsMedicineOrthopa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSanDiegoSportsMedicineOrthopa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "San Diego Sports Medicine  Orthopa"
	sourceDef.SourceType = pkg.SourceTypeSanDiegoSportsMedicineOrthopa
