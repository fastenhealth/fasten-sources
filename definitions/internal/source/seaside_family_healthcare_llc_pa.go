...
func GetSourceSeasideFamilyHealthcareLlcPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSeasideFamilyHealthcareLlcPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Seaside Family Healthcare LLC PA"
	sourceDef.SourceType = pkg.SourceTypeSeasideFamilyHealthcareLlcPa
