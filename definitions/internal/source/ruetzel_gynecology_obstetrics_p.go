...
func GetSourceRuetzelGynecologyObstetricsP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRuetzelGynecologyObstetricsP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RUETZEL GYNECOLOGY  OBSTETRICS P"
	sourceDef.SourceType = pkg.SourceTypeRuetzelGynecologyObstetricsP
