...
func GetSourceMeridianPhysiciansConsultingPll(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMeridianPhysiciansConsultingPll]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Meridian Physicians Consulting PLL"
	sourceDef.SourceType = pkg.SourceTypeMeridianPhysiciansConsultingPll
