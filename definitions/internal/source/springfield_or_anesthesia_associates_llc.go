...
func GetSourceSpringfieldOrAnesthesiaAssociatesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSpringfieldOrAnesthesiaAssociatesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Springfield OR Anesthesia Associates LLC"
	sourceDef.SourceType = pkg.SourceTypeSpringfieldOrAnesthesiaAssociatesLlc
	sourceDef.Category = []string{"367500000X"}
	sourceDef.Aliases = []string{"SPRINGFIELD OR ANESTHESIA"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1487158390"}}
