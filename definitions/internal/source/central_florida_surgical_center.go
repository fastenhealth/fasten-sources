...
func GetSourceCentralFloridaSurgicalCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralFloridaSurgicalCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Central Florida Surgical Center"
	sourceDef.SourceType = pkg.SourceTypeCentralFloridaSurgicalCenter
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"CENTRAL FLORIDA SURGICAL CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1386619708"}}
