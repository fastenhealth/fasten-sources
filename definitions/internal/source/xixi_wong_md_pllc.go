...
func GetSourceXixiWongMdPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeXixiWongMdPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "XiXi Wong MD PLLC"
	sourceDef.SourceType = pkg.SourceTypeXixiWongMdPllc
	sourceDef.Category = []string{"2084P0800X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1801358726"}}
	sourceDef.PatientAccessUrl = "https://www.drwongmd.com"
