...
func GetSourceNorthernPlainsAnesthesiaAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthernPlainsAnesthesiaAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTHERN PLAINS ANESTHESIA ASSOCIATES"
	sourceDef.SourceType = pkg.SourceTypeNorthernPlainsAnesthesiaAssociates
