...
func GetSourceEastMetroFamilyMedicalClinicP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEastMetroFamilyMedicalClinicP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "East Metro Family Medical Clinic P"
	sourceDef.SourceType = pkg.SourceTypeEastMetroFamilyMedicalClinicP
