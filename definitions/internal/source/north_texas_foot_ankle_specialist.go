...
func GetSourceNorthTexasFootAnkleSpecialist(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthTexasFootAnkleSpecialist]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Texas Foot  Ankle Specialist"
	sourceDef.SourceType = pkg.SourceTypeNorthTexasFootAnkleSpecialist
