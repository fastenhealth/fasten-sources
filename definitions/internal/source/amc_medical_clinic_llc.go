...
func GetSourceAmcMedicalClinicLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmcMedicalClinicLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AMC Medical Clinic LLC"
	sourceDef.SourceType = pkg.SourceTypeAmcMedicalClinicLlc
	sourceDef.Category = []string{"207Q00000X", "261QR1300X"}
	sourceDef.Aliases = []string{"AMC MEDICAL CLINIC LLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1295368587"}}
