...
func GetSourcePeninsulaOrthopaedicAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePeninsulaOrthopaedicAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Peninsula Orthopaedic Associates"
	sourceDef.SourceType = pkg.SourceTypePeninsulaOrthopaedicAssociates
