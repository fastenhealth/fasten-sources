...
func GetSourceGreenwichPodiatryGroupLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGreenwichPodiatryGroupLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Greenwich Podiatry Group LLC"
	sourceDef.SourceType = pkg.SourceTypeGreenwichPodiatryGroupLlc
	sourceDef.Category = []string{"213E00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1285990986"}}
	sourceDef.PatientAccessUrl = "http://greenwichpodiatrygroup.com"
