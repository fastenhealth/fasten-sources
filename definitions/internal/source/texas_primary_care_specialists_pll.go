...
func GetSourceTexasPrimaryCareSpecialistsPll(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexasPrimaryCareSpecialistsPll]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Texas Primary Care Specialists PLL"
	sourceDef.SourceType = pkg.SourceTypeTexasPrimaryCareSpecialistsPll
