...
func GetSourceOrthopedicInsOfRioGrandeValley(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOrthopedicInsOfRioGrandeValley]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Orthopedic Ins of Rio Grande Valley"
	sourceDef.SourceType = pkg.SourceTypeOrthopedicInsOfRioGrandeValley
