...
func GetSourceInvogueTotalWomensHealthcarePl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInvogueTotalWomensHealthcarePl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Invogue Total Womens Healthcare PL"
	sourceDef.SourceType = pkg.SourceTypeInvogueTotalWomensHealthcarePl
