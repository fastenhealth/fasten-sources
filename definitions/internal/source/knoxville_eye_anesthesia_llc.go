...
func GetSourceKnoxvilleEyeAnesthesiaLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKnoxvilleEyeAnesthesiaLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Knoxville Eye Anesthesia LLC"
	sourceDef.SourceType = pkg.SourceTypeKnoxvilleEyeAnesthesiaLlc
	sourceDef.Category = []string{"367500000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1275084386"}}
