...
func GetSourceCraigRanchOrthopaedicsAndSportsMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCraigRanchOrthopaedicsAndSportsMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CRAIG RANCH ORTHOPAEDICS AND SPORTS MEDICINE"
	sourceDef.SourceType = pkg.SourceTypeCraigRanchOrthopaedicsAndSportsMedicine
