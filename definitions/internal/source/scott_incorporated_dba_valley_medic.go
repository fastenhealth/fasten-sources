...
func GetSourceScottIncorporatedDbaValleyMedic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeScottIncorporatedDbaValleyMedic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Scott Incorporated DBA Valley Medic"
	sourceDef.SourceType = pkg.SourceTypeScottIncorporatedDbaValleyMedic
