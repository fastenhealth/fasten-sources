...
func GetSourceWinstonMedicalCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWinstonMedicalCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Winston Medical Center"
	sourceDef.SourceType = pkg.SourceTypeWinstonMedicalCenter
	sourceDef.Category = []string{"282N00000X", "314000000X"}
	sourceDef.Aliases = []string{"WINSTON MEDICAL CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1073597126", "1316078108", "1427189216"}}
