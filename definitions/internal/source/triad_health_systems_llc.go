...
func GetSourceTriadHealthSystemsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTriadHealthSystemsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Triad Health Systems LLC"
	sourceDef.SourceType = pkg.SourceTypeTriadHealthSystemsLlc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"TRIAD COMPLETE HEALTHCARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1245867258"}}
