...
func GetSourceRetreatDoctorsOccupationalHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRetreatDoctorsOccupationalHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Retreat Doctors Occupational Health"
	sourceDef.SourceType = pkg.SourceTypeRetreatDoctorsOccupationalHealth
