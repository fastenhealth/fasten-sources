...
func GetSourceCareExcellenceLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCareExcellenceLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Care Excellence LLC"
	sourceDef.SourceType = pkg.SourceTypeCareExcellenceLlc
	sourceDef.Category = []string{"207QA0505X"}
	sourceDef.Aliases = []string{"CARE EXCELLENCE LLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1952950453"}}
	sourceDef.PatientAccessUrl = "http://www.careexcellencellc.com"
