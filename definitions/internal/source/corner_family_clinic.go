...
func GetSourceCornerFamilyClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCornerFamilyClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Corner Family Clinic"
	sourceDef.SourceType = pkg.SourceTypeCornerFamilyClinic
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{"CORNER FAMILY CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1437884830"}}
	sourceDef.PatientAccessUrl = "http://cornerfamilyclinic.com"
