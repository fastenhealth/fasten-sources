...
func GetSourceAbqDnaLlcDbaZiaMedicalConcie(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAbqDnaLlcDbaZiaMedicalConcie]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ABQ DNA LLC dba Zia Medical Concie"
	sourceDef.SourceType = pkg.SourceTypeAbqDnaLlcDbaZiaMedicalConcie
