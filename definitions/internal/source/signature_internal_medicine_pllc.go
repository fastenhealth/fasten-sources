...
func GetSourceSignatureInternalMedicinePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSignatureInternalMedicinePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SIGNATURE INTERNAL MEDICINE PLLC"
	sourceDef.SourceType = pkg.SourceTypeSignatureInternalMedicinePllc
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1215681176"}}
	sourceDef.PatientAccessUrl = "http://www.signatureinternalmedicine.com"
