...
func GetSourceLouisianaHeartAndVascularInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLouisianaHeartAndVascularInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Louisiana Heart and Vascular institute"
	sourceDef.SourceType = pkg.SourceTypeLouisianaHeartAndVascularInstitute
