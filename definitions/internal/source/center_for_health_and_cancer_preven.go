...
func GetSourceCenterForHealthAndCancerPreven(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForHealthAndCancerPreven]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center for Health and Cancer Preven"
	sourceDef.SourceType = pkg.SourceTypeCenterForHealthAndCancerPreven
