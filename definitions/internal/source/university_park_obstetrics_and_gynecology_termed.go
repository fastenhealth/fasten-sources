...
func GetSourceUniversityParkObstetricsAndGynecologyTermed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUniversityParkObstetricsAndGynecologyTermed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "UNIVERSITY PARK OBSTETRICS AND GYNECOLOGY TERMED"
	sourceDef.SourceType = pkg.SourceTypeUniversityParkObstetricsAndGynecologyTermed
