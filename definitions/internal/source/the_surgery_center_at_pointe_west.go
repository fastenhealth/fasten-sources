...
func GetSourceTheSurgeryCenterAtPointeWest(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheSurgeryCenterAtPointeWest]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "The Surgery Center At Pointe West"
	sourceDef.SourceType = pkg.SourceTypeTheSurgeryCenterAtPointeWest
