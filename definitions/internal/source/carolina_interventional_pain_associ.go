...
func GetSourceCarolinaInterventionalPainAssoci(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCarolinaInterventionalPainAssoci]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Carolina Interventional Pain Associ"
	sourceDef.SourceType = pkg.SourceTypeCarolinaInterventionalPainAssoci
