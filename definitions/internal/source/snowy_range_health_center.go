...
func GetSourceSnowyRangeHealthCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSnowyRangeHealthCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Snowy Range Health Center"
	sourceDef.SourceType = pkg.SourceTypeSnowyRangeHealthCenter
	sourceDef.Category = []string{"261Q00000X", "261QP2300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1831580240"}}
	sourceDef.PatientAccessUrl = "http://www.snowyrangehealthcenter.com"
