...
func GetSourcePrimePrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrimePrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Prime Primary Care"
	sourceDef.SourceType = pkg.SourceTypePrimePrimaryCare
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"PRIME PRIMARY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1477286979"}}
	sourceDef.PatientAccessUrl = "http://www.primeprimarycare.com"
