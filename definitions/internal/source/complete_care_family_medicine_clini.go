...
func GetSourceCompleteCareFamilyMedicineClini(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCompleteCareFamilyMedicineClini]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Complete Care Family Medicine Clini"
	sourceDef.SourceType = pkg.SourceTypeCompleteCareFamilyMedicineClini
