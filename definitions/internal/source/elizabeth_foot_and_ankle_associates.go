...
func GetSourceElizabethFootAndAnkleAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeElizabethFootAndAnkleAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Elizabeth Foot and Ankle Associates"
	sourceDef.SourceType = pkg.SourceTypeElizabethFootAndAnkleAssociates
