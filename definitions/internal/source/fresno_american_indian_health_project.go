...
func GetSourceFresnoAmericanIndianHealthProject(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFresnoAmericanIndianHealthProject]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Fresno American Indian Health Project"
	sourceDef.SourceType = pkg.SourceTypeFresnoAmericanIndianHealthProject
	sourceDef.Category = []string{"261QF0400X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1427317981"}}
