...
func GetSourceMichiganOrthopaedicSurgeons(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMichiganOrthopaedicSurgeons]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Michigan Orthopaedic Surgeons"
	sourceDef.SourceType = pkg.SourceTypeMichiganOrthopaedicSurgeons
	sourceDef.Category = []string{"207X00000X"}
	sourceDef.Aliases = []string{"MICHIGAN ORTHOPAEDIC SURGEONS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1811562184"}}
