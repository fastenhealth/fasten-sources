...
func GetSourceSouthernUtahKidneyAndHypertensionCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernUtahKidneyAndHypertensionCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southern Utah Kidney and Hypertension Center"
	sourceDef.SourceType = pkg.SourceTypeSouthernUtahKidneyAndHypertensionCenter
