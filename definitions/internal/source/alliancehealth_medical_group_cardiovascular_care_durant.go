...
func GetSourceAlliancehealthMedicalGroupCardiovascularCareDurant(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupCardiovascularCareDurant]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Cardiovascular Care Durant"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupCardiovascularCareDurant
