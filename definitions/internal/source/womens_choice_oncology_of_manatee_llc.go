...
func GetSourceWomensChoiceOncologyOfManateeLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWomensChoiceOncologyOfManateeLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WOMENS CHOICE ONCOLOGY OF MANATEE LLC"
	sourceDef.SourceType = pkg.SourceTypeWomensChoiceOncologyOfManateeLlc
