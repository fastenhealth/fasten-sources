...
func GetSourceNorthwestMedicalPlazaBentonville(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestMedicalPlazaBentonville]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Medical Plaza  Bentonville"
	sourceDef.SourceType = pkg.SourceTypeNorthwestMedicalPlazaBentonville
