...
func GetSourceCommunityUrgentCarePlus(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCommunityUrgentCarePlus]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Community Urgent Care Plus"
	sourceDef.SourceType = pkg.SourceTypeCommunityUrgentCarePlus
	sourceDef.Category = []string{"261QU0200X"}
	sourceDef.Aliases = []string{"COMMUNITY URGENT CARE PLUS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1194365932"}}
