...
func GetSourceKernPrimaryCareMedicalClinicIn(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKernPrimaryCareMedicalClinicIn]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kern Primary Care Medical Clinic In"
	sourceDef.SourceType = pkg.SourceTypeKernPrimaryCareMedicalClinicIn
