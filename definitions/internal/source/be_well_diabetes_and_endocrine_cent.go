...
func GetSourceBeWellDiabetesAndEndocrineCent(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBeWellDiabetesAndEndocrineCent]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BE WELL DIABETES AND ENDOCRINE CENT"
	sourceDef.SourceType = pkg.SourceTypeBeWellDiabetesAndEndocrineCent
