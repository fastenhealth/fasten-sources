...
func GetSourceJacksonMedicalCenterUrgentCareClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJacksonMedicalCenterUrgentCareClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Jackson Medical Center Urgent Care Clinic"
	sourceDef.SourceType = pkg.SourceTypeJacksonMedicalCenterUrgentCareClinic
