...
func GetSourceCrossgatesRiverOaksHospitalists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCrossgatesRiverOaksHospitalists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Crossgates River Oaks Hospitalists"
	sourceDef.SourceType = pkg.SourceTypeCrossgatesRiverOaksHospitalists
