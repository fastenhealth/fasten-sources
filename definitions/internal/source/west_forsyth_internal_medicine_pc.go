...
func GetSourceWestForsythInternalMedicinePc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWestForsythInternalMedicinePc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WEST FORSYTH INTERNAL MEDICINE PC"
	sourceDef.SourceType = pkg.SourceTypeWestForsythInternalMedicinePc
