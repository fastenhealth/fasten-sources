...
func GetSourceHolisticLifeServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHolisticLifeServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Holistic Life Services"
	sourceDef.SourceType = pkg.SourceTypeHolisticLifeServices
	sourceDef.Category = []string{"261QM0801X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1114557022"}}
	sourceDef.PatientAccessUrl = "https://holisticlifeservice.com/"
