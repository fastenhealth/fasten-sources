...
func GetSourcePrimeHealthcareGadsdenPhysicianManagementLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrimeHealthcareGadsdenPhysicianManagementLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PRIME HEALTHCARE  GADSDEN PHYSICIAN MANAGEMENT LLC"
	sourceDef.SourceType = pkg.SourceTypePrimeHealthcareGadsdenPhysicianManagementLlc
