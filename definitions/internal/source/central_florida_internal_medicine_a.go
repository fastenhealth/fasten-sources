...
func GetSourceCentralFloridaInternalMedicineA(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralFloridaInternalMedicineA]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Central Florida Internal Medicine a"
	sourceDef.SourceType = pkg.SourceTypeCentralFloridaInternalMedicineA
