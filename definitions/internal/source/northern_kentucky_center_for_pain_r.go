...
func GetSourceNorthernKentuckyCenterForPainR(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthernKentuckyCenterForPainR]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northern Kentucky Center for Pain R"
	sourceDef.SourceType = pkg.SourceTypeNorthernKentuckyCenterForPainR
