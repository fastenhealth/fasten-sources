...
func GetSourceBabylonVillageMedicalAssocLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBabylonVillageMedicalAssocLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Babylon Village Medical Assoc LLC"
	sourceDef.SourceType = pkg.SourceTypeBabylonVillageMedicalAssocLlc
