...
func GetSourceCc223SchwartzbardSalazarFeldmanObgyn(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc223SchwartzbardSalazarFeldmanObgyn]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC223SCHWARTZBARD SALAZAR FELDMAN OBGYN"
	sourceDef.SourceType = pkg.SourceTypeCc223SchwartzbardSalazarFeldmanObgyn
