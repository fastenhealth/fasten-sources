...
func GetSourceHdaiSpecialtyAccessCenterSacMecurahealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHdaiSpecialtyAccessCenterSacMecurahealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HDAI Specialty Access Center SAC  MecuraHealth"
	sourceDef.SourceType = pkg.SourceTypeHdaiSpecialtyAccessCenterSacMecurahealth
