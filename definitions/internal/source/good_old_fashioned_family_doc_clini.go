...
func GetSourceGoodOldFashionedFamilyDocClini(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGoodOldFashionedFamilyDocClini]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Good Old Fashioned Family Doc Clini"
	sourceDef.SourceType = pkg.SourceTypeGoodOldFashionedFamilyDocClini
