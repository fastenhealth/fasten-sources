...
func GetSourceSmmcMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSmmcMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SMMC Medical Group"
	sourceDef.SourceType = pkg.SourceTypeSmmcMedicalGroup
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"SMMC GREGG STREET CLINIC", "SMMC MEDICAL GROUP"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1194239905"}}
