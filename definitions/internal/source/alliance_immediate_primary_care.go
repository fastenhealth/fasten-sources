...
func GetSourceAllianceImmediatePrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllianceImmediatePrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ALLIANCE IMMEDIATE  PRIMARY CARE"
	sourceDef.SourceType = pkg.SourceTypeAllianceImmediatePrimaryCare
