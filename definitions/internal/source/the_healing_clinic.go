...
func GetSourceTheHealingClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheHealingClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Healing Clinic"
	sourceDef.SourceType = pkg.SourceTypeTheHealingClinic
	sourceDef.Category = []string{"261QH0100X"}
	sourceDef.Aliases = []string{"THE HEALING CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1104514942"}}
