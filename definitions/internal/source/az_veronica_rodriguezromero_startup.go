...
func GetSourceAzVeronicaRodriguezromeroStartup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAzVeronicaRodriguezromeroStartup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AZ  Veronica RodriguezRomero  Startup"
	sourceDef.SourceType = pkg.SourceTypeAzVeronicaRodriguezromeroStartup
