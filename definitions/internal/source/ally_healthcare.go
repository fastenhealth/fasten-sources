...
func GetSourceAllyHealthcare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllyHealthcare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ally Healthcare"
	sourceDef.SourceType = pkg.SourceTypeAllyHealthcare
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"ALLY HEALTHCARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1538835103"}}
