...
func GetSourceStLucieCountyHealthAccessNetw(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStLucieCountyHealthAccessNetw]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "St Lucie County Health Access Netw"
	sourceDef.SourceType = pkg.SourceTypeStLucieCountyHealthAccessNetw
