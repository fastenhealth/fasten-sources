...
func GetSourceAlliancehealthMedicalGroupPediatricsDurantCampus(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupPediatricsDurantCampus]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Pediatrics Durant Campus"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupPediatricsDurantCampus
