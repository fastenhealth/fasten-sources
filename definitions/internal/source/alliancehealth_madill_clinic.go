...
func GetSourceAlliancehealthMadillClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMadillClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Madill Clinic"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMadillClinic
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"ALLIANCEHEALTH MADILL CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1992246565"}}
