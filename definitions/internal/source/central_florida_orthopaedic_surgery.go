...
func GetSourceCentralFloridaOrthopaedicSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralFloridaOrthopaedicSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Central Florida Orthopaedic Surgery"
	sourceDef.SourceType = pkg.SourceTypeCentralFloridaOrthopaedicSurgery
