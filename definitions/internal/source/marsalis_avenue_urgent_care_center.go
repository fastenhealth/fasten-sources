...
func GetSourceMarsalisAvenueUrgentCareCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMarsalisAvenueUrgentCareCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Marsalis Avenue Urgent Care Center"
	sourceDef.SourceType = pkg.SourceTypeMarsalisAvenueUrgentCareCenter
