...
func GetSourceTrinityWomensCenterDrSmithblair(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTrinityWomensCenterDrSmithblair]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Trinity Womens Center Dr SmithBlair"
	sourceDef.SourceType = pkg.SourceTypeTrinityWomensCenterDrSmithblair
