...
func GetSourceCoastalGeorgiaWellnessCenterAnd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCoastalGeorgiaWellnessCenterAnd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Coastal Georgia Wellness Center and"
	sourceDef.SourceType = pkg.SourceTypeCoastalGeorgiaWellnessCenterAnd
