...
func GetSourceAnesthesia1(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAnesthesia1]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Anesthesia"
	sourceDef.SourceType = pkg.SourceTypeAnesthesia1
	sourceDef.Category = []string{"207L00000X", "208800000X", "367500000X"}
	sourceDef.Aliases = []string{"ANESTHESIA", "UROLOGY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1225547979", "1265181044", "1295244226", "1831780394"}}
