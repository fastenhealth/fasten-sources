...
func GetSourceFamilyFirstHealthcareClinicPll(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFamilyFirstHealthcareClinicPll]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Family First Healthcare Clinic PLL"
	sourceDef.SourceType = pkg.SourceTypeFamilyFirstHealthcareClinicPll
