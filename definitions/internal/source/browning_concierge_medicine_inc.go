...
func GetSourceBrowningConciergeMedicineInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBrowningConciergeMedicineInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BROWNING CONCIERGE MEDICINE INC"
	sourceDef.SourceType = pkg.SourceTypeBrowningConciergeMedicineInc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1356033401"}}
