...
func GetSourcePreferredPrimaryCareNewMilford(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePreferredPrimaryCareNewMilford]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Preferred Primary Care New Milford"
	sourceDef.SourceType = pkg.SourceTypePreferredPrimaryCareNewMilford
