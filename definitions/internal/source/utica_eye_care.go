...
func GetSourceUticaEyeCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUticaEyeCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Utica Eye Care"
	sourceDef.SourceType = pkg.SourceTypeUticaEyeCare
	sourceDef.Category = []string{"207W00000X"}
	sourceDef.Aliases = []string{"UTICA EYE CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1821036823"}}
