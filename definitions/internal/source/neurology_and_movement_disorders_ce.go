...
func GetSourceNeurologyAndMovementDisordersCe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeurologyAndMovementDisordersCe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEUROLOGY AND MOVEMENT DISORDERS CE"
	sourceDef.SourceType = pkg.SourceTypeNeurologyAndMovementDisordersCe
