...
func GetSourceBradfordFamilyHealthcarePc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBradfordFamilyHealthcarePc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bradford Family Healthcare PC"
	sourceDef.SourceType = pkg.SourceTypeBradfordFamilyHealthcarePc
	sourceDef.Category = []string{"261QH0100X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1407284532"}}
