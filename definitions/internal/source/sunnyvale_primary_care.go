...
func GetSourceSunnyvalePrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSunnyvalePrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sunnyvale Primary Care"
	sourceDef.SourceType = pkg.SourceTypeSunnyvalePrimaryCare
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"SUNNYVALE PRIMARY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1679164495"}}
