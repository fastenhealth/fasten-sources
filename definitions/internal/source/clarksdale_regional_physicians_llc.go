...
func GetSourceClarksdaleRegionalPhysiciansLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeClarksdaleRegionalPhysiciansLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Clarksdale Regional Physicians LLC"
	sourceDef.SourceType = pkg.SourceTypeClarksdaleRegionalPhysiciansLlc
