...
func GetSourceWesternArizonaPulmonaryCriticalCareSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWesternArizonaPulmonaryCriticalCareSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Western Arizona Pulmonary Critical Care Specialists"
	sourceDef.SourceType = pkg.SourceTypeWesternArizonaPulmonaryCriticalCareSpecialists
