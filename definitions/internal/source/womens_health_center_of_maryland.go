...
func GetSourceWomensHealthCenterOfMaryland(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWomensHealthCenterOfMaryland]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WOMENS HEALTH CENTER OF MARYLAND"
	sourceDef.SourceType = pkg.SourceTypeWomensHealthCenterOfMaryland
