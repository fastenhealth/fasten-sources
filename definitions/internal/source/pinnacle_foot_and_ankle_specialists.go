...
func GetSourcePinnacleFootAndAnkleSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePinnacleFootAndAnkleSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pinnacle Foot and Ankle Specialists"
	sourceDef.SourceType = pkg.SourceTypePinnacleFootAndAnkleSpecialists
	sourceDef.Category = []string{"261QP1100X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1548778749"}}
