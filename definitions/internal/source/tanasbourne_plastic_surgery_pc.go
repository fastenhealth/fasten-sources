...
func GetSourceTanasbournePlasticSurgeryPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTanasbournePlasticSurgeryPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tanasbourne Plastic Surgery PC"
	sourceDef.SourceType = pkg.SourceTypeTanasbournePlasticSurgeryPc
	sourceDef.Category = []string{"208200000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1174708457"}}
	sourceDef.PatientAccessUrl = "http://www.tanasbourneplasticsurgery.com/"
