...
func GetSourceCaliforniaAdvancedPainAndSpineSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCaliforniaAdvancedPainAndSpineSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "California Advanced Pain and Spine Specialists"
	sourceDef.SourceType = pkg.SourceTypeCaliforniaAdvancedPainAndSpineSpecialists
