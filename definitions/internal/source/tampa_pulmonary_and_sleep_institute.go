...
func GetSourceTampaPulmonaryAndSleepInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTampaPulmonaryAndSleepInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "TAMPA PULMONARY AND SLEEP INSTITUTE"
	sourceDef.SourceType = pkg.SourceTypeTampaPulmonaryAndSleepInstitute
