...
func GetSourceCc505HeightsObstetricsAndGynecology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc505HeightsObstetricsAndGynecology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC505HEIGHTS OBSTETRICS AND GYNECOLOGY"
	sourceDef.SourceType = pkg.SourceTypeCc505HeightsObstetricsAndGynecology
