...
func GetSourceCompassionateFamilyUrgentCareLl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCompassionateFamilyUrgentCareLl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COMPASSIONATE FAMILY URGENT CARE LL"
	sourceDef.SourceType = pkg.SourceTypeCompassionateFamilyUrgentCareLl
