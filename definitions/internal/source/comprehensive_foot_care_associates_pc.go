...
func GetSourceComprehensiveFootCareAssociatesPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeComprehensiveFootCareAssociatesPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COMPREHENSIVE FOOT CARE ASSOCIATES PC"
	sourceDef.SourceType = pkg.SourceTypeComprehensiveFootCareAssociatesPc
