...
func GetSourceEmeraldCoastObgynOfThePanhandleLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEmeraldCoastObgynOfThePanhandleLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "EMERALD COAST OBGYN OF THE PANHANDLE LLC"
	sourceDef.SourceType = pkg.SourceTypeEmeraldCoastObgynOfThePanhandleLlc
