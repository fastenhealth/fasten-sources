...
func GetSourceKidneySpecialistsOfGeorgiaPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKidneySpecialistsOfGeorgiaPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kidney Specialists of Georgia PC"
	sourceDef.SourceType = pkg.SourceTypeKidneySpecialistsOfGeorgiaPc
	sourceDef.Category = []string{"207RN0300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1063070654"}}
