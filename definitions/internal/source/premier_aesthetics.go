...
func GetSourcePremierAesthetics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePremierAesthetics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Premier Aesthetics"
	sourceDef.SourceType = pkg.SourceTypePremierAesthetics
	sourceDef.Category = []string{"363L00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1134862311"}}
	sourceDef.PatientAccessUrl = "http://www.premieraestheticsva.com"
