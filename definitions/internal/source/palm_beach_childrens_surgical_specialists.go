...
func GetSourcePalmBeachChildrensSurgicalSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePalmBeachChildrensSurgicalSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Palm Beach Childrens Surgical Specialists"
	sourceDef.SourceType = pkg.SourceTypePalmBeachChildrensSurgicalSpecialists
