...
func GetSourceHealingJourneyTherapyServicesL(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHealingJourneyTherapyServicesL]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Healing Journey Therapy Services L"
	sourceDef.SourceType = pkg.SourceTypeHealingJourneyTherapyServicesL
