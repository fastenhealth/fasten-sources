...
func GetSourceLakeNormanOrthopedicSportsMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLakeNormanOrthopedicSportsMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lake Norman Orthopedic  Sports Medicine"
	sourceDef.SourceType = pkg.SourceTypeLakeNormanOrthopedicSportsMedicine
