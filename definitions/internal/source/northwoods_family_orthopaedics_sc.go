...
func GetSourceNorthwoodsFamilyOrthopaedicsSc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwoodsFamilyOrthopaedicsSc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwoods Family Orthopaedics SC"
	sourceDef.SourceType = pkg.SourceTypeNorthwoodsFamilyOrthopaedicsSc
