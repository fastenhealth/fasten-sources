...
func GetSourceOrthocarePainAndRehabilitationMedicineLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOrthocarePainAndRehabilitationMedicineLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "OrthoCare Pain and Rehabilitation Medicine LLC"
	sourceDef.SourceType = pkg.SourceTypeOrthocarePainAndRehabilitationMedicineLlc
	sourceDef.Category = []string{"208100000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1386131555"}}
