...
func GetSourceNavarroPediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNavarroPediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Navarro Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeNavarroPediatrics
	sourceDef.Category = []string{"208000000X", "261QR1300X"}
	sourceDef.Aliases = []string{"NAVARRO PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1114370632", "1851842736"}}
