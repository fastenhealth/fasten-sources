...
func GetSourceHiltonHeadRegionalPhysicianNetwork(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHiltonHeadRegionalPhysicianNetwork]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hilton Head Regional Physician Network"
	sourceDef.SourceType = pkg.SourceTypeHiltonHeadRegionalPhysicianNetwork
