...
func GetSourceMarkeyCancerCenterAtLexingtonClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMarkeyCancerCenterAtLexingtonClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Markey Cancer Center at Lexington Clinic"
	sourceDef.SourceType = pkg.SourceTypeMarkeyCancerCenterAtLexingtonClinic
