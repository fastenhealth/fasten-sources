...
func GetSourceTexasIntegrativePainInstituteIn(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexasIntegrativePainInstituteIn]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Texas Integrative Pain Institute In"
	sourceDef.SourceType = pkg.SourceTypeTexasIntegrativePainInstituteIn
