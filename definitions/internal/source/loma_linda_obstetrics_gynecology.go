...
func GetSourceLomaLindaObstetricsGynecology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLomaLindaObstetricsGynecology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Loma Linda Obstetrics  Gynecology"
	sourceDef.SourceType = pkg.SourceTypeLomaLindaObstetricsGynecology
