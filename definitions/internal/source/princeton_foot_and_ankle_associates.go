...
func GetSourcePrincetonFootAndAnkleAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrincetonFootAndAnkleAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PRINCETON FOOT AND ANKLE ASSOCIATES"
	sourceDef.SourceType = pkg.SourceTypePrincetonFootAndAnkleAssociates
	sourceDef.Category = []string{"213E00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1528256187", "1538114475"}}
