...
func GetSourceMedicalCenterUrologyPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMedicalCenterUrologyPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Medical Center Urology PA"
	sourceDef.SourceType = pkg.SourceTypeMedicalCenterUrologyPa
	sourceDef.Category = []string{"208800000X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1639285943"}}
	sourceDef.PatientAccessUrl = "https://www.medicalcenterurology.com/gallery"
