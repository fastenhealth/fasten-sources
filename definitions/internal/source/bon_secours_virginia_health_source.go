...
func GetSourceBonSecoursVirginiaHealthSource(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBonSecoursVirginiaHealthSource]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bon Secours Virginia Health Source"
	sourceDef.SourceType = pkg.SourceTypeBonSecoursVirginiaHealthSource
