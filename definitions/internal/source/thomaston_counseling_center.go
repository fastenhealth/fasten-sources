...
func GetSourceThomastonCounselingCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeThomastonCounselingCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Thomaston Counseling Center"
	sourceDef.SourceType = pkg.SourceTypeThomastonCounselingCenter
	sourceDef.Category = []string{"101YM0800X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1356824502"}}
	sourceDef.PatientAccessUrl = "http://www.thomastoncounseling.com"
