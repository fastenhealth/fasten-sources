...
func GetSourceEmkeyArthritisOsteoporosisClin(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEmkeyArthritisOsteoporosisClin]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Emkey Arthritis  Osteoporosis Clin"
	sourceDef.SourceType = pkg.SourceTypeEmkeyArthritisOsteoporosisClin
