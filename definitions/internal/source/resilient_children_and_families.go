...
func GetSourceResilientChildrenAndFamilies(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeResilientChildrenAndFamilies]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Resilient Children and Families"
	sourceDef.SourceType = pkg.SourceTypeResilientChildrenAndFamilies
	sourceDef.Category = []string{"261QM0801X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1043920838"}}
	sourceDef.PatientAccessUrl = "http://www.resilient-children.com"
