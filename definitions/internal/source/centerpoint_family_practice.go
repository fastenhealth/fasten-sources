...
func GetSourceCenterpointFamilyPractice(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterpointFamilyPractice]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Centerpoint Family Practice"
	sourceDef.SourceType = pkg.SourceTypeCenterpointFamilyPractice
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"CENTERPOINT FAMILY PRACTICE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1124659164"}}
