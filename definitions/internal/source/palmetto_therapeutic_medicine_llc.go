...
func GetSourcePalmettoTherapeuticMedicineLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePalmettoTherapeuticMedicineLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PALMETTO THERAPEUTIC MEDICINE LLC"
	sourceDef.SourceType = pkg.SourceTypePalmettoTherapeuticMedicineLlc
