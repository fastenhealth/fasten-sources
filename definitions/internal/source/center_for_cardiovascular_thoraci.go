...
func GetSourceCenterForCardiovascularThoraci(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForCardiovascularThoraci]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center For Cardiovascular  Thoraci"
	sourceDef.SourceType = pkg.SourceTypeCenterForCardiovascularThoraci
