...
func GetSourceCc205FloridaMammographyCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc205FloridaMammographyCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC205FLORIDA MAMMOGRAPHY CENTER LLC"
	sourceDef.SourceType = pkg.SourceTypeCc205FloridaMammographyCenterLlc
