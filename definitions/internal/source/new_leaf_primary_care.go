...
func GetSourceNewLeafPrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNewLeafPrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "New Leaf Primary Care"
	sourceDef.SourceType = pkg.SourceTypeNewLeafPrimaryCare
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"NEW LEAF PRIMARY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1619409075"}}
