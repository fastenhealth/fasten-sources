...
func GetSourceTexas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Texas"
	sourceDef.SourceType = pkg.SourceTypeTexas
	sourceDef.Category = []string{"171M00000X", "207QH0002X", "207RH0002X", "2080N0001X"}
	sourceDef.Aliases = []string{"TEXAS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1558866814", "1730562091"}}
