...
func GetSourceBatesvilleRegionalMedCenterRhcs(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBatesvilleRegionalMedCenterRhcs]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Batesville Regional Med Center  RHCs"
	sourceDef.SourceType = pkg.SourceTypeBatesvilleRegionalMedCenterRhcs
