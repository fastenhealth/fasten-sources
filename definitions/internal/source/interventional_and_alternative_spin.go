...
func GetSourceInterventionalAndAlternativeSpin(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInterventionalAndAlternativeSpin]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Interventional and Alternative Spin"
	sourceDef.SourceType = pkg.SourceTypeInterventionalAndAlternativeSpin
