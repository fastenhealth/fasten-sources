...
func GetSourceCc045ArboretumObstetricsGynecologyPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc045ArboretumObstetricsGynecologyPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC045ARBORETUM OBSTETRICS  GYNECOLOGY PA"
	sourceDef.SourceType = pkg.SourceTypeCc045ArboretumObstetricsGynecologyPa
