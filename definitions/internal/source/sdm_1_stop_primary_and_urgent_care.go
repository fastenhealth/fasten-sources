...
func GetSourceSdm1StopPrimaryAndUrgentCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSdm1StopPrimaryAndUrgentCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SDM1 Stop Primary and Urgent Care"
	sourceDef.SourceType = pkg.SourceTypeSdm1StopPrimaryAndUrgentCare
