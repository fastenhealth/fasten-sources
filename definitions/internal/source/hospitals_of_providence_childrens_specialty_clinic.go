...
func GetSourceHospitalsOfProvidenceChildrensSpecialtyClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHospitalsOfProvidenceChildrensSpecialtyClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hospitals of Providence Childrens Specialty Clinic"
	sourceDef.SourceType = pkg.SourceTypeHospitalsOfProvidenceChildrensSpecialtyClinic
