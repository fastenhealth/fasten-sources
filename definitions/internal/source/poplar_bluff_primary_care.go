...
func GetSourcePoplarBluffPrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePoplarBluffPrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Poplar Bluff Primary Care"
	sourceDef.SourceType = pkg.SourceTypePoplarBluffPrimaryCare
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"POPLAR BLUFF PRIMARY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1225380108"}}
