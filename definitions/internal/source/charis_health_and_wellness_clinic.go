...
func GetSourceCharisHealthAndWellnessClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCharisHealthAndWellnessClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Charis Health And Wellness Clinic"
	sourceDef.SourceType = pkg.SourceTypeCharisHealthAndWellnessClinic
