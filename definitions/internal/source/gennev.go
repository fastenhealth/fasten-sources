...
func GetSourceGennev(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGennev]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gennev"
	sourceDef.SourceType = pkg.SourceTypeGennev
	sourceDef.Category = []string{"207V00000X", "207VG0400X"}
	sourceDef.Aliases = []string{"GENNEV"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1336888163", "1952048654"}}
