...
func GetSourceAlliancehealthMedicalGroupSpecialistsDurant(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupSpecialistsDurant]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Specialists  Durant"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupSpecialistsDurant
