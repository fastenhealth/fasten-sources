...
func GetSourceMontgomeryFamilyClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMontgomeryFamilyClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Montgomery Family Clinic"
	sourceDef.SourceType = pkg.SourceTypeMontgomeryFamilyClinic
	sourceDef.Category = []string{"261QP2300X", "261QR1300X"}
	sourceDef.Aliases = []string{"MONTGOMERY FAMILY CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1235429648", "1730429952"}}
