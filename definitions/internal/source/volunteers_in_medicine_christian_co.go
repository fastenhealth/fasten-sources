...
func GetSourceVolunteersInMedicineChristianCo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVolunteersInMedicineChristianCo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Volunteers in Medicine Christian Co"
	sourceDef.SourceType = pkg.SourceTypeVolunteersInMedicineChristianCo
