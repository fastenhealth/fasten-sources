...
func GetSourceTheHealingRheum(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheHealingRheum]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Healing Rheum"
	sourceDef.SourceType = pkg.SourceTypeTheHealingRheum
	sourceDef.Category = []string{"207RR0500X"}
	sourceDef.Aliases = []string{"THE HEALING RHEUM"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1922767268"}}
	sourceDef.PatientAccessUrl = "http://www.thehealingrheum.com"
