...
func GetSourceLchHealthAndCommunityServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLchHealthAndCommunityServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LCH HEALTH AND COMMUNITY SERVICES"
	sourceDef.SourceType = pkg.SourceTypeLchHealthAndCommunityServices
