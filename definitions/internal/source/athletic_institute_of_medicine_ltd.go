...
func GetSourceAthleticInstituteOfMedicineLtd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAthleticInstituteOfMedicineLtd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Athletic Institute Of Medicine LTD"
	sourceDef.SourceType = pkg.SourceTypeAthleticInstituteOfMedicineLtd
	sourceDef.Category = []string{"207XX0005X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1326189564"}}
