...
func GetSourceSurgicalAssociatesOfWestFlorida(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSurgicalAssociatesOfWestFlorida]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Surgical Associates of West Florida"
	sourceDef.SourceType = pkg.SourceTypeSurgicalAssociatesOfWestFlorida
