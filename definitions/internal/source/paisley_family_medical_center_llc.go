...
func GetSourcePaisleyFamilyMedicalCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePaisleyFamilyMedicalCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PAISLEY FAMILY MEDICAL CENTER LLC"
	sourceDef.SourceType = pkg.SourceTypePaisleyFamilyMedicalCenterLlc
