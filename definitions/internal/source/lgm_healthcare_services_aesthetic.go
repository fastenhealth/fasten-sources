...
func GetSourceLgmHealthcareServicesAesthetic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLgmHealthcareServicesAesthetic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LGM HEALTHCARE SERVICES  AESTHETIC"
	sourceDef.SourceType = pkg.SourceTypeLgmHealthcareServicesAesthetic
