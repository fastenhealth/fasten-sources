...
func GetSourceRobinsHealthAndWellnessCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRobinsHealthAndWellnessCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Robins Health and Wellness Center"
	sourceDef.SourceType = pkg.SourceTypeRobinsHealthAndWellnessCenter
