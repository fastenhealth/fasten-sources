...
func GetSourceNorthSuburbanFamilyPhysicians(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthSuburbanFamilyPhysicians]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Suburban Family Physicians"
	sourceDef.SourceType = pkg.SourceTypeNorthSuburbanFamilyPhysicians
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{"NORTH SUBURBAN FAMILY PHYSICIANS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1740635432"}}
