...
func GetSourceBryanSangMedicalAssociationPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBryanSangMedicalAssociationPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bryan  Sang Medical Association PA"
	sourceDef.SourceType = pkg.SourceTypeBryanSangMedicalAssociationPa
