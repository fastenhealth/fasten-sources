...
func GetSourceColoradoClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeColoradoClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Colorado Clinic"
	sourceDef.SourceType = pkg.SourceTypeColoradoClinic
	sourceDef.Category = []string{"207LP2900X"}
	sourceDef.Aliases = []string{"COLORADO CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1750636650"}}
	sourceDef.PatientAccessUrl = "https://www.coloradoclinic.com/"
