...
func GetSourceUnityMedicalAndWellnessLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUnityMedicalAndWellnessLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "UNITY MEDICAL AND WELLNESS LLC"
	sourceDef.SourceType = pkg.SourceTypeUnityMedicalAndWellnessLlc
