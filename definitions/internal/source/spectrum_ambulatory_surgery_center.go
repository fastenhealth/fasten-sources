...
func GetSourceSpectrumAmbulatorySurgeryCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSpectrumAmbulatorySurgeryCenter]; clientIdOk {
	...
	sourceDef.Aliases = []string{"SPECTRUM AMBULATORY SURGERY CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1518382936"}}
