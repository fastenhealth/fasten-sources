...
func GetSourceGillMedicalGeriatricAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGillMedicalGeriatricAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gill Medical  Geriatric Associates"
	sourceDef.SourceType = pkg.SourceTypeGillMedicalGeriatricAssociates
