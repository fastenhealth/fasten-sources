...
func GetSourceGeorgiaPhysicianManagementLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGeorgiaPhysicianManagementLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GEORGIA PHYSICIAN MANAGEMENT LLC"
	sourceDef.SourceType = pkg.SourceTypeGeorgiaPhysicianManagementLlc
