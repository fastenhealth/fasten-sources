...
func GetSourceAdvancedPainWellnessInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedPainWellnessInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ADVANCED PAIN  WELLNESS INSTITUTE"
	sourceDef.SourceType = pkg.SourceTypeAdvancedPainWellnessInstitute
