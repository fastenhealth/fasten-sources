...
func GetSourceGreenbrookTmsNeurohealthCenters(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGreenbrookTmsNeurohealthCenters]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Greenbrook TMS NeuroHealth Centers"
	sourceDef.SourceType = pkg.SourceTypeGreenbrookTmsNeurohealthCenters
