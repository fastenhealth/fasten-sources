...
func GetSourceSharpeMedicalServicesDeanGlassman(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSharpeMedicalServicesDeanGlassman]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sharpe Medical Services  Dean Glassman"
	sourceDef.SourceType = pkg.SourceTypeSharpeMedicalServicesDeanGlassman
