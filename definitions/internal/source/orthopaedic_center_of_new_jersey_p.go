...
func GetSourceOrthopaedicCenterOfNewJerseyP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOrthopaedicCenterOfNewJerseyP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ORTHOPAEDIC CENTER OF NEW JERSEY P"
	sourceDef.SourceType = pkg.SourceTypeOrthopaedicCenterOfNewJerseyP
