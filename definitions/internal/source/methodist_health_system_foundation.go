...
func GetSourceMethodistHealthSystemFoundation(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMethodistHealthSystemFoundation]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "METHODIST HEALTH SYSTEM FOUNDATION"
	sourceDef.SourceType = pkg.SourceTypeMethodistHealthSystemFoundation
