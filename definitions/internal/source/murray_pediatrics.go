...
func GetSourceMurrayPediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMurrayPediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Murray Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeMurrayPediatrics
	sourceDef.Category = []string{"363L00000X"}
	sourceDef.Aliases = []string{"MURRAY PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1144608159", "1952553927"}}
