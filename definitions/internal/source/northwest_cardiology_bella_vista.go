...
func GetSourceNorthwestCardiologyBellaVista(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestCardiologyBellaVista]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Cardiology  Bella Vista"
	sourceDef.SourceType = pkg.SourceTypeNorthwestCardiologyBellaVista
