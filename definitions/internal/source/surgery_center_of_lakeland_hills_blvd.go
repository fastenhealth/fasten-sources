...
func GetSourceSurgeryCenterOfLakelandHillsBlvd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSurgeryCenterOfLakelandHillsBlvd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Surgery Center Of Lakeland Hills Blvd"
	sourceDef.SourceType = pkg.SourceTypeSurgeryCenterOfLakelandHillsBlvd
