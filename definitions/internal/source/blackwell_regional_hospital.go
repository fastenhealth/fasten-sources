...
func GetSourceBlackwellRegionalHospital(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBlackwellRegionalHospital]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Blackwell Regional Hospital"
	sourceDef.SourceType = pkg.SourceTypeBlackwellRegionalHospital
	sourceDef.Category = []string{"282N00000X"}
	sourceDef.Aliases = []string{"BLACKWELL REGIONAL HOSPITAL", "STILLWATER MEDICAL - BLACKWELL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1104375237"}}
