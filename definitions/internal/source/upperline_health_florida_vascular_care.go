...
func GetSourceUpperlineHealthFloridaVascularCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUpperlineHealthFloridaVascularCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Upperline Health Florida Vascular Care"
	sourceDef.SourceType = pkg.SourceTypeUpperlineHealthFloridaVascularCare
