...
func GetSourceChesapeakeRegionalNeurosciencesneurosurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChesapeakeRegionalNeurosciencesneurosurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Chesapeake Regional NeurosciencesNeurosurgery"
	sourceDef.SourceType = pkg.SourceTypeChesapeakeRegionalNeurosciencesneurosurgery
