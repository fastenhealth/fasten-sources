...
func GetSourceJxeVenturesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJxeVenturesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "JXE VENTURES LLC"
	sourceDef.SourceType = pkg.SourceTypeJxeVenturesLlc
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{"SAN JUAN MEDICAL CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1881263309"}}
	sourceDef.PatientAccessUrl = "http://www.sanjuanmedicalcenter.com"
