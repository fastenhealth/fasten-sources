...
func GetSourceClinicOfEastEndAssociationLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeClinicOfEastEndAssociationLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Clinic of East End Association LLC"
	sourceDef.SourceType = pkg.SourceTypeClinicOfEastEndAssociationLlc
