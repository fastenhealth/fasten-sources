...
func GetSourcePodiatricManagementAssociatesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePodiatricManagementAssociatesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PODIATRIC MANAGEMENT ASSOCIATES LLC"
	sourceDef.SourceType = pkg.SourceTypePodiatricManagementAssociatesLlc
