...
func GetSourceConsultantsInCardiovascularMedic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeConsultantsInCardiovascularMedic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CONSULTANTS IN CARDIOVASCULAR MEDIC"
	sourceDef.SourceType = pkg.SourceTypeConsultantsInCardiovascularMedic
