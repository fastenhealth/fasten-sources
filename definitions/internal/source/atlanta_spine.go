...
func GetSourceAtlantaSpine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAtlantaSpine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Atlanta Spine"
	sourceDef.SourceType = pkg.SourceTypeAtlantaSpine
	sourceDef.Category = []string{"207X00000X", "2081P2900X", "208VP0014X"}
	sourceDef.Aliases = []string{"ATLANTA SPINE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1104396415"}}
