...
func GetSourceStatesvilleObgyn(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStatesvilleObgyn]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Statesville OBGYN"
	sourceDef.SourceType = pkg.SourceTypeStatesvilleObgyn
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{"STATESVILLE OBGYN"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1710261151"}}
