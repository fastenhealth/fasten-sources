...
func GetSourceFranklinwilliamsonBicountyHealt(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFranklinwilliamsonBicountyHealt]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FranklinWilliamson BiCounty Healt"
	sourceDef.SourceType = pkg.SourceTypeFranklinwilliamsonBicountyHealt
