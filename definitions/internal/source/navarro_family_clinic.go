...
func GetSourceNavarroFamilyClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNavarroFamilyClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Navarro Family Clinic"
	sourceDef.SourceType = pkg.SourceTypeNavarroFamilyClinic
	sourceDef.Category = []string{"207Q00000X", "261QR1300X"}
	sourceDef.Aliases = []string{"NAVARRO FAMILY CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1730635202", "1831640747"}}
