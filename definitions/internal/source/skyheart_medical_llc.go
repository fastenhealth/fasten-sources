...
func GetSourceSkyheartMedicalLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSkyheartMedicalLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SkyHeart Medical LLC"
	sourceDef.SourceType = pkg.SourceTypeSkyheartMedicalLlc
	sourceDef.Category = []string{"261QH0100X", "261QP2300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1972194215"}}
	sourceDef.PatientAccessUrl = "http://www.SkyHeartMedical.com"
