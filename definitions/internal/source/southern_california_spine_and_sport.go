...
func GetSourceSouthernCaliforniaSpineAndSport(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernCaliforniaSpineAndSport]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southern California Spine and Sport"
	sourceDef.SourceType = pkg.SourceTypeSouthernCaliforniaSpineAndSport
