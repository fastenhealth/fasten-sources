...
func GetSourceSpineSpecialistsOfLouisianaLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSpineSpecialistsOfLouisianaLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Spine Specialists of Louisiana LLC"
	sourceDef.SourceType = pkg.SourceTypeSpineSpecialistsOfLouisianaLlc
