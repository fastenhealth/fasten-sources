...
func GetSourceChesapeakeRegionalNeurosciencespainManagement(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChesapeakeRegionalNeurosciencespainManagement]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Chesapeake Regional NeurosciencesPain Management"
	sourceDef.SourceType = pkg.SourceTypeChesapeakeRegionalNeurosciencespainManagement
