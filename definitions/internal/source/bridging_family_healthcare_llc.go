...
func GetSourceBridgingFamilyHealthcareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBridgingFamilyHealthcareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bridging Family Healthcare LLC"
	sourceDef.SourceType = pkg.SourceTypeBridgingFamilyHealthcareLlc
	sourceDef.Category = []string{}
	sourceDef.Aliases = []string{}
	sourceDef.PatientAccessUrl = "http://Bridgingfamilyhealthcare.com\t\t\t\t\t\t\t\t\t\t\t\t"
