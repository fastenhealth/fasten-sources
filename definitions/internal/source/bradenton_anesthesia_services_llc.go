...
func GetSourceBradentonAnesthesiaServicesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBradentonAnesthesiaServicesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Bradenton Anesthesia Services LLC"
	sourceDef.SourceType = pkg.SourceTypeBradentonAnesthesiaServicesLlc
