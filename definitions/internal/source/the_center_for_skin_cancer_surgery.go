...
func GetSourceTheCenterForSkinCancerSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheCenterForSkinCancerSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Center for Skin Cancer Surgery"
	sourceDef.SourceType = pkg.SourceTypeTheCenterForSkinCancerSurgery
