...
func GetSourceBciAmHealthLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBciAmHealthLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BCI AM HEALTH LLC"
	sourceDef.SourceType = pkg.SourceTypeBciAmHealthLlc
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{"BCI", "BCI AM WELLNESS LLC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1073103438"}}
