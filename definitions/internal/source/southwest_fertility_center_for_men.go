...
func GetSourceSouthwestFertilityCenterForMen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthwestFertilityCenterForMen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SOUTHWEST FERTILITY CENTER FOR MEN"
	sourceDef.SourceType = pkg.SourceTypeSouthwestFertilityCenterForMen
