...
func GetSourceDooleyChiropracticLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDooleyChiropracticLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Dooley Chiropractic LLC"
	sourceDef.SourceType = pkg.SourceTypeDooleyChiropracticLlc
	sourceDef.Category = []string{"261QH0100X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1588998132"}}
	sourceDef.PatientAccessUrl = "http://www.chiropractorincenterville.com"
