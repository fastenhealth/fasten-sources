...
func GetSourceTrinityMedicalAssociatesPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTrinityMedicalAssociatesPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Trinity Medical Associates Pa"
	sourceDef.SourceType = pkg.SourceTypeTrinityMedicalAssociatesPa
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1376897090"}}
	sourceDef.PatientAccessUrl = "http://www.trinitymedicalassociates.org"
