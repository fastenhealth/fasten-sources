...
func GetSourceImaginemic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeImaginemic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ImagineMIC"
	sourceDef.SourceType = pkg.SourceTypeImaginemic
	sourceDef.Category = []string{"207P00000X", "207R00000X"}
	sourceDef.Aliases = []string{"IMAGINEMIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1437622057"}}
