...
func GetSourceUniversityAllergyAsthmaCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUniversityAllergyAsthmaCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "University Allergy  Asthma Center"
	sourceDef.SourceType = pkg.SourceTypeUniversityAllergyAsthmaCenter
