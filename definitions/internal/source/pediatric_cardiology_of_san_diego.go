...
func GetSourcePediatricCardiologyOfSanDiego(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePediatricCardiologyOfSanDiego]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pediatric Cardiology of San Diego"
	sourceDef.SourceType = pkg.SourceTypePediatricCardiologyOfSanDiego
	sourceDef.Category = []string{"2080P0202X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1750891727"}}
