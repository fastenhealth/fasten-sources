...
func GetSourceColliervilleOrthopedicsAndSports(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeColliervilleOrthopedicsAndSports]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Collierville Orthopedics and Sports"
	sourceDef.SourceType = pkg.SourceTypeColliervilleOrthopedicsAndSports
