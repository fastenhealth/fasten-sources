...
func GetSourceEmeraldCoastWomanCareLlcTermed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEmeraldCoastWomanCareLlcTermed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "EMERALD COAST WOMAN CARE LLC TERMED"
	sourceDef.SourceType = pkg.SourceTypeEmeraldCoastWomanCareLlcTermed
