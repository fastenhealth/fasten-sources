...
func GetSourceNeurologicalCareOfIndianaInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeurologicalCareOfIndianaInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Neurological Care of Indiana Inc"
	sourceDef.SourceType = pkg.SourceTypeNeurologicalCareOfIndianaInc
	sourceDef.Category = []string{"261QM2500X"}
	sourceDef.Aliases = []string{"HEADACHES AND PAIN CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1801116223"}}
