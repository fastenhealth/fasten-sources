...
func GetSourceAlliancehealthClinicPoncaCity(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthClinicPoncaCity]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Clinic Ponca City"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthClinicPoncaCity
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"ALLIANCEHEALTH CLINIC PONCA CITY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1932634573"}}
