...
func GetSourceGsgPrideHealthcareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGsgPrideHealthcareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GSG Pride Healthcare LLC"
	sourceDef.SourceType = pkg.SourceTypeGsgPrideHealthcareLlc
	sourceDef.Category = []string{"261QP2300X", "261QR1300X"}
	sourceDef.Aliases = []string{"MEDICAL PRACTICE", "TARIS GONZALEZ"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1437622701"}}
