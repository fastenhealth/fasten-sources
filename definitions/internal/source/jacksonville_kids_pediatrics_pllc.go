...
func GetSourceJacksonvilleKidsPediatricsPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJacksonvilleKidsPediatricsPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Jacksonville Kids Pediatrics PLLC"
	sourceDef.SourceType = pkg.SourceTypeJacksonvilleKidsPediatricsPllc
