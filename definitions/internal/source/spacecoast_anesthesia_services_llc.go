...
func GetSourceSpacecoastAnesthesiaServicesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSpacecoastAnesthesiaServicesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Spacecoast Anesthesia Services LLC"
	sourceDef.SourceType = pkg.SourceTypeSpacecoastAnesthesiaServicesLlc
