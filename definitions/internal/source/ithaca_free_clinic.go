...
func GetSourceIthacaFreeClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIthacaFreeClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ithaca Free Clinic"
	sourceDef.SourceType = pkg.SourceTypeIthacaFreeClinic
	sourceDef.Category = []string{"261QH0100X"}
	sourceDef.Aliases = []string{"ITHACA FREE CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1306309000"}}
