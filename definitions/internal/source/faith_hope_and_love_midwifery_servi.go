...
func GetSourceFaithHopeAndLoveMidwiferyServi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFaithHopeAndLoveMidwiferyServi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Faith Hope and Love Midwifery Servi"
	sourceDef.SourceType = pkg.SourceTypeFaithHopeAndLoveMidwiferyServi
