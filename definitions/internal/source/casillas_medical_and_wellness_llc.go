...
func GetSourceCasillasMedicalAndWellnessLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCasillasMedicalAndWellnessLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CASILLAS MEDICAL AND WELLNESS LLC"
	sourceDef.SourceType = pkg.SourceTypeCasillasMedicalAndWellnessLlc
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1437875929"}}
	sourceDef.PatientAccessUrl = "https://casillasmedicalandwellness.com/"
