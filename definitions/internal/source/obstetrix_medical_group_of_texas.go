...
func GetSourceObstetrixMedicalGroupOfTexas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeObstetrixMedicalGroupOfTexas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Obstetrix Medical Group Of Texas"
	sourceDef.SourceType = pkg.SourceTypeObstetrixMedicalGroupOfTexas
