...
func GetSourceStarrRegionalPhysiciansServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStarrRegionalPhysiciansServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Starr Regional Physicians Services"
	sourceDef.SourceType = pkg.SourceTypeStarrRegionalPhysiciansServices
