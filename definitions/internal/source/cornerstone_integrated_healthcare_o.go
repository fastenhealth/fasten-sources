...
func GetSourceCornerstoneIntegratedHealthcareO(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCornerstoneIntegratedHealthcareO]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CORNERSTONE INTEGRATED HEALTHCARE O"
	sourceDef.SourceType = pkg.SourceTypeCornerstoneIntegratedHealthcareO
