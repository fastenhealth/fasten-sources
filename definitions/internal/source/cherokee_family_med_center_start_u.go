...
func GetSourceCherokeeFamilyMedCenterStartU(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCherokeeFamilyMedCenterStartU]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cherokee Family Med Center Start u"
	sourceDef.SourceType = pkg.SourceTypeCherokeeFamilyMedCenterStartU
