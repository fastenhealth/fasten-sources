...
func GetSourceAscensionMedicalGroupProvidenceDepaulClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAscensionMedicalGroupProvidenceDepaulClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ascension Medical Group Providence DePaul Clinic"
	sourceDef.SourceType = pkg.SourceTypeAscensionMedicalGroupProvidenceDepaulClinic
