...
func GetSourceBeaconChristianCommunityHealthCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBeaconChristianCommunityHealthCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BEACON CHRISTIAN COMMUNITY HEALTH CENTER"
	sourceDef.SourceType = pkg.SourceTypeBeaconChristianCommunityHealthCenter
