...
func GetSourceBarbaraAnneHesselMdPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBarbaraAnneHesselMdPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Barbara Anne Hessel MD PLLC"
	sourceDef.SourceType = pkg.SourceTypeBarbaraAnneHesselMdPllc
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1053617050"}}
	sourceDef.PatientAccessUrl = "http://www.drhesselmd.com"
