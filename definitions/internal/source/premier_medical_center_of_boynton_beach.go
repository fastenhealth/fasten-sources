...
func GetSourcePremierMedicalCenterOfBoyntonBeach(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePremierMedicalCenterOfBoyntonBeach]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PREMIER MEDICAL CENTER OF BOYNTON BEACH"
	sourceDef.SourceType = pkg.SourceTypePremierMedicalCenterOfBoyntonBeach
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1114520673"}}
