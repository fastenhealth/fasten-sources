...
func GetSourceBerniceGonzalezMdPaDbaVitalL(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBerniceGonzalezMdPaDbaVitalL]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bernice Gonzalez MD PA DBA Vital L"
	sourceDef.SourceType = pkg.SourceTypeBerniceGonzalezMdPaDbaVitalL
