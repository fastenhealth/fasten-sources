...
func GetSourceMccannelEyeSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMccannelEyeSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "McCannel Eye Surgery"
	sourceDef.SourceType = pkg.SourceTypeMccannelEyeSurgery
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Aliases = []string{"MCCANNEL EYE SURGERY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1437123270"}}
