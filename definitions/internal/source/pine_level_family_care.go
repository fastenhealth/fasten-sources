...
func GetSourcePineLevelFamilyCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePineLevelFamilyCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pine Level Family Care"
	sourceDef.SourceType = pkg.SourceTypePineLevelFamilyCare
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"PINE LEVEL FAMILY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1437653037"}}
