...
func GetSourceNewEnglandOrthopaedicAndSpineSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNewEnglandOrthopaedicAndSpineSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEW ENGLAND ORTHOPAEDIC AND SPINE SURGERY"
	sourceDef.SourceType = pkg.SourceTypeNewEnglandOrthopaedicAndSpineSurgery
