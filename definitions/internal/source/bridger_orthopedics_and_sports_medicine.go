...
func GetSourceBridgerOrthopedicsAndSportsMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBridgerOrthopedicsAndSportsMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bridger Orthopedics and Sports Medicine"
	sourceDef.SourceType = pkg.SourceTypeBridgerOrthopedicsAndSportsMedicine
