...
func GetSourceCc021GeorgiaCenterForFemaleHealthIiLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc021GeorgiaCenterForFemaleHealthIiLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC021GEORGIA CENTER FOR FEMALE HEALTH II LLC"
	sourceDef.SourceType = pkg.SourceTypeCc021GeorgiaCenterForFemaleHealthIiLlc
