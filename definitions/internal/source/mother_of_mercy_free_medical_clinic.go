...
func GetSourceMotherOfMercyFreeMedicalClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMotherOfMercyFreeMedicalClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Mother of Mercy Free Medical Clinic"
	sourceDef.SourceType = pkg.SourceTypeMotherOfMercyFreeMedicalClinic
