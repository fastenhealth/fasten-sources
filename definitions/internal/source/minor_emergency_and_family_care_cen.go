...
func GetSourceMinorEmergencyAndFamilyCareCen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMinorEmergencyAndFamilyCareCen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Minor Emergency and Family Care Cen"
	sourceDef.SourceType = pkg.SourceTypeMinorEmergencyAndFamilyCareCen
