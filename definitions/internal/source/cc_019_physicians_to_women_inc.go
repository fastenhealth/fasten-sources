...
func GetSourceCc019PhysiciansToWomenInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc019PhysiciansToWomenInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC019PHYSICIANS TO WOMEN INC"
	sourceDef.SourceType = pkg.SourceTypeCc019PhysiciansToWomenInc
