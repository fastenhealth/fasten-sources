...
func GetSourceHolisticBehavioralHealthMultiSe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHolisticBehavioralHealthMultiSe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HOLISTIC BEHAVIORAL HEALTH MULTI SE"
	sourceDef.SourceType = pkg.SourceTypeHolisticBehavioralHealthMultiSe
