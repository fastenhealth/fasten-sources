...
func GetSourceCommunityHealthImagingCenters(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCommunityHealthImagingCenters]; clientIdOk {
	...
	sourceDef.Category = []string{"261QR0200X", "261QR0206X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1154763407"}}
