...
func GetSourceJacksonvilleCtrForReproductiveMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJacksonvilleCtrForReproductiveMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "JACKSONVILLE CTR FOR REPRODUCTIVE MEDICINE"
	sourceDef.SourceType = pkg.SourceTypeJacksonvilleCtrForReproductiveMedicine
