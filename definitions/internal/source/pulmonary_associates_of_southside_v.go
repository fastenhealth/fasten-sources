...
func GetSourcePulmonaryAssociatesOfSouthsideV(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePulmonaryAssociatesOfSouthsideV]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PULMONARY ASSOCIATES OF SOUTHSIDE V"
	sourceDef.SourceType = pkg.SourceTypePulmonaryAssociatesOfSouthsideV
