...
func GetSourceAmsurgSanLuisObispoAnesthesia(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmsurgSanLuisObispoAnesthesia]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Amsurg San Luis Obispo Anesthesia"
	sourceDef.SourceType = pkg.SourceTypeAmsurgSanLuisObispoAnesthesia
