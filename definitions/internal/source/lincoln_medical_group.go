...
func GetSourceLincolnMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLincolnMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lincoln Medical Group"
	sourceDef.SourceType = pkg.SourceTypeLincolnMedicalGroup
	sourceDef.Category = []string{"174400000X"}
	sourceDef.Aliases = []string{"LINCOLN MEDICAL GROUP"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1578885620", "1740914324"}}
