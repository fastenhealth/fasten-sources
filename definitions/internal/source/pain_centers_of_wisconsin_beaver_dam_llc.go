...
func GetSourcePainCentersOfWisconsinBeaverDamLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePainCentersOfWisconsinBeaverDamLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Pain Centers Of Wisconsin Beaver Dam LLC"
	sourceDef.SourceType = pkg.SourceTypePainCentersOfWisconsinBeaverDamLlc
	sourceDef.PatientAccessUrl = "https://npino.com/medical-supplies/1356904254-pain-centers-of-wisconsin---beaver-dam%2C-llc/"
