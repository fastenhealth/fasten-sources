...
func GetSourceTexasMovementDisorderSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexasMovementDisorderSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "TEXAS MOVEMENT DISORDER SPECIALISTS"
	sourceDef.SourceType = pkg.SourceTypeTexasMovementDisorderSpecialists
