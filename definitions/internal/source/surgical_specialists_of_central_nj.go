...
func GetSourceSurgicalSpecialistsOfCentralNj(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSurgicalSpecialistsOfCentralNj]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Surgical Specialists of Central NJ"
	sourceDef.SourceType = pkg.SourceTypeSurgicalSpecialistsOfCentralNj
