...
func GetSourceSacredHeartPrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSacredHeartPrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sacred Heart Primary Care"
	sourceDef.SourceType = pkg.SourceTypeSacredHeartPrimaryCare
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"SACRED HEART PRIMARY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1497105597"}}
