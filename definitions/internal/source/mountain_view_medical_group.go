...
func GetSourceMountainViewMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMountainViewMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Mountain View Medical Group"
	sourceDef.SourceType = pkg.SourceTypeMountainViewMedicalGroup
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{"MOUNTAIN VIEW MEDICAL GROUP"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1184190464"}}
