...
func GetSourceAethenaGynecologyUnifiedPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAethenaGynecologyUnifiedPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AETHENA GYNECOLOGY UNIFIED PLLC"
	sourceDef.SourceType = pkg.SourceTypeAethenaGynecologyUnifiedPllc
