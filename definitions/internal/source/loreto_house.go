...
func GetSourceLoretoHouse(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLoretoHouse]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Loreto House"
	sourceDef.SourceType = pkg.SourceTypeLoretoHouse
	sourceDef.Category = []string{"261QC1500X"}
	sourceDef.Aliases = []string{"LORETO HOUSE, INC."}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1891411492"}}
