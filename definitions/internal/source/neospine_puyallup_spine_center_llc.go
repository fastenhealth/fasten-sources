...
func GetSourceNeospinePuyallupSpineCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeospinePuyallupSpineCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Neospine Puyallup Spine Center LLC"
	sourceDef.SourceType = pkg.SourceTypeNeospinePuyallupSpineCenterLlc
