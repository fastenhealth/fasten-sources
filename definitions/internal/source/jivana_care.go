...
func GetSourceJivanaCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJivanaCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "JIVANA CARE"
	sourceDef.SourceType = pkg.SourceTypeJivanaCare
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"JIVANA CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1710375662"}}
