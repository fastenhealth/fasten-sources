...
func GetSourceBalanceAndNeurologicCenterOfTheRockies(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBalanceAndNeurologicCenterOfTheRockies]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BALANCE AND NEUROLOGIC CENTER OF THE ROCKIES"
	sourceDef.SourceType = pkg.SourceTypeBalanceAndNeurologicCenterOfTheRockies
