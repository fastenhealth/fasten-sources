...
func GetSourceSouthwesternCardiacArrhythmiaIns(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthwesternCardiacArrhythmiaIns]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southwestern Cardiac Arrhythmia Ins"
	sourceDef.SourceType = pkg.SourceTypeSouthwesternCardiacArrhythmiaIns
