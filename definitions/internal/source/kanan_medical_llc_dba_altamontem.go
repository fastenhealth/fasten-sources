...
func GetSourceKananMedicalLlcDbaAltamontem(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKananMedicalLlcDbaAltamontem]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "KANAN MEDICAL LLC dba AltamonteM"
	sourceDef.SourceType = pkg.SourceTypeKananMedicalLlcDbaAltamontem
