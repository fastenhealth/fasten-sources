...
func GetSourceFlourishPediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFlourishPediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Flourish Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeFlourishPediatrics
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{"FLOURISH PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1063025690"}}
	sourceDef.PatientAccessUrl = "http://www.flourishpeds.com"
