...
func GetSourceFnpConciergeHomeMedicalServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFnpConciergeHomeMedicalServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FNP CONCIERGE HOME MEDICAL SERVICES"
	sourceDef.SourceType = pkg.SourceTypeFnpConciergeHomeMedicalServices
