...
func GetSourceHospitalistsMeetingHouseLaneMedical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHospitalistsMeetingHouseLaneMedical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Hospitalists/Meeting House Lane Medical"
	sourceDef.SourceType = pkg.SourceTypeHospitalistsMeetingHouseLaneMedical
