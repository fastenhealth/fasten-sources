...
func GetSourceNorthwestAnesthesiologyPainServicesPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestAnesthesiologyPainServicesPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Anesthesiology  Pain Services PA"
	sourceDef.SourceType = pkg.SourceTypeNorthwestAnesthesiologyPainServicesPa
