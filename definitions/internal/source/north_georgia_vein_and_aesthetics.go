...
func GetSourceNorthGeorgiaVeinAndAesthetics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthGeorgiaVeinAndAesthetics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NORTH GEORGIA VEIN AND AESTHETICS"
	sourceDef.SourceType = pkg.SourceTypeNorthGeorgiaVeinAndAesthetics
