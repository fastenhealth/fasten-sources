...
func GetSourceBlissfulNursingAndMentalHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBlissfulNursingAndMentalHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Blissful Nursing and Mental Health"
	sourceDef.SourceType = pkg.SourceTypeBlissfulNursingAndMentalHealth
