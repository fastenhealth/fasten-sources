...
func GetSourceSeasonsMidwiferyAndBirthCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSeasonsMidwiferyAndBirthCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Seasons Midwifery and Birth Center"
	sourceDef.SourceType = pkg.SourceTypeSeasonsMidwiferyAndBirthCenter
