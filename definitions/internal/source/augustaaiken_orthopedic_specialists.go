...
func GetSourceAugustaaikenOrthopedicSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAugustaaikenOrthopedicSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AugustaAiken Orthopedic Specialists"
	sourceDef.SourceType = pkg.SourceTypeAugustaaikenOrthopedicSpecialists
