...
func GetSourceTheMontgomeryEyeSurgeryCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheMontgomeryEyeSurgeryCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "The Montgomery Eye Surgery Center"
	sourceDef.SourceType = pkg.SourceTypeTheMontgomeryEyeSurgeryCenter
