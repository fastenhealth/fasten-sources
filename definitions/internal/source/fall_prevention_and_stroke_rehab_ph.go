...
func GetSourceFallPreventionAndStrokeRehabPh(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFallPreventionAndStrokeRehabPh]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FALL PREVENTION AND STROKE REHAB PH"
	sourceDef.SourceType = pkg.SourceTypeFallPreventionAndStrokeRehabPh
