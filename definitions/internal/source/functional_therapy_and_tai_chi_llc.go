...
func GetSourceFunctionalTherapyAndTaiChiLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFunctionalTherapyAndTaiChiLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FUNctional Therapy and Tai Chi LLC"
	sourceDef.SourceType = pkg.SourceTypeFunctionalTherapyAndTaiChiLlc
