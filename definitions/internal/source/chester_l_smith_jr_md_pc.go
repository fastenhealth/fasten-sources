...
func GetSourceChesterLSmithJrMdPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChesterLSmithJrMdPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CHESTER L SMITH JR MD PC"
	sourceDef.SourceType = pkg.SourceTypeChesterLSmithJrMdPc
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{"CHESTER L SMITH MD"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1720626310"}}
