...
func GetSourceValueInPreventionOfHoustonTx(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeValueInPreventionOfHoustonTx]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Value in Prevention of Houston TX"
	sourceDef.SourceType = pkg.SourceTypeValueInPreventionOfHoustonTx
