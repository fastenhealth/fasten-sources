...
func GetSourceClevelandCareClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeClevelandCareClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cleveland Care Clinic"
	sourceDef.SourceType = pkg.SourceTypeClevelandCareClinic
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{"CLEVELAND CARE CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1588372965"}}
