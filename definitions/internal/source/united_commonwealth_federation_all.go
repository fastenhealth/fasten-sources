...
func GetSourceUnitedCommonwealthFederationAll(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUnitedCommonwealthFederationAll]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "United Commonwealth Federation All"
	sourceDef.SourceType = pkg.SourceTypeUnitedCommonwealthFederationAll
