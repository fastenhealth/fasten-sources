...
func GetSourceAbcWellnessGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAbcWellnessGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ABC Wellness Group"
	sourceDef.SourceType = pkg.SourceTypeAbcWellnessGroup
	sourceDef.Category = []string{"363LP0808X"}
	sourceDef.Aliases = []string{"ABC WELLNESS GROUP"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1205393709"}}
	sourceDef.PatientAccessUrl = "https://awgderidder.com"
