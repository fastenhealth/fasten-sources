...
func GetSourceIntegrativeSportsMedicineWelln(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIntegrativeSportsMedicineWelln]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Integrative Sports Medicine  Welln"
	sourceDef.SourceType = pkg.SourceTypeIntegrativeSportsMedicineWelln
