...
func GetSourceMountaineerWellnessAndRecoveryC(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMountaineerWellnessAndRecoveryC]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Mountaineer Wellness and Recovery C"
	sourceDef.SourceType = pkg.SourceTypeMountaineerWellnessAndRecoveryC
