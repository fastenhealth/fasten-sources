...
func GetSourceCc013VirginiaCenterForWomenInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc013VirginiaCenterForWomenInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC013VIRGINIA CENTER FOR WOMEN INC"
	sourceDef.SourceType = pkg.SourceTypeCc013VirginiaCenterForWomenInc
