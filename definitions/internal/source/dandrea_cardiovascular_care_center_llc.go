...
func GetSourceDandreaCardiovascularCareCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDandreaCardiovascularCareCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DANDREA CARDIOVASCULAR CARE CENTER LLC"
	sourceDef.SourceType = pkg.SourceTypeDandreaCardiovascularCareCenterLlc
