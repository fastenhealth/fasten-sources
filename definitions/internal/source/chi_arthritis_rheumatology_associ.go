...
func GetSourceChiArthritisRheumatologyAssoci(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChiArthritisRheumatologyAssoci]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CHI ARTHRITIS  RHEUMATOLOGY ASSOCI"
	sourceDef.SourceType = pkg.SourceTypeChiArthritisRheumatologyAssoci
