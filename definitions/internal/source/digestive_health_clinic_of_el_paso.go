...
func GetSourceDigestiveHealthClinicOfElPaso(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDigestiveHealthClinicOfElPaso]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Digestive Health Clinic of El Paso"
	sourceDef.SourceType = pkg.SourceTypeDigestiveHealthClinicOfElPaso
