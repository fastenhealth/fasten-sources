...
func GetSourceOrlandoPainCentersOfExcellence(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOrlandoPainCentersOfExcellence]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Orlando Pain Centers of Excellence"
	sourceDef.SourceType = pkg.SourceTypeOrlandoPainCentersOfExcellence
