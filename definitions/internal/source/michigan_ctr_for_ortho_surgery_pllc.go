...
func GetSourceMichiganCtrForOrthoSurgeryPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMichiganCtrForOrthoSurgeryPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Michigan Ctr for Ortho Surgery PLLC"
	sourceDef.SourceType = pkg.SourceTypeMichiganCtrForOrthoSurgeryPllc
