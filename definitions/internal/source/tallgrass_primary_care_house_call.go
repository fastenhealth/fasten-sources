...
func GetSourceTallgrassPrimaryCareHouseCall(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTallgrassPrimaryCareHouseCall]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tallgrass Primary Care  House Call"
	sourceDef.SourceType = pkg.SourceTypeTallgrassPrimaryCareHouseCall
