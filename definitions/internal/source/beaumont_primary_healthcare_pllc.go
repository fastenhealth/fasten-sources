...
func GetSourceBeaumontPrimaryHealthcarePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBeaumontPrimaryHealthcarePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Beaumont Primary Healthcare PLLC"
	sourceDef.SourceType = pkg.SourceTypeBeaumontPrimaryHealthcarePllc
