...
func GetSourceRappahannockGastroenterologyAssoc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRappahannockGastroenterologyAssoc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Rappahannock Gastroenterology Assoc"
	sourceDef.SourceType = pkg.SourceTypeRappahannockGastroenterologyAssoc
