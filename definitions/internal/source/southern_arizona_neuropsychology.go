...
func GetSourceSouthernArizonaNeuropsychology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernArizonaNeuropsychology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southern Arizona Neuropsychology"
	sourceDef.SourceType = pkg.SourceTypeSouthernArizonaNeuropsychology
