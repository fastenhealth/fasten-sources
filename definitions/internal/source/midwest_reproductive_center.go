...
func GetSourceMidwestReproductiveCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidwestReproductiveCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Midwest Reproductive Center"
	sourceDef.SourceType = pkg.SourceTypeMidwestReproductiveCenter
	sourceDef.Category = []string{"207VE0102X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1992893200"}}
	sourceDef.PatientAccessUrl = "http://www.midwestreproductive.com"
