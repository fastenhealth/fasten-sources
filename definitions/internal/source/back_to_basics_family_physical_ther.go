...
func GetSourceBackToBasicsFamilyPhysicalTher(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBackToBasicsFamilyPhysicalTher]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Back To Basics Family Physical Ther"
	sourceDef.SourceType = pkg.SourceTypeBackToBasicsFamilyPhysicalTher
