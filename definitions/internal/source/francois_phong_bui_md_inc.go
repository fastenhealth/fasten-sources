...
func GetSourceFrancoisPhongBuiMdInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFrancoisPhongBuiMdInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FRANCOIS PHONG BUI MD INC"
	sourceDef.SourceType = pkg.SourceTypeFrancoisPhongBuiMdInc
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1962644138"}}
	sourceDef.PatientAccessUrl = "http://www.venushealthbeautycenter.com"
