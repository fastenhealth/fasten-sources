...
func GetSourceJuanMocegaMd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJuanMocegaMd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Juan Mocega MD"
	sourceDef.SourceType = pkg.SourceTypeJuanMocegaMd
	sourceDef.PatientAccessUrl = "https://www.angelescommunity.org/"
