...
func GetSourceBufclaLlcDbaBenchmarkUrgentAn(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBufclaLlcDbaBenchmarkUrgentAn]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BUFCLA LLC DBA BENCHMARK URGENT AN"
	sourceDef.SourceType = pkg.SourceTypeBufclaLlcDbaBenchmarkUrgentAn
