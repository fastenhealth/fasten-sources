...
func GetSourceAdultAndChildrenAllergyAsthmaC(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdultAndChildrenAllergyAsthmaC]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Adult and Children Allergy Asthma C"
	sourceDef.SourceType = pkg.SourceTypeAdultAndChildrenAllergyAsthmaC
