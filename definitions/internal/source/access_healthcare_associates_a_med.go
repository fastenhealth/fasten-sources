...
func GetSourceAccessHealthcareAssociatesAMed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAccessHealthcareAssociatesAMed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Access Healthcare Associates A Med"
	sourceDef.SourceType = pkg.SourceTypeAccessHealthcareAssociatesAMed
