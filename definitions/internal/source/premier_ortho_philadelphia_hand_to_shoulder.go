...
func GetSourcePremierOrthoPhiladelphiaHandToShoulder(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePremierOrthoPhiladelphiaHandToShoulder]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Premier Ortho  Philadelphia Hand to Shoulder"
	sourceDef.SourceType = pkg.SourceTypePremierOrthoPhiladelphiaHandToShoulder
