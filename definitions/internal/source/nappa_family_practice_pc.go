...
func GetSourceNappaFamilyPracticePc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNappaFamilyPracticePc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NAPPA FAMILY PRACTICE PC"
	sourceDef.SourceType = pkg.SourceTypeNappaFamilyPracticePc
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1508528944"}}
	sourceDef.PatientAccessUrl = "http://www.nappafamilypractice.com"
