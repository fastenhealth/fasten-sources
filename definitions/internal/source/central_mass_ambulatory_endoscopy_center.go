...
func GetSourceCentralMassAmbulatoryEndoscopyCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralMassAmbulatoryEndoscopyCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Central Mass Ambulatory Endoscopy Center"
	sourceDef.SourceType = pkg.SourceTypeCentralMassAmbulatoryEndoscopyCenter
