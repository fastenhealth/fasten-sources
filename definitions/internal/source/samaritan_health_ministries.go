...
func GetSourceSamaritanHealthMinistries(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSamaritanHealthMinistries]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Samaritan Health Ministries"
	sourceDef.SourceType = pkg.SourceTypeSamaritanHealthMinistries
	sourceDef.Category = []string{"261QC1500X"}
	sourceDef.Aliases = []string{"SAMARITAN HEALTH MINISTRIES"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1376911636"}}
