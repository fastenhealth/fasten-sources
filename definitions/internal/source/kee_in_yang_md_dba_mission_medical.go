...
func GetSourceKeeInYangMdDbaMissionMedical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKeeInYangMdDbaMissionMedical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kee In Yang MD DBA Mission Medical"
	sourceDef.SourceType = pkg.SourceTypeKeeInYangMdDbaMissionMedical
