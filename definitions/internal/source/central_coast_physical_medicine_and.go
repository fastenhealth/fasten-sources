...
func GetSourceCentralCoastPhysicalMedicineAnd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralCoastPhysicalMedicineAnd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CENTRAL COAST PHYSICAL MEDICINE AND"
	sourceDef.SourceType = pkg.SourceTypeCentralCoastPhysicalMedicineAnd
