...
func GetSourceKentuckyMedicalSpecialistsPsc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKentuckyMedicalSpecialistsPsc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Kentucky Medical Specialists PSC"
	sourceDef.SourceType = pkg.SourceTypeKentuckyMedicalSpecialistsPsc
