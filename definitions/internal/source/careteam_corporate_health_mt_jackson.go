...
func GetSourceCareteamCorporateHealthMtJackson(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCareteamCorporateHealthMtJackson]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CareTeam Corporate Health Mt Jackson"
	sourceDef.SourceType = pkg.SourceTypeCareteamCorporateHealthMtJackson
