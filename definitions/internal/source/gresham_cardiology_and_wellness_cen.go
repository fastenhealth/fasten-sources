...
func GetSourceGreshamCardiologyAndWellnessCen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGreshamCardiologyAndWellnessCen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gresham Cardiology and Wellness Cen"
	sourceDef.SourceType = pkg.SourceTypeGreshamCardiologyAndWellnessCen
