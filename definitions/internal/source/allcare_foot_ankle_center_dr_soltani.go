...
func GetSourceAllcareFootAnkleCenterDrSoltani(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllcareFootAnkleCenterDrSoltani]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ALLCARE FOOT  ANKLE CENTER  Dr Soltani"
	sourceDef.SourceType = pkg.SourceTypeAllcareFootAnkleCenterDrSoltani
