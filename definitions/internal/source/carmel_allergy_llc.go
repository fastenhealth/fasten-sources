...
func GetSourceCarmelAllergyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCarmelAllergyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Carmel Allergy LLC"
	sourceDef.SourceType = pkg.SourceTypeCarmelAllergyLlc
	sourceDef.Category = []string{"207K00000X"}
	sourceDef.Aliases = []string{"ALLERGY & ASTHMA CENTERS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1215434774"}}
