...
func GetSourceGreatBendRegionalHospitalClinics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGreatBendRegionalHospitalClinics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Great Bend Regional Hospital  Clinics"
	sourceDef.SourceType = pkg.SourceTypeGreatBendRegionalHospitalClinics
