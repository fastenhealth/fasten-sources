...
func GetSourcePaulaPilgrimMdGynecologyPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePaulaPilgrimMdGynecologyPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Paula Pilgrim MD Gynecology PLLC"
	sourceDef.SourceType = pkg.SourceTypePaulaPilgrimMdGynecologyPllc
	sourceDef.Category = []string{"207VG0400X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1184137572"}}
