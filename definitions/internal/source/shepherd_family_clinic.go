...
func GetSourceShepherdFamilyClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShepherdFamilyClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Shepherd Family Clinic"
	sourceDef.SourceType = pkg.SourceTypeShepherdFamilyClinic
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"SHEPHERD FAMILY CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1780396549"}}
