...
func GetSourceContraCostaMedicalGrpForWomen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeContraCostaMedicalGrpForWomen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Contra Costa Medical Grp for Women"
	sourceDef.SourceType = pkg.SourceTypeContraCostaMedicalGrpForWomen
