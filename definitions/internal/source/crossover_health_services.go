...
func GetSourceCrossoverHealthServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCrossoverHealthServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Crossover Health Services"
	sourceDef.SourceType = pkg.SourceTypeCrossoverHealthServices
	sourceDef.Category = []string{"261QM2500X"}
	sourceDef.Aliases = []string{"CROSSOVER HEALTH SERVICES"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1942958111"}}
