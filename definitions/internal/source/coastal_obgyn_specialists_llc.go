...
func GetSourceCoastalObgynSpecialistsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCoastalObgynSpecialistsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COASTAL OBGYN SPECIALISTS LLC"
	sourceDef.SourceType = pkg.SourceTypeCoastalObgynSpecialistsLlc
