...
func GetSourceCenterForPulmonarySleepMedici(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForPulmonarySleepMedici]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center for Pulmonary  Sleep Medici"
	sourceDef.SourceType = pkg.SourceTypeCenterForPulmonarySleepMedici
