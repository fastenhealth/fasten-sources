...
func GetSourceTexasOrthopedicSurgeryConsultant(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexasOrthopedicSurgeryConsultant]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Texas Orthopedic Surgery Consultant"
	sourceDef.SourceType = pkg.SourceTypeTexasOrthopedicSurgeryConsultant
