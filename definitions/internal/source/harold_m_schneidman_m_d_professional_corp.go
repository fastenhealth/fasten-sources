...
func GetSourceHaroldMSchneidmanMDProfessionalCorp(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHaroldMSchneidmanMDProfessionalCorp]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HAROLD M SCHNEIDMAN M D PROFESSIONAL CORP"
	sourceDef.SourceType = pkg.SourceTypeHaroldMSchneidmanMDProfessionalCorp
