...
func GetSourceArthritisCareCenterOklahomaPll(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArthritisCareCenterOklahomaPll]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arthritis Care Center Oklahoma PLL"
	sourceDef.SourceType = pkg.SourceTypeArthritisCareCenterOklahomaPll
