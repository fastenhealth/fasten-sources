...
func GetSourceAmeriImmunizationWellnessCente(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmeriImmunizationWellnessCente]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ameri Immunization  Wellness Cente"
	sourceDef.SourceType = pkg.SourceTypeAmeriImmunizationWellnessCente
