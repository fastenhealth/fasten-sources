...
func GetSourceNortheastIowaFamilyPracticeCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNortheastIowaFamilyPracticeCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northeast Iowa Family Practice Center"
	sourceDef.SourceType = pkg.SourceTypeNortheastIowaFamilyPracticeCenter
