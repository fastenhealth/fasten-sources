...
func GetSourceFortWorthInterventionalLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFortWorthInterventionalLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Fort Worth Interventional LLC"
	sourceDef.SourceType = pkg.SourceTypeFortWorthInterventionalLlc
	sourceDef.Category = []string{"207LP2900X"}
	sourceDef.Aliases = []string{"RATINO INTERVENTIONAL PAIN MANAGEMENT"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1902569445"}}
