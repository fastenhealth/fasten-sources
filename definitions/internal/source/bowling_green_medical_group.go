...
func GetSourceBowlingGreenMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBowlingGreenMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bowling Green Medical Group"
	sourceDef.SourceType = pkg.SourceTypeBowlingGreenMedicalGroup
	sourceDef.Category = []string{"207Q00000X", "261QR1300X"}
	sourceDef.Aliases = []string{"BOWLING GREEN MEDICAL GROUP"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1144856246", "1588168215"}}
