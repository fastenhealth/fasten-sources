...
func GetSourceHamiltonMillPediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHamiltonMillPediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hamilton Mill Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeHamiltonMillPediatrics
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"HAMILTON MILL PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1942938311"}}
