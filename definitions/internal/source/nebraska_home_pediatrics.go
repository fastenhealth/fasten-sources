...
func GetSourceNebraskaHomePediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNebraskaHomePediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Nebraska Home Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeNebraskaHomePediatrics
	sourceDef.Category = []string{"363L00000X"}
	sourceDef.Aliases = []string{"NEBRASKA HOME PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1043726524"}}
