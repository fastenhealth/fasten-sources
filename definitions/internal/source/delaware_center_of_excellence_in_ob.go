...
func GetSourceDelawareCenterOfExcellenceInOb(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDelawareCenterOfExcellenceInOb]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Delaware Center of Excellence in Ob"
	sourceDef.SourceType = pkg.SourceTypeDelawareCenterOfExcellenceInOb
