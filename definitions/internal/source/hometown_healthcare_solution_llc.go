...
func GetSourceHometownHealthcareSolutionLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHometownHealthcareSolutionLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HOMETOWN HEALTHCARE SOLUTION LLC"
	sourceDef.SourceType = pkg.SourceTypeHometownHealthcareSolutionLlc
