...
func GetSourceBellinghamHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBellinghamHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bellingham Health"
	sourceDef.SourceType = pkg.SourceTypeBellinghamHealth
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{"BELLINGHAM HEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1376594093"}}
