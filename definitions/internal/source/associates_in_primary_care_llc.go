...
func GetSourceAssociatesInPrimaryCareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAssociatesInPrimaryCareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ASSOCIATES IN PRIMARY CARE LLC"
	sourceDef.SourceType = pkg.SourceTypeAssociatesInPrimaryCareLlc
