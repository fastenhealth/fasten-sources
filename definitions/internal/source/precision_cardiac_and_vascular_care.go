...
func GetSourcePrecisionCardiacAndVascularCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrecisionCardiacAndVascularCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Precision Cardiac and Vascular Care"
	sourceDef.SourceType = pkg.SourceTypePrecisionCardiacAndVascularCare
