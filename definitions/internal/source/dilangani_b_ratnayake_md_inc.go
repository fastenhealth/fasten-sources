...
func GetSourceDilanganiBRatnayakeMdInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDilanganiBRatnayakeMdInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DILANGANI B RATNAYAKE MD INC"
	sourceDef.SourceType = pkg.SourceTypeDilanganiBRatnayakeMdInc
	sourceDef.Category = []string{"261QP3300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1245870278"}}
	sourceDef.PatientAccessUrl = "http://dratnayakemd.com/"
