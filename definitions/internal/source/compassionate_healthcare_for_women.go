...
func GetSourceCompassionateHealthcareForWomen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCompassionateHealthcareForWomen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Compassionate Healthcare for Women"
	sourceDef.SourceType = pkg.SourceTypeCompassionateHealthcareForWomen
