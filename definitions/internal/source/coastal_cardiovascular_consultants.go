...
func GetSourceCoastalCardiovascularConsultants(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCoastalCardiovascularConsultants]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COASTAL CARDIOVASCULAR CONSULTANTS"
	sourceDef.SourceType = pkg.SourceTypeCoastalCardiovascularConsultants
