...
func GetSourceGreenwoodLefloreClinicNetwork(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGreenwoodLefloreClinicNetwork]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Greenwood Leflore Clinic Network"
	sourceDef.SourceType = pkg.SourceTypeGreenwoodLefloreClinicNetwork
