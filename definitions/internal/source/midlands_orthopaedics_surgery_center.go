...
func GetSourceMidlandsOrthopaedicsSurgeryCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidlandsOrthopaedicsSurgeryCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Midlands Orthopaedics Surgery Center"
	sourceDef.SourceType = pkg.SourceTypeMidlandsOrthopaedicsSurgeryCenter
