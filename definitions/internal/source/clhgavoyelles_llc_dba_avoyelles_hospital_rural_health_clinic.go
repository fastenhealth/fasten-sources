...
func GetSourceClhgavoyellesLlcDbaAvoyellesHospitalRuralHealthClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeClhgavoyellesLlcDbaAvoyellesHospitalRuralHealthClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CLHGAvoyelles LLC Dba Avoyelles Hospital Rural Health Clinic"
	sourceDef.SourceType = pkg.SourceTypeClhgavoyellesLlcDbaAvoyellesHospitalRuralHealthClinic
