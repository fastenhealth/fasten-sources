...
func GetSourceShilohUrgentCarePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShilohUrgentCarePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SHILOH URGENT CARE PLLC"
	sourceDef.SourceType = pkg.SourceTypeShilohUrgentCarePllc
	sourceDef.Category = []string{"261QU0200X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1982236170"}}
	sourceDef.PatientAccessUrl = "http://www.shilohurgentcare.com"
