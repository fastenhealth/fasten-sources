...
func GetSourceNortheastProhealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNortheastProhealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northeast ProHealth"
	sourceDef.SourceType = pkg.SourceTypeNortheastProhealth
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"NORTHEAST PROHEALTH"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1336491315"}}
	sourceDef.PatientAccessUrl = "http://www.alamowellnessalliance.com"
