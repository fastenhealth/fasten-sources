...
func GetSourceBayAreaMedicalAndSurgicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBayAreaMedicalAndSurgicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bay Area Medical and Surgical Group"
	sourceDef.SourceType = pkg.SourceTypeBayAreaMedicalAndSurgicalGroup
	sourceDef.Category = []string{"208600000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1497380638"}}
