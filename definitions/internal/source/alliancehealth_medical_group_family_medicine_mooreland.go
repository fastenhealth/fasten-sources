...
func GetSourceAlliancehealthMedicalGroupFamilyMedicineMooreland(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupFamilyMedicineMooreland]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Family Medicine Mooreland"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupFamilyMedicineMooreland
