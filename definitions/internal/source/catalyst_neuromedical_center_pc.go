...
func GetSourceCatalystNeuromedicalCenterPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCatalystNeuromedicalCenterPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Catalyst Neuromedical Center PC"
	sourceDef.SourceType = pkg.SourceTypeCatalystNeuromedicalCenterPc
	sourceDef.Category = []string{"2084N0400X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1558973792"}}
