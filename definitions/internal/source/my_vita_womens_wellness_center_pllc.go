...
func GetSourceMyVitaWomensWellnessCenterPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMyVitaWomensWellnessCenterPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MY VITA WOMENS WELLNESS CENTER PLLC"
	sourceDef.SourceType = pkg.SourceTypeMyVitaWomensWellnessCenterPllc
