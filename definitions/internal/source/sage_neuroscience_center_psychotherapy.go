...
func GetSourceSageNeuroscienceCenterPsychotherapy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSageNeuroscienceCenterPsychotherapy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SAGE NEUROSCIENCE CENTER PSYCHOTHERAPY"
	sourceDef.SourceType = pkg.SourceTypeSageNeuroscienceCenterPsychotherapy
