...
func GetSourcePorterRegionalHospitalEmergency(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePorterRegionalHospitalEmergency]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Porter Regional Hospital Emergency"
	sourceDef.SourceType = pkg.SourceTypePorterRegionalHospitalEmergency
