...
func GetSourceUrbanCommunityActionProjectsDba(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUrbanCommunityActionProjectsDba]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Urban Community Action Projects dba"
	sourceDef.SourceType = pkg.SourceTypeUrbanCommunityActionProjectsDba
