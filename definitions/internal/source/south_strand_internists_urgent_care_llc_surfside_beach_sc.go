...
func GetSourceSouthStrandInternistsUrgentCareLlcSurfsideBeachSc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthStrandInternistsUrgentCareLlcSurfsideBeachSc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "South Strand Internists  Urgent Care LLC  Surfside Beach SC"
	sourceDef.SourceType = pkg.SourceTypeSouthStrandInternistsUrgentCareLlcSurfsideBeachSc
