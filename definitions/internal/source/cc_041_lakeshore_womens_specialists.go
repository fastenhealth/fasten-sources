...
func GetSourceCc041LakeshoreWomensSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc041LakeshoreWomensSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC041LAKESHORE WOMENS SPECIALISTS"
	sourceDef.SourceType = pkg.SourceTypeCc041LakeshoreWomensSpecialists
