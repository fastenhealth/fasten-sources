// Copyright (C) Fasten Health, Inc. - All Rights Reserved.
//
// THIS FILE IS GENERATED BY https://github.com/fastenhealth/fasten-sources-gen
// PLEASE DO NOT EDIT BY HAND

package source

import (
	platform "github.com/fastenhealth/fasten-sources/definitions/internal/platform"
	models "github.com/fastenhealth/fasten-sources/definitions/models"
	pkg "github.com/fastenhealth/fasten-sources/pkg"
)

// https://fhir-myrecord.cerner.com/r4/95143858-4077-4cf1-b5d2-c2a999f62bb3/metadata
func GetSourceMobridgeRegionalHospital(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	sourceDef, err := platform.GetSourceCerner(env, clientIdLookup)
	sourceDef.AuthorizationEndpoint = "https://authorization.cerner.com/tenants/95143858-4077-4cf1-b5d2-c2a999f62bb3/protocols/oauth2/profiles/smart-v1/personas/patient/authorize"
	sourceDef.TokenEndpoint = "https://authorization.cerner.com/tenants/95143858-4077-4cf1-b5d2-c2a999f62bb3/protocols/oauth2/profiles/smart-v1/token"
	sourceDef.IntrospectionEndpoint = "https://authorization.cerner.com/tokeninfo"

	sourceDef.Audience = "https://fhir-myrecord.cerner.com/r4/95143858-4077-4cf1-b5d2-c2a999f62bb3"

	sourceDef.ApiEndpointBaseUrl = "https://fhir-myrecord.cerner.com/r4/95143858-4077-4cf1-b5d2-c2a999f62bb3"
	// retrieve client-id, if available
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMobridgeRegionalHospital]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeCerner))

	sourceDef.Display = "Mobridge Regional Hospital"
	sourceDef.SourceType = pkg.SourceTypeMobridgeRegionalHospital
	sourceDef.Category = []string{"207Q00000X", "251E00000X", "261QR1300X", "275N00000X", "282NC0060X", "310400000X", "333600000X", "3336I0012X"}
	sourceDef.Aliases = []string{"MOBRIDGE REGIONAL HOSPITAL", "MOBRIDGE REGIONAL HOSPITAL-HOME HEALTH", "SWINGBED"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1184766073", "1265574156", "1275502577", "1356483242", "1639211527", "1710029608", "1871554311", "1902948896", "1942243274", "1992847883"}}
	sourceDef.BrandLogo = "mobridge-regional-hospital.svg"
	sourceDef.PatientAccessUrl = "https://www.mobridgehospital.org/"
	sourceDef.SecretKeyPrefix = "cerner"

	return sourceDef, err
}
