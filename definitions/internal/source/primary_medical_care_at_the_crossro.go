...
func GetSourcePrimaryMedicalCareAtTheCrossro(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrimaryMedicalCareAtTheCrossro]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Primary Medical Care at the Crossro"
	sourceDef.SourceType = pkg.SourceTypePrimaryMedicalCareAtTheCrossro
