...
func GetSourceHaigPhysicalMedicinePlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHaigPhysicalMedicinePlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Haig Physical Medicine PLC"
	sourceDef.SourceType = pkg.SourceTypeHaigPhysicalMedicinePlc
	sourceDef.Category = []string{"261QM2500X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1215466107"}}
