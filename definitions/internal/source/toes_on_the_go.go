...
func GetSourceToesOnTheGo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeToesOnTheGo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Toes on the Go"
	sourceDef.SourceType = pkg.SourceTypeToesOnTheGo
	sourceDef.Category = []string{"213E00000X"}
	sourceDef.Aliases = []string{"TOES ON THE GO"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1124120654"}}
