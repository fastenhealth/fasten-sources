...
func GetSourceUrgentCareAndWalkinMedicalSuite(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUrgentCareAndWalkinMedicalSuite]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Urgent Care and Walkin Medical Suite"
	sourceDef.SourceType = pkg.SourceTypeUrgentCareAndWalkinMedicalSuite
