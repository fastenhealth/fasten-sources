...
func GetSourceCorePhysicalTherapyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCorePhysicalTherapyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Core Physical Therapy LLC"
	sourceDef.SourceType = pkg.SourceTypeCorePhysicalTherapyLlc
	sourceDef.Category = []string{"261QP2000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1073086914"}}
	sourceDef.PatientAccessUrl = "http://www.coretherapyservices.com"
