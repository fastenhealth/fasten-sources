...
func GetSourceBlackDiamondMedicalLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBlackDiamondMedicalLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BLACK DIAMOND MEDICAL LLC"
	sourceDef.SourceType = pkg.SourceTypeBlackDiamondMedicalLlc
	sourceDef.Category = []string{"207Q00000X", "261QU0200X", "363LP2300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1639644909"}}
