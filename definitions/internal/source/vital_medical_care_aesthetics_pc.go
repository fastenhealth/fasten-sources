...
func GetSourceVitalMedicalCareAestheticsPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVitalMedicalCareAestheticsPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VITAL MEDICAL CARE AESTHETICS PC"
	sourceDef.SourceType = pkg.SourceTypeVitalMedicalCareAestheticsPc
