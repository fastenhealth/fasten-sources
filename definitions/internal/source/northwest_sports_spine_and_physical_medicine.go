...
func GetSourceNorthwestSportsSpineAndPhysicalMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestSportsSpineAndPhysicalMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Sports Spine and Physical Medicine"
	sourceDef.SourceType = pkg.SourceTypeNorthwestSportsSpineAndPhysicalMedicine
