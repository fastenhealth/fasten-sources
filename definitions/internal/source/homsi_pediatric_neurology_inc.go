...
func GetSourceHomsiPediatricNeurologyInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHomsiPediatricNeurologyInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HOMSI PEDIATRIC NEUROLOGY INC"
	sourceDef.SourceType = pkg.SourceTypeHomsiPediatricNeurologyInc
