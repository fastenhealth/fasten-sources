...
func GetSourceFamilyEyeCareCentersOfVirginia(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFamilyEyeCareCentersOfVirginia]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Family Eye Care Centers of Virginia"
	sourceDef.SourceType = pkg.SourceTypeFamilyEyeCareCentersOfVirginia
