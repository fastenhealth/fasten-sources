...
func GetSourceDrBradfordWolkHibiscusWomensCareLl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDrBradfordWolkHibiscusWomensCareLl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DR BRADFORD WOLK HIBISCUS WOMENS CARE LL"
	sourceDef.SourceType = pkg.SourceTypeDrBradfordWolkHibiscusWomensCareLl
