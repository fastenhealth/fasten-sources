...
func GetSourceSouthernTennesseeOrthopedics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernTennesseeOrthopedics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southern Tennessee Orthopedics"
	sourceDef.SourceType = pkg.SourceTypeSouthernTennesseeOrthopedics
	sourceDef.Category = []string{"207X00000X", "332B00000X"}
	sourceDef.Aliases = []string{"SOUTHERN TENNESSEE ORTHOPEDICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1861838476"}}
