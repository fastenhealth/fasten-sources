...
func GetSourceWilliamsburgFamilyMedicineCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWilliamsburgFamilyMedicineCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WILLIAMSBURG FAMILY MEDICINE CENTER"
	sourceDef.SourceType = pkg.SourceTypeWilliamsburgFamilyMedicineCenter
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1376689620"}}
