...
func GetSourceAdvancedCommunityMedicalCareGro(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedCommunityMedicalCareGro]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Advanced Community Medical Care Gro"
	sourceDef.SourceType = pkg.SourceTypeAdvancedCommunityMedicalCareGro
