...
func GetSourceMaryvilleWomensCenterPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMaryvilleWomensCenterPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MARYVILLE WOMENS CENTER PC"
	sourceDef.SourceType = pkg.SourceTypeMaryvilleWomensCenterPc
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1356857171"}}
	sourceDef.PatientAccessUrl = "http://www.mymwc.org"
