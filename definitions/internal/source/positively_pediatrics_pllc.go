...
func GetSourcePositivelyPediatricsPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePositivelyPediatricsPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "POSITIVELY PEDIATRICS PLLC"
	sourceDef.SourceType = pkg.SourceTypePositivelyPediatricsPllc
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1700498946"}}
	sourceDef.PatientAccessUrl = "http://www.positivelypediatrics.com"
