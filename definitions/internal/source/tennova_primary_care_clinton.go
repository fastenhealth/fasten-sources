...
func GetSourceTennovaPrimaryCareClinton(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaPrimaryCareClinton]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Primary Care Clinton"
	sourceDef.SourceType = pkg.SourceTypeTennovaPrimaryCareClinton
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"TENNOVA PRIMARY CARE CLINTON"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1093300717"}}
