...
func GetSourceWoodlandHeightsCardiothoracicAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWoodlandHeightsCardiothoracicAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Woodland Heights Cardiothoracic Associates"
	sourceDef.SourceType = pkg.SourceTypeWoodlandHeightsCardiothoracicAssociates
