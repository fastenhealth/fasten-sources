...
func GetSourcePriviaMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePriviaMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Privia Medical Group"
	sourceDef.SourceType = pkg.SourceTypePriviaMedicalGroup
	sourceDef.Category = []string{"207X00000X"}
	sourceDef.Aliases = []string{"FAMILY ORTHOPAEDICS AND SPORTS MEDICINE OF NORTHERN VIRGINIA"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1467908061"}}
