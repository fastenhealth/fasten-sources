...
func GetSourceAllianceHealthDurantClinicsInternalMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllianceHealthDurantClinicsInternalMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Alliance Health Durant Clinics Internal Medicine"
	sourceDef.SourceType = pkg.SourceTypeAllianceHealthDurantClinicsInternalMedicine
