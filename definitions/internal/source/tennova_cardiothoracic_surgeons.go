...
func GetSourceTennovaCardiothoracicSurgeons(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaCardiothoracicSurgeons]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Cardiothoracic Surgeons"
	sourceDef.SourceType = pkg.SourceTypeTennovaCardiothoracicSurgeons
