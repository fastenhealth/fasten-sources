...
func GetSourceGenaspireLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGenaspireLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GenAspire LLC"
	sourceDef.SourceType = pkg.SourceTypeGenaspireLlc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"GENASPIRE GERIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1245997576"}}
