...
func GetSourceGeneralAndLaparoendoscopicSurgeo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGeneralAndLaparoendoscopicSurgeo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "General And Laparoendoscopic Surgeo"
	sourceDef.SourceType = pkg.SourceTypeGeneralAndLaparoendoscopicSurgeo
