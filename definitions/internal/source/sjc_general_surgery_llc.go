...
func GetSourceSjcGeneralSurgeryLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSjcGeneralSurgeryLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SJC GENERAL SURGERY LLC"
	sourceDef.SourceType = pkg.SourceTypeSjcGeneralSurgeryLlc
	sourceDef.Category = []string{"208600000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1124758966"}}
	sourceDef.PatientAccessUrl = "http://www.southcoasthealth.com/"
