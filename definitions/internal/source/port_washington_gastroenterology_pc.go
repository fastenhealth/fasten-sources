...
func GetSourcePortWashingtonGastroenterologyPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePortWashingtonGastroenterologyPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Port Washington Gastroenterology PC"
	sourceDef.SourceType = pkg.SourceTypePortWashingtonGastroenterologyPc
