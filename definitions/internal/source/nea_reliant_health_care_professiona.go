...
func GetSourceNeaReliantHealthCareProfessiona(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeaReliantHealthCareProfessiona]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEA Reliant Health Care Professiona"
	sourceDef.SourceType = pkg.SourceTypeNeaReliantHealthCareProfessiona
