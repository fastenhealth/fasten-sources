...
func GetSourceMemorialRegionalHealthMedicalClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMemorialRegionalHealthMedicalClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Memorial Regional Health Medical Clinic"
	sourceDef.SourceType = pkg.SourceTypeMemorialRegionalHealthMedicalClinic
