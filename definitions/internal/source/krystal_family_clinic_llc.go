...
func GetSourceKrystalFamilyClinicLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKrystalFamilyClinicLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "KRYSTAL FAMILY CLINIC LLC"
	sourceDef.SourceType = pkg.SourceTypeKrystalFamilyClinicLlc
	sourceDef.Category = []string{"363LF0000X", "363LP0808X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1811424708"}}
	sourceDef.PatientAccessUrl = "http://www.krystalfamilyclinic.com"
