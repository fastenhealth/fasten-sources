...
func GetSourcePrimaryCareAssociatesPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrimaryCareAssociatesPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Primary Care Associates PC"
	sourceDef.SourceType = pkg.SourceTypePrimaryCareAssociatesPc
	sourceDef.Category = []string{"207Q00000X", "208000000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1275528630"}}
	sourceDef.PatientAccessUrl = "https://primarycareassociatesbartlesville.com/physicians.html"
