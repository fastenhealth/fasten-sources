...
func GetSourceLiviMedicalServicesLlcDbaSage(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLiviMedicalServicesLlcDbaSage]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Livi Medical Services LLC DBA Sage"
	sourceDef.SourceType = pkg.SourceTypeLiviMedicalServicesLlcDbaSage
