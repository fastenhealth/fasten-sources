...
func GetSourceAlliancehealthMedicalGroupPediatricsClinton(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupPediatricsClinton]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Pediatrics Clinton"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupPediatricsClinton
