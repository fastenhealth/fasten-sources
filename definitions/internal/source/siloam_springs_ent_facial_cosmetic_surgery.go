...
func GetSourceSiloamSpringsEntFacialCosmeticSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSiloamSpringsEntFacialCosmeticSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Siloam Springs ENT  Facial Cosmetic Surgery"
	sourceDef.SourceType = pkg.SourceTypeSiloamSpringsEntFacialCosmeticSurgery
