...
func GetSourcePittsboroPineRidgeUrgentCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePittsboroPineRidgeUrgentCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pittsboro  Pine Ridge Urgent Care"
	sourceDef.SourceType = pkg.SourceTypePittsboroPineRidgeUrgentCare
