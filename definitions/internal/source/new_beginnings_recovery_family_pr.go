...
func GetSourceNewBeginningsRecoveryFamilyPr(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNewBeginningsRecoveryFamilyPr]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEW BEGINNINGS RECOVERY  FAMILY PR"
	sourceDef.SourceType = pkg.SourceTypeNewBeginningsRecoveryFamilyPr
