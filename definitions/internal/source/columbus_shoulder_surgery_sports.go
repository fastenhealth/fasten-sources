...
func GetSourceColumbusShoulderSurgerySports(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeColumbusShoulderSurgerySports]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Columbus Shoulder Surgery  Sports"
	sourceDef.SourceType = pkg.SourceTypeColumbusShoulderSurgerySports
