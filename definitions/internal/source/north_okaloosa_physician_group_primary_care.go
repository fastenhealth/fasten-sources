...
func GetSourceNorthOkaloosaPhysicianGroupPrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthOkaloosaPhysicianGroupPrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North Okaloosa Physician Group  Primary Care"
	sourceDef.SourceType = pkg.SourceTypeNorthOkaloosaPhysicianGroupPrimaryCare
