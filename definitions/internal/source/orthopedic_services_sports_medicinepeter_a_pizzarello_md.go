...
func GetSourceOrthopedicServicesSportsMedicinepeterAPizzarelloMd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOrthopedicServicesSportsMedicinepeterAPizzarelloMd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Orthopedic Services  Sports MedicinePeter A Pizzarello MD"
