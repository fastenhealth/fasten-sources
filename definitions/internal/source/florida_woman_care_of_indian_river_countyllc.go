...
func GetSourceFloridaWomanCareOfIndianRiverCountyllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFloridaWomanCareOfIndianRiverCountyllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FLORIDA WOMAN CARE OF INDIAN RIVER COUNTYLLC"
	sourceDef.SourceType = pkg.SourceTypeFloridaWomanCareOfIndianRiverCountyllc
