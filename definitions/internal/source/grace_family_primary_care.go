...
func GetSourceGraceFamilyPrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGraceFamilyPrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Grace Family Primary Care"
	sourceDef.SourceType = pkg.SourceTypeGraceFamilyPrimaryCare
	sourceDef.Category = []string{"363L00000X"}
	sourceDef.Aliases = []string{"GRACE FAMILY PRIMARY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1740850999"}}
