...
func GetSourceEliteHandsPhysicianAssistantSer(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEliteHandsPhysicianAssistantSer]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Elite Hands Physician Assistant Ser"
	sourceDef.SourceType = pkg.SourceTypeEliteHandsPhysicianAssistantSer
