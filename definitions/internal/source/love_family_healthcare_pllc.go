...
func GetSourceLoveFamilyHealthcarePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLoveFamilyHealthcarePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LOVE FAMILY HEALTHCARE PLLC"
	sourceDef.SourceType = pkg.SourceTypeLoveFamilyHealthcarePllc
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1689281537"}}
	sourceDef.PatientAccessUrl = "http://www.lovefamilyhealthcare.com"
