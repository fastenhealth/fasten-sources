...
func GetSourceNorthSanAntonioHealthcareAssoci(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthSanAntonioHealthcareAssoci]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "North San Antonio Healthcare Associ"
	sourceDef.SourceType = pkg.SourceTypeNorthSanAntonioHealthcareAssoci
