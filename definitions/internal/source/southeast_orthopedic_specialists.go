...
func GetSourceSoutheastOrthopedicSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSoutheastOrthopedicSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southeast Orthopedic Specialists"
	sourceDef.SourceType = pkg.SourceTypeSoutheastOrthopedicSpecialists
	sourceDef.Category = []string{"332900000X"}
	sourceDef.Aliases = []string{"SOUTHEAST ORTHOPEDIC SPECIALISTS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1164143905"}}
