...
func GetSourceBerkeleyPrimaryCareMedicalCorpo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBerkeleyPrimaryCareMedicalCorpo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Berkeley Primary Care Medical Corpo"
	sourceDef.SourceType = pkg.SourceTypeBerkeleyPrimaryCareMedicalCorpo
