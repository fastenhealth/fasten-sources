...
func GetSourcePoplarBluffMedicalClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePoplarBluffMedicalClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Poplar Bluff Medical Clinic"
	sourceDef.SourceType = pkg.SourceTypePoplarBluffMedicalClinic
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"POPLAR BLUFF MEDICAL CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1881965267"}}
