...
func GetSourceLanierFootAndAnkleLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLanierFootAndAnkleLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lanier Foot and Ankle LLC"
	sourceDef.SourceType = pkg.SourceTypeLanierFootAndAnkleLlc
	sourceDef.Category = []string{"213ES0103X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1417511700"}}
	sourceDef.PatientAccessUrl = "https://lanierfootandankle.com/"
