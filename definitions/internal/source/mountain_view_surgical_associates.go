...
func GetSourceMountainViewSurgicalAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMountainViewSurgicalAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Mountain View Surgical Associates"
	sourceDef.SourceType = pkg.SourceTypeMountainViewSurgicalAssociates
	sourceDef.Category = []string{"208600000X"}
	sourceDef.Aliases = []string{"MOUNTAIN VIEW SURGICAL ASSOCIATES"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1821181876"}}
