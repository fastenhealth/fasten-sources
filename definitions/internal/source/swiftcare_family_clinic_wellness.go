...
func GetSourceSwiftcareFamilyClinicWellness(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSwiftcareFamilyClinicWellness]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Swiftcare Family Clinic  Wellness"
	sourceDef.SourceType = pkg.SourceTypeSwiftcareFamilyClinicWellness
