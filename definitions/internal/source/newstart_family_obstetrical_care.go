...
func GetSourceNewstartFamilyObstetricalCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNewstartFamilyObstetricalCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEWSTART FAMILY  OBSTETRICAL CARE"
	sourceDef.SourceType = pkg.SourceTypeNewstartFamilyObstetricalCare
