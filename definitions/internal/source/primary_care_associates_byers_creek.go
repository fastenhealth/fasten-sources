...
func GetSourcePrimaryCareAssociatesByersCreek(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrimaryCareAssociatesByersCreek]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Primary Care Associates  Byers Creek"
	sourceDef.SourceType = pkg.SourceTypePrimaryCareAssociatesByersCreek
