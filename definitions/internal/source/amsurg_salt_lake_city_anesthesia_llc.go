...
func GetSourceAmsurgSaltLakeCityAnesthesiaLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmsurgSaltLakeCityAnesthesiaLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Amsurg Salt Lake City Anesthesia LLC"
	sourceDef.SourceType = pkg.SourceTypeAmsurgSaltLakeCityAnesthesiaLlc
	sourceDef.Category = []string{"207L00000X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1871898031"}}
	sourceDef.BrandLogo = "amsurg-salt-lake-city-anesthesia-llc.jpg"
