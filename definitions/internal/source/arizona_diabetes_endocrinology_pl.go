...
func GetSourceArizonaDiabetesEndocrinologyPl(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArizonaDiabetesEndocrinologyPl]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arizona Diabetes  Endocrinology PL"
	sourceDef.SourceType = pkg.SourceTypeArizonaDiabetesEndocrinologyPl
