...
func GetSourceSaintMarysPrimaryCareNetworkLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSaintMarysPrimaryCareNetworkLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Saint Marys Primary Care Network LLC"
	sourceDef.SourceType = pkg.SourceTypeSaintMarysPrimaryCareNetworkLlc
