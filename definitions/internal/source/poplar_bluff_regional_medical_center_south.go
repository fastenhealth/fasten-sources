...
func GetSourcePoplarBluffRegionalMedicalCenterSouth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePoplarBluffRegionalMedicalCenterSouth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Poplar Bluff Regional Medical Center  South"
	sourceDef.SourceType = pkg.SourceTypePoplarBluffRegionalMedicalCenterSouth
