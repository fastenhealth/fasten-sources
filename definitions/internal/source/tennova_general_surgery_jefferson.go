...
func GetSourceTennovaGeneralSurgeryJefferson(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaGeneralSurgeryJefferson]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova General Surgery  Jefferson"
	sourceDef.SourceType = pkg.SourceTypeTennovaGeneralSurgeryJefferson
