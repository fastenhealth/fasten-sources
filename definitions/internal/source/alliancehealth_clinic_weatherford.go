...
func GetSourceAlliancehealthClinicWeatherford(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthClinicWeatherford]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Clinic Weatherford"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthClinicWeatherford
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"ALLIANCEHEALTH CLINIC WEATHERFORD"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1548797954"}}
