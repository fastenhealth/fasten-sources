...
func GetSourceAdultPrimaryCarePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdultPrimaryCarePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ADULT PRIMARY CARE PLLC"
	sourceDef.SourceType = pkg.SourceTypeAdultPrimaryCarePllc
	sourceDef.Category = []string{"207R00000X", "207RG0300X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1073287488"}}
	sourceDef.PatientAccessUrl = "https://aprimarycare.com/"
