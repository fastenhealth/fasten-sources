...
func GetSourceHoustonCoVolunteerMedicalClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHoustonCoVolunteerMedicalClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Houston Co Volunteer Medical Clinic"
	sourceDef.SourceType = pkg.SourceTypeHoustonCoVolunteerMedicalClinic
