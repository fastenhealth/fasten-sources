...
func GetSourceBrowardOphthalmologyGroupLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBrowardOphthalmologyGroupLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Broward Ophthalmology Group LLC"
	sourceDef.SourceType = pkg.SourceTypeBrowardOphthalmologyGroupLlc
