...
func GetSourceTakleEyeSurgeryCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTakleEyeSurgeryCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Takle Eye Surgery Center LLC"
	sourceDef.SourceType = pkg.SourceTypeTakleEyeSurgeryCenterLlc
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1740776517"}}
