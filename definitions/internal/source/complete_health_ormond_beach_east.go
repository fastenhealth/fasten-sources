...
func GetSourceCompleteHealthOrmondBeachEast(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCompleteHealthOrmondBeachEast]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Complete Health Ormond Beach East"
	sourceDef.SourceType = pkg.SourceTypeCompleteHealthOrmondBeachEast
