...
func GetSourceFloridaWellnessAndRegenerativeM(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFloridaWellnessAndRegenerativeM]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FLORIDA WELLNESS AND REGENERATIVE M"
	sourceDef.SourceType = pkg.SourceTypeFloridaWellnessAndRegenerativeM
