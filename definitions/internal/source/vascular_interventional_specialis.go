...
func GetSourceVascularInterventionalSpecialis(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVascularInterventionalSpecialis]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Vascular  Interventional Specialis"
	sourceDef.SourceType = pkg.SourceTypeVascularInterventionalSpecialis
