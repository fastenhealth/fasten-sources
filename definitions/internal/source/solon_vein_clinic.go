...
func GetSourceSolonVeinClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSolonVeinClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Solon Vein Clinic"
	sourceDef.SourceType = pkg.SourceTypeSolonVeinClinic
	sourceDef.Category = []string{"2086S0129X"}
	sourceDef.Aliases = []string{"SOLON VEIN CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1194373704"}}
	sourceDef.PatientAccessUrl = "http://www.solonveinclinic.com"
