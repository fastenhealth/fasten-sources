...
func GetSourceCentralArkansasScreeningAndMedi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralArkansasScreeningAndMedi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CENTRAL ARKANSAS SCREENING AND MEDI"
	sourceDef.SourceType = pkg.SourceTypeCentralArkansasScreeningAndMedi
