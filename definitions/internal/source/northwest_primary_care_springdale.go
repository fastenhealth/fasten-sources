...
func GetSourceNorthwestPrimaryCareSpringdale(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestPrimaryCareSpringdale]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Primary Care  Springdale"
	sourceDef.SourceType = pkg.SourceTypeNorthwestPrimaryCareSpringdale
