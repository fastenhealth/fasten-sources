...
func GetSourceCentralCaliforniaMovementDisorde(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralCaliforniaMovementDisorde]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Central California Movement Disorde"
	sourceDef.SourceType = pkg.SourceTypeCentralCaliforniaMovementDisorde
