...
func GetSourceResilienceHealthcareLakefrontMedicalAssociatesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeResilienceHealthcareLakefrontMedicalAssociatesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RESILIENCE HEALTHCARE LAKEFRONT MEDICAL ASSOCIATES LLC"
	sourceDef.SourceType = pkg.SourceTypeResilienceHealthcareLakefrontMedicalAssociatesLlc
