...
func GetSourceEastMemphisOrthopedicGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEastMemphisOrthopedicGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "East Memphis Orthopedic Group"
	sourceDef.SourceType = pkg.SourceTypeEastMemphisOrthopedicGroup
	sourceDef.Category = []string{"207X00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1023087475"}}
	sourceDef.PatientAccessUrl = "http://www.eastmemphisortho.com"
