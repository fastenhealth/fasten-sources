...
func GetSourceInsightTmsAndMentalHealthCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInsightTmsAndMentalHealthCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "INSIGHT TMS AND MENTAL HEALTH CARE"
	sourceDef.SourceType = pkg.SourceTypeInsightTmsAndMentalHealthCare
