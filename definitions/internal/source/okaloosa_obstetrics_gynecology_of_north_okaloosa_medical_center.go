...
func GetSourceOkaloosaObstetricsGynecologyOfNorthOkaloosaMedicalCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOkaloosaObstetricsGynecologyOfNorthOkaloosaMedicalCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Okaloosa Obstetrics  Gynecology of North Okaloosa Medical Center"
	sourceDef.SourceType = pkg.SourceTypeOkaloosaObstetricsGynecologyOfNorthOkaloosaMedicalCenter
