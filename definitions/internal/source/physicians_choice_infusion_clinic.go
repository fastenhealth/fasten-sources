...
func GetSourcePhysiciansChoiceInfusionClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePhysiciansChoiceInfusionClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Physicians Choice Infusion Clinic"
	sourceDef.SourceType = pkg.SourceTypePhysiciansChoiceInfusionClinic
