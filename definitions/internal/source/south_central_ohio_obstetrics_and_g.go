...
func GetSourceSouthCentralOhioObstetricsAndG(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthCentralOhioObstetricsAndG]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "South Central Ohio Obstetrics And G"
	sourceDef.SourceType = pkg.SourceTypeSouthCentralOhioObstetricsAndG
