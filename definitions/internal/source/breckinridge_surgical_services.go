...
func GetSourceBreckinridgeSurgicalServices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBreckinridgeSurgicalServices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Breckinridge Surgical Services"
	sourceDef.SourceType = pkg.SourceTypeBreckinridgeSurgicalServices
	sourceDef.Category = []string{"174400000X"}
	sourceDef.Aliases = []string{"BRECKINRIDGE SURGICAL SERVICES"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1730520875"}}
