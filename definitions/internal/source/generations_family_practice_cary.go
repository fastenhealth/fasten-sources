...
func GetSourceGenerationsFamilyPracticeCary(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGenerationsFamilyPracticeCary]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GENERATIONS FAMILY PRACTICE  CARY"
	sourceDef.SourceType = pkg.SourceTypeGenerationsFamilyPracticeCary
