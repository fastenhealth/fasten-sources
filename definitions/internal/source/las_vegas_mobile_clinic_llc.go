...
func GetSourceLasVegasMobileClinicLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLasVegasMobileClinicLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LAS VEGAS MOBILE CLINIC LLC"
	sourceDef.SourceType = pkg.SourceTypeLasVegasMobileClinicLlc
	sourceDef.Category = []string{"261QP2300X", "364SF0001X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1801560495"}}
