...
func GetSourceRvaAllergyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRvaAllergyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RVA Allergy LLC"
	sourceDef.SourceType = pkg.SourceTypeRvaAllergyLlc
	sourceDef.Category = []string{"207K00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1023610946"}}
	sourceDef.PatientAccessUrl = "http://rvaallergy.com"
