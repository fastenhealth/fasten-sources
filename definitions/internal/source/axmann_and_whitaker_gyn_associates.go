...
func GetSourceAxmannAndWhitakerGynAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAxmannAndWhitakerGynAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AXMANN AND WHITAKER GYN ASSOCIATES"
	sourceDef.SourceType = pkg.SourceTypeAxmannAndWhitakerGynAssociates
