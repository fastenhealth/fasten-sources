...
func GetSourceSandhillsSportsPerformance(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSandhillsSportsPerformance]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sandhills Sports Performance"
	sourceDef.SourceType = pkg.SourceTypeSandhillsSportsPerformance
	sourceDef.Category = []string{"225100000X", "261QP2000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1144632118", "1598186082"}}
