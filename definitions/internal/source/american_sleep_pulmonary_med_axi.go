...
func GetSourceAmericanSleepPulmonaryMedAxi(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmericanSleepPulmonaryMedAxi]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "American Sleep  Pulmonary Med Axi"
	sourceDef.SourceType = pkg.SourceTypeAmericanSleepPulmonaryMedAxi
