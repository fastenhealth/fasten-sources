...
func GetSourceRidgelinePhysicianServicesPlcD(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRidgelinePhysicianServicesPlcD]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ridgeline Physician Services PLC D"
	sourceDef.SourceType = pkg.SourceTypeRidgelinePhysicianServicesPlcD
