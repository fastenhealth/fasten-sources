...
func GetSourceRootedHealthAndWellnessLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRootedHealthAndWellnessLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ROOTED HEALTH AND WELLNESS LLC"
	sourceDef.SourceType = pkg.SourceTypeRootedHealthAndWellnessLlc
