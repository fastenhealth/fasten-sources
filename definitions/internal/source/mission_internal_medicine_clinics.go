...
func GetSourceMissionInternalMedicineClinics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMissionInternalMedicineClinics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MISSION INTERNAL MEDICINE CLINICS"
	sourceDef.SourceType = pkg.SourceTypeMissionInternalMedicineClinics
