...
func GetSourceSleepDiagnosticResearchOfAz(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSleepDiagnosticResearchOfAz]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SLEEP DIAGNOSTIC  RESEARCH OF AZ"
	sourceDef.SourceType = pkg.SourceTypeSleepDiagnosticResearchOfAz
