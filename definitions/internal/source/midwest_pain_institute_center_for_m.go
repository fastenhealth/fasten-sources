...
func GetSourceMidwestPainInstituteCenterForM(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidwestPainInstituteCenterForM]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MIDWEST PAIN INSTITUTE CENTER FOR M"
	sourceDef.SourceType = pkg.SourceTypeMidwestPainInstituteCenterForM
