...
func GetSourceJohnsonHealthSolutionsPllcNei(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJohnsonHealthSolutionsPllcNei]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Johnson Health Solutions PLLC Nei"
	sourceDef.SourceType = pkg.SourceTypeJohnsonHealthSolutionsPllcNei
