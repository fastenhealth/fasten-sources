...
func GetSourceRoyaleFootAndAnkleSurgeonsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRoyaleFootAndAnkleSurgeonsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ROYALE FOOT AND ANKLE SURGEONS LLC"
	sourceDef.SourceType = pkg.SourceTypeRoyaleFootAndAnkleSurgeonsLlc
