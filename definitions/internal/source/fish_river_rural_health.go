...
func GetSourceFishRiverRuralHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFishRiverRuralHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Fish River Rural Health"
	sourceDef.SourceType = pkg.SourceTypeFishRiverRuralHealth
	sourceDef.Category = []string{"261QF0400X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1174741094", "1376761288", "1467902668", "1528608437", "1568838928"}}
	sourceDef.PatientAccessUrl = "http://www.frrh.org"
