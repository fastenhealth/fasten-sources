...
func GetSourceFamilyPracticeAssociatesAtHamil(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFamilyPracticeAssociatesAtHamil]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FAMILY PRACTICE ASSOCIATES AT HAMIL"
	sourceDef.SourceType = pkg.SourceTypeFamilyPracticeAssociatesAtHamil
