...
func GetSourcePioneerCommunityCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePioneerCommunityCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pioneer Community Care"
	sourceDef.SourceType = pkg.SourceTypePioneerCommunityCare
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"PIONEER COMMUNITY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1285277566"}}
