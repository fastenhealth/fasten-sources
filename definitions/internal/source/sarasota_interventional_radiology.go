...
func GetSourceSarasotaInterventionalRadiology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSarasotaInterventionalRadiology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sarasota Interventional Radiology"
	sourceDef.SourceType = pkg.SourceTypeSarasotaInterventionalRadiology
	sourceDef.Category = []string{"2085R0204X"}
	sourceDef.Aliases = []string{"SARASOTA INTERVENTIONAL RADIOLOGY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1548879794"}}
