...
func GetSourceSouthBendOrthopaedicAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthBendOrthopaedicAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SOUTH BEND ORTHOPAEDIC ASSOCIATES"
	sourceDef.SourceType = pkg.SourceTypeSouthBendOrthopaedicAssociates
