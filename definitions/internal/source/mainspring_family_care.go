...
func GetSourceMainspringFamilyCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMainspringFamilyCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Mainspring Family Care"
	sourceDef.SourceType = pkg.SourceTypeMainspringFamilyCare
	sourceDef.Category = []string{"261Q00000X"}
	sourceDef.Aliases = []string{"MAINSPRING FAMILY CARE", "MAINSPRING FAMILY CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1619583754"}}
