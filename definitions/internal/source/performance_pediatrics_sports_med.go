...
func GetSourcePerformancePediatricsSportsMed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePerformancePediatricsSportsMed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Performance Pediatrics  Sports Med"
	sourceDef.SourceType = pkg.SourceTypePerformancePediatricsSportsMed
