...
func GetSourceUscAmbulatorySurgicalCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUscAmbulatorySurgicalCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "USC Ambulatory Surgical Center"
	sourceDef.SourceType = pkg.SourceTypeUscAmbulatorySurgicalCenter
	sourceDef.Category = []string{"261QA1903X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1518129576"}}
