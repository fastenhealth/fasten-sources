...
func GetSourceBlissHomeBasedPrimaryMedicalCa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBlissHomeBasedPrimaryMedicalCa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bliss Home Based Primary Medical Ca"
	sourceDef.SourceType = pkg.SourceTypeBlissHomeBasedPrimaryMedicalCa
