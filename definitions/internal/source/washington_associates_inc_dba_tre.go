...
func GetSourceWashingtonAssociatesIncDbaTre(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWashingtonAssociatesIncDbaTre]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Washington  Associates INC dba Tre"
	sourceDef.SourceType = pkg.SourceTypeWashingtonAssociatesIncDbaTre
