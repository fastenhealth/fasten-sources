...
func GetSourceHenricoDoctorsPediatricsEmergency(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHenricoDoctorsPediatricsEmergency]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Henrico Doctors Pediatrics Emergency"
	sourceDef.SourceType = pkg.SourceTypeHenricoDoctorsPediatricsEmergency
