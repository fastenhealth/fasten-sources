...
func GetSourceWeightLossMedicalProvidersPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWeightLossMedicalProvidersPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Weight Loss Medical Providers PC"
	sourceDef.SourceType = pkg.SourceTypeWeightLossMedicalProvidersPc
