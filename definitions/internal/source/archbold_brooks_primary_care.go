...
func GetSourceArchboldBrooksPrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArchboldBrooksPrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Archbold Brooks Primary Care"
	sourceDef.SourceType = pkg.SourceTypeArchboldBrooksPrimaryCare
	sourceDef.Category = []string{"261QM1300X"}
	sourceDef.Aliases = []string{"ARCHBOLD BROOKS PRIMARY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1063841138"}}
