...
func GetSourceJacksonPurchasePhysicianPractices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJacksonPurchasePhysicianPractices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Jackson Purchase Physician Practices"
	sourceDef.SourceType = pkg.SourceTypeJacksonPurchasePhysicianPractices
