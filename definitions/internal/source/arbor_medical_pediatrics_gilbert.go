...
func GetSourceArborMedicalPediatricsGilbert(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArborMedicalPediatricsGilbert]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arbor Medical Pediatrics Gilbert"
	sourceDef.SourceType = pkg.SourceTypeArborMedicalPediatricsGilbert
