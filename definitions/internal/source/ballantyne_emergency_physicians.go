...
func GetSourceBallantyneEmergencyPhysicians(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBallantyneEmergencyPhysicians]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BALLANTYNE EMERGENCY PHYSICIANS"
	sourceDef.SourceType = pkg.SourceTypeBallantyneEmergencyPhysicians
