...
func GetSourceCommunityFirstTherapyAndConsult(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCommunityFirstTherapyAndConsult]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Community First Therapy and Consult"
	sourceDef.SourceType = pkg.SourceTypeCommunityFirstTherapyAndConsult
