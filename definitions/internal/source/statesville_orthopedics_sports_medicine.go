...
func GetSourceStatesvilleOrthopedicsSportsMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStatesvilleOrthopedicsSportsMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Statesville Orthopedics  Sports Medicine"
	sourceDef.SourceType = pkg.SourceTypeStatesvilleOrthopedicsSportsMedicine
