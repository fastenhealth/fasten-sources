...
func GetSourceSouthStrandInternistsUrgentCareLlcCarolinaForest(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthStrandInternistsUrgentCareLlcCarolinaForest]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "South Strand Internists  Urgent Care LLC  Carolina Forest"
	sourceDef.SourceType = pkg.SourceTypeSouthStrandInternistsUrgentCareLlcCarolinaForest
