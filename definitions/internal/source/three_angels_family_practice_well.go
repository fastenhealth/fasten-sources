...
func GetSourceThreeAngelsFamilyPracticeWell(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeThreeAngelsFamilyPracticeWell]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Three Angels Family Practice  Well"
	sourceDef.SourceType = pkg.SourceTypeThreeAngelsFamilyPracticeWell
