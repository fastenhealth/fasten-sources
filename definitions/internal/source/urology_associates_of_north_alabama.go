...
func GetSourceUrologyAssociatesOfNorthAlabama(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUrologyAssociatesOfNorthAlabama]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Urology Associates of North Alabama"
	sourceDef.SourceType = pkg.SourceTypeUrologyAssociatesOfNorthAlabama
	sourceDef.Category = []string{"208800000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1245505536"}}
