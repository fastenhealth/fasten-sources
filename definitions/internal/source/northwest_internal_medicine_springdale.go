...
func GetSourceNorthwestInternalMedicineSpringdale(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestInternalMedicineSpringdale]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Internal Medicine  Springdale"
	sourceDef.SourceType = pkg.SourceTypeNorthwestInternalMedicineSpringdale
