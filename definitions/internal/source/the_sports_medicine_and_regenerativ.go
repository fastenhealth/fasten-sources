...
func GetSourceTheSportsMedicineAndRegenerativ(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheSportsMedicineAndRegenerativ]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "THE SPORTS MEDICINE AND REGENERATIV"
	sourceDef.SourceType = pkg.SourceTypeTheSportsMedicineAndRegenerativ
