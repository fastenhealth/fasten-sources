...
func GetSourceEpilepsyFoundationCentralSouth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEpilepsyFoundationCentralSouth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Epilepsy Foundation Central  South"
	sourceDef.SourceType = pkg.SourceTypeEpilepsyFoundationCentralSouth
