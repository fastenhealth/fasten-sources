...
func GetSourceIowaPediatricAsthmaAllergyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIowaPediatricAsthmaAllergyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Iowa Pediatric Asthma  Allergy LLC"
	sourceDef.SourceType = pkg.SourceTypeIowaPediatricAsthmaAllergyLlc
