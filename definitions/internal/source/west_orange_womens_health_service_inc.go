...
func GetSourceWestOrangeWomensHealthServiceInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWestOrangeWomensHealthServiceInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "West Orange Womens Health Service Inc"
	sourceDef.SourceType = pkg.SourceTypeWestOrangeWomensHealthServiceInc
