...
func GetSourceFortWayneallenCountyDepartment(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFortWayneallenCountyDepartment]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Fort WayneAllen County Department"
	sourceDef.SourceType = pkg.SourceTypeFortWayneallenCountyDepartment
