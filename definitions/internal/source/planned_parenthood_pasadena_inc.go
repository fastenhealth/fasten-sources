...
func GetSourcePlannedParenthoodPasadenaInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePlannedParenthoodPasadenaInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "PLANNED PARENTHOOD PASADENA INC"
	sourceDef.SourceType = pkg.SourceTypePlannedParenthoodPasadenaInc
