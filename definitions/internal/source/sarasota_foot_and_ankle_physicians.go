...
func GetSourceSarasotaFootAndAnklePhysicians(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSarasotaFootAndAnklePhysicians]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Sarasota Foot And Ankle Physicians"
	sourceDef.SourceType = pkg.SourceTypeSarasotaFootAndAnklePhysicians
