...
func GetSourceDrPatrickHeronFacogLlcTermed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDrPatrickHeronFacogLlcTermed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DR PATRICK HERON FACOG LLC TERMED"
	sourceDef.SourceType = pkg.SourceTypeDrPatrickHeronFacogLlcTermed
