...
func GetSourceAmsurgChattanoogaAnesthesiaLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmsurgChattanoogaAnesthesiaLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "AmSurg Chattanooga Anesthesia LLC"
	sourceDef.SourceType = pkg.SourceTypeAmsurgChattanoogaAnesthesiaLlc
	sourceDef.Category = []string{"207L00000X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1841586922"}}
