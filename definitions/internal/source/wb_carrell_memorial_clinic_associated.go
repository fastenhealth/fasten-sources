...
func GetSourceWbCarrellMemorialClinicAssociated(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWbCarrellMemorialClinicAssociated]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "WB Carrell Memorial Clinic Associated"
	sourceDef.SourceType = pkg.SourceTypeWbCarrellMemorialClinicAssociated
