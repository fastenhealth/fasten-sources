...
func GetSourcePediatricsOfBartlesville(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePediatricsOfBartlesville]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pediatrics of Bartlesville"
	sourceDef.SourceType = pkg.SourceTypePediatricsOfBartlesville
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1164545216"}}
	sourceDef.PatientAccessUrl = "http://www.pediatricsofbartlesville.com/"
