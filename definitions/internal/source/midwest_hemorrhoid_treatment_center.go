...
func GetSourceMidwestHemorrhoidTreatmentCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidwestHemorrhoidTreatmentCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Midwest Hemorrhoid Treatment Center"
	sourceDef.SourceType = pkg.SourceTypeMidwestHemorrhoidTreatmentCenter
