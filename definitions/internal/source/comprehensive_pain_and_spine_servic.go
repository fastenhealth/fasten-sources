...
func GetSourceComprehensivePainAndSpineServic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeComprehensivePainAndSpineServic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COMPREHENSIVE PAIN AND SPINE SERVIC"
	sourceDef.SourceType = pkg.SourceTypeComprehensivePainAndSpineServic
