...
func GetSourceMelbourneHandCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMelbourneHandCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Melbourne Hand Center"
	sourceDef.SourceType = pkg.SourceTypeMelbourneHandCenter
	sourceDef.Category = []string{"207XS0106X"}
	sourceDef.Aliases = []string{"MELBOURNE HAND CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1568869113"}}
