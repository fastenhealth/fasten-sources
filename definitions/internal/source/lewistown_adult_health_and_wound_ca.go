...
func GetSourceLewistownAdultHealthAndWoundCa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLewistownAdultHealthAndWoundCa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Lewistown Adult Health and Wound Ca"
	sourceDef.SourceType = pkg.SourceTypeLewistownAdultHealthAndWoundCa
