...
func GetSourceBreamMedicalSalisbury(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBreamMedicalSalisbury]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bream Medical Salisbury"
	sourceDef.SourceType = pkg.SourceTypeBreamMedicalSalisbury
	sourceDef.Category = []string{"208D00000X"}
	sourceDef.Aliases = []string{"BREAM MEDICAL SALISBURY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1275291361"}}
