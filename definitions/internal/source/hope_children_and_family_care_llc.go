...
func GetSourceHopeChildrenAndFamilyCareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHopeChildrenAndFamilyCareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hope Children and family Care LLC"
	sourceDef.SourceType = pkg.SourceTypeHopeChildrenAndFamilyCareLlc
