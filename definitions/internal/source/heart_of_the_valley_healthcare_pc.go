...
func GetSourceHeartOfTheValleyHealthcarePc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHeartOfTheValleyHealthcarePc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Heart of the Valley Healthcare PC"
	sourceDef.SourceType = pkg.SourceTypeHeartOfTheValleyHealthcarePc
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1710925797"}}
	sourceDef.PatientAccessUrl = "http://heartofthevalleyhealthcare.com"
