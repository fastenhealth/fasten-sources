...
func GetSourcePsychiatricProfessionalsOfGeorgiaPoweredByGeodeHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePsychiatricProfessionalsOfGeorgiaPoweredByGeodeHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Psychiatric Professionals of Georgia Powered by Geode Health"
	sourceDef.SourceType = pkg.SourceTypePsychiatricProfessionalsOfGeorgiaPoweredByGeodeHealth
