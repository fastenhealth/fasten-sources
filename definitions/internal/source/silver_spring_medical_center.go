...
func GetSourceSilverSpringMedicalCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSilverSpringMedicalCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Silver Spring Medical Center"
	sourceDef.SourceType = pkg.SourceTypeSilverSpringMedicalCenter
	sourceDef.Category = []string{"302R00000X"}
	sourceDef.Aliases = []string{"SILVER SPRING MEDICAL CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1649390808"}}
