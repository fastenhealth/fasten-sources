...
func GetSourceEvanstonHospitalCorporationBvmgbridgerValleyMedicalGroup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEvanstonHospitalCorporationBvmgbridgerValleyMedicalGroup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "EVANSTON HOSPITAL CORPORATION BVMGBridger Valley Medical Group"
	sourceDef.SourceType = pkg.SourceTypeEvanstonHospitalCorporationBvmgbridgerValleyMedicalGroup
