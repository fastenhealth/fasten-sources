...
func GetSourceAscensionViaChristiBehavioralHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAscensionViaChristiBehavioralHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ascension Via Christi Behavioral Health"
	sourceDef.SourceType = pkg.SourceTypeAscensionViaChristiBehavioralHealth
