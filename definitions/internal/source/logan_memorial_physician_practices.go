...
func GetSourceLoganMemorialPhysicianPractices(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLoganMemorialPhysicianPractices]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Logan Memorial Physician Practices"
	sourceDef.SourceType = pkg.SourceTypeLoganMemorialPhysicianPractices
