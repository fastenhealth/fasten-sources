...
func GetSourceJasmeetGillMdInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeJasmeetGillMdInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Jasmeet Gill MD Inc"
	sourceDef.SourceType = pkg.SourceTypeJasmeetGillMdInc
	sourceDef.PatientAccessUrl = "https://www.healthgrades.com/physician/dr-jasmeet-gill-2s3yl"
