...
func GetSourceRowleyPrimaryCareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRowleyPrimaryCareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ROWLEY PRIMARY CARE LLC"
	sourceDef.SourceType = pkg.SourceTypeRowleyPrimaryCareLlc
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{"PROVENTIVE CARE SOLUTIONS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1811652894"}}
