...
func GetSourceGoodShepherdCommunityClinicInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGoodShepherdCommunityClinicInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Good Shepherd Community Clinic Inc"
	sourceDef.SourceType = pkg.SourceTypeGoodShepherdCommunityClinicInc
