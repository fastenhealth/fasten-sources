...
func GetSourceSanAntonioWomensHealthPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSanAntonioWomensHealthPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SAN ANTONIO WOMENS HEALTH PLLC"
	sourceDef.SourceType = pkg.SourceTypeSanAntonioWomensHealthPllc
	sourceDef.Category = []string{"174400000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1104369818"}}
	sourceDef.PatientAccessUrl = "http://www.sanantoniowomenshealth.com"
