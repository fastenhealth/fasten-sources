...
func GetSourceDiscoveryMentalHealthServicesPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDiscoveryMentalHealthServicesPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Discovery Mental Health Services PA"
	sourceDef.SourceType = pkg.SourceTypeDiscoveryMentalHealthServicesPa
