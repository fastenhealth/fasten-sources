...
func GetSourceElizabethJMcconnellMdPlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeElizabethJMcconnellMdPlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Elizabeth J McConnell MD PLC"
	sourceDef.SourceType = pkg.SourceTypeElizabethJMcconnellMdPlc
	sourceDef.Category = []string{"208600000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1255309712"}}
	sourceDef.PatientAccessUrl = "http://www.mccolon.com"
