...
func GetSourceWalkerConciergeHealthAndWellnes(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWalkerConciergeHealthAndWellnes]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Walker Concierge Health and Wellnes"
	sourceDef.SourceType = pkg.SourceTypeWalkerConciergeHealthAndWellnes
