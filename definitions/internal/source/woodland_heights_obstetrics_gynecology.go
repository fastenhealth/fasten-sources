...
func GetSourceWoodlandHeightsObstetricsGynecology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWoodlandHeightsObstetricsGynecology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Woodland Heights Obstetrics  Gynecology"
	sourceDef.SourceType = pkg.SourceTypeWoodlandHeightsObstetricsGynecology
