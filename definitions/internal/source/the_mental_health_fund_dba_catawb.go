...
func GetSourceTheMentalHealthFundDbaCatawb(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheMentalHealthFundDbaCatawb]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Mental Health Fund dba Catawb"
	sourceDef.SourceType = pkg.SourceTypeTheMentalHealthFundDbaCatawb
