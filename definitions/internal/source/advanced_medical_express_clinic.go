...
func GetSourceAdvancedMedicalExpressClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedMedicalExpressClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Advanced Medical Express Clinic"
	sourceDef.SourceType = pkg.SourceTypeAdvancedMedicalExpressClinic
	sourceDef.Category = []string{"261QP2300X", "363LF0000X"}
	sourceDef.Aliases = []string{"ADVANCED MEDICAL EXPRESS CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1083088058"}}
