...
func GetSourceAnnapolisNephrologyAssociatesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAnnapolisNephrologyAssociatesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Annapolis Nephrology Associates LLC"
	sourceDef.SourceType = pkg.SourceTypeAnnapolisNephrologyAssociatesLlc
