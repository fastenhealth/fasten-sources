...
func GetSourceCardiovascularInstituteOfAmerica(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCardiovascularInstituteOfAmerica]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cardiovascular Institute Of America"
	sourceDef.SourceType = pkg.SourceTypeCardiovascularInstituteOfAmerica
