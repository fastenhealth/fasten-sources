...
func GetSourcePlasticreconstructiveSurgeryInst(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePlasticreconstructiveSurgeryInst]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PlasticReconstructive Surgery Inst"
	sourceDef.SourceType = pkg.SourceTypePlasticreconstructiveSurgeryInst
