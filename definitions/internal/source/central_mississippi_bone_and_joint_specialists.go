...
func GetSourceCentralMississippiBoneAndJointSpecialists(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralMississippiBoneAndJointSpecialists]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Central Mississippi Bone and Joint Specialists"
	sourceDef.SourceType = pkg.SourceTypeCentralMississippiBoneAndJointSpecialists
