...
func GetSourceVincentAlarconPhysicalTherapist(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVincentAlarconPhysicalTherapist]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VINCENT ALARCON PHYSICAL THERAPIST"
	sourceDef.SourceType = pkg.SourceTypeVincentAlarconPhysicalTherapist
