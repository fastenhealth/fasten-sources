...
func GetSourceGlaucomaInstituteOfAustinPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGlaucomaInstituteOfAustinPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "GLAUCOMA INSTITUTE OF AUSTIN PA"
	sourceDef.SourceType = pkg.SourceTypeGlaucomaInstituteOfAustinPa
	sourceDef.Category = []string{"207W00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1285671271"}}
	sourceDef.PatientAccessUrl = "http://www.glaucomainstitute.net/"
