...
func GetSourceArkansasCenterForArthroscopyPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArkansasCenterForArthroscopyPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arkansas Center for Arthroscopy PA"
	sourceDef.SourceType = pkg.SourceTypeArkansasCenterForArthroscopyPa
