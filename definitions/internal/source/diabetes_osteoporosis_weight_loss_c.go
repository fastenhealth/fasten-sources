...
func GetSourceDiabetesOsteoporosisWeightLossC(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDiabetesOsteoporosisWeightLossC]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Diabetes Osteoporosis Weight Loss C"
	sourceDef.SourceType = pkg.SourceTypeDiabetesOsteoporosisWeightLossC
