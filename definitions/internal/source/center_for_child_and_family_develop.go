...
func GetSourceCenterForChildAndFamilyDevelop(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCenterForChildAndFamilyDevelop]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Center for Child and Family Develop"
	sourceDef.SourceType = pkg.SourceTypeCenterForChildAndFamilyDevelop
