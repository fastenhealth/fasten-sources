...
func GetSourceGraystoneOphthalmologyAssociatesPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGraystoneOphthalmologyAssociatesPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Graystone Ophthalmology Associates PA"
	sourceDef.SourceType = pkg.SourceTypeGraystoneOphthalmologyAssociatesPa
	sourceDef.PatientAccessUrl = "https://www.graystone-eye.com/offices/"
