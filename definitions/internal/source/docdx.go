...
func GetSourceDocdx(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDocdx]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DocDX"
	sourceDef.SourceType = pkg.SourceTypeDocdx
	sourceDef.Category = []string{"207Q00000X", "207R00000X", "261QP2300X"}
	sourceDef.Aliases = []string{"DOCDX", "DOCDX.COM, INC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1770204638"}}
