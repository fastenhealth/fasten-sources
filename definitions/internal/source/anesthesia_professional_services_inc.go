...
func GetSourceAnesthesiaProfessionalServicesInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAnesthesiaProfessionalServicesInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Anesthesia Professional Services Inc"
	sourceDef.SourceType = pkg.SourceTypeAnesthesiaProfessionalServicesInc
