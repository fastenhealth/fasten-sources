...
func GetSourceNeurosurgicalServicesOfOaklandP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeurosurgicalServicesOfOaklandP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEUROSURGICAL SERVICES OF OAKLAND P"
	sourceDef.SourceType = pkg.SourceTypeNeurosurgicalServicesOfOaklandP
