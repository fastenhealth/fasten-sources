...
func GetSourceNortheastRegionalMedicalcenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNortheastRegionalMedicalcenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northeast Regional MedicalCenter"
	sourceDef.SourceType = pkg.SourceTypeNortheastRegionalMedicalcenter
