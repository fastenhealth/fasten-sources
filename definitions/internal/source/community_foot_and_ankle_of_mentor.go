...
func GetSourceCommunityFootAndAnkleOfMentor(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCommunityFootAndAnkleOfMentor]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COMMUNITY FOOT AND ANKLE OF MENTOR"
	sourceDef.SourceType = pkg.SourceTypeCommunityFootAndAnkleOfMentor
