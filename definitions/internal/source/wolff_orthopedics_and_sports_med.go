...
func GetSourceWolffOrthopedicsAndSportsMed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeWolffOrthopedicsAndSportsMed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Wolff Orthopedics and Sports Med"
	sourceDef.SourceType = pkg.SourceTypeWolffOrthopedicsAndSportsMed
