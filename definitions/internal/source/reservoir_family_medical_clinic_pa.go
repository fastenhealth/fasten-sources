...
func GetSourceReservoirFamilyMedicalClinicPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeReservoirFamilyMedicalClinicPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Reservoir Family Medical Clinic PA"
	sourceDef.SourceType = pkg.SourceTypeReservoirFamilyMedicalClinicPa
