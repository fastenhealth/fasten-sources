...
func GetSourceAlliancehealthMedicalGroupGeneralSurgeryPoncaCity(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupGeneralSurgeryPoncaCity]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group General Surgery Ponca City"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupGeneralSurgeryPoncaCity
