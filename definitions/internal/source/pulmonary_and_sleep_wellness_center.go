...
func GetSourcePulmonaryAndSleepWellnessCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePulmonaryAndSleepWellnessCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PULMONARY AND SLEEP WELLNESS CENTER"
	sourceDef.SourceType = pkg.SourceTypePulmonaryAndSleepWellnessCenter
