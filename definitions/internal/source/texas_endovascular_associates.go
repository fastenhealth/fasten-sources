...
func GetSourceTexasEndovascularAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexasEndovascularAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Texas Endovascular Associates"
	sourceDef.SourceType = pkg.SourceTypeTexasEndovascularAssociates
	sourceDef.Category = []string{"2085R0204X"}
	sourceDef.Aliases = []string{"TEXAS ENDOVASCULAR ASSOCIATES"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1972221166"}}
