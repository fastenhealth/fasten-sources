...
func GetSourceSeattleOrthopedicCenterMri(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSeattleOrthopedicCenterMri]; clientIdOk {
	...
	sourceDef.Category = []string{"2085R0202X", "261QM1200X"}
	sourceDef.Aliases = []string{"SEATTLE ORTHOPEDIC CENTER MRI"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1003441783"}}
