...
func GetSourcePediatricHealthcareUnlimited(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePediatricHealthcareUnlimited]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PEDIATRIC HEALTHCARE UNLIMITED"
	sourceDef.SourceType = pkg.SourceTypePediatricHealthcareUnlimited
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1982766390"}}
	sourceDef.PatientAccessUrl = "http://www.altonpeds.com"
