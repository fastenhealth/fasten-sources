...
func GetSourceGreenfieldFamilyMedicineClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGreenfieldFamilyMedicineClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GREENFIELD FAMILY MEDICINE CLINIC"
	sourceDef.SourceType = pkg.SourceTypeGreenfieldFamilyMedicineClinic
