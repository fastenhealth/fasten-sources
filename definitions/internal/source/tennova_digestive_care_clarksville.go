...
func GetSourceTennovaDigestiveCareClarksville(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaDigestiveCareClarksville]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Digestive Care  Clarksville"
	sourceDef.SourceType = pkg.SourceTypeTennovaDigestiveCareClarksville
