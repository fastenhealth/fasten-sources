...
func GetSourcePrimeTelecareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrimeTelecareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PRIME TELECARE LLC"
	sourceDef.SourceType = pkg.SourceTypePrimeTelecareLlc
	sourceDef.Category = []string{"363L00000X"}
	sourceDef.Aliases = []string{"PRIME FAMILY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1629678347"}}
	sourceDef.PatientAccessUrl = "http://www.primefamilycare.com"
