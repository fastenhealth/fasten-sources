...
func GetSourceCommunityHealthClinic1(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCommunityHealthClinic1]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Community Health Clinic"
	sourceDef.SourceType = pkg.SourceTypeCommunityHealthClinic1
	sourceDef.Category = []string{"207Q00000X", "208000000X", "363LF0000X", "363LW0102X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1063407922"}}
