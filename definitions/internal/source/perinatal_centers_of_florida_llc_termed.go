...
func GetSourcePerinatalCentersOfFloridaLlcTermed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePerinatalCentersOfFloridaLlcTermed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PERINATAL CENTERS OF FLORIDA LLC TERMED"
	sourceDef.SourceType = pkg.SourceTypePerinatalCentersOfFloridaLlcTermed
