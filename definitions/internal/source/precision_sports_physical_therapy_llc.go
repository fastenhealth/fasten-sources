...
func GetSourcePrecisionSportsPhysicalTherapyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrecisionSportsPhysicalTherapyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PRECISION SPORTS PHYSICAL THERAPY LLC"
	sourceDef.SourceType = pkg.SourceTypePrecisionSportsPhysicalTherapyLlc
