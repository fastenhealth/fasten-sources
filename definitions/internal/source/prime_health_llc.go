...
func GetSourcePrimeHealthLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePrimeHealthLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Prime Health LLC"
	sourceDef.SourceType = pkg.SourceTypePrimeHealthLlc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"PRIME HEALTH MEDICAL CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1326510652"}}
