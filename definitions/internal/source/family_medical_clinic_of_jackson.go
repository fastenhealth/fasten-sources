...
func GetSourceFamilyMedicalClinicOfJackson(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFamilyMedicalClinicOfJackson]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Family Medical Clinic of Jackson"
	sourceDef.SourceType = pkg.SourceTypeFamilyMedicalClinicOfJackson
	sourceDef.Category = []string{"261QP2300X"}
	sourceDef.Aliases = []string{"FAMILY MEDICAL CLINIC OF JACKSON"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1215693171"}}
