...
func GetSourceSpinemdPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSpinemdPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SPINEMD PLLC"
	sourceDef.SourceType = pkg.SourceTypeSpinemdPllc
	sourceDef.Category = []string{"207X00000X", "207XS0117X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1851850291"}}
	sourceDef.PatientAccessUrl = "http://www.myspinemd.com"
