...
func GetSourceFlowTherapy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFlowTherapy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Flow Therapy"
	sourceDef.SourceType = pkg.SourceTypeFlowTherapy
	sourceDef.Category = []string{"207RC0000X"}
	sourceDef.Aliases = []string{"FLOW THERAPY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1144520503"}}
