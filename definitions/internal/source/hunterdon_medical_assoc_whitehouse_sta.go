...
func GetSourceHunterdonMedicalAssocWhitehouseSta(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHunterdonMedicalAssocWhitehouseSta]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Hunterdon Medical Assoc Whitehouse Sta"
	sourceDef.SourceType = pkg.SourceTypeHunterdonMedicalAssocWhitehouseSta
