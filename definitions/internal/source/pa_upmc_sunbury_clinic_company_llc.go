...
func GetSourcePaUpmcSunburyClinicCompanyLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePaUpmcSunburyClinicCompanyLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PA  UPMC  Sunbury Clinic Company LLC"
	sourceDef.SourceType = pkg.SourceTypePaUpmcSunburyClinicCompanyLlc
