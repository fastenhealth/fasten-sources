...
func GetSourceGynWomensCentreOfLakewoodRanch(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGynWomensCentreOfLakewoodRanch]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "GYN WOMENS CENTRE OF LAKEWOOD RANCH"
	sourceDef.SourceType = pkg.SourceTypeGynWomensCentreOfLakewoodRanch
