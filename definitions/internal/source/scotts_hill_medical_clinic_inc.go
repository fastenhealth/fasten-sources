...
func GetSourceScottsHillMedicalClinicInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeScottsHillMedicalClinicInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "SCOTTS HILL MEDICAL CLINIC INC"
	sourceDef.SourceType = pkg.SourceTypeScottsHillMedicalClinicInc
