...
func GetSourceDwellFamilyDoctors(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDwellFamilyDoctors]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Dwell Family Doctors"
	sourceDef.SourceType = pkg.SourceTypeDwellFamilyDoctors
	sourceDef.Category = []string{"174400000X"}
	sourceDef.Aliases = []string{"DWELL FAMILY DOCTORS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1801127576"}}
