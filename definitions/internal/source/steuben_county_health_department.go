...
func GetSourceSteubenCountyHealthDepartment(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSteubenCountyHealthDepartment]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Steuben County Health Department"
	sourceDef.SourceType = pkg.SourceTypeSteubenCountyHealthDepartment
