...
func GetSourceArkansasHealthcareServicesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeArkansasHealthcareServicesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Arkansas Healthcare Services LLC"
	sourceDef.SourceType = pkg.SourceTypeArkansasHealthcareServicesLlc
