...
func GetSourceAlMedicalCaresAssociatesPrattvil(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlMedicalCaresAssociatesPrattvil]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AL  Medical Cares Associates  Prattvil"
	sourceDef.SourceType = pkg.SourceTypeAlMedicalCaresAssociatesPrattvil
