...
func GetSourceShoalsFamilyPracticeUrgentCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeShoalsFamilyPracticeUrgentCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Shoals Family Practice Urgent Care"
	sourceDef.SourceType = pkg.SourceTypeShoalsFamilyPracticeUrgentCare
