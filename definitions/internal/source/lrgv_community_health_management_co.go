...
func GetSourceLrgvCommunityHealthManagementCo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLrgvCommunityHealthManagementCo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LRGV Community Health Management Co"
	sourceDef.SourceType = pkg.SourceTypeLrgvCommunityHealthManagementCo
