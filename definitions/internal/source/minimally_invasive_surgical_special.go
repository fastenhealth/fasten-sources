...
func GetSourceMinimallyInvasiveSurgicalSpecial(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMinimallyInvasiveSurgicalSpecial]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Minimally Invasive Surgical Special"
	sourceDef.SourceType = pkg.SourceTypeMinimallyInvasiveSurgicalSpecial
