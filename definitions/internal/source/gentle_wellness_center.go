...
func GetSourceGentleWellnessCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGentleWellnessCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gentle Wellness Center"
	sourceDef.SourceType = pkg.SourceTypeGentleWellnessCenter
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"GENTLE WELLNESS CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1194209544"}}
