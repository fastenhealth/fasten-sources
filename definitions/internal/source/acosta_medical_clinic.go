...
func GetSourceAcostaMedicalClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAcostaMedicalClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Acosta Medical Clinic"
	sourceDef.SourceType = pkg.SourceTypeAcostaMedicalClinic
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"ACOSTA MEDICAL CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1316519937"}}
	sourceDef.PatientAccessUrl = "https://acostamedicalclinic.com/"
