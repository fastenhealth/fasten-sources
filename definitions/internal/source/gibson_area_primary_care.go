...
func GetSourceGibsonAreaPrimaryCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGibsonAreaPrimaryCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Gibson Area Primary Care"
	sourceDef.SourceType = pkg.SourceTypeGibsonAreaPrimaryCare
	sourceDef.Category = []string{"261QM1300X", "261QR1300X"}
	sourceDef.Aliases = []string{"GIBSON AREA PRIMARY CARE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1740641745"}}
	sourceDef.BrandLogo = "gibson-area-primary-care.jpg"
