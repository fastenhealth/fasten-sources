...
func GetSourceBreckinridgeHealthClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBreckinridgeHealthClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BRECKINRIDGE HEALTH CLINIC"
	sourceDef.SourceType = pkg.SourceTypeBreckinridgeHealthClinic
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{"BRECKINRIDGE HEALTH INC."}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1598384745"}}
