...
func GetSourceDuvielIrizarryLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDuvielIrizarryLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Duviel Irizarry LLC"
	sourceDef.SourceType = pkg.SourceTypeDuvielIrizarryLlc
	sourceDef.Category = []string{"261QM2500X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1437597432"}}
	sourceDef.PatientAccessUrl = "http://www.nomercysportsmedicine.com/"
