...
func GetSourcePureLifeNmInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePureLifeNmInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pure Life NM Inc"
	sourceDef.SourceType = pkg.SourceTypePureLifeNmInc
	sourceDef.Category = []string{"363AM0700X"}
	sourceDef.Aliases = []string{"PURE LIFE FAMILY PRACTICE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1275139958"}}
