...
func GetSourceZzzdeleteseattleDispatchhealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeZzzdeleteseattleDispatchhealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "zzzDeleteSeattle DispatchHealth"
	sourceDef.SourceType = pkg.SourceTypeZzzdeleteseattleDispatchhealth
