...
func GetSourceAdamsEndocrinologyAssociatesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdamsEndocrinologyAssociatesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Adams Endocrinology Associates LLC"
	sourceDef.SourceType = pkg.SourceTypeAdamsEndocrinologyAssociatesLlc
