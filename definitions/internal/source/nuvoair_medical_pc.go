...
func GetSourceNuvoairMedicalPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNuvoairMedicalPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NuvoAir Medical PC"
	sourceDef.SourceType = pkg.SourceTypeNuvoairMedicalPc
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"NUVOAIR MEDICAL"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1417688680"}}
