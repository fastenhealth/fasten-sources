...
func GetSourcePerfectFormCenterForWellness(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePerfectFormCenterForWellness]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PERFECT FORM CENTER FOR WELLNESS "
	sourceDef.SourceType = pkg.SourceTypePerfectFormCenterForWellness
