...
func GetSourcePlannedParenthoodOfMetropolitanNj(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePlannedParenthoodOfMetropolitanNj]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Planned Parenthood of Metropolitan NJ"
	sourceDef.SourceType = pkg.SourceTypePlannedParenthoodOfMetropolitanNj
	sourceDef.Category = []string{"261QA0005X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1386617686", "1508839804", "1649243932", "1760455695"}}
