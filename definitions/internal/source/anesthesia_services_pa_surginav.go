...
func GetSourceAnesthesiaServicesPaSurginav(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAnesthesiaServicesPaSurginav]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = " Anesthesia Services PA SurgiNav"
	sourceDef.SourceType = pkg.SourceTypeAnesthesiaServicesPaSurginav
