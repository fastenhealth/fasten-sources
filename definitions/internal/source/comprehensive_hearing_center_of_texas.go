...
func GetSourceComprehensiveHearingCenterOfTexas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeComprehensiveHearingCenterOfTexas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COMPREHENSIVE HEARING CENTER OF TEXAS"
	sourceDef.SourceType = pkg.SourceTypeComprehensiveHearingCenterOfTexas
