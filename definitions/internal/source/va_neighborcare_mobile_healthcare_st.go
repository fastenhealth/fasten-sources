...
func GetSourceVaNeighborcareMobileHealthcareSt(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVaNeighborcareMobileHealthcareSt]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "VA  NeighborCare Mobile Healthcare  St"
	sourceDef.SourceType = pkg.SourceTypeVaNeighborcareMobileHealthcareSt
