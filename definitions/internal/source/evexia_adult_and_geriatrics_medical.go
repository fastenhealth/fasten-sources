...
func GetSourceEvexiaAdultAndGeriatricsMedical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEvexiaAdultAndGeriatricsMedical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Evexia Adult and Geriatrics Medical"
	sourceDef.SourceType = pkg.SourceTypeEvexiaAdultAndGeriatricsMedical
