...
func GetSourceIntegrativeCardiovascularHealthAndWellness(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIntegrativeCardiovascularHealthAndWellness]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Integrative Cardiovascular Health and Wellness"
	sourceDef.SourceType = pkg.SourceTypeIntegrativeCardiovascularHealthAndWellness
