...
func GetSourceDunnahoeManagementHoldingsLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDunnahoeManagementHoldingsLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "DUNNAHOE MANAGEMENT  HOLDINGS LLC"
	sourceDef.SourceType = pkg.SourceTypeDunnahoeManagementHoldingsLlc
