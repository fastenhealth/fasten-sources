...
func GetSourceMidlandsOrthopaedicsAndNeurosurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidlandsOrthopaedicsAndNeurosurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Midlands Orthopaedics and Neurosurgery"
	sourceDef.SourceType = pkg.SourceTypeMidlandsOrthopaedicsAndNeurosurgery
