...
func GetSourceNorthsidePulmonarySleepMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthsidePulmonarySleepMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northside Pulmonary Sleep Medicine"
	sourceDef.SourceType = pkg.SourceTypeNorthsidePulmonarySleepMedicine
