...
func GetSourceBluffSportsMedicineOrthopedicClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBluffSportsMedicineOrthopedicClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bluff Sports Medicine  Orthopedic Clinic"
	sourceDef.SourceType = pkg.SourceTypeBluffSportsMedicineOrthopedicClinic
