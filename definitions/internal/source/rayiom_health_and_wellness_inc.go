...
func GetSourceRayiomHealthAndWellnessInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRayiomHealthAndWellnessInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RAYIOM HEALTH AND WELLNESS INC"
	sourceDef.SourceType = pkg.SourceTypeRayiomHealthAndWellnessInc
