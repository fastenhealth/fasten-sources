...
func GetSourcePikesPeakInternalMedicineAssoci(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePikesPeakInternalMedicineAssoci]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pikes Peak Internal Medicine Associ"
	sourceDef.SourceType = pkg.SourceTypePikesPeakInternalMedicineAssoci
