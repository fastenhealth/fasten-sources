...
func GetSourceUspiJacksonvilleEndoscopy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUspiJacksonvilleEndoscopy]; clientIdOk {
	...
	sourceDef.Aliases = []string{"JACKSONVILLE CENTER FOR ENDOSCOPY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1366982845"}}
	sourceDef.PatientAccessUrl = "https://www.jacksonvilleendoscopy.com/"
