...
func GetSourceHalifaxEndocrinologyAndOsteoporo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHalifaxEndocrinologyAndOsteoporo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Halifax Endocrinology and Osteoporo"
	sourceDef.SourceType = pkg.SourceTypeHalifaxEndocrinologyAndOsteoporo
