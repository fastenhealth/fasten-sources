...
func GetSourceHopeFamilyTherapyAndConsultatio(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHopeFamilyTherapyAndConsultatio]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hope Family Therapy and Consultatio"
	sourceDef.SourceType = pkg.SourceTypeHopeFamilyTherapyAndConsultatio
