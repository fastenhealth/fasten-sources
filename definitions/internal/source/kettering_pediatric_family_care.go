...
func GetSourceKetteringPediatricFamilyCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKetteringPediatricFamilyCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "KETTERING PEDIATRIC  FAMILY CARE"
	sourceDef.SourceType = pkg.SourceTypeKetteringPediatricFamilyCare
