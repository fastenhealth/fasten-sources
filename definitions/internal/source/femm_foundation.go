...
func GetSourceFemmFoundation(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFemmFoundation]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Femm Foundation"
	sourceDef.SourceType = pkg.SourceTypeFemmFoundation
	sourceDef.Category = []string{"207Q00000X", "207VG0400X"}
	sourceDef.Aliases = []string{"FEMM HEALTH CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1144642794"}}
