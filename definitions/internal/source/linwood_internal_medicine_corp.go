...
func GetSourceLinwoodInternalMedicineCorp(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLinwoodInternalMedicineCorp]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LINWOOD INTERNAL MEDICINE CORP"
	sourceDef.SourceType = pkg.SourceTypeLinwoodInternalMedicineCorp
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1225705775"}}
	sourceDef.PatientAccessUrl = "http://drgracefiorentino.com"
