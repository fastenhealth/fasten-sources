...
func GetSourceFlFamiliarMedicalServicesLlcAth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFlFamiliarMedicalServicesLlcAth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FL  Familiar Medical Services LLC  ath"
	sourceDef.SourceType = pkg.SourceTypeFlFamiliarMedicalServicesLlcAth
