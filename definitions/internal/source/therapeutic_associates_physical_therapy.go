...
func GetSourceTherapeuticAssociatesPhysicalTherapy(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTherapeuticAssociatesPhysicalTherapy]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Therapeutic Associates Physical Therapy"
	sourceDef.SourceType = pkg.SourceTypeTherapeuticAssociatesPhysicalTherapy
