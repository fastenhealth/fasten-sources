...
func GetSourceDiabetesAndEndocrineTreatmentCe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDiabetesAndEndocrineTreatmentCe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Diabetes and Endocrine Treatment Ce"
	sourceDef.SourceType = pkg.SourceTypeDiabetesAndEndocrineTreatmentCe
