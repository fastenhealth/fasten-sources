...
func GetSourceTexasIntensivistsNetworkParkhillAffiliate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTexasIntensivistsNetworkParkhillAffiliate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Texas Intensivists Network ParkHill Affiliate"
	sourceDef.SourceType = pkg.SourceTypeTexasIntensivistsNetworkParkhillAffiliate
