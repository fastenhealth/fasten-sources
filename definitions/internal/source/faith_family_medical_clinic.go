...
func GetSourceFaithFamilyMedicalClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFaithFamilyMedicalClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Faith Family Medical Clinic"
	sourceDef.SourceType = pkg.SourceTypeFaithFamilyMedicalClinic
	sourceDef.Category = []string{"251V00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1801929617"}}
	sourceDef.PatientAccessUrl = "http://www.Faithmedical.org"
