...
func GetSourceCentralFloridaHealthCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentralFloridaHealthCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Central Florida Health Care"
	sourceDef.SourceType = pkg.SourceTypeCentralFloridaHealthCare
	sourceDef.Category = []string{"261QM1300X"}
	sourceDef.Aliases = []string{"CENTRAL FLORIDA HEALTH CARE, INC. - MULBERRY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1013301720"}}
