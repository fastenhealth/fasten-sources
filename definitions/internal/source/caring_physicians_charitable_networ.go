...
func GetSourceCaringPhysiciansCharitableNetwor(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCaringPhysiciansCharitableNetwor]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Caring Physicians Charitable Networ"
	sourceDef.SourceType = pkg.SourceTypeCaringPhysiciansCharitableNetwor
