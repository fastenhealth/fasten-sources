...
func GetSourceBreastfeedingSupportWellnessP(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBreastfeedingSupportWellnessP]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BREASTFEEDING SUPPORT  WELLNESS P"
	sourceDef.SourceType = pkg.SourceTypeBreastfeedingSupportWellnessP
