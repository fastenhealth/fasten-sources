...
func GetSourceProspectBehavioralPediatricsAnd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeProspectBehavioralPediatricsAnd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Prospect Behavioral Pediatrics and"
	sourceDef.SourceType = pkg.SourceTypeProspectBehavioralPediatricsAnd
