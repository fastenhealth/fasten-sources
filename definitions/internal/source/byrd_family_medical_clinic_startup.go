...
func GetSourceByrdFamilyMedicalClinicStartup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeByrdFamilyMedicalClinicStartup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Byrd Family Medical Clinic Startup"
	sourceDef.SourceType = pkg.SourceTypeByrdFamilyMedicalClinicStartup
