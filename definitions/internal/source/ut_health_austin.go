...
func GetSourceUtHealthAustin(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUtHealthAustin]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "UT Health Austin"
	sourceDef.SourceType = pkg.SourceTypeUtHealthAustin
	sourceDef.Category = []string{"208D00000X"}
	sourceDef.Aliases = []string{"UT HEALTH AUSTIN"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1073066239"}}
