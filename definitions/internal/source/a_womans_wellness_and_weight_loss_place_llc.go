...
func GetSourceAWomansWellnessAndWeightLossPlaceLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAWomansWellnessAndWeightLossPlaceLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "A WOMANS WELLNESS AND WEIGHT LOSS PLACE LLC"
	sourceDef.SourceType = pkg.SourceTypeAWomansWellnessAndWeightLossPlaceLlc
