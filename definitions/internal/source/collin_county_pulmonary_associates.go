...
func GetSourceCollinCountyPulmonaryAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCollinCountyPulmonaryAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COLLIN COUNTY PULMONARY ASSOCIATES"
	sourceDef.SourceType = pkg.SourceTypeCollinCountyPulmonaryAssociates
