...
func GetSourceTheOrthopedicCenterAtNorthwest(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTheOrthopedicCenterAtNorthwest]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "The Orthopedic Center at Northwest"
	sourceDef.SourceType = pkg.SourceTypeTheOrthopedicCenterAtNorthwest
