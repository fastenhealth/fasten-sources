...
func GetSourceMontanaArthritisCenterLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMontanaArthritisCenterLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Montana Arthritis Center LLC"
	sourceDef.SourceType = pkg.SourceTypeMontanaArthritisCenterLlc
	sourceDef.Category = []string{"261QM1300X"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1114550829"}}
	sourceDef.PatientAccessUrl = "https://montanaarthritis.com/"
