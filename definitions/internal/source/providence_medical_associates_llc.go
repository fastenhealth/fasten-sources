...
func GetSourceProvidenceMedicalAssociatesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeProvidenceMedicalAssociatesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Providence Medical Associates LLC"
	sourceDef.SourceType = pkg.SourceTypeProvidenceMedicalAssociatesLlc
	sourceDef.Category = []string{"261QM2500X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1326654070"}}
	sourceDef.PatientAccessUrl = "http://www.providence-health.net"
