...
func GetSourceOrtholouisiana(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOrtholouisiana]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "OrthoLouisiana"
	sourceDef.SourceType = pkg.SourceTypeOrtholouisiana
	sourceDef.Category = []string{"332900000X"}
	sourceDef.Aliases = []string{"ORTHOLOUISIANA"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1720589013"}}
	sourceDef.PatientAccessUrl = "http://ortho-louisiana.com"
