...
func GetSourceKnjPropertiesLlcDbaBerlinInte(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeKnjPropertiesLlcDbaBerlinInte]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "KNJ Properties LLC dba Berlin Inte"
	sourceDef.SourceType = pkg.SourceTypeKnjPropertiesLlcDbaBerlinInte
