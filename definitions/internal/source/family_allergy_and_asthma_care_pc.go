...
func GetSourceFamilyAllergyAndAsthmaCarePc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFamilyAllergyAndAsthmaCarePc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FAMILY ALLERGY AND ASTHMA CARE PC"
	sourceDef.SourceType = pkg.SourceTypeFamilyAllergyAndAsthmaCarePc
	sourceDef.Category = []string{"207K00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1114679461"}}
	sourceDef.PatientAccessUrl = "http://allergyofmichigan.com"
