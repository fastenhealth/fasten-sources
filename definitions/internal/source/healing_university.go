...
func GetSourceHealingUniversity(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHealingUniversity]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Healing University"
	sourceDef.SourceType = pkg.SourceTypeHealingUniversity
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"HEALING UNIVERSITY"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1285392639"}}
