...
func GetSourceSouthCentralColfaxCountySpecial(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthCentralColfaxCountySpecial]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "South Central Colfax County Special"
	sourceDef.SourceType = pkg.SourceTypeSouthCentralColfaxCountySpecial
