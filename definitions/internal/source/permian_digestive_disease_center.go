...
func GetSourcePermianDigestiveDiseaseCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePermianDigestiveDiseaseCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Permian Digestive Disease Center"
	sourceDef.SourceType = pkg.SourceTypePermianDigestiveDiseaseCenter
