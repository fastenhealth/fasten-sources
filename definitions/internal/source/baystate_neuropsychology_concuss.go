...
func GetSourceBaystateNeuropsychologyConcuss(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBaystateNeuropsychologyConcuss]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Baystate Neuropsychology  Concuss"
	sourceDef.SourceType = pkg.SourceTypeBaystateNeuropsychologyConcuss
