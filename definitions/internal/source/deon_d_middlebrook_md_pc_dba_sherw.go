...
func GetSourceDeonDMiddlebrookMdPcDbaSherw(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeDeonDMiddlebrookMdPcDbaSherw]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Deon D Middlebrook MD PC DBA Sherw"
	sourceDef.SourceType = pkg.SourceTypeDeonDMiddlebrookMdPcDbaSherw
