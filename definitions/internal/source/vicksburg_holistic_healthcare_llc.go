...
func GetSourceVicksburgHolisticHealthcareLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVicksburgHolisticHealthcareLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Vicksburg Holistic Healthcare LLC"
	sourceDef.SourceType = pkg.SourceTypeVicksburgHolisticHealthcareLlc
