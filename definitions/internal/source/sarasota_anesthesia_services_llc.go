...
func GetSourceSarasotaAnesthesiaServicesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSarasotaAnesthesiaServicesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Sarasota Anesthesia Services LLC"
	sourceDef.SourceType = pkg.SourceTypeSarasotaAnesthesiaServicesLlc
