...
func GetSourceEvergreenCareCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEvergreenCareCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Evergreen Care Center"
	sourceDef.SourceType = pkg.SourceTypeEvergreenCareCenter
	sourceDef.Category = []string{"314000000X"}
	sourceDef.Aliases = []string{"EVERGREEN CARE CENTER"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1598323669"}}
