...
func GetSourcePathwayToWellnessHealthcareCent(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePathwayToWellnessHealthcareCent]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PATHWAY TO WELLNESS HEALTHCARE CENT"
	sourceDef.SourceType = pkg.SourceTypePathwayToWellnessHealthcareCent
