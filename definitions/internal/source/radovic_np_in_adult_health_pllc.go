...
func GetSourceRadovicNpInAdultHealthPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRadovicNpInAdultHealthPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "RADOVIC NP IN ADULT HEALTH PLLC"
	sourceDef.SourceType = pkg.SourceTypeRadovicNpInAdultHealthPllc
	sourceDef.Category = []string{"363LA2200X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1780392068"}}
	sourceDef.PatientAccessUrl = "http://www.novuminitiishealth.com"
