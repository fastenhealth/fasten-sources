...
func GetSourceHillsboroClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHillsboroClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hillsboro Clinic"
	sourceDef.SourceType = pkg.SourceTypeHillsboroClinic
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"HILLSBORO CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1063792919"}}
