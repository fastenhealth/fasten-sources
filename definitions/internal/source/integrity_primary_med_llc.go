...
func GetSourceIntegrityPrimaryMedLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIntegrityPrimaryMedLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Integrity Primary Med LLC"
	sourceDef.SourceType = pkg.SourceTypeIntegrityPrimaryMedLlc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"INTEGRITY PRIMARY MED"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1497402531"}}
