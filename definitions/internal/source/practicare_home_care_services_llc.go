...
func GetSourcePracticareHomeCareServicesLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePracticareHomeCareServicesLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Practicare Home Care Services LLC"
	sourceDef.SourceType = pkg.SourceTypePracticareHomeCareServicesLlc
