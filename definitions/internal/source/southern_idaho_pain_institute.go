...
func GetSourceSouthernIdahoPainInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernIdahoPainInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southern Idaho Pain Institute"
	sourceDef.SourceType = pkg.SourceTypeSouthernIdahoPainInstitute
	sourceDef.Category = []string{"174400000X"}
	sourceDef.Aliases = []string{"SOUTHERN IDAHO PAIN INSTITUTE"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1780398586"}}
