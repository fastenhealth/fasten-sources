...
func GetSourceMomdLtd(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMomdLtd]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MOMD LTD"
	sourceDef.SourceType = pkg.SourceTypeMomdLtd
	sourceDef.Category = []string{"207R00000X"}
	sourceDef.Aliases = []string{"LARRY KASKEL M.D.", "MOMD LTD"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1881108314"}}
