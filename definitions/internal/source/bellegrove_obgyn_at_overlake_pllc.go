...
func GetSourceBellegroveObgynAtOverlakePllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBellegroveObgynAtOverlakePllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BELLEGROVE OBGYN AT OVERLAKE PLLC"
	sourceDef.SourceType = pkg.SourceTypeBellegroveObgynAtOverlakePllc
