...
func GetSourceCaClementClinicPaAnantVinjamo(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCaClementClinicPaAnantVinjamo]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CA  CLEMENT CLINIC PA  Anant Vinjamo"
	sourceDef.SourceType = pkg.SourceTypeCaClementClinicPaAnantVinjamo
