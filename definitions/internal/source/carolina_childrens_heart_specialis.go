...
func GetSourceCarolinaChildrensHeartSpecialis(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCarolinaChildrensHeartSpecialis]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Carolina Childrens Heart Specialis"
	sourceDef.SourceType = pkg.SourceTypeCarolinaChildrensHeartSpecialis
