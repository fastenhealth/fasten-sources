...
func GetSourceZanjabeeIntegrativeMedPrimCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeZanjabeeIntegrativeMedPrimCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Zanjabee Integrative Med Prim Care"
	sourceDef.SourceType = pkg.SourceTypeZanjabeeIntegrativeMedPrimCare
