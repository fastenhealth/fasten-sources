...
func GetSourceSnakeRiverCommunityClinicInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSnakeRiverCommunityClinicInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Snake River Community Clinic Inc"
	sourceDef.SourceType = pkg.SourceTypeSnakeRiverCommunityClinicInc
