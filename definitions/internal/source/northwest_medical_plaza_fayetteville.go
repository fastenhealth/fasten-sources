...
func GetSourceNorthwestMedicalPlazaFayetteville(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestMedicalPlazaFayetteville]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Medical Plaza  Fayetteville"
	sourceDef.SourceType = pkg.SourceTypeNorthwestMedicalPlazaFayetteville
