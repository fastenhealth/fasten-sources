...
func GetSourceEastBayCenterForDigestiveHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEastBayCenterForDigestiveHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "East Bay Center for Digestive Health"
	sourceDef.SourceType = pkg.SourceTypeEastBayCenterForDigestiveHealth
