...
func GetSourceSimmondsMartinAndHelmbrechtLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSimmondsMartinAndHelmbrechtLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Simmonds Martin and Helmbrecht LLC"
	sourceDef.SourceType = pkg.SourceTypeSimmondsMartinAndHelmbrechtLlc
