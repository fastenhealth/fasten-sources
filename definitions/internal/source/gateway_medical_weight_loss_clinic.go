...
func GetSourceGatewayMedicalWeightLossClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGatewayMedicalWeightLossClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Gateway Medical Weight Loss Clinic"
	sourceDef.SourceType = pkg.SourceTypeGatewayMedicalWeightLossClinic
