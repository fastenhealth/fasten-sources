...
func GetSourcePinnacleIntegrativeOrthopedicsAn(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePinnacleIntegrativeOrthopedicsAn]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pinnacle Integrative Orthopedics an"
	sourceDef.SourceType = pkg.SourceTypePinnacleIntegrativeOrthopedicsAn
