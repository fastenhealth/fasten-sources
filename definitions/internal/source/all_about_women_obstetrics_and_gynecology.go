...
func GetSourceAllAboutWomenObstetricsAndGynecology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllAboutWomenObstetricsAndGynecology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ALL ABOUT WOMEN OBSTETRICS AND GYNECOLOGY"
	sourceDef.SourceType = pkg.SourceTypeAllAboutWomenObstetricsAndGynecology
