...
func GetSourceNewHorizonsObgynOfTrinityLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNewHorizonsObgynOfTrinityLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NEW HORIZONS OBGYN OF TRINITY LLC"
	sourceDef.SourceType = pkg.SourceTypeNewHorizonsObgynOfTrinityLlc
