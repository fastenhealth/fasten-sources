...
func GetSourceBestStartPediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBestStartPediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Best Start Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeBestStartPediatrics
	sourceDef.Category = []string{"2080A0000X"}
	sourceDef.Aliases = []string{"BEST START PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1689182412"}}
