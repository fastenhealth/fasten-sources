...
func GetSourceRemedyInternalMedicineWellness(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRemedyInternalMedicineWellness]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Remedy Internal Medicine  Wellness"
	sourceDef.SourceType = pkg.SourceTypeRemedyInternalMedicineWellness
