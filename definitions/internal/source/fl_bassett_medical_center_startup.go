...
func GetSourceFlBassettMedicalCenterStartup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFlBassettMedicalCenterStartup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FL  Bassett Medical Center  Startup"
	sourceDef.SourceType = pkg.SourceTypeFlBassettMedicalCenterStartup
