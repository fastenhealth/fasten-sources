...
func GetSourceAlliancehealthMedicalGroupOrthopedicsPoncaCity(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAlliancehealthMedicalGroupOrthopedicsPoncaCity]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "AllianceHealth Medical Group Orthopedics Ponca City"
	sourceDef.SourceType = pkg.SourceTypeAlliancehealthMedicalGroupOrthopedicsPoncaCity
