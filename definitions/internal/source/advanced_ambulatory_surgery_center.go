...
func GetSourceAdvancedAmbulatorySurgeryCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdvancedAmbulatorySurgeryCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ADVANCED AMBULATORY SURGERY CENTER"
	sourceDef.SourceType = pkg.SourceTypeAdvancedAmbulatorySurgeryCenter
