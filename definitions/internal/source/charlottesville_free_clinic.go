...
func GetSourceCharlottesvilleFreeClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCharlottesvilleFreeClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Charlottesville Free Clinic"
	sourceDef.SourceType = pkg.SourceTypeCharlottesvilleFreeClinic
	sourceDef.Category = []string{"333600000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1477089639"}}
	sourceDef.PatientAccessUrl = "https://www.cvillefreeclinic.org/"
