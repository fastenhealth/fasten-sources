...
func GetSourceSouthernMaineGeriatricsAssociate(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthernMaineGeriatricsAssociate]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Southern Maine Geriatrics Associate"
	sourceDef.SourceType = pkg.SourceTypeSouthernMaineGeriatricsAssociate
