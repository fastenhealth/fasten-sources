...
func GetSourceBouldercentreForOrthopedics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBouldercentreForOrthopedics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BoulderCentre for Orthopedics"
	sourceDef.SourceType = pkg.SourceTypeBouldercentreForOrthopedics
	sourceDef.Category = []string{"225100000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1952427049"}}
	sourceDef.PatientAccessUrl = "http:// www.bouldercentre.com"
