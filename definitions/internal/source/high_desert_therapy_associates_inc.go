...
func GetSourceHighDesertTherapyAssociatesInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHighDesertTherapyAssociatesInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HIGH DESERT THERAPY ASSOCIATES INC"
	sourceDef.SourceType = pkg.SourceTypeHighDesertTherapyAssociatesInc
