...
func GetSourceStatelineHeartAndVascularHealth(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStatelineHeartAndVascularHealth]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Stateline Heart and Vascular Health"
	sourceDef.SourceType = pkg.SourceTypeStatelineHeartAndVascularHealth
