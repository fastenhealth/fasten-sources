...
func GetSourceAccess2HealthPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAccess2HealthPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ACCESS2HEALTH PLLC"
	sourceDef.SourceType = pkg.SourceTypeAccess2HealthPllc
	sourceDef.Category = []string{"363L00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1093339434"}}
	sourceDef.PatientAccessUrl = "http://access2healthpllc.com"
