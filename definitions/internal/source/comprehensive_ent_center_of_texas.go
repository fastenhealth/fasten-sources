...
func GetSourceComprehensiveEntCenterOfTexas(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeComprehensiveEntCenterOfTexas]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "COMPREHENSIVE ENT CENTER OF TEXAS"
	sourceDef.SourceType = pkg.SourceTypeComprehensiveEntCenterOfTexas
	sourceDef.Category = []string{"207Y00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1780709675"}}
