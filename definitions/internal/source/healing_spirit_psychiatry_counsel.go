...
func GetSourceHealingSpiritPsychiatryCounsel(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHealingSpiritPsychiatryCounsel]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Healing Spirit Psychiatry  Counsel"
	sourceDef.SourceType = pkg.SourceTypeHealingSpiritPsychiatryCounsel
