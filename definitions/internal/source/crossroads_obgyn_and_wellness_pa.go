...
func GetSourceCrossroadsObgynAndWellnessPa(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCrossroadsObgynAndWellnessPa]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Crossroads OBGYN and Wellness PA"
	sourceDef.SourceType = pkg.SourceTypeCrossroadsObgynAndWellnessPa
	sourceDef.Category = []string{"207V00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1144550617"}}
	sourceDef.PatientAccessUrl = "http://www.crossroadsobgyn.com"
