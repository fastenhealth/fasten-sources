...
func GetSourceCompassionCareClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCompassionCareClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Compassion Care Clinic"
	sourceDef.SourceType = pkg.SourceTypeCompassionCareClinic
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"STACEY MICHELIN"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1164982443"}}
