...
func GetSourceInternationalPediatricsClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeInternationalPediatricsClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "International Pediatrics Clinic"
	sourceDef.SourceType = pkg.SourceTypeInternationalPediatricsClinic
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{"INTERNATIONAL PEDIATRICS CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1922131374"}}
