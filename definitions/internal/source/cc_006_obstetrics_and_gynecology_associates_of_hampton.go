...
func GetSourceCc006ObstetricsAndGynecologyAssociatesOfHampton(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc006ObstetricsAndGynecologyAssociatesOfHampton]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC006OBSTETRICS AND GYNECOLOGY ASSOCIATES OF HAMPTON"
	sourceDef.SourceType = pkg.SourceTypeCc006ObstetricsAndGynecologyAssociatesOfHampton
