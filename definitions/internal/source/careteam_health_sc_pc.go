...
func GetSourceCareteamHealthScPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCareteamHealthScPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CareTeam Health SC PC"
	sourceDef.SourceType = pkg.SourceTypeCareteamHealthScPc
	sourceDef.Category = []string{"207Q00000X"}
	sourceDef.Aliases = []string{"CARETEAM CORPORATE HEALTH: WATERTOWN"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1104491984"}}
