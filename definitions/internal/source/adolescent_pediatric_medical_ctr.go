...
func GetSourceAdolescentPediatricMedicalCtr(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAdolescentPediatricMedicalCtr]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Adolescent  Pediatric Medical Ctr"
	sourceDef.SourceType = pkg.SourceTypeAdolescentPediatricMedicalCtr
