...
func GetSourceBeeWellElPasoPediatricsPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBeeWellElPasoPediatricsPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "BEE WELL EL PASO PEDIATRICS PLLC"
	sourceDef.SourceType = pkg.SourceTypeBeeWellElPasoPediatricsPllc
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1164083358"}}
	sourceDef.PatientAccessUrl = "http://beewellelpasopediatrics.com"
