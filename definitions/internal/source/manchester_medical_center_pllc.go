...
func GetSourceManchesterMedicalCenterPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeManchesterMedicalCenterPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MANCHESTER MEDICAL CENTER PLLC"
	sourceDef.SourceType = pkg.SourceTypeManchesterMedicalCenterPllc
	sourceDef.Category = []string{"207P00000X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1730682766"}}
