...
func GetSourceSouthFloridaMedicalAndSurgicalAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSouthFloridaMedicalAndSurgicalAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "South Florida Medical and Surgical Associates"
	sourceDef.SourceType = pkg.SourceTypeSouthFloridaMedicalAndSurgicalAssociates
