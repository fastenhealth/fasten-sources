...
func GetSourceRainbowCityClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRainbowCityClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Rainbow City Clinic"
	sourceDef.SourceType = pkg.SourceTypeRainbowCityClinic
	sourceDef.Category = []string{"363LF0000X"}
	sourceDef.Aliases = []string{"RAINBOW CITY CLINIC, INC."}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1134799547"}}
