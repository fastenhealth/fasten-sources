...
func GetSourceMoberlyRuralHealthClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMoberlyRuralHealthClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Moberly Rural Health Clinic"
	sourceDef.SourceType = pkg.SourceTypeMoberlyRuralHealthClinic
	sourceDef.Category = []string{"261Q00000X", "261QR1300X"}
	sourceDef.Aliases = []string{"MOBERLY RURAL HEALTH CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1407989510"}}
