...
func GetSourceSuryaHealthCareLimitedLiability(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeSuryaHealthCareLimitedLiability]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Surya Health Care Limited Liability"
	sourceDef.SourceType = pkg.SourceTypeSuryaHealthCareLimitedLiability
