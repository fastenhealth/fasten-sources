...
func GetSourceCabrilloCenterForRheumaticDisea(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCabrilloCenterForRheumaticDisea]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Cabrillo Center for Rheumatic Disea"
	sourceDef.SourceType = pkg.SourceTypeCabrilloCenterForRheumaticDisea
