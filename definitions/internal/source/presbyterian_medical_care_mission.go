...
func GetSourcePresbyterianMedicalCareMission(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePresbyterianMedicalCareMission]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Presbyterian Medical Care Mission"
	sourceDef.SourceType = pkg.SourceTypePresbyterianMedicalCareMission
