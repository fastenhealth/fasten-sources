...
func GetSourceExcelsisHealthLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeExcelsisHealthLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Excelsis Health LLC"
	sourceDef.SourceType = pkg.SourceTypeExcelsisHealthLlc
	sourceDef.Category = []string{"207QB0002X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1770247736"}}
	sourceDef.PatientAccessUrl = "http://www.excelsishealth.com"
