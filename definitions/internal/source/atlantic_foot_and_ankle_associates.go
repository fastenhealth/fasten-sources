...
func GetSourceAtlanticFootAndAnkleAssociates(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAtlanticFootAndAnkleAssociates]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Atlantic Foot and Ankle Associates"
	sourceDef.SourceType = pkg.SourceTypeAtlanticFootAndAnkleAssociates
