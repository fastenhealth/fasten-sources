...
func GetSourceFlemingIslandObstetricsGynecologyLlcTermed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFlemingIslandObstetricsGynecologyLlcTermed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "FLEMING ISLAND OBSTETRICS  GYNECOLOGY LLC TERMED"
	sourceDef.SourceType = pkg.SourceTypeFlemingIslandObstetricsGynecologyLlcTermed
