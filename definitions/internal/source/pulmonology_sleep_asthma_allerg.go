...
func GetSourcePulmonologySleepAsthmaAllerg(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePulmonologySleepAsthmaAllerg]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pulmonology Sleep Asthma  Allerg"
	sourceDef.SourceType = pkg.SourceTypePulmonologySleepAsthmaAllerg
