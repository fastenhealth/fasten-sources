...
func GetSourceMathisHealthClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMathisHealthClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MATHIS HEALTH CLINIC"
	sourceDef.SourceType = pkg.SourceTypeMathisHealthClinic
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"MATHIS HEALTH CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1023641545"}}
