...
func GetSourceHealedAndEmpoweredLlc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHealedAndEmpoweredLlc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "HEALED AND EMPOWERED LLC"
	sourceDef.SourceType = pkg.SourceTypeHealedAndEmpoweredLlc
