...
func GetSourceNorthwestCardiologySiloamSprings(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNorthwestCardiologySiloamSprings]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Northwest Cardiology  Siloam Springs"
	sourceDef.SourceType = pkg.SourceTypeNorthwestCardiologySiloamSprings
