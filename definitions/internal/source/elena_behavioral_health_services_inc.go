...
func GetSourceElenaBehavioralHealthServicesInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeElenaBehavioralHealthServicesInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ELENA BEHAVIORAL HEALTH SERVICES INC"
	sourceDef.SourceType = pkg.SourceTypeElenaBehavioralHealthServicesInc
