...
func GetSourceLaIntegrativeGastroenterologyRahbarMedical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLaIntegrativeGastroenterologyRahbarMedical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LA Integrative Gastroenterology  Rahbar Medical"
	sourceDef.SourceType = pkg.SourceTypeLaIntegrativeGastroenterologyRahbarMedical
