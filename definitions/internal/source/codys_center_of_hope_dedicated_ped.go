...
func GetSourceCodysCenterOfHopeDedicatedPed(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCodysCenterOfHopeDedicatedPed]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Codys Center of Hope Dedicated Ped"
	sourceDef.SourceType = pkg.SourceTypeCodysCenterOfHopeDedicatedPed
