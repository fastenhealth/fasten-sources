...
func GetSourceGeneralAndLaparoscopicSurgicalA(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGeneralAndLaparoscopicSurgicalA]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "General and Laparoscopic Surgical A"
	sourceDef.SourceType = pkg.SourceTypeGeneralAndLaparoscopicSurgicalA
