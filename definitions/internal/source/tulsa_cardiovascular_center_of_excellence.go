...
func GetSourceTulsaCardiovascularCenterOfExcellence(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTulsaCardiovascularCenterOfExcellence]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "TULSA CARDIOVASCULAR CENTER OF EXCELLENCE"
	sourceDef.SourceType = pkg.SourceTypeTulsaCardiovascularCenterOfExcellence
