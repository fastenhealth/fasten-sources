...
func GetSourceParkingWayPediatrics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeParkingWayPediatrics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Parking Way Pediatrics"
	sourceDef.SourceType = pkg.SourceTypeParkingWayPediatrics
	sourceDef.Category = []string{"208000000X"}
	sourceDef.Aliases = []string{"PARKING WAY PEDIATRICS"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1598815896"}}
