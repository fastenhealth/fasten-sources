...
func GetSourceAmericanInterventionalPainInstit(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAmericanInterventionalPainInstit]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "American Interventional Pain Instit"
	sourceDef.SourceType = pkg.SourceTypeAmericanInterventionalPainInstit
