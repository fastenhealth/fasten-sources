...
func GetSourceAllcareFootAnkleCenterDrRaugh(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAllcareFootAnkleCenterDrRaugh]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ALLCARE FOOT  ANKLE CENTER  Dr Raugh"
	sourceDef.SourceType = pkg.SourceTypeAllcareFootAnkleCenterDrRaugh
