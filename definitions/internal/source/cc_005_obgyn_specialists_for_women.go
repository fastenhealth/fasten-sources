...
func GetSourceCc005ObgynSpecialistsForWomen(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCc005ObgynSpecialistsForWomen]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CC005OBGYN SPECIALISTS FOR WOMEN"
	sourceDef.SourceType = pkg.SourceTypeCc005ObgynSpecialistsForWomen
