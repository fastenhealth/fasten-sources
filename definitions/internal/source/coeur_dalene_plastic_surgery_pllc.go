...
func GetSourceCoeurDalenePlasticSurgeryPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCoeurDalenePlasticSurgeryPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Coeur DAlene Plastic Surgery PLLC"
	sourceDef.SourceType = pkg.SourceTypeCoeurDalenePlasticSurgeryPllc
