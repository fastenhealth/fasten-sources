...
func GetSourceNovaPulmonaryCriticalCareSlee(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNovaPulmonaryCriticalCareSlee]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "NOVA Pulmonary Critical Care  Slee"
	sourceDef.SourceType = pkg.SourceTypeNovaPulmonaryCriticalCareSlee
