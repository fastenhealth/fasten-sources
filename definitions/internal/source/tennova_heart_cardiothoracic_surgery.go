...
func GetSourceTennovaHeartCardiothoracicSurgery(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeTennovaHeartCardiothoracicSurgery]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Tennova Heart  Cardiothoracic Surgery"
	sourceDef.SourceType = pkg.SourceTypeTennovaHeartCardiothoracicSurgery
