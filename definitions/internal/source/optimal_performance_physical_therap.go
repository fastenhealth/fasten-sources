...
func GetSourceOptimalPerformancePhysicalTherap(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeOptimalPerformancePhysicalTherap]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Optimal Performance Physical Therap"
	sourceDef.SourceType = pkg.SourceTypeOptimalPerformancePhysicalTherap
