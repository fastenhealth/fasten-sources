...
func GetSourceComprehensiveSpinePainRelievePainRestoreLives(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeComprehensiveSpinePainRelievePainRestoreLives]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Comprehensive Spine  Pain Relieve Pain Restore Lives"
	sourceDef.SourceType = pkg.SourceTypeComprehensiveSpinePainRelievePainRestoreLives
