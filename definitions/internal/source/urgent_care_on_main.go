...
func GetSourceUrgentCareOnMain(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeUrgentCareOnMain]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Urgent Care on Main"
	sourceDef.SourceType = pkg.SourceTypeUrgentCareOnMain
	sourceDef.Category = []string{"332900000X"}
	sourceDef.Aliases = []string{"URGENT CARE ON MAIN"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1700444619"}}
