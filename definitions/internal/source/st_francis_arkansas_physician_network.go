...
func GetSourceStFrancisArkansasPhysicianNetwork(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeStFrancisArkansasPhysicianNetwork]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "St Francis Arkansas Physician Network"
	sourceDef.SourceType = pkg.SourceTypeStFrancisArkansasPhysicianNetwork
