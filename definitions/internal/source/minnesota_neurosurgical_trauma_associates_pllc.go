...
func GetSourceMinnesotaNeurosurgicalTraumaAssociatesPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMinnesotaNeurosurgicalTraumaAssociatesPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Minnesota Neurosurgical Trauma Associates PLLC"
	sourceDef.SourceType = pkg.SourceTypeMinnesotaNeurosurgicalTraumaAssociatesPllc
