...
func GetSourceCareteamCorporateHealthDanville(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCareteamCorporateHealthDanville]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CareTeam Corporate Health Danville"
	sourceDef.SourceType = pkg.SourceTypeCareteamCorporateHealthDanville
