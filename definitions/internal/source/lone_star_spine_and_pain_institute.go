...
func GetSourceLoneStarSpineAndPainInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeLoneStarSpineAndPainInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "LONE STAR SPINE AND PAIN INSTITUTE"
	sourceDef.SourceType = pkg.SourceTypeLoneStarSpineAndPainInstitute
