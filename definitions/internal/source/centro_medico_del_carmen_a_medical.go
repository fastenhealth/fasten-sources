...
func GetSourceCentroMedicoDelCarmenAMedical(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCentroMedicoDelCarmenAMedical]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Centro Medico Del Carmen A Medical"
	sourceDef.SourceType = pkg.SourceTypeCentroMedicoDelCarmenAMedical
