...
func GetSourceBoiseWomensHealthBirthCenter(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBoiseWomensHealthBirthCenter]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Boise Womens Health  Birth Center"
	sourceDef.SourceType = pkg.SourceTypeBoiseWomensHealthBirthCenter
