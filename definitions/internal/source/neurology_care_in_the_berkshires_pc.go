...
func GetSourceNeurologyCareInTheBerkshiresPc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeNeurologyCareInTheBerkshiresPc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Neurology Care in the Berkshires PC"
	sourceDef.SourceType = pkg.SourceTypeNeurologyCareInTheBerkshiresPc
	sourceDef.Category = []string{"2084N0400X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1124331681"}}
