...
func GetSourceRichardtonClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeRichardtonClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Richardton Clinic"
	sourceDef.SourceType = pkg.SourceTypeRichardtonClinic
	sourceDef.Category = []string{"261QR1300X"}
	sourceDef.Aliases = []string{"RICHARDTON CLINIC"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1043649486"}}
