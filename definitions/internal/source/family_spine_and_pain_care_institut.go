...
func GetSourceFamilySpineAndPainCareInstitut(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFamilySpineAndPainCareInstitut]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Family Spine And Pain Care Institut"
	sourceDef.SourceType = pkg.SourceTypeFamilySpineAndPainCareInstitut
