...
func GetSourceBeverlyHillsAdvancedPainSpine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBeverlyHillsAdvancedPainSpine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Beverly Hills Advanced Pain  Spine"
	sourceDef.SourceType = pkg.SourceTypeBeverlyHillsAdvancedPainSpine
