...
func GetSourceApexHealthNorthShoreGynecology(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeApexHealthNorthShoreGynecology]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Apex Health North Shore Gynecology"
	sourceDef.SourceType = pkg.SourceTypeApexHealthNorthShoreGynecology
