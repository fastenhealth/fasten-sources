...
func GetSourcePaDrAndreaVanestenbergStartup(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePaDrAndreaVanestenbergStartup]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PA  Dr Andrea VanEstenberg  Startup"
	sourceDef.SourceType = pkg.SourceTypePaDrAndreaVanestenbergStartup
