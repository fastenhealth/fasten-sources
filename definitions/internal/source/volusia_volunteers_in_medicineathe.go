...
func GetSourceVolusiaVolunteersInMedicineathe(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeVolusiaVolunteersInMedicineathe]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Volusia Volunteers in Medicineathe"
	sourceDef.SourceType = pkg.SourceTypeVolusiaVolunteersInMedicineathe
