...
func GetSourceMidatlanticWomensCareImagingCenters(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMidatlanticWomensCareImagingCenters]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "MidAtlantic Womens Care Imaging Centers"
	sourceDef.SourceType = pkg.SourceTypeMidatlanticWomensCareImagingCenters
