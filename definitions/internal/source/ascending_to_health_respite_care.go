...
func GetSourceAscendingToHealthRespiteCare(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeAscendingToHealthRespiteCare]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Ascending to Health Respite Care"
	sourceDef.SourceType = pkg.SourceTypeAscendingToHealthRespiteCare
