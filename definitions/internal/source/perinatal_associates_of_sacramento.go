...
func GetSourcePerinatalAssociatesOfSacramento(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePerinatalAssociatesOfSacramento]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeNextgen))

	sourceDef.Display = "Perinatal Associates of Sacramento"
	sourceDef.SourceType = pkg.SourceTypePerinatalAssociatesOfSacramento
