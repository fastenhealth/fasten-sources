...
func GetSourcePcrmClinicDbaBarnardMedicalC(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePcrmClinicDbaBarnardMedicalC]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "PCRM Clinic dba Barnard Medical C"
	sourceDef.SourceType = pkg.SourceTypePcrmClinicDbaBarnardMedicalC
