...
func GetSourceIslandTimeMedicalAndWellness(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIslandTimeMedicalAndWellness]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Island Time Medical and Wellness"
	sourceDef.SourceType = pkg.SourceTypeIslandTimeMedicalAndWellness
