...
func GetSourceFlorida(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeFlorida]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Florida"
	sourceDef.SourceType = pkg.SourceTypeFlorida
	sourceDef.Category = []string{"207Q00000X", "207R00000X", "208D00000X", "251C00000X"}
	sourceDef.Aliases = []string{"FLORIDA"}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1952777229", "1992472880"}}
