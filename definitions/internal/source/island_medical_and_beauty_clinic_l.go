...
func GetSourceIslandMedicalAndBeautyClinicL(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeIslandMedicalAndBeautyClinicL]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ISLAND MEDICAL AND BEAUTY CLINIC L"
	sourceDef.SourceType = pkg.SourceTypeIslandMedicalAndBeautyClinicL
