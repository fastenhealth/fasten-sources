...
func GetSourceCityPainManagementPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCityPainManagementPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "City Pain Management PLLC"
	sourceDef.SourceType = pkg.SourceTypeCityPainManagementPllc
	sourceDef.Category = []string{"208VP0014X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1669860326"}}
	sourceDef.PatientAccessUrl = "http://www.citypainmanagement.com"
