...
func GetSourceGenerationsFamilyMedicalClinc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeGenerationsFamilyMedicalClinc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Generations Family Medical Clinc"
	sourceDef.SourceType = pkg.SourceTypeGenerationsFamilyMedicalClinc
