...
func GetSourceCareplusHighlandColonyFamilyMedicine(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeCareplusHighlandColonyFamilyMedicine]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "CarePlus Highland Colony Family Medicine"
	sourceDef.SourceType = pkg.SourceTypeCareplusHighlandColonyFamilyMedicine
