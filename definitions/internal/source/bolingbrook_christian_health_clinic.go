...
func GetSourceBolingbrookChristianHealthClinic(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeBolingbrookChristianHealthClinic]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Bolingbrook Christian Health Clinic"
	sourceDef.SourceType = pkg.SourceTypeBolingbrookChristianHealthClinic
