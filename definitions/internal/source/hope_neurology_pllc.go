...
func GetSourceHopeNeurologyPllc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeHopeNeurologyPllc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Hope Neurology Pllc"
	sourceDef.SourceType = pkg.SourceTypeHopeNeurologyPllc
	sourceDef.Category = []string{"2084N0400X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1881895183"}}
	sourceDef.PatientAccessUrl = "https://hopeneuro.com/"
