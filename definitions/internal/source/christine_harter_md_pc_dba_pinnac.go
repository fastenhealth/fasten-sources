...
func GetSourceChristineHarterMdPcDbaPinnac(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeChristineHarterMdPcDbaPinnac]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Christine Harter MD PC dba Pinnac"
	sourceDef.SourceType = pkg.SourceTypeChristineHarterMdPcDbaPinnac
