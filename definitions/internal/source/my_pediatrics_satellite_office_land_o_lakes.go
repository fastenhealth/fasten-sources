...
func GetSourceMyPediatricsSatelliteOfficeLandOLakes(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMyPediatricsSatelliteOfficeLandOLakes]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "My Pediatrics SATELLITE OFFICE  Land O Lakes"
	sourceDef.SourceType = pkg.SourceTypeMyPediatricsSatelliteOfficeLandOLakes
