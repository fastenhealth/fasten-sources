...
func GetSourceEstrellaFamilyCareAndAesthetics(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeEstrellaFamilyCareAndAesthetics]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "ESTRELLA FAMILY CARE AND AESTHETICS"
	sourceDef.SourceType = pkg.SourceTypeEstrellaFamilyCareAndAesthetics
