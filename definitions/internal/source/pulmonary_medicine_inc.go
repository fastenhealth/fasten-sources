...
func GetSourcePulmonaryMedicineInc(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePulmonaryMedicineInc]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Pulmonary Medicine Inc"
	sourceDef.SourceType = pkg.SourceTypePulmonaryMedicineInc
	sourceDef.Category = []string{"207RC0200X", "207RP1001X", "207RS0012X"}
	sourceDef.Aliases = []string{}
	sourceDef.Identifiers = map[string][]string{"http://hl7.org/fhir/sid/us-npi": []string{"1043270697"}}
