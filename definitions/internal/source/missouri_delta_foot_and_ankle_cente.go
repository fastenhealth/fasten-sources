...
func GetSourceMissouriDeltaFootAndAnkleCente(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypeMissouriDeltaFootAndAnkleCente]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Missouri Delta Foot and Ankle Cente"
	sourceDef.SourceType = pkg.SourceTypeMissouriDeltaFootAndAnkleCente
