...
func GetSourcePalmBeachNeuroscienceInstitute(env pkg.FastenLighthouseEnvType, clientIdLookup map[pkg.SourceType]string) (models.LighthouseSourceDefinition, error) {
	...
	if clientId, clientIdOk := clientIdLookup[pkg.SourceTypePalmBeachNeuroscienceInstitute]; clientIdOk {
		sourceDef.ClientId = clientId
	}
	sourceDef.RedirectUri = pkg.GetCallbackEndpoint(string(pkg.SourceTypeAthena))

	sourceDef.Display = "Palm Beach Neuroscience Institute"
	sourceDef.SourceType = pkg.SourceTypePalmBeachNeuroscienceInstitute
